...
function clientCalendarCapacity(PDO $pdo,int $org,string $from,string $to,?int $onlyResource=null,int $excludeBooking=0,bool $studiosOnly=false): array {
    ...
    foreach(recurringBookingBlockOccurrences($pdo,$org,$from,$to) as $r)$mark($r['resource_id'],$r['block_date'],$r['start_time'],$r['end_time']);
...
function clientPackageCalendar(PDO $pdo,array $user,int $packageId,int $duration,string $startDate,int $days,int $bookingId=0): array {
    ...
    $capacity=clientCalendarCapacity($pdo,$org,$from,$to,$booking?(int)$booking['resource_id']:null,$bookingId,!$booking);if(!$capacity['resources'])fail('لا يوجد استديو متاح للحجز الآن.',409,'no_booking_resource');
