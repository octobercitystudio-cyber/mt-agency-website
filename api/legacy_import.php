...
function legacyImportReusableRecords(PDO $pdo,int $organizationId,array $packageRows,array $appointmentRows,array $byPhone,array $byId): array {
    ...
    $importedPackage=$pdo->prepare("SELECT cp.id FROM client_packages cp LEFT JOIN services s ON s.id=cp.service_id AND s.organization_id=cp.organization_id WHERE cp.organization_id=? AND cp.client_id=? AND cp.billing_unit=? AND (cp.name=? OR s.name=?) AND cp.notes LIKE '%منقول من البرنامج القديم%' ORDER BY cp.id DESC LIMIT 2 FOR UPDATE");
    ...
    foreach($packageRows as $row){
        ...
        $client=legacyImportVerifiedClient($row,$byPhone,$byId);$name=legacyImportText($row['source_service_name']??'',180);$unit=(string)($row['billing_unit']??'hour');$importedPackage->execute([$organizationId,$client['id'],$unit,$name,$name]);$importedMatches=$importedPackage->fetchAll();if(count($importedMatches)===1){$packageMap[$reference]=(int)$importedMatches[0]['id'];continue;}
        if(($row['status']??'')!=='active')continue;$shape->execute([$organizationId,$client['id'],$unit,legacyImportQuantity($row['purchased_quantity']??0),(float)legacyImportMoney($row['total_price']??0),(float)legacyImportMoney($row['paid_amount']??0),$name,$name]);$matches=$shape->fetchAll();if(count($matches)===1)$packageMap[$reference]=(int)$matches[0]['id'];
    }
    $exact=[];$futureExpected=[];$findImported=$pdo->prepare("SELECT id,client_package_id FROM bookings WHERE organization_id=? AND client_id=? AND notes LIKE ? ORDER BY id LIMIT 2 FOR UPDATE");$findExact=$pdo->prepare("SELECT id,client_package_id FROM bookings WHERE organization_id=? AND client_id=? AND resource_id=? AND date=? AND start_time=? AND end_time=? AND status IN ('confirmed','completed') ORDER BY id LIMIT 2 FOR UPDATE");
    foreach($appointmentRows as $row){
        if(!is_array($row))continue;$packageReference=legacyImportText($row['package_reference']??'',120);$reference=legacyImportText($row['legacy_reference']??'',120);if($reference==='')continue;$client=legacyImportVerifiedClient($row,$byPhone,$byId);$date=legacyImportDate($row['date']??'');$start=normalizeBusinessTime($row['start_time']??'');$end=normalizeBusinessTime($row['end_time']??'',true);if($start===''||$end==='')continue;$storedEnd=$end==='24:00'?'23:59':$end;if($packageReference!==''&&($row['status']??'')==='confirmed'&&$date>=$today)$futureExpected[$packageReference]=($futureExpected[$packageReference]??0)+1;$sourceBookingId=(int)($row['source_booking_id']??0);if($sourceBookingId>0){$findImported->execute([$organizationId,$client['id'],'%[قديم #'.$sourceBookingId.']%']);$matches=$findImported->fetchAll();if(count($matches)>1)fail('يوجد أكثر من موعد مطابق لرقم الموعد القديم '.$sourceBookingId.'؛ أوقفت المزامنة لمنع التكرار.',409,'legacy_booking_match_ambiguous',['source_booking_id'=>$sourceBookingId]);if(count($matches)===1){if($packageReference!==''&&((int)$matches[0]['client_package_id'])>0)$exact[$packageReference][$reference]=['booking_id'=>(int)$matches[0]['id'],'package_id'=>(int)$matches[0]['client_package_id']];else$bookingMap[$reference]=(int)$matches[0]['id'];continue;}}$findExact->execute([$organizationId,$client['id'],(int)($row['resource_id']??0),$date,$start.':00',$storedEnd.':00']);$matches=$findExact->fetchAll();if(count($matches)===1){if($packageReference!==''&&((int)$matches[0]['client_package_id'])>0)$exact[$packageReference][$reference]=['booking_id'=>(int)$matches[0]['id'],'package_id'=>(int)$matches[0]['client_package_id']];else$bookingMap[$reference]=(int)$matches[0]['id'];}
    }
...
function legacyPrepareBookingSync(PDO $pdo,array $user,array $rows,array $reuseMap): int {
    $organizationId=(int)$user['organization_id'];$transitions=0;$lockBooking=$pdo->prepare('SELECT * FROM bookings WHERE id=? AND organization_id=? FOR UPDATE');$lockPackage=$pdo->prepare('SELECT * FROM client_packages WHERE id=? AND organization_id=? FOR UPDATE');
    foreach($rows as $row){if(!is_array($row)||($row['status']??'')!=='completed')continue;$reference=legacyImportText($row['legacy_reference']??'',120);$bookingId=(int)($reuseMap[$reference]??0);if($bookingId<1)continue;$lockBooking->execute([$bookingId,$organizationId]);$booking=$lockBooking->fetch();if(!$booking||(string)$booking['status']!=='confirmed')continue;releaseBookingSlots($pdo,$bookingId);$packageId=(int)($booking['client_package_id']??0);if($packageId>0){$held=bookingHeldQuantity($pdo,$bookingId,$packageId);if($held>0){$lockPackage->execute([$packageId,$organizationId]);$package=$lockPackage->fetch();if(!$package)fail('تعذر تجهيز رصيد الموعد القديم المكتمل للمزامنة.',409,'legacy_booking_package_missing');mutateLockedPackageQuantities($pdo,$package,0,-$held,0);$actual=(string)($package['billing_unit']??'hour')==='reel'?max(0,(int)($row['actual_reels']??0)):legacyImportQuantity($row['actual_hours']??0);$covered=min($held,$actual);if($covered>0)insertPackageUsage($pdo,$package,$bookingId,'consume',$covered,'استهلاك جلسة اكتملت في البرنامج القديم','legacy-booking-id:'.(int)($row['source_booking_id']??0).':consume',$user['id']);$release=max(0,$held-$covered);if($release>0)insertPackageUsage($pdo,$package,$bookingId,'release',$release,'إعادة رصيد غير مستخدم من جلسة قديمة','legacy-booking-id:'.(int)($row['source_booking_id']??0).':release',$user['id']);}}$transitions++;}
    return $transitions;
}
...
function legacyImportProjects(PDO $pdo,array $user,array $rows,array $byPhone,array $byId): array {
    $map=[];$count=0;$payments=0;$reused=0;$organizationId=(int)$user['organization_id'];
    $find=$pdo->prepare("SELECT * FROM projects WHERE organization_id=? AND client_id=? AND name=? AND notes LIKE '%منقول من البرنامج القديم%' ORDER BY id DESC LIMIT 2 FOR UPDATE");
    $insert=$pdo->prepare("INSERT INTO projects (organization_id,client_id,name,category,service_type,pricing_model,quantity,unit_label,agreed_price,requires_booking,progress_percent,status,starts_at,due_at,notes,created_by) VALUES (?,?,?,'custom',?,'custom',1,'project',?,?,?,?,?,?,?,?)");
    $update=$pdo->prepare("UPDATE projects SET service_type=?,agreed_price=?,requires_booking=?,progress_percent=?,status=?,starts_at=?,due_at=?,notes=? WHERE id=? AND organization_id=?");
    foreach($rows as $row){if(!is_array($row))fail('سجل خدمة قديم غير صالح.',422,'invalid_legacy_project');$client=legacyImportVerifiedClient($row,$byPhone,$byId);$reference=legacyImportText($row['legacy_reference']??'',120);$name=legacyImportText($row['name']??'',190);if($reference===''||$name==='')fail('بيانات مشروع قديم غير مكتملة.',422,'invalid_legacy_project');$starts=legacyImportDate($row['starts_at']??'');$due=legacyImportDate($row['due_at']??'',true);$status=in_array((string)($row['status']??''),['active','completed'],true)?(string)$row['status']:'active';$progress=$status==='completed'?100:max(0,min(99,(int)($row['progress_percent']??35)));$price=legacyImportMoney($row['agreed_price']??0);$paid=legacyImportMoney($row['paid_amount']??0);$sourceNotes=legacyImportText(($row['notes']??'').' — منقول من البرنامج القديم: '.$reference,3000);$invoiceId=null;
        $find->execute([$organizationId,$client['id'],$name]);$matches=$find->fetchAll();if(count($matches)>1)fail('تعذر تحديد المشروع القديم المطابق بصورة آمنة.',409,'legacy_project_match_ambiguous',['client_name'=>$client['name'],'project_name'=>$name]);$existing=$matches[0]??null;
        if($existing){$projectId=(int)$existing['id'];$invoiceId=(int)($existing['invoice_id']??0)?:null;$notes=str_contains((string)($existing['notes']??''),'منقول من البرنامج القديم')?(string)$existing['notes']:$sourceNotes;$update->execute([legacyImportText($row['service_type']??'custom',40),$price,!empty($row['requires_booking'])?1:0,$progress,$status,$starts,$due,$notes,$projectId,$organizationId]);$reused++;}
        else{$insert->execute([$organizationId,$client['id'],$name,legacyImportText($row['service_type']??'custom',40),$price,!empty($row['requires_booking'])?1:0,$progress,$status,$starts,$due,$sourceNotes,$user['id']]);$projectId=(int)$pdo->lastInsertId();}
        if(packageMoneyCents($price)>0){$invoiceStatus=packageMoneyCents($paid)>=packageMoneyCents($price)?'paid':'issued';if($invoiceId){$invoice=$pdo->prepare('SELECT * FROM invoices WHERE id=? AND organization_id=? FOR UPDATE');$invoice->execute([$invoiceId,$organizationId]);$invoiceRow=$invoice->fetch();if(!$invoiceRow)fail('فاتورة المشروع القديم المطابق غير موجودة.',409,'legacy_project_invoice_missing');$allocated=$pdo->prepare('SELECT COALESCE(SUM(amount),0) FROM payment_allocations WHERE organization_id=? AND invoice_id=?');$allocated->execute([$organizationId,$invoiceId]);$allocatedAmount=(float)$allocated->fetchColumn();if($allocatedAmount>(float)$paid+.01)fail('دفعات المشروع الحالية تتجاوز رصيد النسخة القديمة، لذلك أوقفت المزامنة للمراجعة.',409,'legacy_project_payment_conflict');$pdo->prepare('UPDATE invoices SET status=?,subtotal=?,total=?,paid_amount=?,issued_at=?,due_at=? WHERE id=? AND organization_id=?')->execute([$invoiceStatus,$price,$price,$paid,$starts,$due,$invoiceId,$organizationId]);$pdo->prepare('UPDATE invoice_items SET description=?,unit_price=?,total=? WHERE invoice_id=?')->execute([$name,$price,$price,$invoiceId]);$missing=max(0,(float)$paid-$allocatedAmount);if(legacyImportOpeningPayment($pdo,$user,(int)$client['id'],(string)$client['name'],number_format($missing,2,'.',''),'cash','LEGACY-PRJ-'.$reference,null,$invoiceId,$starts))$payments++;}
            else{$invoiceNumber='LEG-'.strtoupper(substr(hash('sha256',$reference),0,12));$pdo->prepare('INSERT INTO invoices (organization_id,client_id,project_id,invoice_number,status,subtotal,discount,total,paid_amount,issued_at,due_at,notes,created_by) VALUES (?,?,?,?,?,?,0,?,?,?,?,?,?)')->execute([$organizationId,$client['id'],$projectId,$invoiceNumber,$invoiceStatus,$price,$price,$paid,$starts,$due,'فاتورة افتتاحية من البرنامج القديم',$user['id']]);$invoiceId=(int)$pdo->lastInsertId();$pdo->prepare('INSERT INTO invoice_items (invoice_id,description,quantity,unit,unit_price,total) VALUES (?,?,1,\'project\',?,?)')->execute([$invoiceId,$name,$price,$price]);$pdo->prepare('UPDATE projects SET invoice_id=? WHERE id=?')->execute([$invoiceId,$projectId]);if(legacyImportOpeningPayment($pdo,$user,(int)$client['id'],(string)$client['name'],$paid,'cash','LEGACY-PRJ-'.$reference,null,$invoiceId,$starts))$payments++;}}
        $map[$reference]=$projectId;$count++;legacyImportAudit($pdo,$user,'projects',$projectId,['legacy_reference'=>$reference,'client_id'=>(int)$client['id'],'invoice_id'=>$invoiceId,'reused_existing'=>(bool)$existing]);
    }
    return ['map'=>$map,'count'=>$count,'payments'=>$payments,'reused'=>$reused];
}
...
function legacyImportAppointments(PDO $pdo,array $user,array $rows,array $byPhone,array $byId,array $packageMap,array $projectMap,array $serviceMap,array $reuseMap=[]): array {
    ...
    foreach($rows as $row){if(!is_array($row))fail('موعد قديم غير صالح.',422,'invalid_legacy_booking');$client=legacyImportVerifiedClient($row,$byPhone,$byId);$reference=legacyImportText($row['legacy_reference']??'',120);$sourceBookingId=(int)($row['source_booking_id']??0);$date=legacyImportDate($row['date']??'');$start=normalizeBusinessTime($row['start_time']??'');$end=normalizeBusinessTime($row['end_time']??'',true);if($reference===''||$sourceBookingId<1||$start===''||$end===''||bookingDurationMinutes($start,$end)<=0)fail('وقت أو رقم الموعد القديم غير صحيح.',422,'invalid_legacy_booking_time');$packageId=!empty($row['package_reference'])?(int)($packageMap[$row['package_reference']]??0):null;$projectId=!empty($row['project_reference'])?(int)($projectMap[$row['project_reference']]??0):null;if(!empty($row['package_reference'])&&!$packageId)fail('تعذر ربط موعد قديم بالباقة.',409,'legacy_booking_package_missing');if(!empty($row['project_reference'])&&!$projectId)fail('تعذر ربط موعد قديم بالمشروع.',409,'legacy_booking_project_missing');$serviceId=$packageId?null:(int)($serviceMap['name:'.legacyImportArabicKey($row['source_service_name']??'')]??0);if($packageId){$s=$pdo->prepare('SELECT service_id FROM client_packages WHERE id=? AND organization_id=?');$s->execute([$packageId,$organizationId]);$serviceId=(int)$s->fetchColumn();}$resourceId=(int)($row['resource_id']??0);$resource=$pdo->prepare('SELECT id FROM resources WHERE id=? AND organization_id=? AND is_active=1');$resource->execute([$resourceId,$organizationId]);if(!$resource->fetch())fail('مورد المواعيد القديمة لم يعد متاحًا.',409,'legacy_booking_resource_missing');$status=(string)($row['status']??'confirmed');if(!in_array($status,['confirmed','completed'],true))$status='confirmed';$duration=max(1,(int)($row['duration_minutes']??bookingDurationMinutes($start,$end)));$actualHours=legacyImportQuantity($row['actual_hours']??0);$actualSeconds=$status==='completed'?max(0,(int)round($actualHours*3600)):0;$timerStart=$status==='completed'?$date.' '.$start.':00':null;$timerEnd=$status==='completed'?$date.' '.($end==='24:00'?'23:59':$end).':00':null;$notes=legacyImportText(($row['notes']??'').' — منقول من البرنامج القديم [قديم #'.$sourceBookingId.']',3000);
        $requested=legacyImportQuantity($row['requested_quantity']??0);$storedEnd=$end==='24:00'?'23:59':$end;
        $bookingId=(int)($reuseMap[$reference]??0);
        if($bookingId>0){
            $lockExisting->execute([$bookingId,$organizationId]);$existing=$lockExisting->fetch();
            if(!$existing||(int)$existing['client_id']!==(int)$client['id']||(int)($existing['client_package_id']??0)!==(int)$packageId)fail('تغيّر الموعد الحالي المطابق أثناء المزامنة.',409,'legacy_reused_booking_changed');
            if((string)$existing['status']==='completed'&&$status!=='completed')fail('لا يمكن إعادة موعد مكتمل إلى موعد قادم أثناء المزامنة.',409,'legacy_completed_booking_regression');
            $timingChanged=(string)$existing['date']!==$date||substr((string)$existing['start_time'],0,5)!==$start||substr((string)$existing['end_time'],0,5)!==$storedEnd||(int)($existing['resource_id']??0)!==$resourceId;
            if($timingChanged&&(string)$existing['status']==='confirmed'){
                releaseBookingSlots($pdo,$bookingId);
                if($status==='confirmed'&&$date>=$today){try{reserveBookingSlots($pdo,['id'=>$bookingId,'organization_id'=>$organizationId,'resource_id'=>$resourceId,'date'=>$date,'start_time'=>$start.':00','end_time'=>$storedEnd.':00']);}catch(PDOException $slotError){if(($slotError->errorInfo[1]??0)===1062)fail('التوقيت الجديد لموعد قديم يتعارض مع موعد محفوظ يوم '.$date.'. لم يتم تغيير أي بيانات.',409,'legacy_booking_conflict',['date'=>$date,'start_time'=>$start,'end_time'=>$storedEnd,'client_name'=>$client['name']]);throw $slotError;}}
            }
            if((string)$existing['status']==='confirmed'&&$status==='confirmed'&&$packageId){
                $currentHeld=bookingHeldQuantity($pdo,$bookingId,$packageId);$heldDelta=round($requested-$currentHeld,4);
                if(abs($heldDelta)>.0001){$packageStmt=$pdo->prepare('SELECT * FROM client_packages WHERE id=? AND organization_id=? FOR UPDATE');$packageStmt->execute([$packageId,$organizationId]);$lockedPackage=$packageStmt->fetch();if(!$lockedPackage)fail('الباقة المرتبطة بالموعد القديم غير موجودة.',409,'legacy_booking_package_missing');mutateLockedPackageQuantities($pdo,$lockedPackage,0,$heldDelta,0);insertPackageUsage($pdo,$lockedPackage,$bookingId,$heldDelta>0?'hold':'release',abs($heldDelta),'تحديث رصيد موعد أثناء مزامنة البرنامج القديم','legacy:'.$reference.':sync-held',$user['id']);}
            }
            $currentNotes=trim((string)($existing['notes']??''));$combinedNotes=str_contains($currentNotes,'[قديم #'.$sourceBookingId.']')?$currentNotes:legacyImportText($currentNotes.($currentNotes!==''?' — ':'').$notes,3000);
            $updateExisting->execute([$packageId?:null,$projectId?:null,$serviceId?:null,$resourceId,$client['name'],legacyImportText($row['source_service_name']??'خدمة',180),$date,$start.':00',$storedEnd.':00',$duration,$requested,$actualHours,max(0,(int)($row['actual_reels']??0)),$timerStart,$timerEnd,$actualSeconds,$requested,$status,legacyImportDate($row['delivery_date']??'',true),legacyImportMoney($row['custom_price']??0),legacyImportMoney($row['discount']??0),legacyImportText($row['discount_reason']??'',255)?:null,$combinedNotes,$user['id'],$date.' 12:00:00',$bookingId,$organizationId]);
            if((string)$existing['status']!==$status)$pdo->prepare('INSERT INTO booking_status_history (booking_id,from_status,to_status,note,changed_by,created_at) VALUES (?,?,?,?,?,?)')->execute([$bookingId,$existing['status'],$status,'مزامنة حالة الموعد من البرنامج القديم',$user['id'],$date.' 12:00:00']);
            $map[$reference]=$bookingId;$count++;$reused++;if($status==='completed')$completed++;legacyImportAudit($pdo,$user,'bookings',$bookingId,['legacy_reference'=>$reference,'client_id'=>(int)$client['id'],'source_booking_id'=>$sourceBookingId,'reused_existing'=>true]);continue;
        }
...
function legacyImportUsageLedger(PDO $pdo,array $user,array $packageRows,array $packageMap,array $appointmentRows,array $bookingMap,array $reusedPackages=[],array $reusedBookings=[]): int {
    ...
    foreach($packageRows as $pkg){
        ...
        if($isReused){$existingConsumption=$pdo->prepare("SELECT COALESCE(SUM(quantity),0) FROM package_usage_ledger WHERE client_package_id=? AND movement_type='consume'");$existingConsumption->execute([$packageId]);$consumedLedger=(float)$existingConsumption->fetchColumn();if($consumedLedger>$remaining+.0001)fail('استهلاك الباقة المسجل في البرنامج الجديد أكبر من النسخة القديمة، لذلك أوقفت المزامنة لحماية الرصيد.',409,'legacy_package_consumption_conflict');$remaining=max(0,round($remaining-$consumedLedger,4));}
...
function legacyImportFinance(PDO $pdo,array $user,array $rows): array {
    $count=0;$created=0;$updated=0;$preserved=0;$organizationId=(int)$user['organization_id'];$employeeMap=legacyImportEmployeeMap($pdo,$organizationId);
    $find=$pdo->prepare('SELECT * FROM finance WHERE organization_id=? AND detail LIKE ? ORDER BY id LIMIT 2 FOR UPDATE');
    ...
    $update=$pdo->prepare('UPDATE finance SET employee_user_id=?,type=?,entry_kind=?,category=?,amount=?,method=?,detail=?,date=?,entity=?,source_type=?,source_id=NULL,correlation_id=?,is_system=? WHERE id=? AND organization_id=?');
    foreach($rows as $row){
        if(!is_array($row))fail('حركة مالية قديمة غير صالحة.',422,'invalid_legacy_finance');
        $sourceId=(int)($row['source_finance_id']??0);if($sourceId<1)fail('رقم الحركة المالية القديمة مفقود.',422,'invalid_legacy_finance_source_id');
        $reference=legacyImportText($row['legacy_reference']??'',120);$kind=(string)($row['entry_kind']??'expense');if(!in_array($kind,['income','expense','transfer_in','transfer_out','advance_in','advance_out','settlement_out'],true))fail('تصنيف حركة مالية قديمة غير صحيح.',422,'invalid_legacy_finance_kind');
        $entity=legacyImportText($row['entity']??'الشركة',80)?:'الشركة';$entityKey=legacyImportArabicKey($entity);$employeeId=count($employeeMap[$entityKey]??[])===1?$employeeMap[$entityKey][0]:null;$isTransfer=in_array($kind,['transfer_in','transfer_out'],true);$correlation='legacy-finance-id:'.$sourceId;if(!empty($row['transfer_pair_reference']))$correlation='legacy-transfer:'.legacyImportText($row['transfer_pair_reference'],100).':'.($kind==='transfer_in'?'in':'out');$date=legacyImportDate($row['date']??'');$marker='[قديم #'.$sourceId.']';$detail=legacyImportText($row['detail']??'حركة من البرنامج القديم',220).' '.$marker;
        $find->execute([$organizationId,'%'.$marker.'%']);$matches=$find->fetchAll();if(count($matches)>1)fail('يوجد أكثر من سجل للحركة المالية القديمة رقم '.$sourceId.'؛ أوقفت المزامنة لمنع التكرار.',409,'legacy_finance_match_ambiguous',['source_finance_id'=>$sourceId]);$existing=$matches[0]??null;
        $values=[$employeeId,legacyImportText($row['type']??($kind==='income'?'إيراد':'مصروف'),48),$kind,legacyImportText($row['category']??'general_expense',80),legacyImportMoney($row['amount']??0),legacyImportText($row['method']??'cash',64),legacyImportText($detail,255),$date,$entity,$isTransfer?'internal_transfer':null,$correlation,$isTransfer?1:0];
        if($existing){if(!empty($existing['voided_at'])){$preserved++;$count++;continue;}$update->execute([...$values,(int)$existing['id'],$organizationId]);$updated++;}
        else{$insertValues=$values;array_splice($insertValues,10,0,[null]);$insert->execute([$organizationId,...$insertValues,$user['id'],$date.' 12:00:00']);$created++;}
        $count++;
    }
    return ['count'=>$count,'created'=>$created,'updated'=>$updated,'preserved'=>$preserved];
}

function legacyImportPreviousSourceRow(PDO $pdo,int $organizationId,string $table,string $key): ?array {
    $stmt=$pdo->prepare("SELECT r.payload_json FROM legacy_import_records r JOIN legacy_import_batches b ON b.id=r.batch_id AND b.organization_id=r.organization_id AND b.status='completed' WHERE r.organization_id=? AND r.source_table=? AND r.source_row_key=? ORDER BY r.batch_id DESC,r.id DESC LIMIT 1");$stmt->execute([$organizationId,$table,$key]);$payload=$stmt->fetchColumn();if($payload===false)return null;$decoded=json_decode((string)$payload,true);return is_array($decoded)?$decoded:null;
}

function legacyImportClientBalances(PDO $pdo,array $user,array $rows,array $byPhone,array $byId): array {
    $updated=0;$preserved=0;$organizationId=(int)$user['organization_id'];foreach($rows as $row){if(!is_array($row))continue;$client=legacyImportVerifiedClient($row,$byPhone,$byId);$sourceClientId=(int)($row['source_client_id']??0);$previous=$sourceClientId>0?legacyImportPreviousSourceRow($pdo,$organizationId,'clients',(string)$sourceClientId):null;$changes=[];$params=[];$pointsChanged=false;foreach(['debt','credit','points'] as $field){$old=(float)($client[$field]??0);$incoming=(float)($row[$field]??0);$canUpdate=$previous?abs($old-(float)($previous[$field]??0))<.0001:abs($old)<.0001;if($canUpdate&&abs($incoming-$old)>.0001){$changes[]="$field=?";$params[]=$field==='points'?max(0,(int)$incoming):legacyImportMoney(max(0,$incoming));if($field==='points')$pointsChanged=true;}elseif(abs($incoming-$old)>.0001)$preserved++;}$job=legacyImportText($row['job']??'',160);$previousJob=legacyImportText($previous['job']??'',160);if($job!==''&&(trim((string)($client['job']??''))===''||($previous&&trim((string)($client['job']??''))===$previousJob))){$changes[]='job=?';$params[]=$job;}$color=legacyImportText($row['color']??'',16);$previousColor=legacyImportText($previous['color']??'',16);if(preg_match('/^#[0-9a-f]{6}$/i',$color)&&(($client['color']??'#6D28D9')==='#6D28D9'||($previous&&strcasecmp((string)($client['color']??''),$previousColor)===0))){$changes[]='color=?';$params[]=$color;}if($pointsChanged&&!empty($row['points_updated_at'])){$changes[]='points_updated_at=?';$params[]=legacyImportDate($row['points_updated_at'],true);}if(!$changes)continue;$params[]=$client['id'];$params[]=$organizationId;$pdo->prepare('UPDATE clients SET '.implode(',',$changes).' WHERE id=? AND organization_id=?')->execute($params);$updated++;}
    return ['updated'=>$updated,'preserved_nonzero'=>$preserved];
}

function legacyImportReminders(PDO $pdo,array $user,array $rows): array {
    $count=0;$created=0;$updated=0;$organizationId=(int)$user['organization_id'];
    $find=$pdo->prepare('SELECT id FROM reminders WHERE organization_id=? AND description LIKE ? ORDER BY id LIMIT 2 FOR UPDATE');
    $findLegacy=$pdo->prepare("SELECT id FROM reminders WHERE organization_id=? AND title=? AND due_date=? AND description LIKE '%منقول من البرنامج القديم%' ORDER BY id LIMIT 2 FOR UPDATE");
    $insert=$pdo->prepare('INSERT INTO reminders (organization_id,title,description,type,due_date,status,recurrence,notify_before,is_recurring,amount,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?)');
    $update=$pdo->prepare('UPDATE reminders SET title=?,description=?,type=?,due_date=?,status=?,recurrence=?,notify_before=?,is_recurring=?,amount=? WHERE id=? AND organization_id=?');
    foreach($rows as $row){if(!is_array($row))continue;$sourceId=(int)($row['source_reminder_id']??0);if($sourceId<1)fail('رقم التذكير القديم مفقود.',422,'invalid_legacy_reminder_source_id');$due=str_replace('T',' ',legacyImportText($row['due_date']??'',19));if(!preg_match('/^\d{4}-\d{2}-\d{2}(?: \d{2}:\d{2}(?::\d{2})?)?$/',$due))fail('تاريخ تذكير قديم غير صحيح.',422,'invalid_legacy_reminder');if(strlen($due)===10)$due.=' 12:00:00';elseif(strlen($due)===16)$due.=':00';$recurring=!empty($row['is_recurring']);$title=legacyImportText($row['title']??'',255);$marker='[قديم #'.$sourceId.']';$description='منقول من البرنامج القديم '.$marker;$values=[$title,$description,legacyImportText($row['type']??'task',48),$due,($row['status']??'pending')==='completed'?'completed':'pending',$recurring?'monthly':null,max(0,(int)($row['notify_before']??0)),$recurring?1:0,legacyImportMoney($row['amount']??0)];$find->execute([$organizationId,'%'.$marker.'%']);$matches=$find->fetchAll();if(!$matches){$findLegacy->execute([$organizationId,$title,$due]);$matches=$findLegacy->fetchAll();}if(count($matches)>1)fail('يوجد أكثر من سجل للتذكير القديم رقم '.$sourceId.'؛ أوقفت المزامنة لمنع التكرار.',409,'legacy_reminder_match_ambiguous',['source_reminder_id'=>$sourceId]);if($matches){$update->execute([...$values,(int)$matches[0]['id'],$organizationId]);$updated++;}else{$insert->execute([$organizationId,...$values,$user['id']]);$created++;}$count++;}
...
function legacyFinanceSnapshot(PDO $pdo,int $organizationId,string $month): array {
    $month=legacyFinanceMonth($month);$start=$month.'-01';$end=(new DateTimeImmutable($start,new DateTimeZone('Africa/Cairo')))->modify('+1 month')->format('Y-m-d');$stmt=$pdo->prepare('SELECT entry_kind,category,amount,method,entity,date FROM finance WHERE organization_id=? AND voided_at IS NULL AND date>=? AND date<? ORDER BY date,id');$stmt->execute([$organizationId,$start,$end]);$opening=['cash'=>0.0,'vodafone_cash'=>0.0,'instapay'=>0.0,'bank_transfer'=>0.0];$closing=$opening;$income=0.0;$expense=0.0;
    foreach($stmt->fetchAll() as $row){$kind=(string)$row['entry_kind'];$amount=(float)$row['amount'];$method=(string)($row['method']?:'other');$sign=0;if($kind==='reversal'){$reversed=preg_replace('/^reversal_/','',(string)$row['category']);if(in_array($reversed,['income','advance_in','transfer_in'],true))$sign=-1;elseif($reversed==='expense')$sign=(string)$row['entity']==='الشركة'?1:0;elseif(in_array($reversed,['advance_out','settlement_out','transfer_out'],true))$sign=1;}elseif(in_array($kind,['income','advance_in','transfer_in'],true))$sign=1;elseif($kind==='expense')$sign=(string)$row['entity']==='الشركة'?-1:0;elseif(in_array($kind,['advance_out','settlement_out','transfer_out'],true))$sign=-1;$closing[$method]=($closing[$method]??0)+$amount*$sign;if($kind==='income'||$kind==='advance_in')$income+=$amount;elseif(in_array($kind,['expense','advance_out','settlement_out'],true))$expense+=$amount;elseif($kind==='reversal'){if($sign<0)$income-=$amount;elseif($sign>0)$expense-=$amount;}}
    foreach($closing as $method=>$amount)$closing[$method]=round($amount,2);
