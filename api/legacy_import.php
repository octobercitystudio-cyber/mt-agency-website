...
function legacyImportPackages(PDO $pdo,array $user,array $rows,array $byPhone,array $byId,array $serviceMap): array {
    ...
    foreach($rows as $row){if(!is_array($row))fail('سجل باقة قديم غير صالح.',422,'invalid_legacy_package');$client=legacyImportVerifiedClient($row,$byPhone,$byId);$service=legacyImportPackageService($pdo,$organizationId,$row,$serviceMap);$reference=legacyImportText($row['legacy_reference']??'',120);if($reference==='')fail('مرجع الباقة القديمة مفقود.',422,'invalid_legacy_package');$unit=(string)($row['billing_unit']??'hour');if(!in_array($unit,['hour','reel'],true))fail('وحدة باقة قديمة غير صحيحة.',422,'invalid_legacy_package_unit');$purchased=legacyImportQuantity($row['purchased_quantity']??0);$consumed=min($purchased,legacyImportQuantity($row['consumed_quantity']??0));$due=legacyImportQuantity($row['payment_due_quantity']??0);$minutes=$unit==='hour'?(int)round($purchased*60):null;$consumedMinutes=$unit==='hour'?(int)round($consumed*60):null;$dueMinutes=$unit==='hour'?(int)round($due*60):null;$starts=legacyImportDate($row['starts_at']??'');$expires=legacyImportDate($row['expires_at']??'');if($expires<$starts)fail('نهاية صلاحية باقة قديمة تسبق بدايتها.',422,'invalid_legacy_package_dates');$status=(string)($row['status']??'active');if(!in_array($status,['active','expired','completed'],true))$status='expired';$total=legacyImportMoney($row['total_price']??0);$paid=legacyImportMoney($row['paid_amount']??0);$notes='منقول من البرنامج القديم — المرجع '.$reference;if((float)($row['overage_quantity']??0)>0)$notes.=' — تجاوز قديم محفوظ: '.legacyImportQuantity($row['overage_quantity']);
        $insert->execute([$organizationId,$client['id'],$service['id'],legacyImportText($row['source_service_name']??$service['name'],180),$notes,$unit,$purchased,$minutes,$consumed,$consumedMinutes,$due,$dueMinutes,max(1,(int)($row['validity_days_snapshot']??1)),max(0,min(100,(float)($service['deposit_percent']??0))),legacyImportMoney($service['overage_price']??0),$total,$paid,$starts,$expires,$status]);$packageId=(int)$pdo->lastInsertId();$map[$reference]=$packageId;$count++;
        $history=is_array($row['source_payment_history']??null)?$row['source_payment_history']:[];$firstDate=$starts;foreach($history as $paymentRow){if(!empty($paymentRow['date'])){$firstDate=legacyImportDate($paymentRow['date']);break;}}$method='cash';$paymentId=legacyImportOpeningPayment($pdo,$user,(int)$client['id'],(string)$client['name'],$paid,$method,'LEGACY-PKG-'.$reference,$packageId,null,$firstDate);if($paymentId)$payments++;
...
function legacyImportAppointments(PDO $pdo,array $user,array $rows,array $byPhone,array $byId,array $packageMap,array $projectMap,array $serviceMap): array {
    $map=[];$count=0;$completed=0;$reserved=0;$held=0;$organizationId=(int)$user['organization_id'];$today=cairoNow()->format('Y-m-d');
    ...
    return ['map'=>$map,'count'=>$count,'completed'=>$completed,'reserved'=>$reserved,'held'=>$held];
}

function legacyImportUsageLedger(PDO $pdo,array $user,array $packageRows,array $packageMap,array $appointmentRows,array $bookingMap): int {
    $appointments=[];
    ...
    foreach($packageRows as $pkg){
        ...
        $reference=(string)($pkg['legacy_reference']??'');$packageId=(int)($packageMap[$reference]??0);$remaining=legacyImportQuantity($pkg['consumed_quantity']??0);$unit=(string)($pkg['billing_unit']??'hour');$package=['id'=>$packageId,'billing_unit'=>$unit];
        insertPackageUsage($pdo,$package,null,'opening',legacyImportQuantity($pkg['purchased_quantity']??0),'رصيد افتتاحي من البرنامج القديم','legacy:'.$reference.':opening',$user['id']);$count++;
        foreach($appointments[$reference]??[] as $booking){if(($booking['status']??'')!=='completed'||$remaining<=0)continue;$quantity=$unit==='reel'?max(0,(int)($booking['actual_reels']??0)):legacyImportQuantity($booking['actual_hours']??0);$quantity=min($remaining,$quantity);if($quantity<=0)continue;insertPackageUsage($pdo,$package,(int)$bookingMap[$booking['legacy_reference']],'consume',$quantity,'استهلاك جلسة من البرنامج القديم','legacy:'.$reference.':booking:'.$booking['legacy_reference'],$user['id']);$remaining=round($remaining-$quantity,4);$count++;}
