...
if ($path === '/cron/push-queue' && $method === 'POST') {
    ...
    respond(['processed'=>count($jobs),'sent_jobs'=>$sent,'failed_jobs'=>$failed,'delivered_devices'=>$devices]);
}
...
if ($path === '/payment-proofs' && $method === 'POST') {
    ...
    $pdo->beginTransaction();try{
        ...
        audit($pdo,$user,'create','payment_proofs',$id,null,['client_id'=>$clientId,'client_package_id'=>$packageId?:null,'invoice_id'=>$invoiceId?:null,'amount'=>$amount]);$pdo->commit();respond(['id'=>$id,'status'=>'pending'],201);
