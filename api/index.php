...
if ($path === '/dashboard/kpis' && $method === 'GET') {
    ...
    if($canViewFinance){try{
        $stmt=$pdo->prepare("SELECT id,client_id,status,GREATEST(ROUND(total*100)-ROUND(paid_amount*100),0) due_cents FROM invoices WHERE organization_id=? AND status NOT IN ('cancelled','void')");$stmt->execute([$organizationId]);$activeInvoices=[];$migratedByClient=[];$invoiceCents=0;
        foreach($stmt->fetchAll() as $invoice){$due=(int)$invoice['due_cents'];$invoiceCents+=$due;$activeInvoices[(int)$invoice['id']]=true;$clientId=(int)$invoice['client_id'];$migratedByClient[$clientId]=($migratedByClient[$clientId]??0)+$due;}
        $stmt=$pdo->prepare("SELECT client_id,ROUND(total_price*100) total_cents,ROUND(overage_amount*100) overage_cents,ROUND(paid_amount*100) paid_cents,source_invoice_id FROM client_packages WHERE organization_id=? AND status NOT IN ('archived','cancelled','draft')");$stmt->execute([$organizationId]);$packageCents=0;
        foreach($stmt->fetchAll() as $package){$totalCents=max(0,(int)$package['total_cents']);$paidCents=max(0,(int)$package['paid_cents']);$overageCents=max(0,(int)$package['overage_cents']);$fullDueCents=max(0,$totalCents+$overageCents-$paidCents);$baseDueCents=max(0,$totalCents-$paidCents);$source=(int)($package['source_invoice_id']??0);$due=$source>0&&isset($activeInvoices[$source])?max(0,$fullDueCents-$baseDueCents):$fullDueCents;$packageCents+=$due;$clientId=(int)$package['client_id'];$migratedByClient[$clientId]=($migratedByClient[$clientId]??0)+$due;}
        $stmt=$pdo->prepare("SELECT id,ROUND(GREATEST(debt,0)*100) debt_cents FROM clients WHERE organization_id=? AND status<>'archived'");$stmt->execute([$organizationId]);$legacyDebtCents=0;$legacyReconciledCents=0;
        foreach($stmt->fetchAll() as $client){$legacy=max(0,(int)$client['debt_cents']);$migrated=$migratedByClient[(int)$client['id']]??0;$legacyDebtCents+=max(0,$legacy-$migrated);$legacyReconciledCents+=min($legacy,$migrated);}
        $receivables=['available'=>true,'amount'=>packageMoney($invoiceCents+$packageCents+$legacyDebtCents),'invoice_amount'=>packageMoney($invoiceCents),'direct_package_and_overage_amount'=>packageMoney($packageCents),'legacy_client_debt_amount'=>packageMoney($legacyDebtCents),'legacy_unreconciled_amount'=>packageMoney($legacyDebtCents),'legacy_reconciled_excluded_amount'=>packageMoney($legacyReconciledCents)];
