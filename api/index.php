...
if ($path === '/finance/manual' && $method === 'POST') {
    ...
    $pdo->beginTransaction();try{$type=match($kind){'income'=>'إيراد','advance_in'=>'سداد سلفة','advance_out'=>'سحب سلفة','settlement_out'=>'سداد مستحقات',default=>'مصروف'};$stmt=$pdo->prepare('INSERT INTO finance (organization_id,client_id,employee_user_id,type,entry_kind,category,amount,method,detail,date,entity,source_type,source_id,is_system,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,0,?)');$stmt->execute([$user['organization_id'],$clientId,$employeeUserId,$type,$kind,$category,$amount,$method,$detail,$date,$entity?:'الشركة',$sourceType,$sourceId,$user['id']]);$id=(int)$pdo->lastInsertId();audit($pdo,$user,'create_manual_finance','finance',$id,null,['client_id'=>$clientId,'employee_user_id'=>$employeeUserId,'entry_kind'=>$kind,'category'=>$category,'amount'=>$amount,'source_type'=>$sourceType,'source_id'=>$sourceId]);$read=$pdo->prepare('SELECT * FROM finance WHERE id=? AND organization_id=?');$read->execute([$id,$user['organization_id']]);$entry=enrichedFinanceEntry($pdo,(int)$user['organization_id'],$read->fetch());$pdo->commit();respond($entry,201);}catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
