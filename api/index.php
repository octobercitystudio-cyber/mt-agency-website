...
if (preg_match('#^/owner/clients/(\d+)/balance-adjustment$#',$path,$m) && $method === 'POST') {
    $user=requireUser($user);requireRole($user,['owner']);$organizationId=(int)$user['organization_id'];$clientId=(int)$m[1];$payload=body();$action=(string)($payload['action']??'');$amountCents=packageMoneyCents($payload['amount']??0);$methodValue=trim((string)($payload['method']??''));$key=trim((string)($payload['idempotency_key']??''));
    if(!in_array($action,['pay_debt','add_credit'],true)||$amountCents<=0||!in_array($methodValue,['cash','bank_transfer','vodafone_cash','instapay'],true)||!preg_match('/^[A-Za-z0-9:_-]{12,120}$/',$key))fail('بيانات حركة رصيد العميل غير صحيحة.',422,'invalid_client_balance_adjustment');$correlation='owner-client-balance:'.$key;$date=cairoNow()->format('Y-m-d');
    $pdo->beginTransaction();try{
...
if (preg_match('#^/booking-blocks/(\d+)/convert$#',$path,$m)&&$method==='POST') {
    ...
    $pdo->beginTransaction();try{
        ...
        if($mode==='existing_package'){
            ...
            $overageCents=strictPackageMoneyCents($newPackage['overage_price_snapshot'],'سعر الساعة الإضافية');$priceCents=strictPackageMoneyCents($newPackage['total_price'],'إجمالي السعر');$paidCents=strictPackageMoneyCents($newPackage['paid_amount'],'المدفوع الآن');if($paidCents>$priceCents)$abort('المدفوع الآن لا يمكن أن يتجاوز إجمالي السعر.',422,'invalid_payment_amount');$method=trim((string)($newPackage['payment_method']??''));if(!in_array($method,['cash','bank_transfer','vodafone_cash','instapay'],true))$abort('طريقة الدفع غير صحيحة.',422,'invalid_payment_method');$notes=trim((string)($newPackage['notes']??''));
...
if (preg_match('#^/payment-proofs/(\d+)/decision$#',$path,$m)&&$method==='POST'){
    ...
    $pdo->beginTransaction();try{
        ...
        if($status==='approved'){
            ...
            $proofMethod=in_array((string)($proof['payment_method']??''),['instapay','vodafone_cash'],true)?(string)$proof['payment_method']:'bank_transfer';
            $methodLabel=['instapay'=>'إنستاباي','vodafone_cash'=>'فودافون كاش','bank_transfer'=>'تحويل بنكي'][$proofMethod];
...
if ($path === '/payment-proofs' && $method === 'POST') {
    ...
    $paymentMethod=trim((string)($_POST['payment_method']??'instapay'));$transferAccounts=['instapay'=>'01114466646','vodafone_cash'=>'01094084424'];
    if(!isset($transferAccounts[$paymentMethod]))fail('اختر إنستاباي أو فودافون كاش.',422,'invalid_payment_method');$transferAccount=$transferAccounts[$paymentMethod];
