...
function bookingBlockDates(string $startDate, bool $repeatDaily, string $repeatUntil): array {
    ...
    if(!$repeatDaily)return [$startDate];
...
if ($path === '/sync' && $method === 'GET') {
    ...
    $scopeSql="organization_id=?";$scopeParams=[$organizationId];if($clientScope){$scopeSql.=" AND (client_id=? OR topic='services' OR (client_id IS NULL AND topic='post_production' AND entity_type='pickup_schedule'))";$scopeParams[]=(int)$user['client_id'];}
...
if ($path==='/booking-blocks'&&$method==='POST') {
    ...
    $dates=bookingBlockDates($date,$repeatDaily,$repeatUntil);$normalized=['date'=>$date,'start_time'=>$start,'end_time'=>$end,'resource_id'=>$resourceId,'repeat_daily'=>$repeatDaily,'repeat_until'=>$repeatDaily?$repeatUntil:null,'title'=>$title,'note'=>$note];$requestHash=hash('sha256',json_encode($normalized,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
    $pdo->beginTransaction();try{
