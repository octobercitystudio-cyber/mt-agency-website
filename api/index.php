...
function schemaColumnExists(PDO $pdo, string $table, string $column): bool {
    ...
    $stmt=$pdo->prepare('SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=? AND COLUMN_NAME=?');$stmt->execute([$table,$column]);return $cache[$key]=(int)$stmt->fetchColumn()>0;
}
...
if ($path === '/dashboard/kpis' && $method === 'GET') {
    ...
    if($canViewFinance){try{
        $stmt=$pdo->prepare("SELECT COALESCE(SUM(GREATEST(total-paid_amount,0)),0) FROM invoices WHERE organization_id=? AND status NOT IN ('cancelled','void')");$stmt->execute([$organizationId]);$invoiceCents=packageMoneyCents($stmt->fetchColumn());
        $stmt=$pdo->prepare("SELECT total_price,overage_amount,paid_amount,source_invoice_id FROM client_packages WHERE organization_id=? AND status NOT IN ('archived','cancelled','draft')");$stmt->execute([$organizationId]);$packageCents=0;
        foreach($stmt->fetchAll() as $package){$totalCents=max(0,packageMoneyCents($package['total_price']));$paidCents=max(0,packageMoneyCents($package['paid_amount']));$overageCents=max(0,packageMoneyCents($package['overage_amount']));$fullDueCents=max(0,$totalCents+$overageCents-$paidCents);$baseDueCents=max(0,$totalCents-$paidCents);$packageCents+=empty($package['source_invoice_id'])?$fullDueCents:max(0,$fullDueCents-$baseDueCents);}
        $stmt=$pdo->prepare("SELECT COALESCE(SUM(GREATEST(debt,0)),0) FROM clients WHERE organization_id=? AND status<>'archived'");$stmt->execute([$organizationId]);$legacyDebtCents=packageMoneyCents($stmt->fetchColumn());
        $receivables=['available'=>true,'amount'=>packageMoney($invoiceCents+$packageCents+$legacyDebtCents),'invoice_amount'=>packageMoney($invoiceCents),'direct_package_and_overage_amount'=>packageMoney($packageCents),'legacy_client_debt_amount'=>packageMoney($legacyDebtCents)];

        $stmt=$pdo->prepare("SELECT type,entry_kind,amount FROM finance WHERE organization_id=? AND date LIKE ?");$stmt->execute([$organizationId,$month.'%']);$cashInCents=0;$cashOutCents=0;
        foreach($stmt->fetchAll() as $entry){$amountCents=max(0,packageMoneyCents($entry['amount']));if(in_array((string)$entry['entry_kind'],['income','transfer_in','advance_in'],true)||in_array((string)$entry['type'],['إيراد','سداد سلفة','income'],true))$cashInCents+=$amountCents;else $cashOutCents+=$amountCents;}
        $cashMovement=['available'=>true,'cash_in'=>packageMoney($cashInCents),'cash_out'=>packageMoney($cashOutCents)];
    ...
    if($canViewPackages){try{
        ...
        $stmt=$pdo->prepare("SELECT COUNT(*) FROM client_packages WHERE organization_id=? AND status='active' AND (expires_at IS NULL OR expires_at>=?)");$stmt->execute([$organizationId,$today]);$activeCount=(int)$stmt->fetchColumn();
        $stmt=$pdo->prepare("SELECT COUNT(*) FROM client_packages WHERE organization_id=? AND status='active' AND expires_at BETWEEN ? AND ?");$stmt->execute([$organizationId,$today,$soon]);$expiringCount=(int)$stmt->fetchColumn();
...
if (preg_match('#^/client-packages/(\d+)/details$#',$path,$m)&&$method==='GET') {
    ...
    $purchased=max(0,(float)$package['purchased_quantity']);$consumed=$authoritativeConsumed;$held=max(0,(float)$package['held_quantity']);$remaining=max(0,$purchased-$consumed);$available=max(0,$remaining-$held);$calendarDays=remainingPackageCalendarDays($package['expires_at']??null,$today);$effectiveStatus=$package['status']==='active'&&!empty($package['expires_at'])&&substr((string)$package['expires_at'],0,10)<$today?'expired':$package['status'];
    respond([
...
if ($path === '/reschedule-requests' && $method === 'POST') {
    ...
    $pdo->beginTransaction();try{
        ...
        $minutes=bookingDurationMinutes($start,$end);$minimum=max(15,(int)$booking['minimum_booking_minutes']);$increment=max(15,(int)$booking['booking_increment_minutes']);if(!preg_match('/^\d{4}-\d{2}-\d{2}$/',$date)||$minutes<$minimum||$minutes%$increment!==0||!validBusinessBooking($start,$end,$minimum)){$pdo->rollBack();fail('الموعد المقترح لا يطابق حدود الحجز المحددة للخدمة.',422);}
