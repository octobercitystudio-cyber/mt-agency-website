...
function deleteEligibleBooking(PDO $pdo, array $user, array $booking): float {
    ...
    appNotification($pdo,$organizationId,(int)$booking['client_id'],'client','booking_deleted','تم حذف موعد','أزيل أحد مواعيدك المجدولة من قائمة الحجوزات.','bookings',$bookingId,'booking:'.$bookingId.':deleted','warning','schedule');
...
if (preg_match('#^/client-packages/(\d+)/details$#',$path,$m)&&$method==='GET') {
    ...
    respond([
      'package'=>['id'=>(int)$package['id'],'name'=>$package['name'],'notes'=>$package['notes'],'billing_unit'=>$package['billing_unit'],'status'=>$package['status'],'effective_status'=>$effectiveStatus,'version'=>(int)($package['version']??1),'validity_mode_snapshot'=>$package['validity_mode_snapshot']??'rolling','validity_days_snapshot'=>(int)($package['validity_days_snapshot']??1),'source_invoice_id'=>$package['source_invoice_id']?(int)$package['source_invoice_id']:null,'invoice_number'=>$package['invoice_number'],'client'=>['id'=>(int)$package['client_id'],'name'=>$package['client_name'],'phone'=>$package['client_phone']],'service'=>['id'=>(int)$package['service_id'],'name'=>$package['service_name']]],
...
if ($path === '/client-packages' && $method === 'POST') {
    ...
    $pdo->beginTransaction();try{
        ...
        $firstBookingDate=$bookings?(string)$bookings[0]['date']:'';$starts=$firstBookingDate?:null;$expires=$firstBookingDate?packageValidityEnd($firstBookingDate,$validity,$validityMode):null;$shootingDate=$validityMode==='shooting_day'?$firstBookingDate:'';
...
if (preg_match('#^/client-packages/(\d+)$#',$path,$m)&&$method==='PATCH') {
    $user=requireUser($user);requireRole($user,['owner']);$id=(int)$m[1];$payload=body();$reason=ownerCorrectionReason($payload);$pdo->beginTransaction();try{
        ...
        $name=trim((string)($payload['name']??$before['name']));$notes=array_key_exists('notes',$payload)?trim((string)$payload['notes']):$before['notes'];$starts=trim((string)($payload['starts_at']??$before['starts_at']??''));$expires=trim((string)($payload['expires_at']??$before['expires_at']??''));$status=(string)($payload['status']??$before['status']);$pendingValidity=$starts===''&&$expires==='';
        $datesValid=$pendingValidity||(preg_match('/^\d{4}-\d{2}-\d{2}$/',$starts)&&preg_match('/^\d{4}-\d{2}-\d{2}$/',$expires)&&$expires>=$starts);
        if($name===''||!$datesValid||!in_array($status,['draft','active','expired','suspended','completed','cancelled','archived'],true)){$pdo->rollBack();fail('بيانات الباقة الوصفية أو تواريخها غير صحيحة.',422,'invalid_package_details');}
        if(!$pendingValidity&&($before['validity_mode_snapshot']??'rolling')==='shooting_day'&&$starts!==$expires){$pdo->rollBack();fail('الباقة اليومية يجب أن تبدأ وتنتهي في يوم التصوير نفسه.',422,'shooting_day_date_mismatch');}
        ...
        $startsValue=$pendingValidity?null:$starts;$expiresValue=$pendingValidity?null:$expires;$pdo->prepare('UPDATE client_packages SET name=?,notes=?,starts_at=?,expires_at=?,status=?,version=version+1 WHERE id=? AND organization_id=?')->execute([$name,$notes?:null,$startsValue,$expiresValue,$status,$id,$user['organization_id']]);
        $after=$before+[];$after['name']=$name;$after['notes']=$notes;$after['starts_at']=$startsValue;$after['expires_at']=$expiresValue;$after['status']=$status;$after['version']=(int)($before['version']??1)+1;ownerAdjustment($pdo,$user,'client_packages',$id,'details',0,0,$reason,$before,$after);audit($pdo,$user,'owner_update_package','client_packages',$id,$before,$after+['reason'=>$reason]);queueClientWhatsAppSummary($pdo,(int)$user['organization_id'],(int)$before['client_id']);$pdo->commit();respond(['id'=>$id,'updated'=>true,'version'=>$after['version']]);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
