...
function employeeFinanceAccountSnapshot(PDO $pdo,int $organizationId,string $month): array {
    ...
    $rows=$pdo->prepare("SELECT id,employee_user_id,entry_kind,category,amount,method,detail,date,entity,source_type,source_id,correlation_id,is_system,created_at FROM finance WHERE organization_id=? AND employee_user_id IS NOT NULL AND voided_at IS NULL AND entry_kind<>'reversal' ORDER BY date DESC,id DESC");
    ...
    $unlinked=$pdo->prepare("SELECT COUNT(*) FROM finance WHERE organization_id=? AND employee_user_id IS NULL AND voided_at IS NULL AND entry_kind<>'reversal' AND TRIM(entity) IN ('اشرف','أشرف','مروة')");$unlinked->execute([$organizationId]);
    return ['month'=>$month,'accounts'=>$accounts,'unlinked_legacy_count'=>(int)$unlinked->fetchColumn()];
}
