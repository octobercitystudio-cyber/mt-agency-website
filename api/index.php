...
if ($path === '/client-packages/legacy-import' && $method === 'POST') {
    $user=requireUser($user);requireRole($user,['owner']);$payload=body();$organizationId=(int)$user['organization_id'];
    $reference=trim((string)($payload['legacy_reference']??''));if($reference===''||mb_strlen($reference)>120)fail('مرجع الباقة القديمة مطلوب وبحد أقصى 120 حرفًا.',422,'invalid_legacy_reference');
    $clientId=(int)($payload['client_id']??0);$serviceId=(int)($payload['service_id']??0);$name=trim((string)($payload['package_name']??''));$unit=(string)($payload['billing_unit']??'');$status=(string)($payload['status']??'active');
    if($name===''||mb_strlen($name)>180)fail('اسم الباقة مطلوب وبحد أقصى 180 حرفًا.',422,'invalid_package_name');
    if(!in_array($unit,['hour','reel'],true))fail('وحدة الرصيد يجب أن تكون ساعة أو ريل.',422,'invalid_billing_unit');
    if(!in_array($status,['active','suspended','completed','expired'],true))fail('حالة الباقة القديمة غير صحيحة.',422,'invalid_legacy_status');
    $quantityText=trim((string)($payload['purchased_quantity']??''));$consumedText=trim((string)($payload['consumed_quantity']??'0'));$dueText=trim((string)($payload['payment_due_quantity']??'0'));
    foreach([$quantityText,$consumedText,$dueText] as $quantityValue)if(!preg_match('/^\d+(?:\.\d{1,4})?$/',$quantityValue))fail('أحد حقول الرصيد غير صحيح.',422,'invalid_legacy_quantity');
    $quantity=(float)$quantityText;$consumed=(float)$consumedText;$paymentDue=(float)$dueText;if($quantity<=0||$consumed<0||$consumed>$quantity+0.000001||$paymentDue<0||$paymentDue>$quantity+0.000001)fail('المستخدم وحد السداد يجب أن يكونا بين صفر وإجمالي الرصيد.',422,'invalid_legacy_quantity');
    if($unit==='reel'&&(!ctype_digit($quantityText)||!ctype_digit($consumedText)||!ctype_digit($dueText)))fail('أعداد الريلز يجب أن تكون أرقامًا صحيحة.',422,'invalid_reel_quantity');
    foreach(['total_price'=>'إجمالي السعر','paid_amount'=>'المبلغ المدفوع'] as $moneyField=>$moneyLabel)if(!array_key_exists($moneyField,$payload))fail('حقل '.$moneyLabel.' مطلوب كنص عشري صريح.',422,'invalid_money_format');
    $totalCents=strictPackageMoneyCents($payload['total_price'],'إجمالي السعر');$paidCents=strictPackageMoneyCents($payload['paid_amount'],'المبلغ المدفوع');if($totalCents<0||$paidCents<0||$paidCents>$totalCents)fail('المدفوع يجب أن يكون بين صفر وإجمالي السعر.',422,'invalid_payment_amount');
    $starts=trim((string)($payload['starts_at']??''));$expires=trim((string)($payload['expires_at']??''));$startDate=DateTimeImmutable::createFromFormat('!Y-m-d',$starts);$expiryDate=DateTimeImmutable::createFromFormat('!Y-m-d',$expires);
    if(!$startDate||!$expiryDate||$startDate->format('Y-m-d')!==$starts||$expiryDate->format('Y-m-d')!==$expires||$expires<$starts)fail('تاريخ بداية أو نهاية الصلاحية غير صحيح.',422,'invalid_package_dates');
    $validityDays=(int)$startDate->diff($expiryDate)->days+1;if($validityDays<1||$validityDays>3650)fail('فترة الصلاحية القديمة خارج النطاق المسموح.',422,'invalid_package_dates');
    $notes=trim((string)($payload['notes']??''));if(mb_strlen($notes)>2000)fail('ملاحظات الاستيراد طويلة جدًا.',422,'legacy_notes_too_long');
    $normalized=['legacy_reference'=>$reference,'client_id'=>$clientId,'service_id'=>$serviceId,'package_name'=>$name,'billing_unit'=>$unit,'purchased_quantity'=>$quantityText,'consumed_quantity'=>$consumedText,'total_price'=>packageMoney($totalCents),'paid_amount'=>packageMoney($paidCents),'starts_at'=>$starts,'expires_at'=>$expires,'payment_due_quantity'=>$dueText,'status'=>$status,'notes'=>$notes];
    $requestHash=hash('sha256',json_encode($normalized,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));$idempotencyKey='legacy-package-import:'.hash('sha256',mb_strtolower($reference,'UTF-8'));
    $readiness=packageSaleSchemaReadiness($pdo);if(!$readiness['ready'])fail('بيئة الباقات غير جاهزة للاستيراد. شغّل ترحيلات الباقات حتى 026 ثم أعد المحاولة.',503,'package_sales_not_ready');
    $pdo->beginTransaction();try{
        ...
        if($prior){if(!hash_equals((string)$prior['request_hash'],$requestHash)){$pdo->rollBack();fail('مرجع الباقة القديمة مستخدم لبيانات مختلفة. غيّر المرجع أو راجع الباقة المُرحّلة.',409,'legacy_reference_mismatch');}if($prior['status']==='completed'&&!empty($prior['response_json'])){$response=json_decode((string)$prior['response_json'],true)?:[];$response['idempotent']=true;$pdo->commit();respond($response);} $pdo->rollBack();fail('ترحيل الباقة نفسها قيد التنفيذ.',409,'request_in_progress');}
        $clientStmt=$pdo->prepare('SELECT id,name FROM clients WHERE id=? AND organization_id=? FOR UPDATE');$clientStmt->execute([$clientId,$organizationId]);$client=$clientStmt->fetch();if(!$client){$pdo->rollBack();fail('العميل غير موجود أو لا يخص هذه الشركة.',404,'client_not_found');}
        $serviceStmt=$pdo->prepare('SELECT * FROM services WHERE id=? AND organization_id=? AND is_active=1 FOR UPDATE');$serviceStmt->execute([$serviceId,$organizationId]);$service=$serviceStmt->fetch();if(!$service){$pdo->rollBack();fail('قالب الخدمة غير موجود أو غير نشط.',404,'service_not_found');}
        if(!isStudioPackageOfferItem($service)||normalizedStudioPackageUnit($service)!==$unit){$pdo->rollBack();fail('قالب الخدمة لا يطابق نوع رصيد الباقة القديمة.',422,'package_service_unit_mismatch');}
        ...
        $purchasedMinutes=$unit==='hour'?(int)round($quantity*60):null;$consumedMinutes=$unit==='hour'?(int)round($consumed*60):null;$dueMinutes=$unit==='hour'?(int)round($paymentDue*60):null;$validityMode=(string)($service['package_validity_mode']??'rolling');if($validityMode==='shooting_day'&&$starts!==$expires)$validityMode='rolling';if(!in_array($validityMode,['rolling','shooting_day'],true))$validityMode='rolling';
        $noteParts=array_values(array_filter([$notes,'مُرحّلة من البرنامج القديم — المرجع: '.$reference]));$storedNotes=implode("\n",$noteParts);
        $insert=$pdo->prepare("INSERT INTO client_packages (organization_id,client_id,service_id,name,notes,billing_unit,purchased_quantity,purchased_minutes,held_quantity,held_minutes,consumed_quantity,consumed_minutes,payment_due_quantity,payment_due_minutes,deposit_percent_snapshot,overage_price_snapshot,total_price,paid_amount,starts_at,expires_at,validity_mode_snapshot,validity_days_snapshot,status) VALUES (?,?,?,?,?,?,?, ?,0,0,?, ?,?,?,0,0,?,?,?,?,?,?,?)");
        $insert->execute([$organizationId,$clientId,$serviceId,$name,$storedNotes,$unit,$unit==='hour'?settlementHours($purchasedMinutes):$quantity,$purchasedMinutes,$unit==='hour'?settlementHours($consumedMinutes):$consumed,$consumedMinutes,$unit==='hour'?settlementHours($dueMinutes):$paymentDue,$dueMinutes,packageMoney($totalCents),packageMoney($paidCents),$starts,$expires,$validityMode,$validityDays,$status]);$packageId=(int)$pdo->lastInsertId();
        insertPackageUsage($pdo,['id'=>$packageId,'billing_unit'=>$unit],null,'opening',$unit==='hour'?settlementHours($purchasedMinutes):$quantity,'رصيد افتتاحي مرحّل من البرنامج القديم','legacy-import:'.$requestId.':opening',$user['id']);
        if($consumed>0)insertPackageUsage($pdo,['id'=>$packageId,'billing_unit'=>$unit],null,'consume',$unit==='hour'?settlementHours($consumedMinutes):$consumed,'استهلاك سابق مرحّل من البرنامج القديم','legacy-import:'.$requestId.':consumed',$user['id']);
        $after=['client_id'=>$clientId,'service_id'=>$serviceId,'name'=>$name,'billing_unit'=>$unit,'purchased_quantity'=>$unit==='hour'?settlementHours($purchasedMinutes):$quantity,'consumed_quantity'=>$unit==='hour'?settlementHours($consumedMinutes):$consumed,'remaining_quantity'=>$unit==='hour'?settlementHours($purchasedMinutes-$consumedMinutes):$quantity-$consumed,'total_price'=>packageMoney($totalCents),'paid_amount'=>packageMoney($paidCents),'outstanding_amount'=>packageMoney($totalCents-$paidCents),'starts_at'=>$starts,'expires_at'=>$expires,'status'=>$status,'legacy_reference'=>$reference,'opening_balance'=>true,'posted_to_current_finance'=>false];
        audit($pdo,$user,'legacy_import','client_packages',$packageId,null,$after);recordChangeEvent($pdo,$organizationId,$clientId,'client_packages','client_packages',$packageId,'created');
        $response=['id'=>$packageId,'legacy_reference'=>$reference,'client_id'=>$clientId,'client_name'=>$client['name'],'paid_amount'=>packageMoney($paidCents),'outstanding_amount'=>packageMoney($totalCents-$paidCents),'opening_balance'=>true,'finance_entry_created'=>false,'idempotent'=>false];
        $pdo->prepare("UPDATE client_package_sale_requests SET status='completed',response_json=?,completed_at=NOW() WHERE id=? AND organization_id=?")->execute([json_encode($response,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$requestId,$organizationId]);$pdo->commit();respond($response,201);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
