...
function fail(string $message, int $status = 400, string $code = 'request_error', array $details = []): never {
    http_response_code($status);
    echo json_encode(['data' => null, 'error' => array_merge(['code' => $code, 'message' => $message], $details)], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    exit;
}

set_exception_handler(function (Throwable $error): never {
    $requestId = substr(hash('sha256', uniqid('erp-error-', true)), 0, 12);
    $method = strtoupper((string)($_SERVER['REQUEST_METHOD'] ?? 'UNKNOWN'));
    $path = (string)(parse_url((string)($_SERVER['REQUEST_URI'] ?? '/api'), PHP_URL_PATH) ?: '/api');
    header('X-Request-Id: ' . $requestId);
    error_log(sprintf('[ERP API][%s][%s %s] %s: %s', $requestId, $method, $path, $error::class, $error->getMessage()));
    fail('حدث خطأ داخلي غير متوقع. حاول مرة أخرى أو تواصل مع الإدارة.', 500, 'server_error', ['request_id' => $requestId]);
});
...
function displayBusinessTime12(mixed $value, string $fallback = ''): string {
    $time = normalizeBusinessTime($value);
    if ($time === '') return $fallback;
    [$rawHour, $minute] = array_map('intval', explode(':', $time));
    $hour = $rawHour === 24 ? 0 : $rawHour;
    return sprintf('%d:%02d %s', ($hour % 12) ?: 12, $minute, $hour < 12 ? 'ص' : 'م');
}
...
/** Fixed time grid used only by new client-created booking requests. */
function validateClientBookingTimeGrid(string $start, string $end, mixed $rawDuration): int {
    $durationText=trim((string)$rawDuration);
    if($durationText===''||!ctype_digit($durationText))fail('أرسل مدة الحجز بالدقائق.',422,'client_booking_duration_required');
    $duration=(int)$durationText;
    if($duration<60||$duration>600)fail('مدة حجز العميل يجب أن تكون بين ساعة و10 ساعات.',422,'client_booking_duration_out_of_range');
    if($duration%30!==0)fail('مدة حجز العميل يجب أن تزيد كل 30 دقيقة.',422,'client_booking_duration_increment_invalid');
    $startMinutes=businessTimeMinutes($start);$endMinutes=businessTimeMinutes($end,true);
    if($startMinutes<0||$startMinutes>=1440||$startMinutes%60!==0)fail('وقت بداية حجز العميل يجب أن يكون عند ساعة كاملة ودقائق :00.',422,'client_booking_start_grid_invalid');
    if($endMinutes<0||$endMinutes%30!==0)fail('وقت نهاية حجز العميل يجب أن ينتهي عند :00 أو :30.',422,'client_booking_end_grid_invalid');
    if($endMinutes<=$startMinutes||$endMinutes>1440)fail('لا يمكن أن يمتد حجز العميل لما بعد منتصف الليل.',422,'client_booking_after_midnight');
    if($startMinutes<720||$endMinutes>1320)fail('حجز العملاء متاح من 12 ظهرًا إلى 10 مساءً.',422,'client_booking_outside_hours');
    $computed=$endMinutes-$startMinutes;
    if($computed!==$duration)fail('المدة المرسلة لا تطابق وقت البداية والنهاية المحسوب.',422,'client_booking_duration_mismatch');
    return $computed;
}
...
function validBusinessBooking(string $start, string $end, int $minimumMinutes = 60): bool {
    ...
    return businessTimeMinutes($start) >= 0
...
/**
 * One date/time contract for every booking workflow. Resource conflicts stay
 * inside the caller transaction so validation and slot reservation are atomic.
 */
function validateBookingSchedule(
    ...
    $dateValue=DateTimeImmutable::createFromFormat('!Y-m-d',$date,$zone);
    $minimum=max(15,$minimumMinutes);$increment=max(15,$incrementMinutes);
    $minutes=bookingDurationMinutes($start,$end);
    if(!$dateValue||$dateValue->format('Y-m-d')!==$date||$minutes<$minimum||$minutes%$increment!==0||!validBusinessBooking($start,$end,$minimum)){
    ...
    if($package){
        ...
        if($starts!==''&&($date<$starts||$date>$expires||(($package['validity_mode_snapshot']??'rolling')==='shooting_day'&&$date!==$starts)))fail('الموعد خارج صلاحية الباقة.',422,'booking_outside_package_validity');
    }
    ...
    // Every booking write uses this check, including pending bookings.
    if($checkConflict)requireBookingSlotAvailable($pdo,$organizationId,$resourceId,$date,$start,$end,$excludeBookingId,$actor,$approval);
    return $minutes;
}

function loginPhoneCandidates(string $identifier): array {
    return identityPhoneCandidates($identifier);
}
...
function changeTopic(string $entityType): string {
    return match ($entityType) {
        ...
        'post_production_jobs', 'post_production_status_history', 'video_delivery_links', 'pickup_schedule' => 'post_production',
        ...
        'offers', 'offer_items', 'promotions', 'promotion_subscriptions' => 'offers',
        'clients' => 'clients',
        'services' => 'services',
...
function audit(PDO $pdo, array $user, string $action, string $entityType, ?int $entityId, mixed $before, mixed $after): int {
    ...
    packageLoyaltyAudit($pdo,$user,$action,$entityType,$entityId,$before,$after);
    return $sourceEventId;
}

/** Client calendar contains only anonymous occupied intervals and selectable slots. */
function clientBookingAvailability(PDO $pdo,array $user,int $packageId,int $durationMinutes,string $startDate,int $windowDays,int $bookingId=0): array {
    return clientPackageCalendar($pdo,$user,$packageId,$durationMinutes,$startDate,$windowDays,$bookingId);
}
...
function auditBookingBlockChange(PDO $pdo, array $user, string $action, ?int $entityId, mixed $before, mixed $after): int {
    ...
    recordChangeEvent($pdo,(int)$user['organization_id'],null,'availability','calendar_availability',null,'changed');
...
function reserveBookingSlots(PDO $pdo, array $booking): void {
    requireBookingSlotAvailable($pdo,(int)$booking['organization_id'],(int)$booking['resource_id'],(string)$booking['date'],(string)$booking['start_time'],(string)$booking['end_time'],(int)$booking['id']);
...
function schemaTableExists(PDO $pdo, string $table): bool {
    ...
    $stmt=$pdo->prepare('SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=?');$stmt->execute([$table]);return $cache[$table]=(int)$stmt->fetchColumn()>0;
}

function schemaColumnExists(PDO $pdo, string $table, string $column): bool {
    return in_array($column,schemaTableColumns($pdo,$table),true);
}

function schemaTableColumns(PDO $pdo,string $table): array {
    static $cache=[];if(array_key_exists($table,$cache))return $cache[$table];
...
function bookingBlockSchemaReady(PDO $pdo): bool {
    ...
    foreach(['id','organization_id','resource_id','block_date','start_time','end_time','duration_minutes','title','note','series_key','idempotency_key','request_hash','response_json','status','converted_booking_id','conversion_idempotency_key','conversion_request_hash','conversion_response_json','created_by','cancelled_by','cancelled_at'] as $column)if(!in_array($column,$blockColumns,true))return false;
    if(!in_array('booking_block_id',$slotColumns,true))return false;
    $nullable=$pdo->prepare("SELECT IS_NULLABLE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='booking_slots' AND COLUMN_NAME='booking_id'");$nullable->execute();return $nullable->fetchColumn()==='YES';
}
...
function installBookingBlockSchema(PDO $pdo): bool {
    ...
    try{
        ...
        $pdo->exec("CREATE TABLE IF NOT EXISTS booking_blocks (
          ...
          title VARCHAR(120) NOT NULL DEFAULT 'حجز مؤقت',
          ...
          conversion_idempotency_key VARCHAR(160) NULL,
          conversion_request_hash CHAR(64) NULL,
          conversion_response_json JSON NULL,
          created_by BIGINT UNSIGNED NOT NULL,
          ...
          UNIQUE KEY uq_booking_blocks_conversion_key (organization_id,conversion_idempotency_key),
          UNIQUE KEY uq_booking_blocks_converted_booking (converted_booking_id),
          ...
          CONSTRAINT fk_booking_blocks_converted_booking FOREIGN KEY (converted_booking_id) REFERENCES bookings(id) ON DELETE SET NULL
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
        ...
        $blockColumn=$pdo->prepare('SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=? AND COLUMN_NAME=?');
        $blockDefinitions=['converted_booking_id'=>'BIGINT UNSIGNED NULL AFTER status','conversion_idempotency_key'=>'VARCHAR(160) NULL AFTER converted_booking_id','conversion_request_hash'=>'CHAR(64) NULL AFTER conversion_idempotency_key','conversion_response_json'=>'JSON NULL AFTER conversion_request_hash'];
        foreach($blockDefinitions as $name=>$definition){$blockColumn->execute(['booking_blocks',$name]);if((int)$blockColumn->fetchColumn()===0)$pdo->exec("ALTER TABLE booking_blocks ADD COLUMN $name $definition");}
        ...
        $index->execute(['booking_blocks','uq_booking_blocks_conversion_key']);if((int)$index->fetchColumn()===0)$pdo->exec('ALTER TABLE booking_blocks ADD UNIQUE KEY uq_booking_blocks_conversion_key (organization_id,conversion_idempotency_key)');
        $index->execute(['booking_blocks','uq_booking_blocks_converted_booking']);if((int)$index->fetchColumn()===0)$pdo->exec('ALTER TABLE booking_blocks ADD UNIQUE KEY uq_booking_blocks_converted_booking (converted_booking_id)');
        ...
        $constraint->execute(['booking_blocks','fk_booking_blocks_converted_booking']);if((int)$constraint->fetchColumn()===0)$pdo->exec('ALTER TABLE booking_blocks ADD CONSTRAINT fk_booking_blocks_converted_booking FOREIGN KEY (converted_booking_id) REFERENCES bookings(id) ON DELETE SET NULL');
...
function bookingBlockDates(string $startDate, bool $repeatDaily, string $repeatUntil): array {
    ...
    if(!$repeatDaily||$repeatUntil==='')return [$startDate];
    ...
    // Future occurrences are resolved from the persistent rule, without a date horizon.
    if((int)$start->diff($end)->days+1>90)return [$startDate];
    $dates=[];for($cursor=$start;$cursor<=$end;$cursor=$cursor->modify('+1 day'))$dates[]=$cursor->format('Y-m-d');
...
function pushConfiguration(array $config): array {
    $push=is_array($config['push']??null)?$config['push']:[];$firebase=is_array($push['firebase']??null)?$push['firebase']:[];
    $public=[
        'apiKey'=>trim((string)($firebase['api_key']??'')),
        'authDomain'=>trim((string)($firebase['auth_domain']??'')),
        'projectId'=>trim((string)($firebase['project_id']??'')),
        'storageBucket'=>trim((string)($firebase['storage_bucket']??'')),
        'messagingSenderId'=>trim((string)($firebase['messaging_sender_id']??'')),
        'appId'=>trim((string)($firebase['app_id']??'')),
    ];
    $vapid=trim((string)($firebase['vapid_public_key']??''));$enabled=!empty($push['enabled'])&&$vapid!=='';
    foreach(['apiKey','projectId','messagingSenderId','appId'] as $required)if($public[$required]==='')$enabled=false;
    return ['enabled'=>$enabled,'firebase'=>$public,'vapid_public_key'=>$vapid];
}
...
function firebaseAccessToken(array $config): array {
    static $cached=null;if(is_array($cached)&&($cached['expires_at']??0)>time()+60)return $cached;
    $file=trim((string)($config['push']['service_account_file']??''));if($file===''||!is_file($file))throw new RuntimeException('firebase_service_account_missing');
    $account=json_decode((string)file_get_contents($file),true);if(!is_array($account)||empty($account['client_email'])||empty($account['private_key'])||empty($account['project_id']))throw new RuntimeException('firebase_service_account_invalid');
    $now=time();$header=pushBase64Url(json_encode(['alg'=>'RS256','typ'=>'JWT'],JSON_UNESCAPED_SLASHES));$claims=pushBase64Url(json_encode(['iss'=>$account['client_email'],'scope'=>'https://www.googleapis.com/auth/firebase.messaging','aud'=>'https://oauth2.googleapis.com/token','iat'=>$now,'exp'=>$now+3600],JSON_UNESCAPED_SLASHES));$unsigned=$header.'.'.$claims;
    $signature='';if(!openssl_sign($unsigned,$signature,$account['private_key'],OPENSSL_ALGO_SHA256))throw new RuntimeException('firebase_jwt_sign_failed');$assertion=$unsigned.'.'.pushBase64Url($signature);
    $curl=curl_init('https://oauth2.googleapis.com/token');curl_setopt_array($curl,[CURLOPT_POST=>true,CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>20,CURLOPT_HTTPHEADER=>['Content-Type: application/x-www-form-urlencoded'],CURLOPT_POSTFIELDS=>http_build_query(['grant_type'=>'urn:ietf:params:oauth:grant-type:jwt-bearer','assertion'=>$assertion])]);$raw=curl_exec($curl);$status=(int)curl_getinfo($curl,CURLINFO_HTTP_CODE);$error=curl_error($curl);curl_close($curl);$decoded=is_string($raw)?json_decode($raw,true):null;
    if($status<200||$status>=300||!is_array($decoded)||empty($decoded['access_token']))throw new RuntimeException('firebase_oauth_failed:'.($error?:$status));
    return $cached=['token'=>(string)$decoded['access_token'],'project_id'=>(string)$account['project_id'],'expires_at'=>$now+max(300,(int)($decoded['expires_in']??3600))];
}

function pushUnreadCount(PDO $pdo,array $notification,?int $subscriptionUserId=null): int {
    $organizationId=(int)($notification['organization_id']??0);if($organizationId<1)return 1;$audience=(string)($notification['audience']??'');
    if($audience==='client'){$stmt=$pdo->prepare("SELECT COUNT(*) FROM app_notifications WHERE organization_id=? AND audience='client' AND client_id=? AND read_at IS NULL AND dismissed_at IS NULL");$stmt->execute([$organizationId,(int)($notification['client_id']??0)]);}
    elseif($audience==='owner'&&$subscriptionUserId){$stmt=$pdo->prepare("SELECT COUNT(*) FROM app_notifications WHERE organization_id=? AND audience='owner' AND recipient_user_id=? AND read_at IS NULL AND dismissed_at IS NULL");$stmt->execute([$organizationId,$subscriptionUserId]);}
    elseif($subscriptionUserId){$stmt=$pdo->prepare("SELECT COUNT(*) FROM app_notifications WHERE organization_id=? AND (audience='staff' OR recipient_user_id=?) AND read_at IS NULL AND dismissed_at IS NULL");$stmt->execute([$organizationId,$subscriptionUserId]);}
    else return 1;
    return max(1,min(999,(int)$stmt->fetchColumn()));
}
...
function deleteEligibleBooking(PDO $pdo, array $user, array $booking): float {
    ...
    $session=$pdo->prepare('SELECT COUNT(*) FROM booking_sessions WHERE booking_id=? AND organization_id=?');$session->execute([$bookingId,$organizationId]);if((int)$session->fetchColumn()>0)fail('لا يمكن حذف موعد له جلسة تصوير مسجلة.',409,'booking_session_protected');
    if(schemaTableExists($pdo,'session_settlements')){$settlement=$pdo->prepare('SELECT COUNT(*) FROM session_settlements WHERE booking_id=? AND organization_id=?');$settlement->execute([$bookingId,$organizationId]);if((int)$settlement->fetchColumn()>0)fail('لا يمكن حذف موعد تمت تسويته.',409,'booking_settlement_protected');}
    if(schemaTableExists($pdo,'booking_archives')){
        $history=$pdo->prepare('SELECT * FROM booking_status_history WHERE booking_id=? ORDER BY id');$history->execute([$bookingId]);
        $requests=$pdo->prepare('SELECT * FROM reschedule_requests WHERE booking_id=? AND organization_id=? ORDER BY id');$requests->execute([$bookingId,$organizationId]);
        $snapshot=['booking'=>$booking,'status_history'=>$history->fetchAll(),'reschedule_requests'=>$requests->fetchAll()];
        $pdo->prepare('INSERT IGNORE INTO booking_archives (organization_id,booking_id,client_id,client_package_id,snapshot_json,archived_by,archived_at) VALUES (?,?,?,?,?,?,?)')->execute([$organizationId,$bookingId,(int)$booking['client_id'],$booking['client_package_id']?:null,json_encode($snapshot,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),(int)$user['id'],cairoNow()->format('Y-m-d H:i:s')]);
    }
    $released=settleCancelledBooking($pdo,$booking,false,(int)$user['id']);
    $pdo->prepare('DELETE FROM reschedule_requests WHERE booking_id=? AND organization_id=?')->execute([$bookingId,$organizationId]);
    $pdo->prepare('DELETE FROM booking_status_history WHERE booking_id=?')->execute([$bookingId]);
    releaseBookingSlots($pdo,$bookingId);
    appNotification($pdo,$organizationId,(int)$booking['client_id'],'client','booking_deleted','تم حذف موعد','أزالت الإدارة الموعد من حجوزاتك: '.bookingNotificationMoment($booking).'.','bookings',$bookingId,'booking:'.$bookingId.':deleted','warning','schedule',['booking_id'=>$bookingId]);
    $deleted=$pdo->prepare('DELETE FROM bookings WHERE id=? AND organization_id=?');$deleted->execute([$bookingId,$organizationId]);if($deleted->rowCount()!==1)fail('تعذر حذف الموعد.',409,'booking_delete_conflict');
    audit($pdo,$user,'delete_appointment','bookings',$bookingId,$booking,['deleted'=>true,'client_id'=>(int)$booking['client_id'],'released_quantity'=>$released]);
    return $released;
}
...
function packageSaleSchemaReadiness(PDO $pdo): array {
    ...
    $requiredColumns=[
        ...
        'client_packages'=>['validity_mode_snapshot','validity_days_snapshot','purchased_minutes','held_minutes','consumed_minutes','payment_due_minutes'],
    ];
    ...
    try{
        ...
        foreach($requiredColumns as $table=>$columns){$available=schemaTableColumns($pdo,$table);foreach($columns as $column)if(!in_array($column,$available,true))$missing[]='column:'.$table.'.'.$column;}
        ...
        return ['ready'=>false,'required_migrations'=>['023_client_package_sale_idempotency.sql','024_package_sale_calendar.sql','026_package_first_booking_validity.sql'],'missing'=>['schema_inspection_failed']];
    }
    return ['ready'=>count($missing)===0,'required_migrations'=>['023_client_package_sale_idempotency.sql','024_package_sale_calendar.sql','026_package_first_booking_validity.sql'],'missing'=>$missing];
}

function packageValidityEnd(string $startsAt, int $validityDays, string $mode='rolling'): string {
    $date=DateTimeImmutable::createFromFormat('!Y-m-d',$startsAt,new DateTimeZone('Africa/Cairo'));
    if(!$date||$date->format('Y-m-d')!==$startsAt)throw new InvalidArgumentException('invalid_package_start');
    if($mode==='shooting_day')return $startsAt;
    return $date->modify('+'.(max(1,$validityDays)-1).' days')->format('Y-m-d');
}

function activatePackageOnFirstBooking(PDO $pdo, array &$package, string $bookingDate, int $organizationId): void {
    if(!empty($package['starts_at'])&&!empty($package['expires_at'])){
        if($bookingDate<(string)$package['starts_at']||$bookingDate>(string)$package['expires_at']||(($package['validity_mode_snapshot']??'rolling')==='shooting_day'&&$bookingDate!==(string)$package['starts_at']))fail('الموعد خارج صلاحية الباقة.',422,'booking_outside_package_validity');
        return;
    }
    $mode=(string)($package['validity_mode_snapshot']??'rolling');$days=max(1,(int)($package['validity_days_snapshot']??1));$expires=packageValidityEnd($bookingDate,$days,$mode);
    $pdo->prepare('UPDATE client_packages SET starts_at=?,expires_at=?,version=version+1 WHERE id=? AND organization_id=? AND starts_at IS NULL AND expires_at IS NULL')->execute([$bookingDate,$expires,$package['id'],$organizationId]);
    $package['starts_at']=$bookingDate;$package['expires_at']=$expires;$package['version']=(int)($package['version']??1)+1;
    $clientId=(int)($package['client_id']??0);if($clientId>0){$packageName=trim((string)($package['name']??''))?:'الباقة';$message='بدأت صلاحية '.$packageName.' مع أول موعد تصوير يوم '.$bookingDate.'، وتنتهي يوم '.$expires.'.';appNotification($pdo,$organizationId,$clientId,'client','package_started','بدأت صلاحية باقتك',$message,'client_packages',(int)$package['id'],'package:'.$package['id'].':started:'.$bookingDate,'success','home',['package_id'=>(int)$package['id']]);}
}
...
function activateScheduledSessions(PDO $pdo, int $organizationId, ?int $clientId=null, int $batchLimit=25): int {
...
function appNotification(PDO $pdo, int $organizationId, ?int $clientId, string $audience, string $type, string $title, string $message, string $entityType, int $entityId, string $dedupeKey, string $severity='info', ?string $actionTab=null, array $payload=[], ?int $recipientUserId=null): bool {
    ...
    $notificationId=(int)$pdo->lastInsertId();recordChangeEvent($pdo,$organizationId,$clientId,'notifications','app_notifications',$notificationId,'created');queuePushNotification($pdo,$organizationId,$notificationId);if($ownTransaction)$pdo->commit();return true;
    }catch(Throwable $error){if($ownTransaction&&$pdo->inTransaction())$pdo->rollBack();throw $error;}
}
...
function packageReminderQuantity(array $package, string $kind): float {
    ...
    if($unit==='hour'){
        $minuteKey=$kind.'_minutes';$quantityKey=$kind.'_quantity';
        $minutes=array_key_exists($minuteKey,$package)&&$package[$minuteKey]!==null?(int)$package[$minuteKey]:(int)round((float)($package[$quantityKey]??0)*60);
        return $minutes/60;
    }
    return (float)($package[$kind.'_quantity']??0);
}
...
function packageReminderOutstandingCents(array $package): int {
    return max(0,packageMoneyCents($package['total_price']??0)+max(0,packageMoneyCents($package['overage_amount']??0))-packageMoneyCents($package['paid_amount']??0));
}

function notifyPackagePaymentDue(PDO $pdo, int $organizationId, int $clientId, int $packageId, string $clientName, ?array $knownPackage=null): int {
...
function ownerClientActionTemplate(string $action, string $entityType, mixed $before, mixed $after): ?array {
    ...
    if($entityType==='bookings'&&$action==='create')return ['client_booking_request','طلب حجز جديد','أرسل طلب حجز جديد من لوحة العميل.','requests','info'];
    if($entityType==='bookings'&&$action==='cancel_request')return ['client_cancellation_request','طلب حذف موعد','طلب حذف أحد مواعيده من لوحة العميل.','requests','warning'];
    if($entityType==='bookings'&&$action==='alternative_decision'){
        $accepted=(string)($after['decision']??'')==='accept';
        return [$accepted?'client_alternative_accepted':'client_alternative_rejected',$accepted?'قبول الموعد البديل':'رفض الموعد البديل',$accepted?'وافق على الموعد البديل المقترح.':'رفض الموعد البديل ويحتاج متابعة.','bookings',$accepted?'success':'warning'];
    }
...
function notifyOwnersOfClientAction(PDO $pdo, array $user, string $action, string $entityType, ?int $entityId, mixed $before, mixed $after, int $sourceEventId): int {
    if(($user['role']??'')!=='client'||!$entityId||$sourceEventId<1)return 0;
    $template=ownerClientActionTemplate($action,$entityType,$before,$after);if(!$template)return 0;
    $organizationId=(int)$user['organization_id'];$clientId=(int)$user['client_id'];
    $clientStmt=$pdo->prepare('SELECT name FROM clients WHERE id=? AND organization_id=? LIMIT 1');$clientStmt->execute([$clientId,$organizationId]);$clientName=trim((string)$clientStmt->fetchColumn())?:'عميل';
    $owners=$pdo->prepare("SELECT id FROM users WHERE organization_id=? AND role='owner' AND is_active=1 ORDER BY id");$owners->execute([$organizationId]);
...
function bookingNotificationMoment(array $booking): string {
    $service=trim((string)($booking['service']??''))?:'جلسة التصوير';
    $date=substr(trim((string)($booking['date']??$booking['proposed_date']??'')),0,10);
    $start=displayBusinessTime12($booking['start_time']??$booking['proposed_start_time']??'');
    $end=displayBusinessTime12($booking['end_time']??$booking['proposed_end_time']??'');
    $parts=[$service];if($date!=='')$parts[]='يوم '.$date;if($start!==''&&$end!=='')$parts[]='من '.$start.' إلى '.$end;
...
function clientNotificationTemplate(string $entityType, string $action, mixed $before, mixed $after): ?array {
    ...
    if($entityType==='client_packages'){
        if($before===[])return ['package_created','بدأ اشتراكك في باقة جديدة','أضيفت باقة جديدة إلى حسابك ويمكنك مراجعة رصيدها وصلاحيتها.','home','success'];
        if(empty($before['starts_at'])&&!empty($merged['starts_at']))return ['package_started','بدأت صلاحية باقتك','بدأت صلاحية باقتك مع أول موعد تصوير، ويمكنك مراجعة تاريخ الانتهاء والرصيد الآن.','home','success'];
    ...
    if($entityType==='bookings'){
        $moment=bookingNotificationMoment($merged);
        if($action==='create'&&$before===[])return ['booking_created','تمت إضافة موعد جديد','أضافت الإدارة موعدًا جديدًا إلى حسابك: '.$moment.'.','schedule',$status==='confirmed'?'success':'info'];
        if($action==='cancel_decision'&&$status!=='cancelled')return ['cancellation_rejected','تم رفض طلب إلغاء الموعد','ظل موعدك قائمًا بعد مراجعة طلب الإلغاء: '.$moment.'.','schedule','info'];
        ...
        if($action==='admin_reschedule'||($action!=='booking_decision'&&array_filter(['date','start_time','end_time'],fn($field)=>array_key_exists($field,$after)&&($before[$field]??null)!==$after[$field])))return ['booking_rescheduled','تم تحديث موعد الحجز','عدّلت الإدارة موعدك إلى: '.$moment.'.','schedule','info'];
        $map=['confirmed'=>['booking_confirmed','تم قبول موعد التصوير','وافقت الإدارة على موعدك: '.$moment.'.','success'],'rejected'=>['booking_rejected','تم رفض موعد التصوير','رفضت الإدارة الموعد المطلوب: '.$moment.'.','danger'],'cancelled'=>['booking_cancelled','تم إلغاء الحجز','تم إلغاء الموعد: '.$moment.'.','warning']];if(isset($map[$status])){[$type,$title,$message,$severity]=$map[$status];return [$type,$title,$message,'schedule',$severity];}return null;
    }
    if($entityType==='reschedule_requests'){if(!in_array($status,['approved','rejected'],true))return null;$moment=bookingNotificationMoment($merged);return ['reschedule_update',$status==='approved'?'تم قبول تغيير الموعد':'تم رفض تغيير الموعد',$status==='approved'?'تم اعتماد تغيير موعدك إلى: '.$moment.'.':'تم رفض الموعد المقترح: '.$moment.'. موعدك الحالي يظل قائمًا.','schedule',$status==='approved'?'success':'warning'];}
    ...
    if($entityType==='payment_proofs'&&!in_array($status,['approved','rejected'],true))return null;
    if($entityType==='payment_proofs')return ['payment_proof_'.$status,$status==='approved'?'تم قبول الدفع':'تم رفض إثبات الدفع',$status==='approved'?'وافقت الإدارة على إثبات الدفع وتم تحديث رصيدك المالي.':'لم يتم اعتماد التحويل. راجع الإثبات وأعد المحاولة.','finance',$status==='approved'?'success':'danger'];
...
function dismissSettledPackageNotifications(PDO $pdo, int $organizationId, int $clientId): void {
    $stmt=$pdo->prepare("UPDATE app_notifications n JOIN client_packages cp ON cp.id=n.entity_id AND n.entity_type='client_packages' SET n.dismissed_at=COALESCE(n.dismissed_at,NOW()),n.read_at=COALESCE(n.read_at,NOW()) WHERE n.organization_id=? AND n.client_id=? AND n.type IN ('payment_due','payment_upcoming') AND cp.total_price+cp.overage_amount-cp.paid_amount<=0.0001");
...
function settleCancelledBooking(PDO $pdo, array $booking, bool $charge, int $userId): float {
    ...
    $packageStmt=$pdo->prepare('SELECT * FROM client_packages WHERE id=? AND organization_id=? AND client_id=? FOR UPDATE');$packageStmt->execute([$booking['client_package_id'],$booking['organization_id'],$booking['client_id']]);$package=$packageStmt->fetch();if(!$package)fail('الباقة المرتبطة بالحجز غير موجودة.',404,'package_not_found');
...
function startBookingSession(PDO $pdo, array $user, int $bookingId, string $source='manual'): array {
    ...
    try{
        ...
        if(!empty($booking['client_package_id'])){$package=$pdo->prepare('SELECT * FROM client_packages WHERE id=? AND organization_id=? AND client_id=? FOR UPDATE');$package->execute([$booking['client_package_id'],$user['organization_id'],$booking['client_id']]);$packageRow=$package->fetch();if(!$packageRow||$packageRow['status']!=='active'||(string)$packageRow['starts_at']>$today||(string)$packageRow['expires_at']<$today)fail('الباقة غير نشطة أو خارج فترة الصلاحية.',422,'invalid_session_package');if(!in_array((string)$packageRow['billing_unit'],['hour','reel'],true))fail('زر بدء التصوير متاح فقط لباقات الساعات أو الريلز.',422,'unsupported_session_package');if(bookingHeldQuantity($pdo,$bookingId,(int)$booking['client_package_id'])<=0)fail('لا يوجد رصيد محجوز لهذا الموعد على الباقة.',422,'missing_package_hold');}
...
function strictPackageMoneyParts(mixed $value): array {
    if(!is_string($value))return ['valid'=>false,'reason'=>'type','cents'=>null];
    $raw=$value;
    if($raw===''||$raw!==trim($raw)||!preg_match('/^(\d+)(?:\.(\d{1,2}))?$/',$raw,$parts))return ['valid'=>false,'reason'=>'format','cents'=>null];
    $digits=ltrim($parts[1].str_pad($parts[2]??'',2,'0'),'0');if($digits==='')$digits='0';$max='9007199254740991';
    if(strlen($digits)>strlen($max)||(strlen($digits)===strlen($max)&&strcmp($digits,$max)>0))return ['valid'=>false,'reason'=>'overflow','cents'=>null];
    return ['valid'=>true,'reason'=>null,'cents'=>(int)$digits];
}

function strictPackageMoneyCents(mixed $value,string $label='المبلغ',string $code='invalid_money_format'): int {
    $parsed=strictPackageMoneyParts($value);if($parsed['valid'])return (int)$parsed['cents'];
    $message=$parsed['reason']==='type'?'يجب إرسال '.$label.' كنص عشري صريح، وليس كرقم JSON.':($parsed['reason']==='overflow'?$label.' أكبر من الحد الآمن المسموح.':'أدخل '.$label.' كرقم عشري عادي بحد أقصى خانتين بعد العلامة، دون مسافات أو صيغة أسية.');
    fail($message,422,$code);
}

function packageMoney(int $cents): string { return number_format($cents/100,2,'.',''); }

function lineTotalCents(mixed $quantity,mixed $unitPrice): int {
    $numericQuantity=filter_var($quantity,FILTER_VALIDATE_FLOAT);
    $rawPrice=trim((string)$unitPrice);
    if(!preg_match('/^\d+(?:\.\d{1,2})?$/',$rawPrice))fail('سعر البند يجب أن يكون بدقة قرشين كحد أقصى.',422,'invalid_line_money');
    $unitCents=packageMoneyCents($unitPrice);
    if($numericQuantity===false||$numericQuantity<=0||$unitCents<0)fail('كمية أو سعر أحد البنود غير صحيح.',422,'invalid_line_money');
    return (int)round($unitCents*(float)$numericQuantity,0,PHP_ROUND_HALF_UP);
}

/** Deterministic largest-remainder allocation; returned cents always sum to total. */
function proportionalCents(int $totalCents,array $weights): array {
    $weights=array_map(fn($value)=>max(0,(int)$value),$weights);$weightTotal=array_sum($weights);
    if($totalCents<0||$weightTotal<=0)return array_fill(0,count($weights),0);
    $allocated=[];$remainders=[];$used=0;
    foreach($weights as $index=>$weight){$numerator=$totalCents*$weight;$base=intdiv($numerator,$weightTotal);$allocated[$index]=$base;$used+=$base;$remainders[$index]=$numerator%$weightTotal;}
    $order=array_keys($weights);usort($order,fn($left,$right)=>$remainders[$right]<=>$remainders[$left]?:$left<=>$right);
    for($remaining=$totalCents-$used,$cursor=0;$remaining>0;$remaining--,$cursor++)$allocated[$order[$cursor%count($order)]]++;
    ksort($allocated);return array_values($allocated);
}

function ownerCorrectionReason(array $payload): string {
    global $path,$method;
    if($method==='POST'&&(preg_match('#^/finance/\d+/void$#',(string)$path)||preg_match('#^/finance/transfers/[^/]+/void$#',(string)$path)))return financeVoidReason($payload);
...
function financeVoidReason(array $payload): string {
    $note=trim((string)($payload['reason']??$payload['internal_note']??''));
    return $note===''?'تم الإلغاء بواسطة المالك.':mb_substr($note,0,500);
}
...
function financeReversal(PDO $pdo,array $user,array $entry,string $reason): int {
    ...
    $stmt=$pdo->prepare("INSERT INTO finance (organization_id,client_id,employee_user_id,type,entry_kind,category,amount,method,detail,date,entity,source_type,source_id,correlation_id,is_system,reversed_entry_id,reversal_reason,created_by) VALUES (?,?,?,?,'reversal',?,?,?,?,?,?,?,?,?,1,?,?,?)");
    $stmt->execute([$user['organization_id'],$entry['client_id']?:null,$entry['employee_user_id']?:null,'قيد عكسي',$category,packageMoney(packageMoneyCents($entry['amount'])),$entry['method'],'عكس: '.$entry['detail'],cairoNow()->format('Y-m-d'),$entry['entity'],$entry['source_type'],$entry['source_id'],$correlation,$entry['id'],$reason,$user['id']]);
...
function voidPayment(PDO $pdo,array $user,int $paymentId,array $payload): array {
    $reason=financeVoidReason($payload);$organizationId=(int)$user['organization_id'];
...
function attendancePolicy(PDO $pdo, array $user, bool $create = true): ?array {
    if (in_array($user['role'], ['client','applicant','owner'], true)) return null;
...
function attendanceSummary(PDO $pdo, array $viewer, string $month, ?int $requestedUserId): array {
    ...
    $where="u.organization_id=? AND u.is_active=1 AND u.role NOT IN ('client','owner','applicant')";
...
function queueClientWhatsAppSummary(PDO $pdo, int $organizationId, int $clientId, ?string $dedupeKey=null): int {
    ...
    $stmt=$pdo->prepare("SELECT name,billing_unit,purchased_quantity,purchased_minutes,held_quantity,held_minutes,consumed_quantity,consumed_minutes,payment_due_quantity,payment_due_minutes,total_price,overage_amount,paid_amount,source_invoice_id,expires_at FROM client_packages WHERE client_id=? AND organization_id=? AND status='active' ORDER BY expires_at");$stmt->execute([$clientId,$organizationId]);$packages=$stmt->fetchAll();
    ...
    $lines=['مرحبًا '.$client['name'].'،','ملخص حسابك لدى Multi Task Agency:'];if($packages){foreach($packages as $package){$unit=(string)$package['billing_unit'];$purchased=packageReminderQuantity($package,'purchased');$held=packageReminderQuantity($package,'held');$consumed=packageReminderQuantity($package,'consumed');$paymentDueQuantity=packageReminderQuantity($package,'payment_due');$remaining=max(0,$purchased-$held-$consumed);$packageDue=max(0,(float)$package['total_price']+(float)$package['overage_amount']-(float)$package['paid_amount']);$remainingText=$unit==='hour'?packageReminderUnitText($remaining,'hour'):number_format($remaining,0,'.','').' '.(['reel'=>'ريل','day'=>'يوم','month'=>'شهر','project'=>'مشروع'][$unit]??$unit);$line='• '.$package['name'].': متبقي '.$remainingText.' — ماليًا '.number_format($packageDue,2,'.',',').' ج.م — تنتهي '.$package['expires_at'];if($paymentDueQuantity>0&&$consumed>=$paymentDueQuantity&&$packageDue>0)$line.=' — مطلوب السداد الآن';$lines[]=$line;}}else{$lines[]='• لا توجد باقات فعالة حاليًا.';}$lines[]='• إجمالي المستحق المالي: '.number_format($outstanding,2,'.',',').' ج.م';$payload=['client_name'=>$client['name'],'packages'=>$packages,'outstanding'=>$outstanding,'currency'=>'EGP','message'=>implode("\n",$lines)];$json=json_encode($payload,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);$recipient=whatsappPhone($client['phone1']);
...
function formationEntryPayload(array $payload,string $type): array {
    ...
    return ['entry_type'=>$type,'amount_cents'=>$amount,'title'=>$title,'category'=>mb_substr(trim((string)($payload['category']??'')),0,80),'payment_method'=>requirePaymentMethod($payload['payment_method']??'cash'),'reference'=>mb_substr(trim((string)($payload['reference']??'')),0,120),'entry_date'=>$date,'note'=>mb_substr(trim((string)($payload['note']??'')),0,3000)];
}
...
function insertSystemBackupRow(PDO $pdo,string $table,array $row,int $organizationId,int $currentOwnerId): bool {
    ...
    if($table==='auth_google_identities'){if((int)($values['user_id']??0)===$currentOwnerId)return false;unset($values['id']);}
...
$resources = [
    ...
    'finance' => ['org' => true, 'read' => ['owner','admin','finance'], 'write' => [], 'columns' => ['id','organization_id','client_id','employee_user_id','type','entry_kind','category','amount','method','detail','date','entity','source_type','source_id','correlation_id','is_system','reversed_entry_id','reversal_reason','corrected_from_id','voided_by','voided_at','version','created_by','created_at']],
...
// Email-verified applicants can track only their own intake until staff approval.
$isPasswordResetRequest=$method==='POST'&&in_array($path,['/auth/password-reset/validate','/auth/password-reset/complete'],true);
if ($user && $user['role'] === 'applicant' && !$isPasswordResetRequest) {
    if (!registrationApplicantRouteAllowed($path,$method)) fail('طلب التسجيل قيد المراجعة. يمكنك متابعة حالة طلباتك فقط.',403,'registration_pending');
}

if ($user && !empty($user['must_change_password']) && !$isPasswordResetRequest && !in_array($path, ['/auth/session','/auth/password','/auth/logout','/health'], true)) {
...
if ($path === '/dashboard/receivables' && $method === 'GET') {
    ...
    $stmt=$pdo->prepare("SELECT cp.id AS package_id,cp.client_id,COALESCE(NULLIF(TRIM(c.name),''),CONCAT('عميل #',cp.client_id)) AS client_name,cp.name AS package_name,cp.status,cp.total_price,COALESCE(cp.overage_amount,0) AS overage_amount,cp.paid_amount,cp.starts_at,cp.expires_at
      ...
      WHERE cp.organization_id=? AND cp.status='active' AND (cp.expires_at IS NULL OR cp.expires_at>=?)");
    $stmt->execute([$organizationId,cairoNow()->format('Y-m-d')]);$items=[];$totalCents=0;$overageCents=0;$paidCents=0;$outstandingCents=0;
...
if ($path === '/dashboard/kpis' && $method === 'GET') {
    ...
    $canViewFinance=in_array($role,['owner','admin','finance'],true);$canViewPackages=in_array($role,['owner','admin','operations','finance'],true);$canViewServices=in_array($role,['owner','admin','operations'],true);
    $receivables=['available'=>false];$cashMovement=['available'=>false];$activePackages=['available'=>false];$activeServices=['available'=>false];$partialErrors=[];
    ...
    if($canViewFinance){try{
        ...
        $stmt=$pdo->prepare("SELECT COALESCE(SUM(GREATEST(GREATEST(ROUND(COALESCE(total_price,0)*100),0)+GREATEST(ROUND(COALESCE(overage_amount,0)*100),0)-GREATEST(ROUND(COALESCE(paid_amount,0)*100),0),0)),0) due_cents FROM client_packages WHERE organization_id=? AND status='active' AND (expires_at IS NULL OR expires_at>=?)");
        $stmt->execute([$organizationId,$today]);$packageCents=(int)$stmt->fetchColumn();
        ...
        $cashMovement=['available'=>true,'definition'=>'operational','transfers_included'=>false,'cash_in'=>packageMoney($cashInCents),'cash_out'=>packageMoney($cashOutCents)];
    }catch(Throwable $error){error_log('[Dashboard KPIs] finance: '.$error->getMessage());$receivables=['available'=>false,'reason'=>'unavailable'];$cashMovement=['available'=>false,'reason'=>'unavailable'];$partialErrors[]='finance';}}

    if($canViewPackages){try{
        $soon=cairoNow()->setTime(0,0)->modify('+14 days')->format('Y-m-d');
        $packageScope=" FROM client_packages cp JOIN services s ON s.id=cp.service_id AND s.organization_id=cp.organization_id WHERE cp.organization_id=? AND cp.status='active' AND s.is_active=1 AND COALESCE(s.is_draft,0)=0 AND s.archived_at IS NULL AND LOWER(TRIM(s.billing_unit)) NOT IN ('project','custom') AND LOWER(TRIM(s.category)) NOT IN ('graphics','graphic','montage','editing','custom','custom_project','project') AND ((LOWER(TRIM(s.billing_unit))='reel' OR (s.total_reels>0 AND s.total_hours<=0)) AND s.total_reels>0 OR LOWER(TRIM(s.billing_unit)) IN ('hour','day','month') AND s.total_hours>0)";
        $stmt=$pdo->prepare("SELECT COUNT(*)".$packageScope." AND (cp.expires_at IS NULL OR cp.expires_at>=?)");$stmt->execute([$organizationId,$today]);$activeCount=(int)$stmt->fetchColumn();
        $stmt=$pdo->prepare("SELECT COUNT(*)".$packageScope." AND cp.expires_at BETWEEN ? AND ?");$stmt->execute([$organizationId,$today,$soon]);$expiringCount=(int)$stmt->fetchColumn();
        $activePackages=['available'=>true,'count'=>$activeCount,'expiring_within_14_days'=>$expiringCount];
    }catch(Throwable $error){error_log('[Dashboard KPIs] packages: '.$error->getMessage());$activePackages=['available'=>false,'reason'=>'unavailable'];$partialErrors[]='packages';}}

    if($canViewServices){try{
        $stmt=$pdo->prepare("SELECT SUM(status IN ('planning','active','on_hold')) AS active_projects,SUM(status='on_hold') AS paused_projects FROM projects WHERE organization_id=?");$stmt->execute([$organizationId]);$projectCounts=$stmt->fetch()?:[];
        $stmt=$pdo->prepare("SELECT COUNT(*) FROM content_items c JOIN projects p ON p.id=c.project_id AND p.organization_id=c.organization_id WHERE c.organization_id=? AND c.archived_at IS NULL AND p.status IN ('planning','active','on_hold') AND c.status IN ('idea','draft','editing','in_progress','in_review','approved','scheduled')");$stmt->execute([$organizationId]);$activeContent=(int)$stmt->fetchColumn();
        $activeServices=['available'=>true,'active_projects'=>(int)($projectCounts['active_projects']??0),'paused_projects'=>(int)($projectCounts['paused_projects']??0),'active_content_items'=>$activeContent];
    }catch(Throwable $error){error_log('[Dashboard KPIs] services: '.$error->getMessage());$activeServices=['available'=>false,'reason'=>'unavailable'];$partialErrors[]='services';}}

    respond(['as_of'=>$today,'receivables'=>$receivables,'cash_movement'=>$cashMovement,'active_packages'=>$activePackages,'active_services'=>$activeServices,'partial_errors'=>$partialErrors]);
}
...
if ($path === '/auth/bootstrap' && $method === 'POST') {
    ...
    $phone = loginMobile($payload['phone'] ?? '');
    if ($phone === '') fail('رقم موبايل المالك مطلوب للدخول.', 422, 'invalid_login_phone');
    ...
    $stmt = $pdo->prepare("INSERT INTO users (organization_id, full_name, email, phone, password_hash, role) VALUES (1, ?, ?, ?, ?, 'owner')");
    $stmt->execute([$name, $email, $phone, password_hash($password, PASSWORD_DEFAULT)]);
    respond(['created' => true], 201);
}

function issueLoginSession(PDO $pdo, array $config, array $found, string $identifier, string $eventType = 'login_succeeded'): array {
...
if ($path === '/sync' && $method === 'GET') {
    ...
    $scopeSql="organization_id=?";$scopeParams=[$organizationId];if($clientScope){$scopeSql.=" AND (client_id=? OR topic='services' OR (client_id IS NULL AND topic='post_production' AND entity_type='pickup_schedule') OR (client_id IS NULL AND topic='availability' AND entity_type='calendar_availability'))";$scopeParams[]=(int)$user['client_id'];}
    ...
    $last=$events?(int)$events[array_key_last($events)]['id']:$cursor;$hasMore=$last<$high;
    respond(['cursor'=>$last,'high_watermark'=>$high,'has_more'=>$hasMore,'server_now'=>cairoNow()->format(DATE_ATOM),'topics'=>$topics,'events'=>$events]);
}
...
function ownerProjectCascadeScope(PDO $pdo,int $organizationId,array $project,bool $lock=false): array {
    $projectId=(int)$project['id'];$invoiceIds=[];$invoiceRows=[];
    $invoiceSql='SELECT id,offer_id FROM invoices WHERE organization_id=? AND (project_id=?';$invoiceParams=[$organizationId,$projectId];
    if(!empty($project['invoice_id'])){$invoiceSql.=' OR id=?';$invoiceParams[]=(int)$project['invoice_id'];}$invoiceSql.=')'.($lock?' FOR UPDATE':'');
    $stmt=$pdo->prepare($invoiceSql);$stmt->execute($invoiceParams);$invoiceRows=$stmt->fetchAll();$invoiceIds=array_values(array_unique(array_map('intval',array_column($invoiceRows,'id'))));
    $bookingStmt=$pdo->prepare('SELECT id,client_package_id FROM bookings WHERE organization_id=? AND project_id=?'.($lock?' FOR UPDATE':''));$bookingStmt->execute([$organizationId,$projectId]);$bookingRows=$bookingStmt->fetchAll();$bookingIds=array_values(array_unique(array_map('intval',array_column($bookingRows,'id'))));
    $paymentIds=[];$proofIds=[];$allocationIds=[];$financeIds=[];
    if($invoiceIds){
        $marks=implode(',',array_fill(0,count($invoiceIds),'?'));
        $sharedProjects=$pdo->prepare("SELECT COUNT(*) FROM projects WHERE organization_id=? AND id<>? AND invoice_id IN ($marks)");$sharedProjects->execute(array_merge([$organizationId,$projectId],$invoiceIds));
        if((int)$sharedProjects->fetchColumn()>0)fail('لا يمكن حذف هذه الباقة لأن فاتورتها مشتركة مع مشروع آخر. افصل الفاتورة أولًا.',409,'shared_project_invoice');
        $linkedPackages=$pdo->prepare("SELECT COUNT(*) FROM client_packages WHERE organization_id=? AND source_invoice_id IN ($marks)");$linkedPackages->execute(array_merge([$organizationId],$invoiceIds));
        if((int)$linkedPackages->fetchColumn()>0)fail('لا يمكن حذف هذه الباقة لأن الفاتورة نفسها مرتبطة بباقة تصوير مباعة. احذف أو افصل الباقة المباعة أولًا.',409,'shared_package_invoice');
        $allocationStmt=$pdo->prepare("SELECT id,payment_id,payment_proof_id,client_package_id FROM payment_allocations WHERE organization_id=? AND invoice_id IN ($marks)".($lock?' FOR UPDATE':''));$allocationStmt->execute(array_merge([$organizationId],$invoiceIds));$allocations=$allocationStmt->fetchAll();
        if(array_filter($allocations,fn($row)=>!empty($row['client_package_id'])))fail('لا يمكن حذف السجل لأن إحدى دفعات الفاتورة موزعة أيضًا على باقة تصوير.',409,'shared_project_allocation');
        $allocationIds=array_values(array_unique(array_map('intval',array_column($allocations,'id'))));$paymentIds=array_values(array_unique(array_filter(array_map('intval',array_column($allocations,'payment_id')))));$proofIds=array_values(array_unique(array_filter(array_map('intval',array_column($allocations,'payment_proof_id')))));
        $proofSql="SELECT id,payment_id,client_package_id,invoice_id FROM payment_proofs WHERE organization_id=? AND (invoice_id IN ($marks)";$proofParams=array_merge([$organizationId],$invoiceIds);if($paymentIds){$paymentMarks=implode(',',array_fill(0,count($paymentIds),'?'));$proofSql.=" OR payment_id IN ($paymentMarks)";$proofParams=array_merge($proofParams,$paymentIds);}$proofSql.=')'.($lock?' FOR UPDATE':'');$proofStmt=$pdo->prepare($proofSql);$proofStmt->execute($proofParams);$proofRows=$proofStmt->fetchAll();$paymentIds=array_values(array_unique(array_merge($paymentIds,array_filter(array_map('intval',array_column($proofRows,'payment_id'))))));
        if($paymentIds){$paymentMarks=implode(',',array_fill(0,count($paymentIds),'?'));$allProofs=$pdo->prepare("SELECT id,payment_id,client_package_id,invoice_id FROM payment_proofs WHERE organization_id=? AND (invoice_id IN ($marks) OR payment_id IN ($paymentMarks))".($lock?' FOR UPDATE':''));$allProofs->execute(array_merge([$organizationId],$invoiceIds,$paymentIds));$proofRows=$allProofs->fetchAll();$outside=$pdo->prepare("SELECT COUNT(*) FROM payment_allocations WHERE organization_id=? AND payment_id IN ($paymentMarks) AND (invoice_id IS NULL OR invoice_id NOT IN ($marks))");$outside->execute(array_merge([$organizationId],$paymentIds,$invoiceIds));if((int)$outside->fetchColumn()>0)fail('لا يمكن حذف السجل لأن إحدى دفعاته موزعة على فاتورة أو باقة أخرى.',409,'shared_project_payment');}
        foreach($proofRows as $proofRow)if(!empty($proofRow['client_package_id'])||(!empty($proofRow['invoice_id'])&&!in_array((int)$proofRow['invoice_id'],$invoiceIds,true)))fail('لا يمكن حذف إثبات دفع مرتبط أيضًا بباقة أو فاتورة أخرى.',409,'shared_project_payment_proof');
        $proofIds=array_values(array_unique(array_merge($proofIds,array_map('intval',array_column($proofRows,'id')))));
        if($proofIds){$proofMarks=implode(',',array_fill(0,count($proofIds),'?'));$outsideProof=$pdo->prepare("SELECT COUNT(*) FROM payment_allocations WHERE organization_id=? AND payment_proof_id IN ($proofMarks) AND (invoice_id IS NULL OR invoice_id NOT IN ($marks))");$outsideProof->execute(array_merge([$organizationId],$proofIds,$invoiceIds));if((int)$outsideProof->fetchColumn()>0)fail('لا يمكن حذف إثبات دفع مشترك مع فاتورة أخرى.',409,'shared_project_payment_proof');}
    }
    if($paymentIds){$marks=implode(',',array_fill(0,count($paymentIds),'?'));$financeStmt=$pdo->prepare("SELECT id FROM finance WHERE organization_id=? AND source_type='payment' AND source_id IN ($marks)".($lock?' FOR UPDATE':''));$financeStmt->execute(array_merge([$organizationId],$paymentIds));$financeIds=array_map('intval',$financeStmt->fetchAll(PDO::FETCH_COLUMN));}
    if($proofIds){$marks=implode(',',array_fill(0,count($proofIds),'?'));$financeStmt=$pdo->prepare("SELECT id FROM finance WHERE organization_id=? AND source_type='payment_proof' AND source_id IN ($marks)".($lock?' FOR UPDATE':''));$financeStmt->execute(array_merge([$organizationId],$proofIds));$financeIds=array_merge($financeIds,array_map('intval',$financeStmt->fetchAll(PDO::FETCH_COLUMN)));}
    if($invoiceIds){$marks=implode(',',array_fill(0,count($invoiceIds),'?'));$financeStmt=$pdo->prepare("SELECT id FROM finance WHERE organization_id=? AND source_type IN ('invoice','invoices') AND source_id IN ($marks)".($lock?' FOR UPDATE':''));$financeStmt->execute(array_merge([$organizationId],$invoiceIds));$financeIds=array_merge($financeIds,array_map('intval',$financeStmt->fetchAll(PDO::FETCH_COLUMN)));}
    $directFinance=$pdo->prepare("SELECT id FROM finance WHERE organization_id=? AND source_type IN ('project','projects','client_project') AND source_id=?".($lock?' FOR UPDATE':''));$directFinance->execute([$organizationId,$projectId]);$financeIds=array_values(array_unique(array_merge($financeIds,array_map('intval',$directFinance->fetchAll(PDO::FETCH_COLUMN)))));
    return ['invoice_rows'=>$invoiceRows,'invoice_ids'=>$invoiceIds,'booking_rows'=>$bookingRows,'booking_ids'=>$bookingIds,'payment_ids'=>$paymentIds,'proof_ids'=>$proofIds,'allocation_ids'=>$allocationIds,'finance_ids'=>$financeIds];
}

function ownerDeleteProjectCascade(PDO $pdo,array $user,array $project,string $reason): array {
    $organizationId=(int)$user['organization_id'];$projectId=(int)$project['id'];$scope=ownerProjectCascadeScope($pdo,$organizationId,$project,true);
    foreach($scope['booking_rows'] as $booking)if(!empty($booking['client_package_id']))fail('لا يمكن حذف موعد مرتبط أيضًا بباقة تصوير مباعة. افصل الموعد عن الباقة أولًا.',409,'shared_project_booking');
    $deleted=['bookings'=>count($scope['booking_ids']),'invoices'=>count($scope['invoice_ids']),'payments'=>count($scope['payment_ids']),'payment_proofs'=>count($scope['proof_ids']),'payment_allocations'=>count($scope['allocation_ids']),'finance'=>count($scope['finance_ids'])];
    if($scope['booking_ids']){
        $marks=implode(',',array_fill(0,count($scope['booking_ids']),'?'));
        if(schemaTableExists($pdo,'post_production_jobs'))$pdo->prepare("DELETE FROM post_production_jobs WHERE organization_id=? AND booking_id IN ($marks)")->execute(array_merge([$organizationId],$scope['booking_ids']));
        if(schemaTableExists($pdo,'session_settlements'))$pdo->prepare("DELETE FROM session_settlements WHERE organization_id=? AND booking_id IN ($marks)")->execute(array_merge([$organizationId],$scope['booking_ids']));
        if(schemaTableExists($pdo,'booking_archives'))$pdo->prepare("DELETE FROM booking_archives WHERE organization_id=? AND booking_id IN ($marks)")->execute(array_merge([$organizationId],$scope['booking_ids']));
        if(schemaTableExists($pdo,'app_notifications'))$pdo->prepare("DELETE FROM app_notifications WHERE organization_id=? AND entity_type IN ('booking','bookings') AND entity_id IN ($marks)")->execute(array_merge([$organizationId],$scope['booking_ids']));
        $pdo->prepare("DELETE FROM bookings WHERE organization_id=? AND id IN ($marks)")->execute(array_merge([$organizationId],$scope['booking_ids']));
    }
    if(schemaTableExists($pdo,'session_settlement_allocations')){
        $conditions=['project_id=?'];$params=[$projectId];
        foreach([['invoice_id',$scope['invoice_ids']],['payment_id',$scope['payment_ids']]] as [$column,$ids])if($ids){$conditions[]="$column IN (".implode(',',array_fill(0,count($ids),'?')).')';array_push($params,...$ids);}
        $pdo->prepare('DELETE FROM session_settlement_allocations WHERE '.implode(' OR ',$conditions))->execute($params);
    }
    if($scope['finance_ids']){$marks=implode(',',array_fill(0,count($scope['finance_ids']),'?'));$pdo->prepare("DELETE FROM finance WHERE organization_id=? AND id IN ($marks)")->execute(array_merge([$organizationId],$scope['finance_ids']));}
    if($scope['allocation_ids']){$marks=implode(',',array_fill(0,count($scope['allocation_ids']),'?'));$pdo->prepare("DELETE FROM payment_allocations WHERE organization_id=? AND id IN ($marks)")->execute(array_merge([$organizationId],$scope['allocation_ids']));}
    if($scope['proof_ids']){$marks=implode(',',array_fill(0,count($scope['proof_ids']),'?'));$pdo->prepare("DELETE FROM payment_proofs WHERE organization_id=? AND id IN ($marks)")->execute(array_merge([$organizationId],$scope['proof_ids']));}
    if($scope['payment_ids']){$marks=implode(',',array_fill(0,count($scope['payment_ids']),'?'));$pdo->prepare("DELETE FROM payments WHERE organization_id=? AND id IN ($marks)")->execute(array_merge([$organizationId],$scope['payment_ids']));}
    $offerIds=array_values(array_unique(array_filter(array_map('intval',array_column($scope['invoice_rows'],'offer_id')))));
    if($scope['invoice_ids']){$marks=implode(',',array_fill(0,count($scope['invoice_ids']),'?'));$pdo->prepare("DELETE FROM invoices WHERE organization_id=? AND id IN ($marks)")->execute(array_merge([$organizationId],$scope['invoice_ids']));}
    if($offerIds){$marks=implode(',',array_fill(0,count($offerIds),'?'));$pdo->prepare("UPDATE offers SET status='cancelled',cancellation_reason=?,cancelled_by=?,cancelled_at=COALESCE(cancelled_at,NOW()),version=version+1 WHERE organization_id=? AND id IN ($marks)")->execute(array_merge([$reason,$user['id'],$organizationId],$offerIds));}
    if(schemaTableExists($pdo,'app_notifications'))$pdo->prepare("DELETE FROM app_notifications WHERE organization_id=? AND entity_type IN ('project','projects') AND entity_id=?")->execute([$organizationId,$projectId]);
    if(schemaTableExists($pdo,'change_events')){
        $conditions=["(entity_type IN ('project','projects') AND entity_id=?)"];$params=[$organizationId,$projectId];
        if($scope['booking_ids']){$marks=implode(',',array_fill(0,count($scope['booking_ids']),'?'));$conditions[]="(entity_type IN ('booking','bookings') AND entity_id IN ($marks))";array_push($params,...$scope['booking_ids']);}
        $pdo->prepare('DELETE FROM change_events WHERE organization_id=? AND ('.implode(' OR ',$conditions).')')->execute($params);
    }
    if(schemaTableExists($pdo,'custom_service_requests'))$pdo->prepare("DELETE FROM custom_service_requests WHERE organization_id=? AND JSON_UNQUOTE(JSON_EXTRACT(response_json,'$.id'))=?")->execute([$organizationId,(string)$projectId]);
    $pdo->prepare('DELETE FROM projects WHERE id=? AND organization_id=?')->execute([$projectId,$organizationId]);
    return $deleted;
}

function ownerPackageCascadeScope(PDO $pdo,int $organizationId,int $packageId,bool $lock=false): array {
    $lockSql=$lock?' FOR UPDATE':'';
    $bookingStmt=$pdo->prepare('SELECT id,status FROM bookings WHERE organization_id=? AND client_package_id=?'.$lockSql);$bookingStmt->execute([$organizationId,$packageId]);$bookings=$bookingStmt->fetchAll();foreach($bookings as $booking)if((string)$booking['status']==='in_progress')fail('أنه جلسة التصوير الجارية قبل حذف الباقة.',409,'package_session_active');$bookingIds=array_map('intval',array_column($bookings,'id'));
    if($bookingIds&&schemaTableExists($pdo,'booking_sessions')){$marks=implode(',',array_fill(0,count($bookingIds),'?'));$active=$pdo->prepare("SELECT COUNT(*) FROM booking_sessions WHERE organization_id=? AND booking_id IN ($marks) AND status='active'");$active->execute(array_merge([$organizationId],$bookingIds));if((int)$active->fetchColumn()>0)fail('أنه جلسة التصوير الجارية قبل حذف الباقة.',409,'package_session_active');}

    $allocationStmt=$pdo->prepare('SELECT id,payment_id,payment_proof_id,invoice_id,amount FROM payment_allocations WHERE organization_id=? AND client_package_id=?'.$lockSql);$allocationStmt->execute([$organizationId,$packageId]);$allocations=$allocationStmt->fetchAll();$allocationIds=array_map('intval',array_column($allocations,'id'));
    $proofStmt=$pdo->prepare('SELECT id,payment_id FROM payment_proofs WHERE organization_id=? AND client_package_id=?'.$lockSql);$proofStmt->execute([$organizationId,$packageId]);$directProofs=$proofStmt->fetchAll();$proofIds=array_map('intval',array_column($directProofs,'id'));
    $candidatePaymentIds=array_values(array_unique(array_filter(array_merge(array_map('intval',array_column($allocations,'payment_id')),array_map('intval',array_column($directProofs,'payment_id'))))));$paymentIds=[];
    foreach($candidatePaymentIds as $paymentId){$outside=$pdo->prepare('SELECT (SELECT COUNT(*) FROM payment_allocations WHERE organization_id=? AND payment_id=? AND (client_package_id IS NULL OR client_package_id<>?))+(SELECT COUNT(*) FROM payment_proofs WHERE organization_id=? AND payment_id=? AND (client_package_id IS NULL OR client_package_id<>?))');$outside->execute([$organizationId,$paymentId,$packageId,$organizationId,$paymentId,$packageId]);if((int)$outside->fetchColumn()===0)$paymentIds[]=$paymentId;}
    if($paymentIds){$marks=implode(',',array_fill(0,count($paymentIds),'?'));$allProofs=$pdo->prepare("SELECT id FROM payment_proofs WHERE organization_id=? AND payment_id IN ($marks)$lockSql");$allProofs->execute(array_merge([$organizationId],$paymentIds));$proofIds=array_values(array_unique(array_merge($proofIds,array_map('intval',$allProofs->fetchAll(PDO::FETCH_COLUMN)))));}

    $adjustmentIds=[];if(schemaTableExists($pdo,'owner_adjustments')){$stmt=$pdo->prepare("SELECT id FROM owner_adjustments WHERE organization_id=? AND entity_type='client_packages' AND entity_id=?$lockSql");$stmt->execute([$organizationId,$packageId]);$adjustmentIds=array_map('intval',$stmt->fetchAll(PDO::FETCH_COLUMN));}
    $financeIds=[];$financeConditions=["(source_type IN ('client_package','client_packages','package') AND source_id=?)"];$financeParams=[$organizationId,$packageId];
    foreach([['payment',$paymentIds],['payment_proof',$proofIds],['owner_adjustment',$adjustmentIds]] as [$sourceType,$ids])if($ids){$financeConditions[]='(source_type=? AND source_id IN ('.implode(',',array_fill(0,count($ids),'?')).'))';$financeParams[]=$sourceType;array_push($financeParams,...$ids);}
    $financeStmt=$pdo->prepare('SELECT id FROM finance WHERE organization_id=? AND ('.implode(' OR ',$financeConditions).')'.$lockSql);$financeStmt->execute($financeParams);$financeIds=array_map('intval',$financeStmt->fetchAll(PDO::FETCH_COLUMN));
    $invoiceDeltas=[];foreach($allocations as $allocation)if(!empty($allocation['invoice_id'])){$invoiceId=(int)$allocation['invoice_id'];$invoiceDeltas[$invoiceId]=($invoiceDeltas[$invoiceId]??0)+packageMoneyCents($allocation['amount']??0);}
    $settlementAllocations=0;if(schemaTableExists($pdo,'session_settlement_allocations')){$stmt=$pdo->prepare('SELECT COUNT(*) FROM session_settlement_allocations WHERE source_client_package_id=? OR target_client_package_id=?');$stmt->execute([$packageId,$packageId]);$settlementAllocations=(int)$stmt->fetchColumn();}
    return ['booking_ids'=>$bookingIds,'allocation_ids'=>$allocationIds,'proof_ids'=>$proofIds,'payment_ids'=>$paymentIds,'finance_ids'=>$financeIds,'adjustment_ids'=>$adjustmentIds,'invoice_deltas'=>$invoiceDeltas,'settlement_allocations'=>$settlementAllocations];
}

function ownerDeletePackageCascade(PDO $pdo,array $user,array $package,string $reason): array {
    $organizationId=(int)$user['organization_id'];$packageId=(int)$package['id'];$scope=ownerPackageCascadeScope($pdo,$organizationId,$packageId,true);
    $deleted=['bookings'=>count($scope['booking_ids']),'ledger'=>ownerLinkedCount($pdo,'SELECT COUNT(*) FROM package_usage_ledger WHERE client_package_id=?',[$packageId]),'payment_allocations'=>count($scope['allocation_ids']),'payment_proofs'=>count($scope['proof_ids']),'payments'=>count($scope['payment_ids']),'finance'=>count($scope['finance_ids']),'settlements'=>$scope['settlement_allocations']];
    if(schemaTableExists($pdo,'session_settlement_allocations'))$pdo->prepare('DELETE FROM session_settlement_allocations WHERE source_client_package_id=? OR target_client_package_id=?')->execute([$packageId,$packageId]);
    if($scope['booking_ids']){$marks=implode(',',array_fill(0,count($scope['booking_ids']),'?'));if(schemaTableExists($pdo,'post_production_jobs'))$pdo->prepare("DELETE FROM post_production_jobs WHERE organization_id=? AND booking_id IN ($marks)")->execute(array_merge([$organizationId],$scope['booking_ids']));if(schemaTableExists($pdo,'session_settlements'))$pdo->prepare("DELETE FROM session_settlements WHERE organization_id=? AND booking_id IN ($marks)")->execute(array_merge([$organizationId],$scope['booking_ids']));if(schemaTableExists($pdo,'booking_archives'))$pdo->prepare("DELETE FROM booking_archives WHERE organization_id=? AND (client_package_id=? OR booking_id IN ($marks))")->execute(array_merge([$organizationId,$packageId],$scope['booking_ids']));if(schemaTableExists($pdo,'app_notifications'))$pdo->prepare("DELETE FROM app_notifications WHERE organization_id=? AND entity_type IN ('booking','bookings') AND entity_id IN ($marks)")->execute(array_merge([$organizationId],$scope['booking_ids']));$pdo->prepare("DELETE FROM bookings WHERE organization_id=? AND id IN ($marks)")->execute(array_merge([$organizationId],$scope['booking_ids']));}
    elseif(schemaTableExists($pdo,'booking_archives'))$pdo->prepare('DELETE FROM booking_archives WHERE organization_id=? AND client_package_id=?')->execute([$organizationId,$packageId]);
    $pdo->prepare('UPDATE projects SET client_package_id=NULL WHERE organization_id=? AND client_package_id=?')->execute([$organizationId,$packageId]);
    if($scope['finance_ids']){$marks=implode(',',array_fill(0,count($scope['finance_ids']),'?'));$pdo->prepare("DELETE FROM finance WHERE organization_id=? AND id IN ($marks)")->execute(array_merge([$organizationId],$scope['finance_ids']));}
    if($scope['allocation_ids']){$marks=implode(',',array_fill(0,count($scope['allocation_ids']),'?'));$pdo->prepare("DELETE FROM payment_allocations WHERE organization_id=? AND id IN ($marks)")->execute(array_merge([$organizationId],$scope['allocation_ids']));}
    if($scope['proof_ids']){$marks=implode(',',array_fill(0,count($scope['proof_ids']),'?'));$pdo->prepare("DELETE FROM payment_proofs WHERE organization_id=? AND id IN ($marks)")->execute(array_merge([$organizationId],$scope['proof_ids']));}
    if($scope['payment_ids']){$marks=implode(',',array_fill(0,count($scope['payment_ids']),'?'));$pdo->prepare("DELETE FROM payments WHERE organization_id=? AND id IN ($marks)")->execute(array_merge([$organizationId],$scope['payment_ids']));}
    foreach($scope['invoice_deltas'] as $invoiceId=>$deltaCents){$stmt=$pdo->prepare("UPDATE invoices SET paid_amount=GREATEST(0,paid_amount-?),status=CASE WHEN status IN ('cancelled','void') THEN status WHEN GREATEST(0,paid_amount-?)<=0 THEN 'issued' WHEN GREATEST(0,paid_amount-?)>=total THEN 'paid' ELSE 'partial' END WHERE id=? AND organization_id=?");$amount=packageMoney($deltaCents);$stmt->execute([$amount,$amount,$amount,$invoiceId,$organizationId]);}
    if(schemaTableExists($pdo,'client_package_payment_requests'))$pdo->prepare('DELETE FROM client_package_payment_requests WHERE organization_id=? AND client_package_id=?')->execute([$organizationId,$packageId]);
    if(schemaTableExists($pdo,'owner_adjustments'))$pdo->prepare("DELETE FROM owner_adjustments WHERE organization_id=? AND entity_type='client_packages' AND entity_id=?")->execute([$organizationId,$packageId]);
    $pdo->prepare('DELETE FROM package_usage_ledger WHERE client_package_id=?')->execute([$packageId]);
    if(schemaTableExists($pdo,'app_notifications'))$pdo->prepare("DELETE FROM app_notifications WHERE organization_id=? AND entity_type IN ('client_package','client_packages','package') AND entity_id=?")->execute([$organizationId,$packageId]);
    if(schemaTableExists($pdo,'change_events')){$conditions=["(entity_type IN ('client_package','client_packages','package') AND entity_id=?)"];$params=[$organizationId,$packageId];if($scope['booking_ids']){$marks=implode(',',array_fill(0,count($scope['booking_ids']),'?'));$conditions[]="(entity_type IN ('booking','bookings') AND entity_id IN ($marks))";array_push($params,...$scope['booking_ids']);}$pdo->prepare('DELETE FROM change_events WHERE organization_id=? AND ('.implode(' OR ',$conditions).')')->execute($params);}
    $pdo->prepare('DELETE FROM client_packages WHERE id=? AND organization_id=?')->execute([$packageId,$organizationId]);
    return $deleted;
}

function ownerRecordImpact(PDO $pdo,array $user,string $entity,int $id,bool $lock=false): array {
    if($entity==='bookings')fail('إجراءات المواعيد تستخدم مسار الحذف الآمن المخصص فقط.',410,'booking_owner_action_retired');
    ...
    if($entity==='clients'){
        ...
        $scope=ownerProjectCascadeScope($pdo,$organizationId,$record,$lock);$links=['bookings'=>count($scope['booking_ids']),'tasks'=>ownerLinkedCount($pdo,'SELECT COUNT(*) FROM project_tasks WHERE organization_id=? AND project_id=?',[$organizationId,$id]),'items'=>ownerLinkedCount($pdo,'SELECT COUNT(*) FROM project_items WHERE organization_id=? AND project_id=?',[$organizationId,$id]),'milestones'=>ownerLinkedCount($pdo,'SELECT COUNT(*) FROM project_milestones WHERE organization_id=? AND project_id=?',[$organizationId,$id]),'content'=>ownerLinkedCount($pdo,'SELECT COUNT(*) FROM content_items WHERE organization_id=? AND project_id=?',[$organizationId,$id]),'invoices'=>count($scope['invoice_ids']),'payments'=>count($scope['payment_ids']),'proofs'=>count($scope['proof_ids']),'finance'=>count($scope['finance_ids'])];
        $action='cascade_delete';$explanation='سيُحذف المشروع نهائيًا مع مواعيده ومهامه ومراحله ومحتواه، وكذلك فواتيره ودفعاته وإثباتات الدفع وقيوده المالية المرتبطة به فقط.';
...
if (preg_match('#^/owner/records/([a-z_]+)/([0-9]+)/action$#',$path,$m) && $method==='POST') {
    $user=requireUser($user);requireRole($user,['owner']);$entity=$m[1];$id=(int)$m[2];$payload=body();if($entity==='bookings'){if(array_key_exists('reason',$payload)||array_key_exists('charge',$payload))fail('حذف الموعد لا يقبل سببًا أو خصمًا.',422,'cancellation_reason_not_supported');fail('استخدم DELETE /bookings/{id} لمسار الحذف الآمن المخصص.',410,'booking_owner_action_retired');}$reason=trim((string)($payload['reason']??''));if(mb_strlen($reason)<5||mb_strlen($reason)>500)fail('اكتب سبب الإجراء بوضوح (5 أحرف على الأقل).',422,'owner_reason_required');$pdo->beginTransaction();
    try{$impact=ownerRecordImpact($pdo,$user,$entity,$id,true);$action=$impact['action'];$before=$impact['record'];if(!empty($payload['expected_action'])&&$payload['expected_action']!==$action)fail('تغيّرت الروابط المرتبطة بالسجل. راجع التأثير مرة أخرى.',409,'stale_owner_impact');if(in_array($action,['hard_delete','cascade_delete'],true)&&trim((string)($payload['confirmation']??''))!=='حذف')fail('اكتب كلمة حذف لتأكيد الحذف النهائي.',422,'hard_delete_confirmation_required');
        ...
        if($action==='cascade_delete'&&$entity==='projects'){$afterDelete=ownerDeleteProjectCascade($pdo,$user,$before,$reason);$impact['deleted_records']=$afterDelete;
...
if ($path==='/studio-sessions/start-direct' && $method==='POST') {
    $user=requireUser($user);requireRole($user,['owner','admin','operations']);$payload=body();$clientId=(int)($payload['client_id']??0);$resourceId=(int)($payload['resource_id']??0);$date=trim((string)($payload['date']??''));$end=normalizeBusinessTime($payload['end_time']??'',true);$title=mb_substr(trim((string)($payload['title']??'')),0,120);if($title==='')$title='جلسة تصوير مباشرة';$note=mb_substr(trim((string)($payload['note']??'')),0,1000);$idempotencyKey=trim((string)($payload['idempotency_key']??''));
    if(!preg_match('/^[A-Za-z0-9:_-]{12,120}$/',$idempotencyKey))fail('مفتاح حفظ الجلسة غير صحيح.',422,'invalid_idempotency_key');$now=cairoNow();$today=$now->format('Y-m-d');if($date==='' )$date=$today;if($date!==$today)fail('يمكن بدء جلسة مباشرة في تاريخ اليوم فقط.',422,'session_date_mismatch');
    $startMinute=(int)$now->format('G')*60+(int)$now->format('i');$startMinute=(int)floor($startMinute/15)*15;if($startMinute<720||$startMinute>=1440)fail('يمكن بدء التصوير المباشر خلال ساعات العمل فقط.',422,'outside_business_hours');$start=sprintf('%02d:%02d',intdiv($startMinute,60),$startMinute%60);$duration=bookingDurationMinutes($start,$end);$requestHash=hash('sha256',json_encode(['client_id'=>$clientId,'resource_id'=>$resourceId,'date'=>$date,'end_time'=>$end,'title'=>$title,'note'=>$note],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));$requestKey='direct-session:'.$idempotencyKey;
    $pdo->beginTransaction();try{
        $userLock=$pdo->prepare('SELECT id FROM users WHERE id=? AND organization_id=? FOR UPDATE');$userLock->execute([$user['id'],$user['organization_id']]);if(!$userLock->fetch())fail('المستخدم غير متاح.',403,'forbidden');
        $idem=$pdo->prepare('SELECT request_hash,response_status,response_body,status FROM idempotency_requests WHERE organization_id=? AND user_id=? AND request_key=? FOR UPDATE');$idem->execute([$user['organization_id'],$user['id'],$requestKey]);$prior=$idem->fetch();if($prior){if(!hash_equals((string)$prior['request_hash'],$requestHash))fail('تم استخدام مفتاح الحفظ نفسه ببيانات مختلفة.',409,'idempotency_payload_mismatch');$response=json_decode((string)($prior['response_body']??''),true);if((string)$prior['status']!=='completed'||!is_array($response))fail('جارٍ حفظ هذه الجلسة من طلب آخر.',409,'idempotency_in_progress');$pdo->commit();$response['idempotent']=true;respond($response);}
        $pdo->prepare("INSERT INTO idempotency_requests (organization_id,user_id,request_key,request_hash,status,expires_at) VALUES (?,?,?,?,'processing',DATE_ADD(NOW(),INTERVAL 7 DAY))")->execute([$user['organization_id'],$user['id'],$requestKey,$requestHash]);
        $clientStmt=$pdo->prepare("SELECT id,name FROM clients WHERE id=? AND organization_id=? AND status='active' FOR UPDATE");$clientStmt->execute([$clientId,$user['organization_id']]);$client=$clientStmt->fetch();if(!$client)fail('اختر عميلًا نشطًا.',422,'invalid_booking_client');
        $duration=validateBookingSchedule($pdo,(int)$user['organization_id'],$resourceId,$date,$start,$end,60,15,null,null,true,$user,$payload);
        $insert=$pdo->prepare("INSERT INTO bookings (organization_id,client_id,client_package_id,service_id,resource_id,client_name,service,date,start_time,end_time,duration_minutes,requested_quantity,status,notes,created_by) VALUES (?,?,NULL,NULL,?,?,?,?,?,?,?,?, 'confirmed',?,?)");$insert->execute([$user['organization_id'],$clientId,$resourceId,$client['name'],$title,$date,$start.':00',$end.':00',$duration,settlementHours($duration),$note?:null,$user['id']]);$bookingId=(int)$pdo->lastInsertId();$booking=['id'=>$bookingId,'organization_id'=>(int)$user['organization_id'],'client_id'=>$clientId,'client_name'=>$client['name'],'client_package_id'=>null,'service_id'=>null,'resource_id'=>$resourceId,'service'=>$title,'date'=>$date,'start_time'=>$start.':00','end_time'=>$end.':00','duration_minutes'=>$duration,'requested_quantity'=>settlementHours($duration),'status'=>'confirmed','notes'=>$note?:null];reserveBookingSlots($pdo,$booking);
        $pdo->prepare('INSERT INTO booking_status_history (booking_id,from_status,to_status,note,changed_by) VALUES (?,NULL,?,?,?)')->execute([$bookingId,'confirmed','إنشاء جلسة تصوير مباشرة دون باقة',$user['id']]);audit($pdo,$user,'create_direct','bookings',$bookingId,null,$booking);$session=startBookingSession($pdo,$user,$bookingId,'manual');$booking['status']='in_progress';$booking['timer_started_at']=$session['started_at']??null;$response=['booking'=>$booking,'session'=>$session,'requires_package_assignment'=>true,'idempotent'=>false];
        $pdo->prepare("UPDATE idempotency_requests SET response_status=201,response_body=?,status='completed' WHERE organization_id=? AND user_id=? AND request_key=?")->execute([json_encode($response,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$user['organization_id'],$user['id'],$requestKey]);recordChangeEvent($pdo,(int)$user['organization_id'],$clientId,'bookings','bookings',$bookingId,'direct_session_started');$pdo->commit();respond($response,201);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
...
if ($path === '/app-notifications/read-all' && $method === 'POST') {
    $user=requireUser($user);$readPayload=body();$upToId=max(0,(int)($readPayload['up_to_id']??0));$channel=trim((string)($readPayload['channel']??''));if($upToId<1)fail('حد تعليم الإشعارات غير صحيح.',422,'invalid_notification_boundary');$pdo->beginTransaction();try{$sql='UPDATE app_notifications SET read_at=NOW() WHERE organization_id=? AND id<=? AND read_at IS NULL AND dismissed_at IS NULL';$params=[(int)$user['organization_id'],$upToId];if($channel==='client-actions'){requireRole($user,['owner']);$sql.=" AND audience='owner' AND recipient_user_id=?";$params[]=(int)$user['id'];}elseif($user['role']==='client'){$sql.=" AND audience='client' AND client_id=?";$params[]=(int)$user['client_id'];}else{$sql.=" AND (audience='staff' OR recipient_user_id=?)";$params[]=(int)$user['id'];}$stmt=$pdo->prepare($sql);$stmt->execute($params);$changed=$stmt->rowCount();if($changed>0)recordChangeEvent($pdo,(int)$user['organization_id'],$user['role']==='client'?(int)$user['client_id']:null,'notifications','app_notifications',$upToId,'read_all');$pdo->commit();respond(['read'=>true,'changed'=>$changed,'up_to_id'=>$upToId]);}catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
...
function enrichedFinanceEntry(PDO $pdo, int $organizationId, array $entry): array {
    ...
    if(!empty($entry['employee_user_id'])){$employee=$pdo->prepare("SELECT id,full_name FROM users WHERE id=? AND organization_id=? AND role NOT IN ('client','applicant') LIMIT 1");$employee->execute([(int)$entry['employee_user_id'],$organizationId]);if($row=$employee->fetch()){$entry['employee_user_id']=(int)$row['id'];$entry['employee_name']=$row['full_name'];}}
    return $entry;
}

function employeeFinanceSemanticKind(array $entry): ?string {
    $kind=(string)($entry['entry_kind']??'');
    if($kind==='expense'&&!empty($entry['employee_user_id']))return 'out_of_pocket';
    return in_array($kind,['advance_out','advance_in','settlement_out'],true)?$kind:null;
}

function employeeFinanceSignedCents(string $kind,int $amountCents): int {
    return in_array($kind,['out_of_pocket','advance_in'],true)?$amountCents:-$amountCents;
}

function employeeOpeningAdjustmentCents(PDO $pdo,int $organizationId,array $employee): int {
    $first=trim((string)explode(' ',trim((string)$employee['full_name']))[0]);
    $alias=in_array($first,['أشرف','اشرف'],true)?'اشرف':($first==='مروة'?'مروة':'');
    if($alias==='')return 0;
    $stmt=$pdo->prepare('SELECT value FROM app_config WHERE organization_id=? AND `key`=? LIMIT 1');$stmt->execute([$organizationId,'partner_'.$alias.'_adj']);
    return packageMoneyCents($stmt->fetchColumn()?:0);
}

function employeeFinanceSchemaStatus(PDO $pdo): array {
    $required=['employee_user_id','entry_kind','category','amount','method','detail','date','source_type','source_id','voided_at'];
    $columns=schemaTableColumns($pdo,'finance');$missing=array_values(array_diff($required,$columns));
    return ['ready'=>count($missing)===0,'missing_columns'=>$missing];
}
...
function employeeFinanceAccountSnapshot(PDO $pdo,int $organizationId,string $month): array {
    ...
    $users=$pdo->prepare("SELECT id,full_name,role FROM users WHERE organization_id=? AND is_active=1 AND role NOT IN ('client','owner','applicant') ORDER BY full_name,id");
    ...
    foreach($users->fetchAll() as $employee){$userId=(int)$employee['id'];$totals=['out_of_pocket'=>0,'advance_out'=>0,'advance_in'=>0,'settlement_out'=>0];$transactions=[];$monthSigned=0;
        foreach($byUser[$userId]??[] as $entry){$kind=employeeFinanceSemanticKind($entry);if($kind===null)continue;$amount=max(0,packageMoneyCents($entry['amount']));$totals[$kind]+=$amount;$signed=employeeFinanceSignedCents($kind,$amount);if(str_starts_with((string)$entry['date'],$month)){$monthSigned+=$signed;$transactions[]=['finance_id'=>(int)$entry['id'],'date'=>$entry['date'],'kind'=>$kind,'amount'=>packageMoney($amount),'signed_amount'=>packageMoney($signed),'method'=>$entry['method'],'detail'=>$entry['detail'],'category'=>$entry['category'],'status'=>'active','source_type'=>$entry['source_type'],'source_id'=>$entry['source_id']?(int)$entry['source_id']:null];}}
        $opening=employeeOpeningAdjustmentCents($pdo,$organizationId,$employee);$net=$totals['out_of_pocket']+$totals['advance_in']-$totals['settlement_out']-$totals['advance_out']+$opening;
        $accounts[]=['user'=>['id'=>$userId,'full_name'=>$employee['full_name'],'role'=>$employee['role']],'totals'=>array_map('packageMoney',$totals),'opening_adjustment'=>packageMoney($opening),'net_due_to_employee'=>packageMoney($net),'selected_month'=>['month'=>$month,'movement_count'=>count($transactions),'signed_amount'=>packageMoney($monthSigned),'transactions'=>$transactions]];
    }
    $unlinked=$pdo->prepare("SELECT COUNT(*) FROM finance WHERE organization_id=? AND employee_user_id IS NULL AND voided_at IS NULL AND (entry_kind IS NULL OR entry_kind<>'reversal') AND TRIM(entity) IN ('اشرف','أشرف','مروة')");$unlinked->execute([$organizationId]);
    return ['month'=>$month,'accounts'=>$accounts,'unlinked_legacy_count'=>(int)$unlinked->fetchColumn(),'schema_ready'=>true,'migration_required'=>null];
}
...
if ($path === '/finance/manual' && $method === 'POST') {
    $user=requireUser($user);requireRole($user,['owner','admin']);$payload=body();$kind=(string)($payload['entry_kind']??'');if(!in_array($kind,['income','expense','advance_in','advance_out','settlement_out'],true))fail('نوع الحركة المالية غير صحيح.',422,'invalid_finance_kind');$amountCents=packageMoneyCents($payload['amount']??0);if($amountCents<=0)fail('مبلغ الحركة يجب أن يكون أكبر من صفر.',422,'invalid_finance_amount');$amount=packageMoney($amountCents);$category=trim((string)($payload['category']??''))?:($kind==='income'?'other_income':'general_expense');$clientId=!empty($payload['client_id'])?(int)$payload['client_id']:null;$employeeUserId=!empty($payload['employee_user_id'])?(int)$payload['employee_user_id']:null;$sourceType=trim((string)($payload['source_type']??''))?:null;$sourceId=!empty($payload['source_id'])?(int)$payload['source_id']:null;
    ...
    $pdo->beginTransaction();try{$type=match($kind){'income'=>'إيراد','advance_in'=>'سداد سلفة','advance_out'=>'سحب سلفة','settlement_out'=>'سداد مستحقات',default=>'مصروف'};
        ...
        if($hasEmployeeColumn){
            $stmt=$pdo->prepare('INSERT INTO finance (organization_id,client_id,employee_user_id,type,entry_kind,category,amount,method,detail,date,entity,source_type,source_id,is_system,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,0,?)');
            $stmt->execute([$user['organization_id'],$clientId,$employeeUserId,$type,$kind,$category,$amount,$method,$detail,$date,$entity?:'الشركة',$sourceType,$sourceId,$user['id']]);
        }else{
            // Migration 027 is imported manually on Hostinger. Ordinary
            // unlinked income/expense entries must remain writable until then.
            $stmt=$pdo->prepare('INSERT INTO finance (organization_id,client_id,type,entry_kind,category,amount,method,detail,date,entity,source_type,source_id,is_system,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,0,?)');
            $stmt->execute([$user['organization_id'],$clientId,$type,$kind,$category,$amount,$method,$detail,$date,$entity?:'الشركة',$sourceType,$sourceId,$user['id']]);
        }
        $id=(int)$pdo->lastInsertId();audit($pdo,$user,'create_manual_finance','finance',$id,null,['client_id'=>$clientId,'employee_user_id'=>$employeeUserId,'entry_kind'=>$kind,'category'=>$category,'amount'=>$amount,'source_type'=>$sourceType,'source_id'=>$sourceId]);$read=$pdo->prepare('SELECT * FROM finance WHERE id=? AND organization_id=?');$read->execute([$id,$user['organization_id']]);$entry=enrichedFinanceEntry($pdo,(int)$user['organization_id'],$read->fetch());$pdo->commit();respond($entry,201);}catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}

if ($path === '/finance/transfer' && $method === 'POST') {
    $user=requireUser($user);requireRole($user,['owner','admin','finance']);$payload=body();$from=requirePaymentMethod($payload['from_method']??'');$to=requirePaymentMethod($payload['to_method']??'');$amount=(float)($payload['amount']??0);$date=(string)($payload['date']??date('Y-m-d'));$note=trim((string)($payload['note']??''));
...
if (preg_match('#^/finance/(\d+)/correct$#',$path,$m)&&$method==='POST') {
    $user=requireUser($user);requireRole($user,['owner']);$id=(int)$m[1];$payload=body();$reason=ownerCorrectionReason($payload);$pdo->beginTransaction();try{$stmt=$pdo->prepare('SELECT * FROM finance WHERE id=? AND organization_id=? FOR UPDATE');$stmt->execute([$id,$user['organization_id']]);$entry=$stmt->fetch();if(!$entry){$pdo->rollBack();fail('الحركة المالية غير موجودة.',404,'finance_not_found');}if(!empty($entry['is_system'])||(!empty($entry['source_type'])&&$entry['source_type']!=='employee_account')){$pdo->rollBack();fail('هذه حركة نظامية؛ يجب تصحيحها من مصدرها الأصلي.',409,'correct_at_source');}if(!empty($entry['voided_at'])||$entry['entry_kind']==='reversal'){$pdo->rollBack();fail('لا يمكن تصحيح حركة ملغاة أو عكسية.',409,'already_voided');}$amountCents=packageMoneyCents($payload['amount']??0);$kind=(string)($payload['entry_kind']??$entry['entry_kind']);$methodValue=requirePaymentMethod($payload['method']??$entry['method']);$detail=trim((string)($payload['detail']??$entry['detail']));$date=(string)($payload['date']??$entry['date']);if($amountCents<=0||!in_array($kind,['income','expense','advance_in','advance_out','settlement_out'],true)||$methodValue===''||$detail===''||!preg_match('/^\d{4}-\d{2}-\d{2}$/',$date)){$pdo->rollBack();fail('بيانات الحركة البديلة غير صحيحة.',422,'invalid_finance_replacement');}$reversalId=financeReversal($pdo,$user,$entry,$reason);$type=match($kind){'income'=>'إيراد','advance_in'=>'سداد سلفة','advance_out'=>'سحب سلفة','settlement_out'=>'سداد مستحقات',default=>'مصروف'};$insert=$pdo->prepare('INSERT INTO finance (organization_id,client_id,employee_user_id,type,entry_kind,category,amount,method,detail,date,entity,source_type,source_id,is_system,corrected_from_id,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,0,?,?)');$insert->execute([$user['organization_id'],$entry['client_id'],$entry['employee_user_id'],$type,$kind,trim((string)($payload['category']??$entry['category']))?:($kind==='income'?'other_income':'general_expense'),packageMoney($amountCents),$methodValue,$detail,$date,$kind==='income'?'الشركة':trim((string)($payload['entity']??$entry['entity'])),$entry['source_type'],$entry['source_id'],$id,$user['id']]);$replacementId=(int)$pdo->lastInsertId();audit($pdo,$user,'correct_finance','finance',$id,$entry,['reversal_id'=>$reversalId,'replacement_id'=>$replacementId,'reason'=>$reason]);$pdo->commit();respond(['id'=>$id,'voided'=>true,'reversal_id'=>$reversalId,'replacement_id'=>$replacementId]);}catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
...
if ($path === '/cron/booking-tick' && $method === 'POST') {
    ...
    $started=0;$materialized=0;foreach($pdo->query('SELECT id FROM organizations')->fetchAll() as $organization){$organizationId=(int)$organization['id'];$started+=activateScheduledSessions($pdo,$organizationId);$materialized+=materializePackageLifecycleNotifications($pdo,$organizationId);}respond(['started'=>$started,'materialized_notifications'=>$materialized,'server_now'=>cairoNow()->format(DATE_ATOM)]);
}
...
if ($path === '/attendance/records' && $method === 'GET') {
    ...
    $stmt=$pdo->prepare("SELECT r.*,u.full_name,u.role FROM attendance_records r JOIN users u ON u.id=r.user_id WHERE r.organization_id=? AND r.user_id=? AND r.work_date LIKE ? AND u.role NOT IN ('client','owner','applicant') ORDER BY r.work_date DESC");$stmt->execute([$user['organization_id'],$target,$month.'-%']);$records=$stmt->fetchAll();
...
if ($path === '/attendance/records/manual' && $method === 'PUT') {
    $user=requireUser($user);requireRole($user,['owner']);$payload=body();$organizationId=(int)$user['organization_id'];$target=(int)($payload['user_id']??0);$workDate=trim((string)($payload['work_date']??''));$status=trim((string)($payload['status']??''));$reason=trim((string)($payload['correction_reason']??''));
    $allowed=['present','late','early_leave','absent','authorized_leave'];
    if($target<1||!preg_match('/^\d{4}-\d{2}-\d{2}$/',$workDate)||!in_array($status,$allowed,true)||mb_strlen($reason)<5)fail('حدد الموظف واليوم والحالة وسببًا واضحًا للتسجيل اليدوي.',422,'invalid_manual_attendance');
    ...
    $pdo->beginTransaction();try{
        ...
        $afterStmt=$pdo->prepare('SELECT * FROM attendance_records WHERE id=? AND organization_id=?');$afterStmt->execute([$id,$organizationId]);$after=$afterStmt->fetch();audit($pdo,$user,$before?'owner_correct_attendance_day':'owner_create_attendance_day','attendance_records',$id,$before,$after+['reason'=>$reason]);recordChangeEvent($pdo,$organizationId,null,'attendance','attendance_records',$id,$before?'corrected':'created');$pdo->commit();respond(['record'=>$after,'created'=>$before===null],$before?200:201);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}

if ($path === '/attendance/adjustments' && $method === 'POST') {
    $user=requireUser($user);requireRole($user,['owner']);$payload=body();$target=(int)($payload['user_id']??0);$month=validMonth((string)($payload['month']??''));$reason=trim((string)($payload['reason']??''));$amount=(float)($payload['amount']??0);if($target<=0||mb_strlen($reason)<5||$amount==0.0)fail('الموظف والمبلغ وسبب واضح مطلوبون.',422,'validation_error');$stmt=$pdo->prepare("SELECT id FROM users WHERE id=? AND organization_id=? AND role NOT IN ('client','owner','applicant')");$stmt->execute([$target,$user['organization_id']]);if(!$stmt->fetch())fail('الموظف غير موجود.',404);
...
if (preg_match('#^/owner/clients/(\d+)/balance-adjustment$#',$path,$m) && $method === 'POST') {
    $user=requireUser($user);requireRole($user,['owner']);$organizationId=(int)$user['organization_id'];$clientId=(int)$m[1];$payload=body();$action=(string)($payload['action']??'');$amountCents=packageMoneyCents($payload['amount']??0);$methodValue=requirePaymentMethod($payload['method']??'');$key=trim((string)($payload['idempotency_key']??''));
    if(!in_array($action,['pay_debt','add_credit'],true)||$amountCents<=0||!in_array($methodValue,['cash','instapay','vodafone_cash'],true)||!preg_match('/^[A-Za-z0-9:_-]{12,120}$/',$key))fail('بيانات حركة رصيد العميل غير صحيحة.',422,'invalid_client_balance_adjustment');$correlation='owner-client-balance:'.$key;$date=cairoNow()->format('Y-m-d');
    $pdo->beginTransaction();try{
        $existingStmt=$pdo->prepare('SELECT * FROM finance WHERE organization_id=? AND correlation_id=? FOR UPDATE');$existingStmt->execute([$organizationId,$correlation]);if($existing=$existingStmt->fetch()){$same=(int)$existing['client_id']===$clientId&&packageMoneyCents($existing['amount'])===$amountCents&&(string)$existing['method']===$methodValue&&(string)$existing['category']===($action==='pay_debt'?'legacy_client_debt':'client_credit');if(!$same)fail('مفتاح الحفظ مستخدم لحركة مختلفة.',409,'idempotency_mismatch');$clientStmt=$pdo->prepare('SELECT id,debt,credit,points,points_updated_at FROM clients WHERE id=? AND organization_id=?');$clientStmt->execute([$clientId,$organizationId]);$pdo->commit();respond(['id'=>(int)$existing['id'],'idempotent'=>true,'client'=>$clientStmt->fetch()]);}
        $clientStmt=$pdo->prepare('SELECT * FROM clients WHERE id=? AND organization_id=? FOR UPDATE');$clientStmt->execute([$clientId,$organizationId]);$before=$clientStmt->fetch();if(!$before)fail('العميل غير موجود.',404,'client_not_found');$amount=packageMoney($amountCents);$debtCents=packageMoneyCents($before['debt']??0);$creditCents=packageMoneyCents($before['credit']??0);if($action==='pay_debt'&&$amountCents>$debtCents)fail('المبلغ أكبر من المديونية الحالية. سجّل الزيادة كرصيد للعميل في عملية مستقلة.',422,'payment_above_client_debt');
        $configStmt=$pdo->prepare("SELECT `key`,value FROM app_config WHERE organization_id=? AND `key` IN ('points_egp_spent','points_earned')");$configStmt->execute([$organizationId]);$config=[];foreach($configStmt->fetchAll() as $row)$config[$row['key']]=$row['value'];$spent=max(0.01,(float)($config['points_egp_spent']??100));$earned=max(0,(float)($config['points_earned']??1));$pointsDelta=(int)floor(((float)$amount/$spent)*$earned);$newDebt=$action==='pay_debt'?$debtCents-$amountCents:$debtCents;$newCredit=$action==='add_credit'?$creditCents+$amountCents:$creditCents;$newPoints=max(0,(int)($before['points']??0)+$pointsDelta);
        $pdo->prepare('UPDATE clients SET debt=?,credit=?,points=?,points_updated_at=? WHERE id=? AND organization_id=?')->execute([packageMoney($newDebt),packageMoney($newCredit),$newPoints,$date,$clientId,$organizationId]);$category=$action==='pay_debt'?'legacy_client_debt':'client_credit';$detail=$action==='pay_debt'?'سداد مديونية من العميل '.$before['name']:'إيداع رصيد للعميل '.$before['name'];$insert=$pdo->prepare("INSERT INTO finance (organization_id,client_id,type,entry_kind,category,amount,method,detail,date,entity,source_type,source_id,correlation_id,is_system,created_by) VALUES (?,?,?,'income',?,?,?,?,?,'الشركة','owner_client_balance',?,?,1,?)");$insert->execute([$organizationId,$clientId,'إيراد',$category,$amount,$methodValue,$detail,$date,$clientId,$correlation,$user['id']]);$financeId=(int)$pdo->lastInsertId();$after=['id'=>$clientId,'debt'=>packageMoney($newDebt),'credit'=>packageMoney($newCredit),'points'=>$newPoints,'points_updated_at'=>$date];audit($pdo,$user,'owner_client_balance_adjustment','clients',$clientId,$before,$after+['finance_id'=>$financeId,'action'=>$action,'amount'=>$amount,'method'=>$methodValue]);recordChangeEvent($pdo,$organizationId,$clientId,'finance','clients',$clientId,'balance_adjusted');$pdo->commit();respond(['id'=>$financeId,'idempotent'=>false,'client'=>$after],201);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
...
if ($path === '/users' && $method === 'POST') {
    ...
    $phone = loginMobile($payload['phone'] ?? '');
    if ($phone === '') fail('رقم الموبايل مطلوب لتسجيل الدخول.', 422, 'invalid_login_phone');
    assertLoginMobileAvailable($pdo, $phone);
    $stmt = $pdo->prepare('INSERT INTO users (organization_id, client_id, full_name, email, phone, password_hash, role, permissions) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
    $stmt->execute([$user['organization_id'], null, trim((string)$payload['full_name']), $payload['email'] ?: null, $phone, password_hash($password, PASSWORD_DEFAULT), $role, json_encode($payload['permissions'] ?? [], JSON_UNESCAPED_UNICODE)]);
...
if (preg_match('#^/users/(\d+)$#', $path, $m) && $method === 'PATCH') {
    ...
    if(array_key_exists('phone',$payload)){ $payload['phone']=loginMobile($payload['phone']); if($payload['phone']==='')fail('رقم الموبايل مطلوب لتسجيل الدخول.',422,'invalid_login_phone'); assertLoginMobileAvailable($pdo,$payload['phone'],$targetId); }
    $updates=[];$params=[];
    foreach(['full_name','email','phone','role','is_active'] as $field){if(array_key_exists($field,$payload)){$updates[]="`$field`=?";$params[]=$payload[$field];}}
...
if ($path === '/clients' && $method === 'POST') {
    $user = requireUser($user); requireRole($user, ['owner','admin','operations']);
    $payload=body();$name=trim((string)($payload['name']??''));$phone=normalizeClientContactPhone($payload['phone1']??'');
    $additionalPhones=normalizeAdditionalClientPhones($payload['additional_phones']??(!empty($payload['phone2'])?[$payload['phone2']]:[]),$phone);
    requireClientPhonesSchema($pdo);
    ...
    try{
        ...
        $stmt=$pdo->prepare('INSERT INTO clients (organization_id,name,company_name,contact_person,phone1,phone2,additional_phones,email,job,address,city,tax_number,commercial_registration,preferred_contact,whatsapp_opt_in,whatsapp_opt_in_at,color,notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
        $stmt->execute([$user['organization_id'],$name,$payload['company_name']??null,$payload['contact_person']??null,$phone,$additionalPhones[0]??null,json_encode($additionalPhones,JSON_UNESCAPED_UNICODE),$payload['email']??null,$payload['job']??null,$payload['address']??null,$payload['city']??null,$payload['tax_number']??null,$payload['commercial_registration']??null,$preferred,$whatsappOptIn?1:0,$whatsappOptIn?date('Y-m-d H:i:s'):null,$color,$payload['notes']??null]);$clientId=(int)$pdo->lastInsertId();
...
if (preg_match('#^/client-packages/(\d+)/details$#',$path,$m)&&$method==='GET') {
    ...
    $paymentNoteSelect=schemaColumnExists($pdo,'payments','note')?'p.note':'NULL AS note';
    $paymentSql="SELECT pa.id AS allocation_id,pa.amount,pa.client_package_id,pa.invoice_id,pa.created_at AS allocated_at,
      p.id AS payment_id,p.method,p.status,p.reference,$paymentNoteSelect,p.created_at,p.reviewed_at,
    ...
    respond([
      ...
      'validity'=>['starts_at'=>$package['starts_at']?substr((string)$package['starts_at'],0,10):null,'expires_at'=>$package['expires_at']?substr((string)$package['expires_at'],0,10):null,'today'=>$today,'remaining_calendar_days'=>$calendarDays,'friday_included'=>true,'state'=>$effectiveStatus==='expired'?'expired':(!$package['expires_at']?'pending_activation':($calendarDays<=14?'near_expiry':'active'))],
...
if ($path === '/client-packages/legacy-db-import' && $method === 'POST') {
    ...
    $rows=$payload['packages']??null;if(!is_array($rows)||count($rows)<1||count($rows)>200)fail('قائمة الباقات القديمة غير صالحة.',422,'invalid_legacy_packages');
    $requestHash=hash('sha256',json_encode(['source_sha256'=>$sourceSha,'packages'=>$rows],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
    $readiness=packageSaleSchemaReadiness($pdo);if(!$readiness['ready'])fail('بيئة الباقات غير جاهزة لاستقبال النقل الآمن.',503,'package_sales_not_ready');
    $abortLegacyImport=static function(string $message,int $status=422,string $code='invalid_legacy_package')use($pdo):never{if($pdo->inTransaction())$pdo->rollBack();fail($message,$status,$code);};
    $pdo->beginTransaction();try{
        $priorStmt=$pdo->prepare('SELECT request_hash,status,response_json FROM client_package_sale_requests WHERE organization_id=? AND idempotency_key=? LIMIT 1 FOR UPDATE');$priorStmt->execute([$organizationId,$idempotencyKey]);$prior=$priorStmt->fetch();
        if($prior){if(!hash_equals((string)$prior['request_hash'],$requestHash))$abortLegacyImport('تم استخدام ملف النسخة نفسه سابقًا ببيانات مختلفة.',409,'idempotency_payload_mismatch');if($prior['status']==='completed'&&!empty($prior['response_json'])){$response=json_decode((string)$prior['response_json'],true)?:[];$response['idempotent']=true;$pdo->rollBack();respond($response);} $abortLegacyImport('عملية نقل الملف نفسها قيد التنفيذ.',409,'request_in_progress');}
        ...
        $clientStmt=$pdo->prepare('SELECT id,name,phone1,phone2 FROM clients WHERE organization_id=? FOR UPDATE');$clientStmt->execute([$organizationId]);$organizationClients=$clientStmt->fetchAll();$clientsByPhone=[];
        foreach($organizationClients as $clientRow){foreach(array_unique([normalizePhone((string)($clientRow['phone1']??'')),normalizePhone((string)($clientRow['phone2']??''))]) as $phone){if($phone==='')continue;$clientsByPhone[$phone]??=[];$clientsByPhone[$phone][]=$clientRow;}}
        $serviceStmt=$pdo->prepare('SELECT * FROM services WHERE id=? AND organization_id=? AND is_active=1 FOR UPDATE');
        $insertPackage=$pdo->prepare("INSERT INTO client_packages (organization_id,client_id,service_id,name,notes,billing_unit,purchased_quantity,purchased_minutes,held_quantity,held_minutes,consumed_quantity,consumed_minutes,payment_due_quantity,payment_due_minutes,deposit_percent_snapshot,overage_price_snapshot,total_price,paid_amount,starts_at,expires_at,validity_mode_snapshot,validity_days_snapshot,status) VALUES (?,?,?,?,?,?,?, ?,0,0,?,?,?, ?,?,?,?,?,?,?,'rolling',?,?)");
        $seenReferences=[];$prepared=[];
        foreach($rows as $index=>$row){
            $number=$index+1;if(!is_array($row))$abortLegacyImport('بيانات الباقة رقم '.$number.' غير صالحة.');
            $reference=strtolower(trim((string)($row['legacy_reference']??'')));if(!preg_match('/^sqlite-[a-f0-9]{16}$/',$reference)||isset($seenReferences[$reference]))$abortLegacyImport('مرجع الباقة رقم '.$number.' غير صالح أو مكرر.',422,'invalid_legacy_reference');$seenReferences[$reference]=true;
            if(strtolower(trim((string)($row['source_sha256']??'')))!==$sourceSha)$abortLegacyImport('الباقة رقم '.$number.' لا تنتمي إلى ملف النسخة المحدد.',422,'legacy_source_mismatch');
            $sourcePhone=normalizePhone((string)($row['source_phone']??''));$clientId=(int)($row['client_id']??0);$phoneMatches=$clientsByPhone[$sourcePhone]??[];
            if($sourcePhone===''||count($phoneMatches)!==1||(int)$phoneMatches[0]['id']!==$clientId)$abortLegacyImport('تعذرت مطابقة عميل الباقة رقم '.$number.' برقم الموبايل وحده.',409,'legacy_client_phone_mismatch');$client=$phoneMatches[0];
            $serviceId=(int)($row['service_id']??0);$serviceStmt->execute([$serviceId,$organizationId]);$service=$serviceStmt->fetch();if(!$service||!isStudioPackageOfferItem($service))$abortLegacyImport('قالب خدمة الباقة رقم '.$number.' غير موجود أو غير صالح.',422,'legacy_service_not_found');
            $unit=trim((string)($row['billing_unit']??''));$serviceUnit=normalizedStudioPackageUnit($service);if(!in_array($unit,['hour','reel'],true)||$unit!==$serviceUnit)$abortLegacyImport('وحدة رصيد الباقة رقم '.$number.' لا تطابق قالب الخدمة.',422,'legacy_service_unit_mismatch');
            $purchased=(float)($row['purchased_quantity']??0);$consumed=(float)($row['consumed_quantity']??0);$paymentDue=(float)($row['payment_due_quantity']??0);if(!is_finite($purchased)||!is_finite($consumed)||!is_finite($paymentDue)||$purchased<=0||$consumed<0||$consumed>$purchased+.000001||$paymentDue<0||$paymentDue>$purchased+.000001)$abortLegacyImport('رصيد الباقة رقم '.$number.' غير متزن.',422,'invalid_legacy_balance');
            if($unit==='reel'&&(floor($purchased)!=$purchased||floor($consumed)!=$consumed))$abortLegacyImport('عدد الريلز في الباقة رقم '.$number.' يجب أن يكون صحيحًا.',422,'invalid_legacy_reel_balance');
            $purchasedMinutes=$unit==='hour'?(int)round($purchased*60):null;$consumedMinutes=$unit==='hour'?(int)round($consumed*60):null;$paymentDueMinutes=$unit==='hour'?(int)round($paymentDue*60):null;if($unit==='hour'){$purchased=settlementHours($purchasedMinutes);$consumed=settlementHours($consumedMinutes);$paymentDue=settlementHours($paymentDueMinutes);}
            foreach(['total_price'=>'إجمالي السعر','paid_amount'=>'المدفوع'] as $moneyField=>$label){if(!array_key_exists($moneyField,$row)||!preg_match('/^\d+(?:\.\d{1,2})?$/',trim((string)$row[$moneyField])))$abortLegacyImport($label.' في الباقة رقم '.$number.' غير صحيح.',422,'invalid_legacy_money');}
            $totalCents=packageMoneyCents($row['total_price']);$paidCents=packageMoneyCents($row['paid_amount']);if($totalCents<0||$paidCents<0||$paidCents>$totalCents)$abortLegacyImport('الحالة المالية للباقة رقم '.$number.' غير صحيحة.',422,'invalid_legacy_money');
            $starts=trim((string)($row['starts_at']??''));$expires=trim((string)($row['expires_at']??''));$startDate=DateTimeImmutable::createFromFormat('!Y-m-d',$starts,new DateTimeZone('Africa/Cairo'));$expiryDate=DateTimeImmutable::createFromFormat('!Y-m-d',$expires,new DateTimeZone('Africa/Cairo'));if(!$startDate||$startDate->format('Y-m-d')!==$starts||!$expiryDate||$expiryDate->format('Y-m-d')!==$expires||$expires<$starts)$abortLegacyImport('فترة صلاحية الباقة رقم '.$number.' غير صحيحة.',422,'invalid_legacy_validity');
            $validityDays=(int)($row['validity_days_snapshot']??0);if($validityDays<1||$validityDays>3660)$abortLegacyImport('مدة صلاحية الباقة رقم '.$number.' غير صحيحة.',422,'invalid_legacy_validity');
            $status=trim((string)($row['status']??''));if(!in_array($status,['active','expired','completed'],true))$abortLegacyImport('حالة الباقة رقم '.$number.' غير صحيحة.',422,'invalid_legacy_status');
            $name=trim((string)($row['source_service_name']??''));if($name===''||mb_strlen($name)>180)$abortLegacyImport('اسم الباقة رقم '.$number.' غير صالح.',422,'invalid_legacy_name');
            $sourceClientName=trim((string)($row['source_client_name']??''));$overage=max(0,(float)($row['overage_quantity']??0));$noteParts=['نُقلت من البرنامج القديم دون إنشاء إيراد جديد.','الاسم في الملف القديم: '.($sourceClientName!==''?$sourceClientName:'غير مسجل').'.','مرجع النقل: '.$reference.'.'];if(!empty($row['archived_source']))$noteParts[]='كان اسم الباقة مؤرشفًا في البرنامج القديم.';if($overage>0)$noteParts[]='الاستهلاك الأصلي تجاوز الرصيد بمقدار '.round($overage,4).'؛ حُفظ المستخدم عند حد الباقة لمنع رصيد سالب.';$notes=implode(' ',$noteParts);
            $prepared[]=['reference'=>$reference,'client'=>$client,'client_id'=>$clientId,'service'=>$service,'service_id'=>$serviceId,'name'=>$name,'notes'=>$notes,'unit'=>$unit,'purchased'=>$purchased,'purchased_minutes'=>$purchasedMinutes,'consumed'=>$consumed,'consumed_minutes'=>$consumedMinutes,'payment_due'=>$paymentDue,'payment_due_minutes'=>$paymentDueMinutes,'total'=>packageMoney($totalCents),'paid'=>packageMoney($paidCents),'starts'=>$starts,'expires'=>$expires,'validity_days'=>$validityDays,'status'=>$status,'source_client_name'=>$sourceClientName,'source_phone'=>$sourcePhone];
        }

        $created=[];
        foreach($prepared as $item){$insertPackage->execute([$organizationId,$item['client_id'],$item['service_id'],$item['name'],$item['notes'],$item['unit'],$item['purchased'],$item['purchased_minutes'],$item['consumed'],$item['consumed_minutes'],$item['payment_due'],$item['payment_due_minutes'],max(0,(float)($item['service']['deposit_percent']??0)),packageMoney(packageMoneyCents($item['service']['overage_price']??0)),$item['total'],$item['paid'],$item['starts'],$item['expires'],$item['validity_days'],$item['status']]);$packageId=(int)$pdo->lastInsertId();$packageForLedger=['id'=>$packageId,'billing_unit'=>$item['unit']];insertPackageUsage($pdo,$packageForLedger,null,'opening',$item['purchased'],'رصيد افتتاحي من البرنامج القديم','legacy-import:'.$item['reference'].':opening',$user['id']);if($item['consumed']>0)insertPackageUsage($pdo,$packageForLedger,null,'consume',$item['consumed'],'استهلاك افتتاحي من البرنامج القديم','legacy-import:'.$item['reference'].':consume',$user['id']);$after=['client_id'=>$item['client_id'],'service_id'=>$item['service_id'],'name'=>$item['name'],'billing_unit'=>$item['unit'],'purchased_quantity'=>$item['purchased'],'consumed_quantity'=>$item['consumed'],'total_price'=>$item['total'],'paid_amount'=>$item['paid'],'starts_at'=>$item['starts'],'expires_at'=>$item['expires'],'status'=>$item['status'],'legacy_reference'=>$item['reference'],'source_phone'=>$item['source_phone'],'source_client_name'=>$item['source_client_name']];audit($pdo,$user,'legacy_package_import','client_packages',$packageId,null,$after);recordChangeEvent($pdo,$organizationId,$item['client_id'],'client_packages','client_packages',$packageId,'created');$created[]=['id'=>$packageId,'legacy_reference'=>$item['reference'],'client_id'=>$item['client_id'],'client_name'=>$item['client']['name'],'package_name'=>$item['name']];}
        $response=['imported_count'=>count($created),'source_sha256'=>$sourceSha,'packages'=>$created,'idempotent'=>false];$pdo->prepare("UPDATE client_package_sale_requests SET status='completed',response_json=?,completed_at=NOW() WHERE id=?")->execute([json_encode($response,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$requestId]);$pdo->commit();respond($response,201);
    }catch(PDOException $error){if($pdo->inTransaction())$pdo->rollBack();if(($error->errorInfo[1]??0)===1062){$replay=$pdo->prepare('SELECT request_hash,status,response_json FROM client_package_sale_requests WHERE organization_id=? AND idempotency_key=? LIMIT 1');$replay->execute([$organizationId,$idempotencyKey]);$existing=$replay->fetch();if($existing&&!hash_equals((string)$existing['request_hash'],$requestHash))fail('تم استخدام ملف النسخة نفسه ببيانات مختلفة.',409,'idempotency_payload_mismatch');if($existing&&$existing['status']==='completed'&&!empty($existing['response_json'])){$response=json_decode((string)$existing['response_json'],true)?:[];$response['idempotent']=true;respond($response);}fail('عملية نقل الملف نفسها قيد التنفيذ.',409,'request_in_progress');}throw $error;}catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
...
if ($path === '/client-packages' && $method === 'POST') {
    ...
    if(array_key_exists('loyalty_enabled',$payload)&&!is_bool($payload['loyalty_enabled']))fail('إعداد الولاء غير صحيح.',422,'invalid_loyalty_enabled');
    $upgradeContext=is_array($payload['upgrade_context']??null)?$payload['upgrade_context']:null;$upgradeSourceId=0;$upgradeReason='';$upgradeCloseSource=false;$upgradeActivation='first_booking';$upgradeExpectedVersion=null;
    if($upgradeContext!==null){requireRole($user,['owner']);$upgradeSourceId=(int)($upgradeContext['source_package_id']??0);$upgradeExpectedVersion=isset($upgradeContext['expected_source_version'])?(int)$upgradeContext['expected_source_version']:null;$upgradeCloseSource=!empty($upgradeContext['close_source_package']);$upgradeActivation=(string)($upgradeContext['activation_mode']??'first_booking');$upgradeReason=trim((string)($upgradeContext['reason']??''));if($upgradeSourceId<=0||$upgradeExpectedVersion===null||$upgradeExpectedVersion<1)fail('بيانات الباقة الأصلية غير مكتملة.',422,'invalid_package_upgrade_source');if(!in_array($upgradeActivation,['first_booking','immediate'],true))fail('طريقة بداية الباقة البديلة غير صحيحة.',422,'invalid_package_upgrade_activation');if(mb_strlen($upgradeReason)<5)fail('اكتب سببًا واضحًا لترقية الباقة.',422,'package_upgrade_reason_required');}
    if($upgradeContext!==null)foreach(['total_price'=>'إجمالي السعر','paid_amount'=>'المدفوع الآن','overage_price_snapshot'=>'سعر الساعة الإضافية'] as $moneyField=>$moneyLabel){if(!array_key_exists($moneyField,$payload))fail('حقل '.$moneyLabel.' مطلوب كنص عشري صريح.',422,'invalid_money_format');strictPackageMoneyCents($payload[$moneyField],$moneyLabel);}
    ...
    $pdo->beginTransaction();try{
        ...
        $pdo->prepare("INSERT INTO client_package_sale_requests (organization_id,idempotency_key,request_hash,status,created_by) VALUES (?,?,?,'processing',?)")->execute([$organizationId,$idempotencyKey,$requestHash,$user['id']]);$requestId=(int)$pdo->lastInsertId();
        $upgradeSource=null;$upgradeHasActiveSession=false;
        if($upgradeContext!==null){$upgradeStmt=$pdo->prepare('SELECT * FROM client_packages WHERE id=? AND organization_id=? FOR UPDATE');$upgradeStmt->execute([$upgradeSourceId,$organizationId]);$upgradeSource=$upgradeStmt->fetch();if(!$upgradeSource)$abortPackageSale('الباقة الأصلية غير موجودة.',404,'package_upgrade_source_not_found');if((int)$upgradeSource['client_id']!==$clientId)$abortPackageSale('الباقة البديلة يجب أن تكون لنفس عميل الباقة الأصلية.',409,'package_upgrade_client_mismatch');if((int)($upgradeSource['version']??1)!==$upgradeExpectedVersion)$abortPackageSale('تم تحديث الباقة الأصلية من شاشة أخرى. حدّث البيانات ثم أعد المحاولة.',409,'stale_package_upgrade_source');$activeStmt=$pdo->prepare("SELECT COUNT(*) FROM bookings b LEFT JOIN booking_sessions bs ON bs.booking_id=b.id AND bs.organization_id=b.organization_id AND bs.status='active' WHERE b.organization_id=? AND b.client_package_id=? AND (b.status='in_progress' OR bs.id IS NOT NULL)");$activeStmt->execute([$organizationId,$upgradeSourceId]);$upgradeHasActiveSession=(int)$activeStmt->fetchColumn()>0;if($upgradeCloseSource&&($upgradeHasActiveSession||authoritativePackageMinutes($upgradeSource,'held')>0||(string)$upgradeSource['billing_unit']!=='hour'&&(float)$upgradeSource['held_quantity']>0))$abortPackageSale('لا يمكن إغلاق الباقة الأصلية مع جلسة جارية أو وقت محجوز. أنهِ الجلسة وعدّل المواعيد أولًا.',409,'package_upgrade_source_committed');}
        ...
        $validity=$validityMode==='shooting_day'?1:$validity;if($validity<1||$validity>3650)$abortPackageSale('مدة صلاحية الباقة غير صحيحة.',422,'invalid_package_dates');
        $firstBookingDate=$bookings?(string)$bookings[0]['date']:'';$starts=$firstBookingDate?:($upgradeContext!==null&&$upgradeActivation==='immediate'?cairoNow()->format('Y-m-d'):null);$expires=$starts?packageValidityEnd($starts,$validity,$validityMode):null;$shootingDate=$validityMode==='shooting_day'?($firstBookingDate?:$starts):'';
        ...
        $overageCents=$upgradeContext!==null?strictPackageMoneyCents($payload['overage_price_snapshot'],'سعر الساعة الإضافية'):packageMoneyCents($payload['overage_price_snapshot']??$service['overage_price']??0);$priceCents=$upgradeContext!==null?strictPackageMoneyCents($payload['total_price'],'إجمالي السعر'):packageMoneyCents($payload['total_price']??$service['price']);$paidCents=$upgradeContext!==null?strictPackageMoneyCents($payload['paid_amount'],'المدفوع الآن'):packageMoneyCents($payload['paid_amount']??0);if($overageCents<0||$priceCents<0||$paidCents<0||$paidCents>$priceCents)$abortPackageSale('السعر والمدفوع أو سعر الزيادة غير صحيح.',422,'invalid_payment_amount');
        $overagePrice=packageMoney($overageCents);$price=packageMoney($priceCents);$paid=packageMoney($paidCents);$method=trim((string)($payload['payment_method']??'cash'));if(!in_array($method,['cash','instapay','vodafone_cash'],true))$abortPackageSale('طريقة الدفع غير صحيحة.',422,'invalid_payment_method');$notes=trim((string)($payload['notes']??''));
        ...
        $stmt=$pdo->prepare("INSERT INTO client_packages (organization_id,client_id,service_id,name,notes,billing_unit,purchased_quantity,purchased_minutes,held_quantity,held_minutes,consumed_quantity,consumed_minutes,payment_due_quantity,payment_due_minutes,deposit_percent_snapshot,overage_price_snapshot,total_price,paid_amount,starts_at,expires_at,validity_mode_snapshot,validity_days_snapshot,status) VALUES (?,?,?,?,?,?,?, ?,0,0,0,0,?,?,?,?,?,?,?,?,?,?, 'active')");
        $stmt->execute([$organizationId,$clientId,$serviceId,$name,$notes?:null,$unit,$unit==='hour'?settlementHours($purchasedMinutes):$quantity,$purchasedMinutes,$unit==='hour'?settlementHours($paymentDueMinutes):$paymentDueQuantity,$paymentDueMinutes,$depositPercent,$overagePrice,$price,$paid,$starts,$expires,$validityMode,$validity]);$id=(int)$pdo->lastInsertId();
        ...
        $newAudit=['loyalty_enabled'=>$payload['loyalty_enabled']??monthlyPackageLoyalty($service),'client_id'=>$clientId,'service_id'=>$serviceId,'billing_unit'=>$unit,'quantity'=>$unit==='hour'?settlementHours($purchasedMinutes):$quantity,'payment_due_quantity'=>$unit==='hour'?settlementHours($paymentDueMinutes):$paymentDueQuantity,'deposit_percent_snapshot'=>$depositPercent,'overage_price_snapshot'=>$overagePrice,'total_price'=>$price,'paid_amount'=>$paid,'expires_at'=>$expires,'idempotency_key'=>$idempotencyKey];if($upgradeContext!==null)$newAudit+=['upgrade_from_package_id'=>$upgradeSourceId,'upgrade_reason'=>$upgradeReason,'activation_mode'=>$upgradeActivation];audit($pdo,$user,$upgradeContext!==null?'owner_upgrade_package_create':'create','client_packages',$id,null,$newAudit);
        $sourceClosed=false;if($upgradeContext!==null&&$upgradeCloseSource){$sourceAfter=$upgradeSource;$sourceAfter['status']='completed';$sourceAfter['version']=(int)($upgradeSource['version']??1)+1;$pdo->prepare("UPDATE client_packages SET status='completed',version=version+1 WHERE id=? AND organization_id=?")->execute([$upgradeSourceId,$organizationId]);ownerAdjustment($pdo,$user,'client_packages',$upgradeSourceId,'package_upgrade_close',0,0,$upgradeReason,$upgradeSource,$sourceAfter);audit($pdo,$user,'owner_upgrade_source_completed','client_packages',$upgradeSourceId,$upgradeSource,['client_id'=>$clientId,'status'=>'completed','replacement_package_id'=>$id,'reason'=>$upgradeReason,'version'=>$sourceAfter['version']]);recordChangeEvent($pdo,$organizationId,$clientId,'client_packages','client_packages',$upgradeSourceId,'updated');$sourceClosed=true;}
        $eventId=recordChangeEvent($pdo,$organizationId,$clientId,'client_packages','client_packages',$id,'created');$notificationType=$upgradeContext!==null?'package_upgraded':'package_created';$notificationTitle=$upgradeContext!==null?'تمت ترقية باقتك':'بدأ اشتراكك في باقة جديدة';$notificationMessage=$upgradeContext!==null?'أضيفت باقة '.$name.' كباقة بديلة، مع حفظ سجل باقتك السابقة.':'أضيفت باقة '.$name.' إلى حسابك ويمكنك مراجعة رصيدها وصلاحيتها.';appNotification($pdo,$organizationId,$clientId,'client',$notificationType,$notificationTitle,$notificationMessage,'client_packages',$id,'change-event:'.$eventId.':'.$notificationType,'success','home',['package_id'=>$id,'source_package_id'=>$upgradeContext!==null?$upgradeSourceId:null]);queueClientWhatsAppSummary($pdo,$organizationId,$clientId);
        $response=['id'=>$id,'expires_at'=>$expires,'payment_id'=>$paymentId,'booking_ids'=>$bookingIds,'billing_unit'=>$unit,'validity_mode_snapshot'=>$validityMode,'purchased_quantity'=>$unit==='hour'?settlementHours($purchasedMinutes):$quantity,'paid_amount'=>$paid,'idempotent'=>false];if($upgradeContext!==null)$response['upgrade']=['source_package_id'=>$upgradeSourceId,'source_closed'=>$sourceClosed,'active_session_preserved'=>$upgradeHasActiveSession,'activation_mode'=>$upgradeActivation];$pdo->prepare("UPDATE client_package_sale_requests SET status='completed',response_json=?,completed_at=NOW() WHERE id=?")->execute([json_encode($response,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$requestId]);$pdo->commit();respond($response,201);
...
if ($path === '/offers' && $method === 'POST') {
    ...
    $allowedUnits=['hour','reel','day','month','project'];$serviceCheck=$pdo->prepare('SELECT id FROM services WHERE id=? AND organization_id=? AND is_active=1');$subtotalCents=0;foreach($items as &$item){$description=trim((string)($item['description']??''));$quantity=(float)($item['quantity']??0);$unitPriceCents=packageMoneyCents($item['unit_price']??'');$unit=(string)($item['unit']??'project');if($description===''||$quantity<=0||$unitPriceCents<0)fail('كل بند في العرض يحتاج وصفًا وكمية موجبة وسعرًا بدقة قرشين.',422,'invalid_offer_item');if(!in_array($unit,$allowedUnits,true))fail('وحدة أحد بنود العرض غير صحيحة.',422,'invalid_offer_unit');if(!empty($item['service_id'])){$serviceCheck->execute([(int)$item['service_id'],$user['organization_id']]);if(!$serviceCheck->fetch())fail('إحدى الخدمات المحددة غير موجودة.',422,'invalid_offer_service');}$lineCents=lineTotalCents($quantity,$item['unit_price']??'');$item['description']=$description;$item['quantity']=$quantity;$item['unit_price']=packageMoney($unitPriceCents);$item['unit']=$unit;$item['total']=packageMoney($lineCents);$item['_total_cents']=$lineCents;$subtotalCents+=$lineCents;}unset($item);$rawDiscount=trim((string)($payload['discount']??'0'));if(!preg_match('/^\d+(?:\.\d{1,2})?$/',$rawDiscount))fail('خصم العرض يجب أن يكون بدقة قرشين كحد أقصى.',422,'invalid_offer_discount');$discountCents=max(0,min($subtotalCents,packageMoneyCents($rawDiscount)));$totalCents=$subtotalCents-$discountCents;$subtotal=packageMoney($subtotalCents);$discount=packageMoney($discountCents);$total=packageMoney($totalCents);$number='OFF-'.date('Ymd-His').'-'.strtoupper(bin2hex(random_bytes(2)));
    $pdo->beginTransaction();try{$stmt=$pdo->prepare("INSERT INTO offers (organization_id,client_id,offer_number,title,status,subtotal,discount,total,valid_until,notes,created_by) VALUES (?,?,?,?, 'draft',?,?,?,?,?,?)");$stmt->execute([$user['organization_id'],$clientId,$number,trim((string)($payload['title']??'عرض سعر')),$subtotal,$discount,$total,$validUntil?:null,$payload['notes']??null,$user['id']]);$id=(int)$pdo->lastInsertId();$itemStmt=$pdo->prepare('INSERT INTO offer_items (offer_id,service_id,description,quantity,unit,unit_price,total,metadata) VALUES (?,?,?,?,?,?,?,?)');foreach($items as $item){$itemStmt->execute([$id,$item['service_id']??null,$item['description'],$item['quantity'],$item['unit'],$item['unit_price'],$item['total'],json_encode($item['metadata']??[],JSON_UNESCAPED_UNICODE)]);}audit($pdo,$user,'create','offers',$id,null,['number'=>$number,'client_id'=>$clientId,'total'=>$total]);$pdo->commit();respond(['id'=>$id,'offer_number'=>$number,'total'=>$total],201);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
}
...
if (preg_match('#^/offers/(\d+)$#',$path,$m)&&$method==='PATCH'){
    ...
    $pdo->beginTransaction();try{
        ...
        $allowedUnits=['hour','reel','day','month','project'];$serviceCheck=$pdo->prepare('SELECT id FROM services WHERE id=? AND organization_id=? AND is_active=1');$subtotalCents=0;
        foreach($items as &$item){$description=trim((string)($item['description']??''));$quantity=(float)($item['quantity']??0);$unit=(string)($item['unit']??'project');if($description===''||$quantity<=0){$pdo->rollBack();fail('كل بند يحتاج وصفًا وكمية موجبة وسعرًا غير سالب.',422,'invalid_offer_item');}if(!in_array($unit,$allowedUnits,true)){$pdo->rollBack();fail('وحدة أحد البنود غير صحيحة.',422,'invalid_offer_unit');}if(!empty($item['service_id'])){$serviceCheck->execute([(int)$item['service_id'],$user['organization_id']]);if(!$serviceCheck->fetch()){$pdo->rollBack();fail('إحدى الخدمات غير موجودة.',422,'invalid_offer_service');}}$unitPriceCents=packageMoneyCents($item['unit_price']??'');$lineCents=lineTotalCents($quantity,$item['unit_price']??'');$item['description']=$description;$item['quantity']=$quantity;$item['unit_price']=packageMoney($unitPriceCents);$item['unit']=$unit;$item['total']=packageMoney($lineCents);$subtotalCents+=$lineCents;}unset($item);
        $rawDiscount=trim((string)($payload['discount']??$before['discount']));if(!preg_match('/^\d+(?:\.\d{1,2})?$/',$rawDiscount)){$pdo->rollBack();fail('خصم العرض يجب أن يكون بدقة قرشين كحد أقصى.',422,'invalid_offer_discount');}$discountCents=max(0,min($subtotalCents,packageMoneyCents($rawDiscount)));$subtotal=packageMoney($subtotalCents);$discount=packageMoney($discountCents);$total=packageMoney($subtotalCents-$discountCents);$title=trim((string)($payload['title']??$before['title']));if($title===''){$pdo->rollBack();fail('عنوان العرض مطلوب.',422,'missing_offer_title');}
...
if (preg_match('#^/offers/(\d+)/accept$#',$path,$m)&&$method==='POST'){
    $user=requireUser($user);if(!in_array($user['role'],['owner','admin','operations','client'],true))fail('ليس لديك صلاحية لقبول العرض.',403);$id=(int)$m[1];$pdo->beginTransaction();try{
        ...
        $stmt=$pdo->prepare('SELECT oi.*,s.validity_days,s.package_validity_mode,s.billing_unit,s.category AS service_category,s.name AS service_name,s.total_hours,s.total_reels,s.payment_due_hours,s.deposit_percent,s.overage_price FROM offer_items oi LEFT JOIN services s ON s.id=oi.service_id WHERE oi.offer_id=?');$stmt->execute([$id]);$items=$stmt->fetchAll();
        if($user['role']==='client' && array_filter($items,fn($item)=>!empty($item['service_id'])&&isStudioPackageOfferItem($item))){
            $clientLock=$pdo->prepare('SELECT id FROM clients WHERE organization_id=? AND id=? FOR UPDATE');$clientLock->execute([$user['organization_id'],$offer['client_id']]);
            requireClientPackagePurchase($pdo,(int)$user['organization_id'],(int)$offer['client_id']);
        }
        ...
        $discountedItemCents=proportionalCents(packageMoneyCents($offer['total']),array_map(fn($item)=>packageMoneyCents($item['total']),$items));
        foreach($items as $itemIndex=>$item){
            $invoiceItem->execute([$invoiceId,$item['service_id'],$item['description'],$item['quantity'],$item['unit'],$item['unit_price'],$item['total']]);
            $packagePrice=packageMoney($discountedItemCents[$itemIndex]??0);
            if($item['service_id']&&isStudioPackageOfferItem($item)){
                $days=max(1,(int)($item['validity_days']??90));$expires=packageValidityEnd(cairoNow()->format('Y-m-d'),$days,'rolling');$billingUnit=$item['billing_unit']?:$item['unit'];$paymentDueQuantity=$billingUnit==='hour'?max(0,(float)($item['payment_due_hours']??0)):0;$purchasedMinutes=$billingUnit==='hour'?(int)round((float)$item['quantity']*60):null;$paymentDueMinutes=$billingUnit==='hour'?(int)round($paymentDueQuantity*60):null;
                $packageName=(string)($item['service_name']?:$item['description']);$packageStmt->execute([$user['organization_id'],$offer['client_id'],$item['service_id'],$invoiceId,$packageName,$billingUnit,$billingUnit==='hour'?settlementHours($purchasedMinutes):$item['quantity'],$purchasedMinutes,$paymentDueQuantity,$paymentDueMinutes,max(0,(float)($item['deposit_percent']??0)),max(0,(float)($item['overage_price']??0)),$packagePrice,cairoNow()->format('Y-m-d'),$expires]);$packageId=(int)$pdo->lastInsertId();insertPackageUsage($pdo,['id'=>$packageId,'billing_unit'=>$billingUnit],null,'opening',(float)$item['quantity'],'قبول عرض سعر','package:'.$packageId.':opening',$user['id']);appNotification($pdo,(int)$user['organization_id'],(int)$offer['client_id'],'client','package_created','بدأ اشتراكك في باقة جديدة','أضيفت باقة '.$packageName.' إلى حسابك بعد قبول العرض.','client_packages',$packageId,'package:'.$packageId.':created-from-offer','success','home',['package_id'=>$packageId]);
            }else{
                $serviceType=inferCustomServiceType($item);$definition=customServiceTypes()[$serviceType];$pricingModel=match($serviceType){'reels'=>'per_reel','podcast'=>'hourly','social_media'=>'monthly','ai_video'=>'per_video',default=>'custom'};$requiresBooking=(int)$definition['booking'];$pdo->prepare("INSERT INTO projects (organization_id,client_id,invoice_id,name,category,service_type,pricing_model,quantity,unit_label,agreed_price,requires_booking,progress_percent,status,starts_at,due_at,notes,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,0,'planning',?,?,?,?)")->execute([$user['organization_id'],$offer['client_id'],$invoiceId,$item['service_name']?:$item['description'],$serviceType,$serviceType,$pricingModel,$item['quantity'],$definition['unit'],$packagePrice,$requiresBooking,cairoNow()->format('Y-m-d'),$offer['valid_until'],trim((string)($offer['notes']??''))?:null,$user['id']]);$projectId=(int)$pdo->lastInsertId();if($firstProjectId===null)$firstProjectId=$projectId;$projectItemStmt->execute([$user['organization_id'],$projectId,$offer['client_id'],'service',$item['description'],$item['quantity'],$item['unit'],$item['unit_price'],$item['total']]);foreach(defaultProjectMilestones($serviceType) as $index=>$title)$milestoneStmt->execute([$user['organization_id'],$projectId,$offer['client_id'],$title,$index]);appNotification($pdo,(int)$user['organization_id'],(int)$offer['client_id'],'client','project_created','تمت إضافة مشروع جديد',(string)($item['service_name']?:$item['description']),'projects',$projectId,'project:'.$projectId.':created','info');
            }
        }
        $pdo->prepare("UPDATE client_packages cp JOIN services s ON s.id=cp.service_id AND s.organization_id=cp.organization_id SET cp.starts_at=NULL,cp.expires_at=NULL,cp.validity_mode_snapshot=IF(s.package_validity_mode='shooting_day','shooting_day','rolling'),cp.validity_days_snapshot=IF(s.package_validity_mode='shooting_day',1,GREATEST(1,s.validity_days)) WHERE cp.organization_id=? AND cp.source_invoice_id=?")->execute([$user['organization_id'],$invoiceId]);
...
if (preg_match('#^/client-packages/(\d+)/payments$#',$path,$m)&&$method==='POST') {
    ...
    $methodValue=requirePaymentMethod($payload['method']??'');if(!in_array($methodValue,['cash','instapay','vodafone_cash'],true))fail('طريقة الدفع غير صحيحة.',422,'invalid_payment_method');
    ...
    $normalized=['client_package_id'=>$id,'amount'=>packageMoney($amountCents),'method'=>$methodValue,'reference'=>$reference,'note'=>$note,'payment_date'=>$paymentDate];$requestHash=hash('sha256',json_encode($normalized,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));$organizationId=(int)$user['organization_id'];
    $pdo->beginTransaction();try{
        ...
        $pdo->prepare("INSERT INTO finance (organization_id,client_id,type,entry_kind,category,amount,method,detail,date,entity,source_type,source_id,correlation_id,is_system,created_by) VALUES (?,?,?,'income','package_payment',?,?,?,?,?,'payment',?,?,1,?)")->execute([$organizationId,$pkg['client_id'],'إيراد',packageMoney($amountCents),$methodValue,$note!==''?$note:'دفعة على الباقة: '.$pkg['name'],$paymentDate,'الشركة',$paymentId,'payment:'.$paymentId,$user['id']]);
        $newOutstanding=max(0,$totalCents-$newPaidCents);if($newOutstanding===0)dismissSettledPackageNotifications($pdo,$organizationId,(int)$pkg['client_id']);audit($pdo,$user,'record_package_payment','payments',$paymentId,null,['client_id'=>(int)$pkg['client_id'],'client_package_id'=>$id,'amount'=>packageMoney($amountCents),'method'=>$methodValue,'remaining'=>packageMoney($newOutstanding)]);$sourceEventId=recordChangeEvent($pdo,$organizationId,(int)$pkg['client_id'],'finance','payments',$paymentId,'package_payment');appNotification($pdo,$organizationId,(int)$pkg['client_id'],'client','payment_recorded','تم تسجيل دفعة على الباقة','تم تسجيل دفعة بقيمة '.packageMoney($amountCents).' ج.م على باقة '.$pkg['name'].'.','payments',$paymentId,'change-event:'.$sourceEventId.':package_payment','success','finance',['package_id'=>$id]);queueClientWhatsAppSummary($pdo,$organizationId,(int)$pkg['client_id'],'package-payment:'.$paymentId);
        $response=['payment_id'=>$paymentId,'package_id'=>$id,'client_id'=>(int)$pkg['client_id'],'amount'=>packageMoney($amountCents),'method'=>$methodValue,'payment_date'=>$paymentDate,'paid_amount'=>packageMoney($newPaidCents),'outstanding'=>packageMoney($newOutstanding),'invoice_id'=>$invoiceId,'idempotent'=>false];$pdo->prepare("UPDATE client_package_payment_requests SET status='completed',response_json=?,completed_at=NOW() WHERE id=?")->execute([json_encode($response,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$requestId]);$pdo->commit();respond($response,201);
...
if (preg_match('#^/clients/(\d+)/payment-history$#',$path,$m)&&$method==='GET') {
    $user=requireUser($user);requireRole($user,['owner','admin','finance']);$clientId=(int)$m[1];$organizationId=(int)$user['organization_id'];
...
if (preg_match('#^/client-packages/(\d+)$#',$path,$m)&&$method==='PATCH') {
    $user=requireUser($user);requireRole($user,['owner']);$id=(int)$m[1];$payload=body();$reason=ownerCorrectionReason($payload);$pdo->beginTransaction();try{
        $stmt=$pdo->prepare('SELECT * FROM client_packages WHERE id=? AND organization_id=? FOR UPDATE');$stmt->execute([$id,$user['organization_id']]);$before=$stmt->fetch();if(!$before){$pdo->rollBack();fail('الباقة غير موجودة.',404,'package_not_found');}
        if(isset($payload['expected_version'])&&(int)$payload['expected_version']!==(int)($before['version']??1)){$pdo->rollBack();fail('تم تحديث الباقة من شاشة أخرى. حدّث البيانات ثم أعد المحاولة.',409,'stale_package_version');}
        $name=trim((string)($payload['name']??$before['name']));$notes=array_key_exists('notes',$payload)?trim((string)$payload['notes']):$before['notes'];$starts=trim((string)($payload['starts_at']??$before['starts_at']??''));$expires=trim((string)($payload['expires_at']??$before['expires_at']??''));$status=(string)($payload['status']??$before['status']);$validityMode=(string)($payload['validity_mode_snapshot']??$before['validity_mode_snapshot']??'rolling');$validityDays=(int)($payload['validity_days_snapshot']??$before['validity_days_snapshot']??1);$paymentDue=max(0,(float)($payload['payment_due_quantity']??$before['payment_due_quantity']??0));$depositPercent=(float)($payload['deposit_percent_snapshot']??$before['deposit_percent_snapshot']??0);$overageCents=packageMoneyCents($payload['overage_price_snapshot']??$before['overage_price_snapshot']??0);$serviceId=(int)($payload['service_id']??$before['service_id']);$pendingValidity=$starts===''&&$expires==='';
        $datesValid=$pendingValidity||(preg_match('/^\d{4}-\d{2}-\d{2}$/',$starts)&&preg_match('/^\d{4}-\d{2}-\d{2}$/',$expires)&&$expires>=$starts);
        $purchased=(string)$before['billing_unit']==='hour'?authoritativePackageMinutes($before,'purchased')/60:(float)$before['purchased_quantity'];
        if($name===''||!$datesValid||!in_array($status,['draft','active','expired','suspended','completed','cancelled','archived'],true)||!in_array($validityMode,['rolling','rolling_first_booking','shooting_day'],true)||$validityDays<1||$validityDays>3650||$paymentDue>$purchased+0.000001||$depositPercent<0||$depositPercent>100||$overageCents<0){$pdo->rollBack();fail('بيانات عقد الباقة أو الصلاحية أو حدود الدفع غير صحيحة.',422,'invalid_package_details');}
        if($validityMode==='rolling_first_booking')$validityMode='rolling';
        if(!$pendingValidity&&$validityMode==='shooting_day'&&$starts!==$expires){$pdo->rollBack();fail('الباقة اليومية يجب أن تبدأ وتنتهي في يوم التصوير نفسه.',422,'shooting_day_date_mismatch');}
        if(!$pendingValidity){$outside=$pdo->prepare("SELECT COUNT(*) FROM bookings WHERE organization_id=? AND client_package_id=? AND status IN ('pending','confirmed','alternative_proposed','cancel_requested','late_cancel_requested','in_progress') AND (date<? OR date>? OR (?='shooting_day' AND date<>?))");$outside->execute([$user['organization_id'],$id,$starts,$expires,$validityMode,$starts]);if((int)$outside->fetchColumn()>0){$pdo->rollBack();fail('توجد مواعيد نشطة خارج فترة الصلاحية الجديدة. عدّل المواعيد أولًا.',409,'bookings_outside_package_validity');}}
        if($serviceId!== (int)$before['service_id']){$serviceStmt=$pdo->prepare('SELECT id,billing_unit FROM services WHERE id=? AND organization_id=? LIMIT 1');$serviceStmt->execute([$serviceId,$user['organization_id']]);$service=$serviceStmt->fetch();if(!$service||(string)$service['billing_unit']!==(string)$before['billing_unit']){$pdo->rollBack();fail('الخدمة البديلة غير موجودة أو وحدتها لا تطابق رصيد الباقة.',422,'package_service_unit_mismatch');}$activity=$pdo->prepare("SELECT (SELECT COUNT(*) FROM bookings WHERE organization_id=? AND client_package_id=?)+(SELECT COUNT(*) FROM payment_allocations WHERE organization_id=? AND client_package_id=?)+(SELECT COUNT(*) FROM package_usage_ledger WHERE client_package_id=? AND movement_type<>'opening')");$activity->execute([$user['organization_id'],$id,$user['organization_id'],$id,$id]);if((int)$activity->fetchColumn()>0||(float)$before['held_quantity']>0||(float)$before['consumed_quantity']>0||packageMoneyCents($before['paid_amount'])>0||packageMoneyCents($before['overage_amount'])>0){$pdo->rollBack();fail('لا يمكن تبديل خدمة باقة لها مواعيد أو استخدام أو دفعات. أنشئ باقة بديلة وأرشف هذه الباقة لحفظ التاريخ.',409,'package_service_has_history');}}
        $paymentDueMinutes=(string)$before['billing_unit']==='hour'?settlementMinutes($paymentDue):null;$paymentDueValue=(string)$before['billing_unit']==='hour'?settlementHours($paymentDueMinutes):round($paymentDue,4);$startsValue=$pendingValidity?null:$starts;$expiresValue=$pendingValidity?null:$expires;
        $pdo->prepare('UPDATE client_packages SET service_id=?,name=?,notes=?,starts_at=?,expires_at=?,status=?,validity_mode_snapshot=?,validity_days_snapshot=?,payment_due_quantity=?,payment_due_minutes=?,deposit_percent_snapshot=?,overage_price_snapshot=?,version=version+1 WHERE id=? AND organization_id=?')->execute([$serviceId,$name,$notes?:null,$startsValue,$expiresValue,$status,$validityMode,$validityDays,$paymentDueValue,$paymentDueMinutes,round($depositPercent,2),packageMoney($overageCents),$id,$user['organization_id']]);
        $after=$before+[];foreach(['service_id'=>$serviceId,'name'=>$name,'notes'=>$notes,'starts_at'=>$startsValue,'expires_at'=>$expiresValue,'status'=>$status,'validity_mode_snapshot'=>$validityMode,'validity_days_snapshot'=>$validityDays,'payment_due_quantity'=>$paymentDueValue,'payment_due_minutes'=>$paymentDueMinutes,'deposit_percent_snapshot'=>round($depositPercent,2),'overage_price_snapshot'=>packageMoney($overageCents)] as $field=>$value)$after[$field]=$value;$after['version']=(int)($before['version']??1)+1;ownerAdjustment($pdo,$user,'client_packages',$id,'contract_details',0,0,$reason,$before,$after);audit($pdo,$user,'owner_update_package_contract','client_packages',$id,$before,$after+['reason'=>$reason]);$pdo->commit();try{queueClientWhatsAppSummary($pdo,(int)$user['organization_id'],(int)$before['client_id']);}catch(Throwable $notificationError){error_log('[Package contract WhatsApp] '.$notificationError->getMessage());}respond(['id'=>$id,'updated'=>true,'version'=>$after['version']]);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
...
if (preg_match('#^/client-packages/(\d+)/commercial-adjustment$#',$path,$m)&&$method==='POST') {
    ...
    if($paidDelta>0){$client=$pdo->prepare('SELECT name FROM clients WHERE id=? AND organization_id=?');$client->execute([$pkg['client_id'],$user['organization_id']]);$methodValue=trim((string)($payload['method']??'cash'));$pdo->prepare("INSERT INTO payments (organization_id,client_id,client_name,amount,method,status,reference,reviewed_by,reviewed_at) VALUES (?,?,?,?,?,'approved',?,?,NOW())")->execute([$user['organization_id'],$pkg['client_id'],$client->fetchColumn(),packageMoney($paidDelta),$methodValue,'OWNER-ADJ-'.$adjustmentId,$user['id']]);$paymentId=(int)$pdo->lastInsertId();$pdo->prepare('INSERT INTO payment_allocations (organization_id,client_id,payment_id,payment_proof_id,client_package_id,invoice_id,amount) VALUES (?,?,?,NULL,?,?,?)')->execute([$user['organization_id'],$pkg['client_id'],$paymentId,$id,$pkg['source_invoice_id']?:null,packageMoney($paidDelta)]);$pdo->prepare("INSERT INTO finance (organization_id,client_id,type,entry_kind,category,amount,method,detail,date,entity,source_type,source_id,correlation_id,is_system,created_by) VALUES (?,?,?,'income','package_paid_correction',?,?,?,?,?,'payment',?,?,1,?)")->execute([$user['organization_id'],$pkg['client_id'],'إيراد',packageMoney($paidDelta),$methodValue,'تصحيح مدفوع الباقة: '.$pkg['name'],cairoNow()->format('Y-m-d'),'الشركة',$paymentId,'payment:'.$paymentId,$user['id']]);}elseif($paidDelta<0){$pdo->prepare("INSERT INTO finance (organization_id,client_id,type,entry_kind,category,amount,method,detail,date,entity,source_type,source_id,correlation_id,is_system,reversal_reason,created_by) VALUES (?,?,?,'reversal','reversal_income',?,?,?,?,?,'owner_adjustment',?,?,1,?,?)")->execute([$user['organization_id'],$pkg['client_id'],'قيد عكسي',packageMoney(abs($paidDelta)),trim((string)($payload['method']??'cash')),'خفض مدفوع الباقة: '.$pkg['name'],cairoNow()->format('Y-m-d'),'الشركة',$adjustmentId,'owner-adjustment:'.$adjustmentId,$reason,$user['id']]);}
...
if (preg_match('#^/client-packages/(\d+)$#',$path,$m)&&$method==='DELETE') {
    $user=requireUser($user);requireRole($user,['owner']);$id=(int)$m[1];$payload=body();$reason=ownerCorrectionReason($payload);if(trim((string)($payload['confirmation']??''))!=='حذف')fail('اكتب كلمة حذف لتأكيد حذف الباقة وسجلاتها.',422,'package_delete_confirmation_required');
    $pdo->beginTransaction();try{$stmt=$pdo->prepare('SELECT * FROM client_packages WHERE id=? AND organization_id=? FOR UPDATE');$stmt->execute([$id,$user['organization_id']]);$pkg=$stmt->fetch();if(!$pkg){$pdo->rollBack();fail('الباقة غير موجودة.',404,'package_not_found');}if(isset($payload['expected_version'])&&(int)$payload['expected_version']!==(int)($pkg['version']??1)){$pdo->rollBack();fail('تم تحديث الباقة من شاشة أخرى. حدّث البيانات ثم أعد المحاولة.',409,'stale_package_version');}$deleted=ownerDeletePackageCascade($pdo,$user,$pkg,$reason);audit($pdo,$user,'owner_cascade_delete','client_packages',$id,$pkg,['client_id'=>(int)$pkg['client_id'],'reason'=>$reason,'deleted_records'=>$deleted]);recordChangeEvent($pdo,(int)$user['organization_id'],(int)$pkg['client_id'],'client_packages','client_packages',$id,'deleted');queueClientWhatsAppSummary($pdo,(int)$user['organization_id'],(int)$pkg['client_id']);$pdo->commit();respond(['id'=>$id,'deleted'=>true,'action'=>'cascade_delete','deleted_records'=>$deleted]);}catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
...
if (preg_match('#^/client-packages/(\d+)/extend$#',$path,$m)&&$method==='POST'){
    $user=requireUser($user);requireRole($user,['owner']);$id=(int)$m[1];$payload=body();$expires=trim((string)($payload['expires_at']??''));$reason=ownerCorrectionReason($payload);if(!preg_match('/^\d{4}-\d{2}-\d{2}$/',$expires))fail('تاريخ الانتهاء الجديد غير صحيح.',422,'invalid_package_expiry');$pdo->beginTransaction();try{
        $stmt=$pdo->prepare('SELECT * FROM client_packages WHERE id=? AND organization_id=? FOR UPDATE');$stmt->execute([$id,$user['organization_id']]);$before=$stmt->fetch();if(!$before){$pdo->rollBack();fail('الباقة غير موجودة.',404,'package_not_found');}
        if(isset($payload['expected_version'])&&(int)$payload['expected_version']!==(int)($before['version']??1)){$pdo->rollBack();fail('تم تحديث الباقة من شاشة أخرى. حدّث البيانات ثم أعد المحاولة.',409,'stale_package_version');}
        $starts=substr((string)($before['starts_at']??''),0,10);$mode=(string)($before['validity_mode_snapshot']??'rolling');if($starts===''||$expires<$starts){$pdo->rollBack();fail('تاريخ الانتهاء يجب ألا يسبق تاريخ بداية الباقة.',422,'invalid_package_expiry');}if($mode==='shooting_day'&&$expires!==$starts){$pdo->rollBack();fail('باقة يوم التصوير يجب أن تبدأ وتنتهي في اليوم نفسه.',422,'shooting_day_date_mismatch');}$startDate=DateTimeImmutable::createFromFormat('!Y-m-d',$starts,new DateTimeZone('Africa/Cairo'));$expiryDate=DateTimeImmutable::createFromFormat('!Y-m-d',$expires,new DateTimeZone('Africa/Cairo'));$validityDays=$mode==='shooting_day'?1:(($startDate&&$expiryDate)?$startDate->diff($expiryDate)->days+1:0);if($validityDays<1||$validityDays>3650){$pdo->rollBack();fail('مدة الصلاحية يجب أن تكون بين يوم و3650 يومًا.',422,'invalid_package_validity_days');}
        $currentExpiry=substr((string)($before['expires_at']??''),0,10);$currentValidityDays=(int)($before['validity_days_snapshot']??1);if($expires===$currentExpiry&&$validityDays===$currentValidityDays){$pdo->commit();respond(['id'=>$id,'expires_at'=>$expires,'validity_days_snapshot'=>$validityDays,'version'=>(int)($before['version']??1),'idempotent'=>true]);}
        $outsideSql="SELECT 1 FROM bookings WHERE organization_id=? AND client_package_id=? AND status IN ('pending','confirmed','alternative_proposed','cancel_requested','late_cancel_requested','in_progress') AND date>?";$outsideParams=[$user['organization_id'],$id,$expires];if($mode==='shooting_day'){$outsideSql.=' OR (organization_id=? AND client_package_id=? AND status IN (\'pending\',\'confirmed\',\'alternative_proposed\',\'cancel_requested\',\'late_cancel_requested\',\'in_progress\') AND date<>?)';$outsideParams[]=$user['organization_id'];$outsideParams[]=$id;$outsideParams[]=$starts;}$outside=$pdo->prepare("$outsideSql LIMIT 1");$outside->execute($outsideParams);if($outside->fetchColumn()!==false){$pdo->rollBack();fail('توجد مواعيد نشطة بعد تاريخ الانتهاء الجديد. عدّل المواعيد أولًا.',409,'bookings_outside_package_validity');}
        $nextStatus=$before['status']==='expired'&&$expires>=cairoNow()->format('Y-m-d')?'active':$before['status'];$sets=['expires_at=?','validity_days_snapshot=?'];$updateParams=[$expires,$validityDays];if($nextStatus!==$before['status']){$sets[]='status=?';$updateParams[]=$nextStatus;}$hasVersion=array_key_exists('version',$before);if($hasVersion)$sets[]='version=version+1';$updateParams[]=$id;$updateParams[]=$user['organization_id'];$update=$pdo->prepare('UPDATE client_packages SET '.implode(',',$sets).' WHERE id=? AND organization_id=?');$update->execute($updateParams);if($update->rowCount()!==1){$pdo->rollBack();fail('تعذر تثبيت تعديل صلاحية الباقة. حدّث البيانات ثم أعد المحاولة.',409,'package_validity_update_conflict');}$after=$before;$after['expires_at']=$expires;$after['validity_days_snapshot']=$validityDays;$after['status']=$nextStatus;$after['version']=$hasVersion?(int)$before['version']+1:(int)($before['version']??1);$pdo->commit();$adjustmentRecorded=true;try{ownerAdjustment($pdo,$user,'client_packages',$id,'validity',0,0,$reason,$before,$after);}catch(Throwable $adjustmentError){$adjustmentRecorded=false;error_log('[Package validity adjustment] '.$adjustmentError->getMessage());}$auditRecorded=true;try{audit($pdo,$user,'extend','client_packages',$id,$before,$after+['reason'=>$reason,'adjustment_recorded'=>$adjustmentRecorded]);}catch(Throwable $auditError){$auditRecorded=false;error_log('[Package validity audit] '.$auditError->getMessage());}try{queueClientWhatsAppSummary($pdo,(int)$user['organization_id'],(int)$before['client_id']);}catch(Throwable $notificationError){error_log('[Package validity WhatsApp] '.$notificationError->getMessage());}respond(['id'=>$id,'expires_at'=>$expires,'validity_days_snapshot'=>$validityDays,'version'=>$after['version'],'idempotent'=>false,'adjustment_recorded'=>$adjustmentRecorded,'audit_recorded'=>$auditRecorded]);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
...
if (preg_match('#^/clients/(\d+)/credentials/password$#',$path,$m)&&$method==='POST') {
    ...
    $pdo->beginTransaction();
    try{$client=$pdo->prepare('SELECT id,name,email,phone1 FROM clients WHERE id=? AND organization_id=? FOR UPDATE');$client->execute([$clientId,$user['organization_id']]);$clientRow=$client->fetch();if(!$clientRow){$pdo->rollBack();fail('العميل غير موجود.',404,'client_not_found');}$clientPhone=loginMobile((string)$clientRow['phone1']);if($clientPhone==='')fail('أضف رقم موبايل صحيحًا للعميل أولًا.',422,'invalid_login_phone');
        $stmt=$pdo->prepare("SELECT * FROM users WHERE client_id=? AND organization_id=? AND role='client' FOR UPDATE");$stmt->execute([$clientId,$user['organization_id']]);$account=$stmt->fetch();assertLoginMobileAvailable($pdo,$clientPhone,$account?(int)$account['id']:null);$accessEnabled=$account?!empty($account['is_active']):false;
...
if ($path==='/booking-blocks'&&$method==='GET') {
    ...
    $resourceId=max(0,(int)($_GET['resource_id']??0));materializeBookingCalendarRange($pdo,(int)$user['organization_id'],$from,$to,$resourceId?:null);$sql="SELECT bb.*,r.name resource_name FROM booking_blocks bb JOIN resources r ON r.id=bb.resource_id AND r.organization_id=bb.organization_id WHERE bb.organization_id=? AND bb.status='active' AND bb.block_date BETWEEN ? AND ?";$params=[$user['organization_id'],$from,$to];
...
if ($path==='/booking-blocks'&&$method==='POST') {
    ...
    $date=trim((string)($payload['date']??''));$start=normalizeBusinessTime($payload['start_time']??'');$end=normalizeBusinessTime($payload['end_time']??'',true);$resourceId=(int)($payload['resource_id']??0);$repeatDaily=!empty($payload['repeat_daily']);$repeatUntil=trim((string)($payload['repeat_until']??''));$title=mb_substr(trim((string)($payload['title']??'')),0,120);if($title==='')$title='حجز مؤقت';$note=mb_substr(trim((string)($payload['note']??'')),0,1000);$idempotencyKey=trim((string)($payload['idempotency_key']??''));
    if(!preg_match('/^[A-Za-z0-9:_-]{12,120}$/',$idempotencyKey))fail('مفتاح حفظ الحظر غير صحيح.',422,'invalid_idempotency_key');$duration=bookingDurationMinutes($start,$end);if(!validBusinessBooking($start,$end,60)||$duration%15!==0)fail('فترة الحظر يجب أن تكون ساعة على الأقل، بزيادات 15 دقيقة، خلال اليوم بالكامل.',422,'invalid_booking_block_time');
    if(($payload['repeat_end_mode']??'')==='never')$repeatUntil='';
    if($repeatDaily&&($payload['repeat_end_mode']??'')==='date'&&$repeatUntil==='')fail('حدد تاريخ نهاية التكرار.',422,'booking_block_repeat_until_required');
    $dates=bookingBlockDates($date,$repeatDaily,$repeatUntil);if($repeatDaily)requireBookingBlockSeriesSchema($pdo);$normalized=['date'=>$date,'start_time'=>$start,'end_time'=>$end,'resource_id'=>$resourceId,'repeat_daily'=>$repeatDaily,'repeat_until'=>$repeatDaily?$repeatUntil:null,'title'=>$title,'note'=>$note];$requestHash=hash('sha256',json_encode($normalized,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
    $pdo->beginTransaction();try{
        ...
        if($repeatDaily)requireBookingBlockSeriesAvailable($pdo,(int)$user['organization_id'],$resourceId,$date,$repeatUntil?:null,$start,$end);
        else materializeRecurringBookingBlocks($pdo,(int)$user['organization_id'],$resourceId,$date,$date);
        foreach($dates as $blockDate){$bookingConflict=$pdo->prepare("SELECT id FROM bookings WHERE organization_id=? AND resource_id=? AND date=? AND status IN ('pending','alternative_proposed','confirmed','in_progress','cancel_requested','late_cancel_requested') AND start_time<? AND (CASE WHEN end_time='00:00:00' THEN '24:00:00' ELSE end_time END)>? LIMIT 1 FOR UPDATE");$bookingConflict->execute([$user['organization_id'],$resourceId,$blockDate,$end.':00',$start.':00']);if($bookingConflict->fetch()){$pdo->rollBack();fail('يتعارض الحظر مع حجز مؤكد يوم '.$blockDate.'. لم يتم إنشاء أي حظر.',409,'booking_conflict');}$blockConflict=$pdo->prepare("SELECT id FROM booking_blocks WHERE organization_id=? AND resource_id=? AND block_date=? AND status='active' AND start_time<? AND end_time>? LIMIT 1 FOR UPDATE");$blockConflict->execute([$user['organization_id'],$resourceId,$blockDate,$end.':00',$start.':00']);if($blockConflict->fetch()){$pdo->rollBack();fail('توجد فترة مغلقة متعارضة يوم '.$blockDate.'. لم يتم إنشاء أي حظر.',409,'booking_block_conflict');}}
        $seriesKey=null;if($repeatDaily){$seriesBytes=random_bytes(16);$seriesBytes[6]=chr((ord($seriesBytes[6])&0x0f)|0x40);$seriesBytes[8]=chr((ord($seriesBytes[8])&0x3f)|0x80);$hex=bin2hex($seriesBytes);$seriesKey=substr($hex,0,8).'-'.substr($hex,8,4).'-'.substr($hex,12,4).'-'.substr($hex,16,4).'-'.substr($hex,20,12);}$items=[];$insert=$pdo->prepare("INSERT INTO booking_blocks (organization_id,resource_id,block_date,start_time,end_time,duration_minutes,title,note,series_key,idempotency_key,request_hash,status,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,'active',?)");
        if($repeatDaily){$rule=$pdo->prepare("INSERT INTO booking_block_series (organization_id,resource_id,series_key,starts_on,repeat_until,start_time,end_time,duration_minutes,title,note,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?)");$rule->execute([$user['organization_id'],$resourceId,$seriesKey,$date,$repeatUntil?:null,$start.':00',$end.':00',$duration,$title,$note?:null,$user['id']]);}
        foreach($dates as $index=>$blockDate){$rowKey=$index===0?$idempotencyKey:$idempotencyKey.':'.$index;$insert->execute([$user['organization_id'],$resourceId,$blockDate,$start.':00',$end.':00',$duration,$title,$note?:null,$seriesKey,$rowKey,$requestHash,$user['id']]);$id=(int)$pdo->lastInsertId();$block=['id'=>$id,'organization_id'=>(int)$user['organization_id'],'resource_id'=>$resourceId,'resource_name'=>$resourceRow['name'],'block_date'=>$blockDate,'start_time'=>$start.':00','end_time'=>$end.':00','duration_minutes'=>$duration,'title'=>$title,'note'=>$note?:null,'series_key'=>$seriesKey,'status'=>'active'];try{reserveBookingBlockSlots($pdo,$block);}catch(PDOException $slotError){if(($slotError->errorInfo[1]??0)===1062){$pdo->rollBack();fail('أصبحت إحدى الفترات غير متاحة. لم يتم إنشاء أي حظر.',409,'booking_conflict');}throw $slotError;}$items[]=$block;}
        $response=['items'=>$items,'count'=>$repeatDaily?($repeatUntil!==''?(int)bookingBlockDate($date)->diff(bookingBlockDate($repeatUntil))->days+1:null):1,'series_key'=>$seriesKey,'repeat_daily'=>$repeatDaily,'repeat_until'=>$repeatUntil?:null,'idempotent'=>false,'skipped_fridays'=>0];$pdo->prepare('UPDATE booking_blocks SET response_json=? WHERE organization_id=? AND idempotency_key=?')->execute([json_encode($response,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$user['organization_id'],$idempotencyKey]);auditBookingBlockChange($pdo,$user,'create',(int)$items[0]['id'],null,['resource_id'=>$resourceId,'dates'=>$dates,'start_time'=>$start,'end_time'=>$end,'series_key'=>$seriesKey]);$pdo->commit();respond($response,201);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}

if (preg_match('#^/booking-blocks/(\d+)/convert$#',$path,$m)&&$method==='POST') {
    $user=requireUser($user);requireRole($user,['owner','admin','operations']);requireBookingBlockSchema($pdo);$id=(int)$m[1];$payload=body();$organizationId=(int)$user['organization_id'];
    $clientId=(int)($payload['client_id']??0);$mode=(string)($payload['package_mode']??'existing_package');if(!in_array($mode,['existing_package','new_package'],true))fail('طريقة ربط الباقة غير صحيحة.',422,'invalid_package_mode');$packageId=$mode==='existing_package'?(int)($payload['client_package_id']??0):0;$newPackage=$mode==='new_package'&&is_array($payload['new_package']??null)?$payload['new_package']:[];$idempotencyKey=trim((string)($payload['idempotency_key']??''));
    if(!preg_match('/^[A-Za-z0-9:_-]{12,120}$/',$idempotencyKey))fail('مفتاح حفظ التحويل غير صحيح.',422,'invalid_idempotency_key');
    $normalizedNew=$mode==='new_package'?['service_id'=>(int)($newPackage['service_id']??0),'name'=>trim((string)($newPackage['name']??'')),'billing_unit'=>(string)($newPackage['billing_unit']??''),'quantity'=>(string)($newPackage['quantity']??''),'validity_days'=>(string)($newPackage['validity_days']??''),'payment_due_quantity'=>(string)($newPackage['payment_due_quantity']??''),'deposit_percent_snapshot'=>(string)($newPackage['deposit_percent_snapshot']??''),'overage_price_snapshot'=>(string)($newPackage['overage_price_snapshot']??''),'total_price'=>(string)($newPackage['total_price']??''),'paid_amount'=>(string)($newPackage['paid_amount']??''),'payment_method'=>(string)($newPackage['payment_method']??''),'notes'=>trim((string)($newPackage['notes']??''))]:null;
    if($mode==='new_package'){foreach(['overage_price_snapshot'=>'سعر الساعة الإضافية','total_price'=>'إجمالي السعر','paid_amount'=>'المدفوع الآن'] as $field=>$label){if(!array_key_exists($field,$newPackage))fail('حقل '.$label.' مطلوب كنص عشري صريح.',422,'invalid_money_format');strictPackageMoneyCents($newPackage[$field],$label);}}
    $requestHash=hash('sha256',json_encode(['block_id'=>$id,'client_id'=>$clientId,'package_mode'=>$mode,'client_package_id'=>$packageId,'new_package'=>$normalizedNew],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
    $abort=static function(string $message,int $status=422,string $code='invalid_booking_block_conversion')use($pdo):never{if($pdo->inTransaction())$pdo->rollBack();fail($message,$status,$code);};
    $lookup=$pdo->prepare('SELECT resource_id FROM booking_blocks WHERE id=? AND organization_id=?');$lookup->execute([$id,$organizationId]);$blockResource=(int)$lookup->fetchColumn();
    $pdo->beginTransaction();try{
        $lock=$pdo->prepare('SELECT id FROM resources WHERE id=? AND organization_id=? FOR UPDATE');$lock->execute([$blockResource,$organizationId]);
        $blockStmt=$pdo->prepare('SELECT bb.*,r.name AS resource_name FROM booking_blocks bb JOIN resources r ON r.id=bb.resource_id AND r.organization_id=bb.organization_id WHERE bb.id=? AND bb.organization_id=? FOR UPDATE');$blockStmt->execute([$id,$organizationId]);$block=$blockStmt->fetch();if(!$block)$abort('الحجز المؤقت غير موجود.',404,'booking_block_not_found');
        $hasConversion=trim((string)($block['conversion_idempotency_key']??''))!==''||trim((string)($block['conversion_request_hash']??''))!==''||trim((string)($block['conversion_response_json']??''))!=='';
        if($hasConversion){$sameKey=hash_equals((string)($block['conversion_idempotency_key']??''),$idempotencyKey);$sameHash=hash_equals((string)($block['conversion_request_hash']??''),$requestHash);if(!$sameKey||!$sameHash)$abort('تم تحويل هذا الحجز المؤقت من قبل.',409,'booking_block_already_converted');$response=json_decode((string)($block['conversion_response_json']??''),true);if(!is_array($response))$abort('تعذر استعادة نتيجة التحويل.',409,'idempotency_incomplete');$pdo->commit();$response['idempotent']=true;respond($response);}
        if((string)$block['status']!=='active')$abort('الحجز المؤقت ملغى ولا يمكن تحويله.',409,'booking_block_inactive');
        $clientStmt=$pdo->prepare("SELECT id,name FROM clients WHERE id=? AND organization_id=? AND status='active' FOR UPDATE");$clientStmt->execute([$clientId,$organizationId]);$client=$clientStmt->fetch();if(!$client)$abort('العميل غير موجود أو غير نشط.',422,'invalid_booking_client');
        requireBookingSlotAvailable($pdo,$organizationId,(int)$block['resource_id'],(string)$block['block_date'],(string)$block['start_time'],(string)$block['end_time'],null,null,[],(int)$block['id']);
        $duration=(int)$block['duration_minutes'];if($duration<15||$duration%15!==0)$abort('مدة الحجز المؤقت لا تطابق شبكة الحجز.',422,'invalid_booking_time');$expectedSlots=intdiv($duration,15);$paymentId=null;$createdPackage=null;
        // Future rule occurrences acquire concrete slots only when converted.
        if(str_starts_with((string)$block['idempotency_key'],'recurrence:')){$slots=$pdo->prepare('SELECT COUNT(*) FROM booking_slots WHERE organization_id=? AND booking_block_id=?');$slots->execute([$organizationId,$id]);if((int)$slots->fetchColumn()===0)reserveBookingBlockSlots($pdo,$block);}
        if($mode==='existing_package'){
            $packageStmt=$pdo->prepare("SELECT cp.*,s.name AS service_name,s.minimum_booking_minutes,s.booking_increment_minutes FROM client_packages cp JOIN services s ON s.id=cp.service_id AND s.organization_id=cp.organization_id WHERE cp.id=? AND cp.organization_id=? AND cp.client_id=? AND cp.status='active' AND cp.billing_unit='hour' FOR UPDATE");$packageStmt->execute([$packageId,$organizationId,$clientId]);$package=$packageStmt->fetch();if(!$package)$abort('اختر باقة ساعات نشطة تخص هذا العميل.',422,'invalid_target_package');
            $starts=trim((string)($package['starts_at']??''));$expires=trim((string)($package['expires_at']??''));if(($starts==='')!==($expires===''))$abort('تواريخ صلاحية الباقة غير مكتملة.',422,'invalid_package_dates');if($starts!==''&&((string)$block['block_date']<$starts||(string)$block['block_date']>$expires))$abort('الحجز المؤقت خارج صلاحية الباقة.',422,'booking_outside_package_validity');
            $minimum=max(15,(int)($package['minimum_booking_minutes']??60));$increment=max(15,(int)($package['booking_increment_minutes']??15));if($duration<$minimum||$duration%$increment!==0)$abort('مدة الحجز لا تطابق الحد الأدنى أو فاصل قالب الباقة.',422,'invalid_booking_time');
            activatePackageOnFirstBooking($pdo,$package,(string)$block['block_date'],$organizationId);$free=authoritativePackageMinutes($package,'purchased')-authoritativePackageMinutes($package,'held')-authoritativePackageMinutes($package,'consumed');if($free<$duration)$abort('رصيد الباقة لا يكفي مدة الحجز المؤقت.',409,'insufficient_package_balance');
        }else{
            $serviceId=(int)($newPackage['service_id']??0);$serviceStmt=$pdo->prepare('SELECT * FROM services WHERE id=? AND organization_id=? AND is_active=1 FOR UPDATE');$serviceStmt->execute([$serviceId,$organizationId]);$service=$serviceStmt->fetch();if(!$service||!isStudioPackageOfferItem($service)||normalizedStudioPackageUnit($service)!=='hour')$abort('اختر قالب باقة ساعات نشطًا وقابلًا للبيع.',422,'invalid_package_template');if((string)($newPackage['billing_unit']??'hour')!=='hour')$abort('وحدة الباقة الجديدة يجب أن تكون ساعات.',422,'invalid_billing_unit');
            $minimum=max(15,(int)($service['minimum_booking_minutes']??60));$increment=max(15,(int)($service['booking_increment_minutes']??15));if($duration<$minimum||$duration%$increment!==0)$abort('مدة الحجز لا تطابق الحد الأدنى أو فاصل القالب المختار.',422,'invalid_booking_time');
            $quantityText=(string)($newPackage['quantity']??'');$dueText=(string)($newPackage['payment_due_quantity']??'0');$validityText=(string)($newPackage['validity_days']??'');$depositText=(string)($newPackage['deposit_percent_snapshot']??'');if(!preg_match('/^\d+(?:\.\d+)?$/',$quantityText)||!preg_match('/^\d+(?:\.\d+)?$/',$dueText)||!preg_match('/^\d+$/',$validityText)||!preg_match('/^\d+(?:\.\d+)?$/',$depositText))$abort('بيانات رصيد الباقة أو صلاحيتها غير صحيحة.',422,'invalid_package_sale');
            $quantity=(float)$quantityText;$paymentDueQuantity=(float)$dueText;$purchasedMinutes=(int)round($quantity*60);$paymentDueMinutes=(int)round($paymentDueQuantity*60);if($quantity<=0||abs($quantity*60-$purchasedMinutes)>.0001||$purchasedMinutes%15!==0||$purchasedMinutes<$duration)$abort('رصيد الباقة الجديدة يجب أن يغطي الحجز وبزيادات 15 دقيقة.',422,'insufficient_package_balance');if($paymentDueQuantity<0||$paymentDueQuantity>$quantity||abs($paymentDueQuantity*60-$paymentDueMinutes)>.0001)$abort('حد الاستحقاق يجب أن يكون بين صفر ورصيد الباقة.',422,'invalid_payment_due_quantity');
            $dailyCategories=['daily','daily package','day package','باقة يومية','باقات يومية','الباقات اليومية','باقة اليوم'];$serviceCategory=mb_strtolower(trim((string)($service['category']??'')),'UTF-8');$validityMode=(string)($service['package_validity_mode']??'rolling');if($validityMode==='shooting_day'||in_array($serviceCategory,$dailyCategories,true))$validityMode='shooting_day';elseif($validityMode!=='rolling')$validityMode='rolling';$validity=$validityMode==='shooting_day'?1:(int)$validityText;if($validity<1||$validity>3650)$abort('مدة صلاحية الباقة غير صحيحة.',422,'invalid_package_dates');$starts=(string)$block['block_date'];$expires=packageValidityEnd($starts,$validity,$validityMode);
            $name=trim((string)($newPackage['name']??$service['name']));if($name===''||mb_strlen($name)>180)$abort('اسم الباقة مطلوب وبحد أقصى 180 حرفًا.',422,'invalid_package_name');$depositPercent=(float)$depositText;if($depositPercent<0||$depositPercent>100)$abort('نسبة المقدم يجب أن تكون بين صفر و100.',422,'invalid_deposit_percent');
            $overageCents=strictPackageMoneyCents($newPackage['overage_price_snapshot'],'سعر الساعة الإضافية');$priceCents=strictPackageMoneyCents($newPackage['total_price'],'إجمالي السعر');$paidCents=strictPackageMoneyCents($newPackage['paid_amount'],'المدفوع الآن');if($paidCents>$priceCents)$abort('المدفوع الآن لا يمكن أن يتجاوز إجمالي السعر.',422,'invalid_payment_amount');$method=trim((string)($newPackage['payment_method']??''));if(!in_array($method,['cash','instapay','vodafone_cash'],true))$abort('طريقة الدفع غير صحيحة.',422,'invalid_payment_method');$notes=trim((string)($newPackage['notes']??''));
            $packageInsert=$pdo->prepare("INSERT INTO client_packages (organization_id,client_id,service_id,name,notes,billing_unit,purchased_quantity,purchased_minutes,held_quantity,held_minutes,consumed_quantity,consumed_minutes,payment_due_quantity,payment_due_minutes,deposit_percent_snapshot,overage_price_snapshot,total_price,paid_amount,starts_at,expires_at,validity_mode_snapshot,validity_days_snapshot,status) VALUES (?,?,?,?,?,'hour',?,?,0,0,0,0,?,?,?,?,?,?,?,?,?,?,'active')");$packageInsert->execute([$organizationId,$clientId,$serviceId,$name,$notes?:null,settlementHours($purchasedMinutes),$purchasedMinutes,settlementHours($paymentDueMinutes),$paymentDueMinutes,$depositPercent,packageMoney($overageCents),packageMoney($priceCents),packageMoney($paidCents),$starts,$expires,$validityMode,$validity]);$packageId=(int)$pdo->lastInsertId();$package=['id'=>$packageId,'client_id'=>$clientId,'service_id'=>$serviceId,'name'=>$name,'service_name'=>$service['name'],'billing_unit'=>'hour','purchased_quantity'=>settlementHours($purchasedMinutes),'purchased_minutes'=>$purchasedMinutes,'held_quantity'=>0,'held_minutes'=>0,'consumed_quantity'=>0,'consumed_minutes'=>0,'payment_due_quantity'=>settlementHours($paymentDueMinutes),'payment_due_minutes'=>$paymentDueMinutes,'deposit_percent_snapshot'=>$depositPercent,'overage_price_snapshot'=>packageMoney($overageCents),'total_price'=>packageMoney($priceCents),'paid_amount'=>packageMoney($paidCents),'starts_at'=>$starts,'expires_at'=>$expires,'validity_mode_snapshot'=>$validityMode,'validity_days_snapshot'=>$validity,'status'=>'active'];insertPackageUsage($pdo,$package,null,'opening',settlementHours($purchasedMinutes),'إنشاء الباقة أثناء تحويل حجز مؤقت','package:'.$packageId.':opening',$user['id']);
            if($paidCents>0){$paid=packageMoney($paidCents);$pdo->prepare("INSERT INTO payments (organization_id,client_id,client_name,amount,method,status,reference,reviewed_by,reviewed_at) VALUES (?,?,?,?,?,'approved',?,?,NOW())")->execute([$organizationId,$clientId,$client['name'],$paid,$method,'package-'.$packageId.'-opening',$user['id']]);$paymentId=(int)$pdo->lastInsertId();$pdo->prepare('INSERT INTO payment_allocations (organization_id,client_id,payment_id,payment_proof_id,client_package_id,invoice_id,amount) VALUES (?,?,?,NULL,?,NULL,?)')->execute([$organizationId,$clientId,$paymentId,$packageId,$paid]);$pdo->prepare("INSERT INTO finance (organization_id,client_id,type,entry_kind,category,amount,method,detail,date,entity,source_type,source_id,correlation_id,is_system,created_by) VALUES (?,?,?,'income','package_payment',?,?,?,?,?,'payment',?,?,1,?)")->execute([$organizationId,$clientId,'إيراد',$paid,$method,'دفعة إنشاء باقة '.$name,cairoNow()->format('Y-m-d'),'الشركة',$paymentId,'payment:'.$paymentId,$user['id']]);}
            $createdPackage=$package;audit($pdo,$user,'create','client_packages',$packageId,null,$package);recordChangeEvent($pdo,$organizationId,$clientId,'client_packages','client_packages',$packageId,'created');
        }
        $serviceLabel=trim((string)($package['name']??''))?:trim((string)($package['service_name']??''));$notes=trim((string)($block['note']??''));$insert=$pdo->prepare("INSERT INTO bookings (organization_id,client_id,client_package_id,service_id,resource_id,client_name,service,date,start_time,end_time,duration_minutes,requested_quantity,status,notes,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?, 'confirmed',?,?)");$insert->execute([$organizationId,$clientId,$packageId,$package['service_id'],$block['resource_id'],$client['name'],$serviceLabel,$block['block_date'],$block['start_time'],$block['end_time'],$duration,settlementHours($duration),$notes?:null,$user['id']]);$bookingId=(int)$pdo->lastInsertId();
        $transfer=$pdo->prepare('UPDATE booking_slots SET booking_id=?,booking_block_id=NULL WHERE organization_id=? AND booking_block_id=?');$transfer->execute([$bookingId,$organizationId,$id]);if($transfer->rowCount()!==$expectedSlots)$abort('تغيرت خانات الموعد أثناء التحويل. لم يتم حفظ أي تغيير.',409,'booking_slot_transfer_conflict');mutateLockedPackageQuantities($pdo,$package,0,settlementHours($duration),0,true);insertPackageUsage($pdo,$package,$bookingId,'hold',settlementHours($duration),'تحويل حجز مؤقت إلى موعد عميل','booking:'.$bookingId.':hold',$user['id']);$pdo->prepare('INSERT INTO booking_status_history (booking_id,from_status,to_status,note,changed_by) VALUES (?,NULL,?,?,?)')->execute([$bookingId,'confirmed',$mode==='new_package'?'بيع باقة وتحويل الحجز المؤقت':'تحويل الحجز المؤقت إلى حجز عميل',$user['id']]);
        $response=['booking'=>['id'=>$bookingId,'client_id'=>$clientId,'client_name'=>$client['name'],'client_package_id'=>$packageId,'service_id'=>(int)$package['service_id'],'service'=>$serviceLabel,'resource_id'=>(int)$block['resource_id'],'resource_name'=>$block['resource_name'],'date'=>$block['block_date'],'start_time'=>$block['start_time'],'end_time'=>$block['end_time'],'duration_minutes'=>$duration,'requested_quantity'=>settlementHours($duration),'status'=>'confirmed','notes'=>$notes?:null],'package_id'=>$packageId,'package'=>$createdPackage,'payment_id'=>$paymentId,'package_mode'=>$mode,'block_id'=>$id,'held_minutes'=>$duration,'remaining_minutes'=>authoritativePackageMinutes($package,'purchased')-authoritativePackageMinutes($package,'consumed')-authoritativePackageMinutes($package,'held')-$duration,'idempotent'=>false];
        $blockUpdate=$pdo->prepare("UPDATE booking_blocks SET status='cancelled',cancelled_by=?,cancelled_at=NOW(),converted_booking_id=?,conversion_idempotency_key=?,conversion_request_hash=?,conversion_response_json=? WHERE id=? AND organization_id=? AND status='active' AND conversion_idempotency_key IS NULL");$blockUpdate->execute([$user['id'],$bookingId,$idempotencyKey,$requestHash,json_encode($response,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$id,$organizationId]);if($blockUpdate->rowCount()!==1)$abort('تغير الحجز المؤقت أثناء التحويل. أعد تحميل التقويم.',409,'booking_block_conversion_conflict');
        auditBookingBlockChange($pdo,$user,$mode==='new_package'?'convert_with_new_package':'convert',$id,$block,$response);audit($pdo,$user,'create','bookings',$bookingId,null,$response['booking']);recordChangeEvent($pdo,$organizationId,$clientId,'bookings','bookings',$bookingId,'created');recordChangeEvent($pdo,$organizationId,$clientId,'client_packages','client_packages',$packageId,'booking_hold');queueClientWhatsAppSummary($pdo,$organizationId,$clientId);$pdo->commit();respond($response,201);
    }catch(PDOException $error){if($pdo->inTransaction())$pdo->rollBack();if(($error->errorInfo[1]??0)===1062){$replay=$pdo->prepare('SELECT conversion_idempotency_key,conversion_request_hash,conversion_response_json FROM booking_blocks WHERE id=? AND organization_id=?');$replay->execute([$id,$organizationId]);$saved=$replay->fetch();if($saved&&hash_equals((string)($saved['conversion_idempotency_key']??''),$idempotencyKey)&&hash_equals((string)($saved['conversion_request_hash']??''),$requestHash)){$response=json_decode((string)($saved['conversion_response_json']??''),true);if(is_array($response)){$response['idempotent']=true;respond($response);}}fail('تم تحويل هذا الحجز المؤقت من طلب آخر.',409,'booking_block_already_converted');}throw $error;}catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
...
if (preg_match('#^/booking-blocks/(\d+)$#',$path,$m)&&$method==='DELETE') {
    ...
    $lookup=$pdo->prepare('SELECT resource_id FROM booking_blocks WHERE id=? AND organization_id=?');$lookup->execute([$id,$user['organization_id']]);$blockResource=(int)$lookup->fetchColumn();
    $pdo->beginTransaction();try{$lock=$pdo->prepare('SELECT id FROM resources WHERE id=? AND organization_id=? FOR UPDATE');$lock->execute([$blockResource,$user['organization_id']]);$stmt=$pdo->prepare("SELECT * FROM booking_blocks WHERE id=? AND organization_id=? AND status='active' FOR UPDATE");$stmt->execute([$id,$user['organization_id']]);$selected=$stmt->fetch();if(!$selected){$pdo->rollBack();fail('فترة الحظر غير موجودة أو ألغيت من قبل.',404,'booking_block_not_found');}$ids=[$id];if($scope==='series'&&!empty($selected['series_key'])){stopBookingBlockSeries($pdo,(int)$user['organization_id'],$selected['series_key'],$selected['block_date']);$series=$pdo->prepare("SELECT id FROM booking_blocks WHERE organization_id=? AND series_key=? AND status='active' AND block_date>=? ORDER BY block_date,id FOR UPDATE");$series->execute([$user['organization_id'],$selected['series_key'],$selected['block_date']]);$ids=array_map('intval',$series->fetchAll(PDO::FETCH_COLUMN));}$marks=implode(',',array_fill(0,count($ids),'?'));$pdo->prepare("UPDATE booking_blocks SET status='cancelled',cancelled_by=?,cancelled_at=NOW() WHERE organization_id=? AND id IN ($marks) AND status='active'")->execute(array_merge([$user['id'],$user['organization_id']],$ids));releaseBookingBlockSlots($pdo,$ids,(int)$user['organization_id']);auditBookingBlockChange($pdo,$user,$scope==='series'?'cancel_series':'cancel',$id,$selected,['cancelled_ids'=>$ids,'scope'=>$scope]);$pdo->commit();respond(['cancelled'=>count($ids),'ids'=>$ids,'scope'=>$scope]);}catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
...
if (preg_match('#^/reschedule-requests/(\d+)/decision$#',$path,$m)&&$method==='POST'){
    ...
    $pdo->beginTransaction();try{$stmt=$pdo->prepare("SELECT r.*,b.organization_id AS booking_organization_id,b.resource_id,b.date AS old_date,b.start_time AS old_start_time,b.end_time AS old_end_time,b.requested_quantity,b.client_package_id,b.status AS booking_status,cp.billing_unit,cp.purchased_quantity,cp.held_quantity,cp.consumed_quantity,cp.starts_at AS package_starts_at,cp.expires_at AS package_expires_at,cp.validity_mode_snapshot,COALESCE(s.minimum_booking_minutes,60) AS minimum_booking_minutes,COALESCE(s.booking_increment_minutes,15) AS booking_increment_minutes FROM reschedule_requests r JOIN bookings b ON b.id=r.booking_id LEFT JOIN client_packages cp ON cp.id=b.client_package_id AND cp.organization_id=b.organization_id LEFT JOIN services s ON s.id=b.service_id AND s.organization_id=b.organization_id WHERE r.id=? AND r.organization_id=? AND r.status='pending' FOR UPDATE");$stmt->execute([$id,$user['organization_id']]);$request=$stmt->fetch();if(!$request){$pdo->rollBack();fail('الطلب غير موجود أو تمت مراجعته.',404);}
        if($action==='approve'){
            if($request['booking_status']!=='confirmed'){$pdo->rollBack();fail('لا يمكن تغيير حجز غير مؤكد.',409,'invalid_booking_state');}
            $proposedStart=normalizeBusinessTime((string)$request['proposed_start_time']);$proposedEnd=normalizeBusinessTime((string)$request['proposed_end_time'],true);$minimum=60;$increment=30;validateClientBookingTimeGrid($proposedStart,$proposedEnd,bookingDurationMinutes($proposedStart,$proposedEnd));lockClientCalendar($pdo,(int)$user['organization_id'],(int)$request['client_id']);requireClientSingleDate($pdo,(int)$user['organization_id'],(int)$request['client_id'],(string)$request['proposed_date'],(int)$request['booking_id']);$packageValidity=empty($request['client_package_id'])?null:['starts_at'=>$request['package_starts_at'],'expires_at'=>$request['package_expires_at'],'validity_mode_snapshot'=>$request['validity_mode_snapshot']];$minutes=validateBookingSchedule($pdo,(int)$user['organization_id'],(int)$request['resource_id'],(string)$request['proposed_date'],$proposedStart,$proposedEnd,$minimum,$increment,(int)$request['booking_id'],$packageValidity,true,$user,$payload);
...
if (preg_match('#^/bookings/(\d+)/admin-reschedule$#',$path,$m)&&$method==='POST'){
    ...
    $pdo->beginTransaction();try{
        ...
        $minimum=max(15,(int)$booking['minimum_booking_minutes']);$increment=max(15,(int)$booking['booking_increment_minutes']);$resourceId=(int)($payload['resource_id']??$booking['resource_id']);$packageValidity=empty($booking['client_package_id'])?null:['starts_at'=>$booking['package_starts_at'],'expires_at'=>$booking['package_expires_at'],'validity_mode_snapshot'=>$booking['validity_mode_snapshot']];$minutes=validateBookingSchedule($pdo,(int)$user['organization_id'],$resourceId,$date,$start,$end,$minimum,$increment,$id,$packageValidity,true,$user,$payload);
...
if (preg_match('#^/bookings/(\d+)/admin-cancel$#',$path,$m)&&$method==='POST'){
    $user=requireUser($user);requireRole($user,['owner','admin','operations']);
    fail('تم إيقاف مسار الإلغاء القديم. استخدم DELETE /bookings/{id} لحذف الموعد دون سبب.',410,'legacy_booking_cancellation_retired');
}

if (preg_match('#^/bookings/(\d+)$#',$path,$m)&&$method==='DELETE'){
    $user=requireUser($user);requireRole($user,['owner','admin','operations']);$id=(int)$m[1];
    $pdo->beginTransaction();try{
        $stmt=$pdo->prepare("SELECT * FROM bookings WHERE id=? AND organization_id=? FOR UPDATE");$stmt->execute([$id,$user['organization_id']]);$booking=$stmt->fetch();if(!$booking){$pdo->rollBack();fail('الموعد غير موجود.',404,'booking_not_found');}
        $released=deleteEligibleBooking($pdo,$user,$booking);$pdo->commit();respond(['id'=>$id,'deleted'=>true,'released_quantity'=>$released]);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}

if (preg_match('#^/bookings/(\d+)/cancel-decision$#',$path,$m)&&$method==='POST'){
    $user=requireUser($user);requireRole($user,['owner','admin','operations']);$id=(int)$m[1];$payload=body();if(array_key_exists('reason',$payload)||array_key_exists('charge',$payload))fail('قرار حذف الموعد لا يقبل سببًا أو خصمًا.',422,'cancellation_reason_not_supported');$approve=(bool)($payload['approve']??false);
    $pdo->beginTransaction();try{
        ...
        if($approve){$settled=deleteEligibleBooking($pdo,$user,$booking);$pdo->commit();respond(['id'=>$id,'deleted'=>true,'released_quantity'=>$settled]);}
        $history=$pdo->prepare("SELECT from_status FROM booking_status_history WHERE booking_id=? AND to_status IN ('cancel_requested','late_cancel_requested') ORDER BY id DESC LIMIT 1");$history->execute([$id]);$restore=(string)$history->fetchColumn();$newStatus=in_array($restore,['pending','confirmed','alternative_proposed'],true)?$restore:'pending';
        $pdo->prepare('UPDATE bookings SET status=?,cancellation_charge=0,cancellation_override_reason=NULL,decided_by=?,decided_at=NOW() WHERE id=? AND organization_id=?')->execute([$newStatus,$user['id'],$id,$user['organization_id']]);
        $pdo->prepare('INSERT INTO booking_status_history (booking_id,from_status,to_status,note,changed_by) VALUES (?,?,?,NULL,?)')->execute([$id,$booking['status'],$newStatus,$user['id']]);
        audit($pdo,$user,'cancel_decision','bookings',$id,$booking,['client_id'=>(int)$booking['client_id'],'status'=>$newStatus,'approved'=>false]);queueClientWhatsAppSummary($pdo,(int)$user['organization_id'],(int)$booking['client_id']);$pdo->commit();respond(['id'=>$id,'status'=>$newStatus]);
    }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
}

if (preg_match('#^/payment-proofs/(\d+)/decision$#',$path,$m)&&$method==='POST'){
    $user=requireUser($user);respond(reviewPaymentProof($pdo,$user,(int)$m[1],body()));
}
...
if ($path === '/client/service-history' && $method === 'GET') {
    ...
    foreach($stmt->fetchAll() as $row){
        ...
        $actualMinutes=$row['actual_minutes']!==null?(int)$row['actual_minutes']:max(0,(int)round($row['actual_seconds']!==null?(float)$row['actual_seconds']/60:(float)$row['actual_hours']*60));$deducted=$row['billing_unit']==='reel'?(float)$row['billable_quantity']:($row['covered_minutes']!==null?(float)$row['covered_minutes']/60:(float)$row['billable_quantity']);$excess=$row['excess_minutes']!==null?(int)$row['excess_minutes']:max(0,(int)round((float)$row['overage_quantity']*60));$mode=(string)($row['settlement_mode']?:'none');$outcome=trim((string)($row['client_note']??''))?:($outcomes[$mode]??'تمت تسوية الجلسة');
...
if ($path === '/client/booking-availability' && $method === 'GET') {
    ...
    $rawPackage=(string)($_GET['client_package_id']??'0');$rawBooking=(string)($_GET['booking_id']??'0');$rawDuration=(string)($_GET['duration_minutes']??'');$rawDays=(string)($_GET['days']??'21');
    if(!ctype_digit($rawBooking)||!ctype_digit($rawPackage)||!ctype_digit($rawDuration)||!ctype_digit($rawDays))fail('حدد الباقة والمدة وفترة البحث بشكل صحيح.',422,'invalid_availability_request');
    $packageId=(int)$rawPackage;$durationMinutes=(int)$rawDuration;$windowDays=(int)$rawDays;
    if(($packageId<1&&(int)$rawBooking<1)||$windowDays<1||$windowDays>31)fail('فترة البحث من يوم واحد إلى 31 يومًا.',422,'availability_window_out_of_range');
    $startDate=trim((string)($_GET['start_date']??cairoNow()->format('Y-m-d')));
    respond(clientBookingAvailability($pdo,$user,$packageId,$durationMinutes,$startDate,$windowDays,(int)$rawBooking));
}

if ($path === '/client/studio-availability' && $method === 'GET') {
    $user=requireUser($user);requireRole($user,['client']);
    respond(clientStudioCalendar($pdo,$user,(int)($_GET['service_id']??0),(int)($_GET['duration_minutes']??60),(string)($_GET['start_date']??cairoNow()->format('Y-m-d')),(int)($_GET['days']??31)));
}
...
if ($path === '/bookings/request' && $method === 'POST') {
    ...
    $minutes = $user['role']==='client' ? validateClientBookingTimeGrid($start,$end,$payload['duration_minutes']??null) : bookingDurationMinutes($start, $end);
    if ($user['role']==='client') requireClientBookingWindow($date,$start,$end);
    ...
    if ($packageId) {
        $p = $pdo->prepare("SELECT cp.*,s.name AS package_service_name,s.minimum_booking_minutes AS package_minimum_minutes,s.booking_increment_minutes AS package_increment_minutes FROM client_packages cp JOIN services s ON s.id=cp.service_id AND s.organization_id=cp.organization_id WHERE cp.id = ? AND cp.client_id = ? AND cp.organization_id=? AND cp.status = 'active'");
        $p->execute([$packageId,$clientId,$user['organization_id']]);$package=$p->fetch();if(!$package)fail('الباقة غير فعالة أو غير موجودة.',422,'invalid_package');if(!empty($package['starts_at'])&&($date<$package['starts_at']||$date>$package['expires_at']||(($package['validity_mode_snapshot']??'rolling')==='shooting_day'&&$date!==$package['starts_at'])))fail('الموعد خارج صلاحية الباقة.',422,'booking_outside_package_validity');if($serviceId&&$serviceId!==(int)$package['service_id'])fail('الخدمة المختارة لا تطابق خدمة الباقة.',422,'package_service_mismatch');$serviceId=(int)$package['service_id'];$serviceName=(string)$package['package_service_name'];$minimumMinutes=max(15,(int)$package['package_minimum_minutes']);$incrementMinutes=max(15,(int)$package['package_increment_minutes']);if($package['billing_unit']==='reel'){$requestedQuantity=max(1,(float)($payload['requested_reels']??$payload['requested_quantity']??1));}
    }
    if ($projectId) {
    ...
    if($user['role']==='client'){$minimumMinutes=60;$incrementMinutes=30;}
    $status = $user['role'] === 'client' ? 'pending' : (string)($payload['status'] ?? 'pending');
    if (!in_array($status, ['pending','confirmed'], true)) $status = 'pending';
    $pdo->beginTransaction();try{
        if($user['role']==='client'){
            lockClientCalendar($pdo,(int)$user['organization_id'],$clientId);requireClientSingleDate($pdo,(int)$user['organization_id'],$clientId,$date);
            if($package){$fresh=$pdo->prepare('SELECT * FROM client_packages WHERE id=? AND organization_id=? FOR UPDATE');$fresh->execute([$packageId,$user['organization_id']]);$package=$fresh->fetch();if(!$package||$package['status']!=='active')fail('الباقة غير فعالة.',422,'invalid_package');}
        }
        $minutes=validateBookingSchedule($pdo,(int)$user['organization_id'],$resourceId,$date,$start,$end,$minimumMinutes,$incrementMinutes,null,$package,true,$user,$payload);
        if($user['role']==='client'&&$package){requireClientDayDuration($package,$minutes);$available=packageAvailableQuantity($package)-clientPendingPackageQuantity($pdo,(int)$user['organization_id'],$clientId,(int)$package['id']);if(($package['billing_unit']==='hour'&&($available*60)+.001<$minutes)||($package['billing_unit']==='reel'&&$available<$requestedQuantity)){$pdo->rollBack();fail('رصيد الباقة المتاح لا يكفي لهذا الحجز.',422,'insufficient_package_balance');}}
        ...
        if($status==='confirmed'){$booking=['id'=>$id,'organization_id'=>$user['organization_id'],'resource_id'=>$resourceId,'date'=>$date,'start_time'=>"$start:00",'end_time'=>"$end:00"];
            ...
            if($packageId){$p=$pdo->prepare('SELECT * FROM client_packages WHERE id=? AND organization_id=? FOR UPDATE');$p->execute([$packageId,$user['organization_id']]);$locked=$p->fetch();if(!$locked)fail('الباقة المرتبطة بالحجز غير موجودة.',404,'package_not_found');activatePackageOnFirstBooking($pdo,$locked,$date,(int)$user['organization_id']);if(packageAvailableQuantity($locked)+0.0001<$requestedQuantity){$pdo->rollBack();fail('رصيد الباقة لا يكفي لتأكيد الحجز.',422,'insufficient_package_balance');}mutateLockedPackageQuantities($pdo,$locked,0,$requestedQuantity,0);insertPackageUsage($pdo,$locked,$id,'hold',$requestedQuantity,'تأكيد الحجز','booking:'.$id.':hold',$user['id']);}}
...
if ($path === '/reschedule-requests' && $method === 'POST') {
    ...
    requireClientBookingWindow($date,$start,$end);
    $pdo->beginTransaction();try{
        lockClientCalendar($pdo,(int)$user['organization_id'],(int)$user['client_id']);
        $stmt = $pdo->prepare("SELECT b.*,cp.starts_at AS package_starts_at,cp.expires_at AS package_expires_at,cp.validity_mode_snapshot,COALESCE(s.minimum_booking_minutes,60) AS minimum_booking_minutes,COALESCE(s.booking_increment_minutes,15) AS booking_increment_minutes FROM bookings b LEFT JOIN client_packages cp ON cp.id=b.client_package_id AND cp.organization_id=b.organization_id LEFT JOIN services s ON s.id=b.service_id AND s.organization_id=b.organization_id WHERE b.id = ? AND b.organization_id=? AND b.client_id = ? AND b.status IN ('confirmed','alternative_proposed') FOR UPDATE");
        $stmt->execute([$bookingId,$user['organization_id'],$user['client_id']]);$booking=$stmt->fetch();if(!$booking){$pdo->rollBack();fail('الحجز غير موجود أو لا يمكن تغييره.',404);}
        $minimum=60;$increment=30;validateClientBookingTimeGrid($start,$end,bookingDurationMinutes($start,$end));requireClientSingleDate($pdo,(int)$user['organization_id'],(int)$user['client_id'],$date,$bookingId);if(($booking['validity_mode_snapshot']??'')==='shooting_day'&&bookingDurationMinutes($start,$end)!==bookingDurationMinutes($booking['start_time'],$booking['end_time']))fail('يجب الاحتفاظ بمدة يوم التصوير كاملة.',422,'daily_full_duration_required');if(clientBookingNoticeIsLate(['date'=>$date,'start_time'=>$start],cairoNow()))fail('الموعد البديل يجب أن يبعد 48 ساعة على الأقل دون احتساب الجمعة.',422,'late_reschedule');$packageValidity=empty($booking['client_package_id'])?null:['starts_at'=>$booking['package_starts_at'],'expires_at'=>$booking['package_expires_at'],'validity_mode_snapshot'=>$booking['validity_mode_snapshot']];$minutes=validateBookingSchedule($pdo,(int)$user['organization_id'],(int)$booking['resource_id'],$date,$start,$end,$minimum,$increment,$bookingId,$packageValidity,true);
        if(clientBookingNoticeIsLate($booking,cairoNow())){$pdo->rollBack();fail('يجب طلب تغيير الموعد قبل الحجز بـ48 ساعة دون احتساب يوم الجمعة. تواصل مع الإدارة للاستثناء.',422,'late_reschedule');}
        if(!empty($booking['client_package_id'])){
            $q=$pdo->prepare('SELECT * FROM client_packages WHERE id=? AND organization_id=? FOR UPDATE');$q->execute([$booking['client_package_id'],$user['organization_id']]);$pkg=$q->fetch();if(!$pkg||$pkg['status']!=='active')fail('الباقة غير فعالة.',422,'invalid_package');
            $available=packageAvailableQuantity($pkg)+bookingHeldQuantity($pdo,$bookingId,(int)$pkg['id'])-clientPendingPackageQuantity($pdo,(int)$user['organization_id'],(int)$user['client_id'],(int)$pkg['id'],$bookingId);
            if($pkg['billing_unit']==='hour'&&$available*60+.001<$minutes)fail('رصيد الباقة لا يكفي للمدة الجديدة.',422,'insufficient_package_balance');
        }
        $existing=$pdo->prepare("SELECT id FROM reschedule_requests WHERE booking_id=? AND organization_id=? AND status='pending' LIMIT 1 FOR UPDATE");$existing->execute([$bookingId,$user['organization_id']]);if($existing->fetch()){$pdo->rollBack();fail('يوجد طلب تغيير قيد المراجعة بالفعل.',409);}
        $stmt=$pdo->prepare("INSERT INTO reschedule_requests (organization_id,booking_id,client_id,proposed_date,proposed_start_time,proposed_end_time,reason,status) VALUES (?,?,?,?,?,?,?,'pending')");$stmt->execute([$user['organization_id'],$bookingId,$user['client_id'],$date,"$start:00","$end:00",trim((string)($payload['reason']??''))]);
        $id=(int)$pdo->lastInsertId();audit($pdo,$user,'create','reschedule_requests',$id,null,['client_id'=>(int)$user['client_id'],'booking_id'=>$bookingId,'date'=>$date,'start'=>$start]);$pdo->commit();respond(['id'=>$id,'status'=>'pending'],201);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}

if (preg_match('#^/bookings/(\d+)/cancel-request$#', $path, $m) && $method === 'POST') {
    $user = requireUser($user); requireRole($user, ['client']);
    $bookingId=(int)$m[1];$pdo->beginTransaction();try{
        $stmt=$pdo->prepare("SELECT * FROM bookings WHERE id=? AND organization_id=? AND client_id=? AND status IN ('pending','confirmed','alternative_proposed') FOR UPDATE");
        $stmt->execute([$bookingId,$user['organization_id'],$user['client_id']]);$booking=$stmt->fetch();if(!$booking){$pdo->rollBack();fail('الحجز غير موجود أو لا يمكن حذفه.',404);}
        if(clientCancellationIsLate($booking)){$pdo->rollBack();fail('يجب طلب الإلغاء قبل الموعد بـ48 ساعة على الأقل دون احتساب يوم الجمعة. يبقى الحجز مؤكدًا وتُحتسب ساعاته من الباقة. تواصل مع الإدارة لأي استثناء.',422,'late_cancellation');}
        $newStatus=clientBookingNoticeIsLate($booking)?'late_cancel_requested':'cancel_requested';
        $pdo->prepare('UPDATE bookings SET status=? WHERE id=? AND organization_id=?')->execute([$newStatus,$bookingId,$user['organization_id']]);
        $pdo->prepare('INSERT INTO booking_status_history (booking_id, from_status, to_status, note, changed_by) VALUES (?, ?, ?, ?, ?)')->execute([$bookingId,$booking['status'],$newStatus,'طلب حذف الموعد من العميل',$user['id']]);
        audit($pdo,$user,'cancel_request','bookings',$bookingId,$booking,['client_id'=>(int)$user['client_id'],'status'=>$newStatus]);$pdo->commit();respond(['id'=>$bookingId,'status'=>$newStatus]);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
...
if (preg_match('#^/bookings/(\d+)/decision$#', $path, $m) && $method === 'POST') {
    ...
    try {
        ...
        $before = $booking;
        if ($action === 'confirm') {
            requireBookingSlotAvailable($pdo,(int)$user['organization_id'],(int)$booking['resource_id'],$booking['date'],$booking['start_time'],$booking['end_time'],$bookingId,$user,$payload);
            ...
            if ($booking['client_package_id']) {
                $pkg = $pdo->prepare('SELECT * FROM client_packages WHERE id = ? AND organization_id=? FOR UPDATE'); $pkg->execute([$booking['client_package_id'],$user['organization_id']]); $package = $pkg->fetch();
                if(!$package){$pdo->rollBack();fail('الباقة المرتبطة بالحجز غير موجودة.',404,'package_not_found');}activatePackageOnFirstBooking($pdo,$package,(string)$booking['date'],(int)$user['organization_id']);
            ...
            validateBookingSchedule($pdo,(int)$user['organization_id'],(int)$booking['resource_id'],$date,$start,$end,$minimum,$increment,$bookingId,null,true);
...
if ($path === '/payment-proofs' && $method === 'POST') {
    ...
    if(!schemaColumnExists($pdo,'payment_proofs','payment_method')||!schemaColumnExists($pdo,'payment_proofs','transfer_account_snapshot'))fail('يلزم تشغيل تحديث لوحة العميل رقم 030.',503,'client_dashboard_migration_required');
    ...
    $pdo->beginTransaction();try{
        $stmt = $pdo->prepare('INSERT INTO payment_proofs (organization_id,client_id,client_package_id,invoice_id,amount,payment_method,transfer_account_snapshot,file_path,original_name,mime_type,status) VALUES (?,?,?,?,?,?,?,?,?,?,\'pending\')');
        $stmt->execute([$organizationId,$clientId,$packageId?:null,$invoiceId?:null,$amount,$paymentMethod,$transferAccount,$relative,basename((string)$file['name']),$mime]); $id=(int)$pdo->lastInsertId();
        audit($pdo,$user,'create','payment_proofs',$id,null,['client_id'=>$clientId,'client_package_id'=>$packageId?:null,'invoice_id'=>$invoiceId?:null,'amount'=>$amount,'payment_method'=>$paymentMethod]);$pdo->commit();respond(['id'=>$id,'status'=>'pending'],201);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();if(is_file($target))@unlink($target);throw $error;}
}
...
if (preg_match('#^/data/([a-z_]+)$#', $path, $m)) {
    ...
    $isPublicConfig = (!$user || $user['role']==='applicant') && $table === 'app_config' && $method === 'GET';
    if (!$isPublicConfig) {
        $user = requireUser($user);
    ...
    if($table==='clients'&&$method==='PATCH'){
        ...
        if(array_key_exists('additional_phones',$values)){
            $values['additional_phones']=normalizeAdditionalClientPhones($values['additional_phones'],(string)($values['phone1']??''));
            $values['phone2']=$values['additional_phones'][0]??null;
            requireClientPhonesSchema($pdo);
        }
        if(array_key_exists('phone1',$values))$values['phone1']=normalizeClientContactPhone($values['phone1']);
    }
    if($table==='clients'&&$method==='POST'&&is_array($rows)){
        foreach($rows as &$clientRow){if(array_key_exists('additional_phones',$clientRow)){
            $clientRow['additional_phones']=normalizeAdditionalClientPhones($clientRow['additional_phones'],(string)($clientRow['phone1']??''));
            $clientRow['phone2']=$clientRow['additional_phones'][0]??null;requireClientPhonesSchema($pdo);
        }}unset($clientRow);
    }
    // Booking creation and schedule/package mutations must pass through the
    // dedicated transactional endpoints that reserve slots and update holds.
    if($table==='bookings'&&$method==='POST')fail('استخدم مسار الحجز المخصص لضمان فحص الموعد والرصيد.',409,'booking_dedicated_route_required');
    if($table==='bookings'&&$method==='PATCH'){
        $candidate=is_array($values)?$values:$payload;$scheduleFields=['client_id','client_package_id','project_id','service_id','resource_id','date','start_time','end_time','duration_minutes','requested_quantity','status'];
        if(array_intersect($scheduleFields,array_keys(is_array($candidate)?$candidate:[])))fail('تغيير الموعد أو الباقة يتم من مسار الحجز المخصص.',409,'booking_dedicated_route_required');
    }
    if($method==='POST') {
        ...
        $inserted=[];$pdo->beginTransaction();try{foreach($rows as $row){
            ...
            $allowed=array_values(array_diff($allowed,['id','created_at','updated_at','organization_id','registration_source']));
