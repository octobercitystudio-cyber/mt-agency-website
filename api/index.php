...
if ($path==='/studio-sessions/start-direct' && $method==='POST') {
    ...
    $pdo->beginTransaction();try{
        ...
        $duration=validateBookingSchedule($pdo,(int)$user['organization_id'],$resourceId,$date,$start,$end,60,15,null,null,true);
...
if (preg_match('#^/bookings/(\d+)/admin-reschedule$#',$path,$m)&&$method==='POST'){
    ...
    $pdo->beginTransaction();try{
        ...
        $minimum=max(15,(int)$booking['minimum_booking_minutes']);$increment=max(15,(int)$booking['booking_increment_minutes']);$resourceId=(int)($payload['resource_id']??$booking['resource_id']);$packageValidity=empty($booking['client_package_id'])?null:['starts_at'=>$booking['package_starts_at'],'expires_at'=>$booking['package_expires_at'],'validity_mode_snapshot'=>$booking['validity_mode_snapshot']];$minutes=validateBookingSchedule($pdo,(int)$user['organization_id'],$resourceId,$date,$start,$end,$minimum,$increment,$id,$packageValidity,true);
...
if ($path === '/bookings/request' && $method === 'POST') {
    ...
    $pdo->beginTransaction();try{
        ...
        if($user['role']==='client'&&$package){$available=packageAvailableQuantity($package)-clientPendingPackageQuantity($pdo,(int)$user['organization_id'],$clientId,(int)$package['id']);if(($package['billing_unit']==='hour'&&($available*60)+.001<$minutes)||($package['billing_unit']==='reel'&&$available<$requestedQuantity)){$pdo->rollBack();fail('رصيد الباقة المتاح لا يكفي لهذا الحجز.',422,'insufficient_package_balance');}}
