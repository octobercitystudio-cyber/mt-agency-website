...
if (preg_match('#^/booking-blocks/(\d+)/convert$#',$path,$m)&&$method==='POST') {
    $user=requireUser($user);requireRole($user,['owner','admin','operations']);requireBookingBlockSchema($pdo);$id=(int)$m[1];$payload=body();
    $clientId=(int)($payload['client_id']??0);$packageId=(int)($payload['client_package_id']??0);$idempotencyKey=trim((string)($payload['idempotency_key']??''));
    if(!preg_match('/^[A-Za-z0-9:_-]{12,120}$/',$idempotencyKey))fail('مفتاح حفظ التحويل غير صحيح.',422,'invalid_idempotency_key');
    $requestHash=hash('sha256',json_encode(['block_id'=>$id,'client_id'=>$clientId,'client_package_id'=>$packageId],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
    $pdo->beginTransaction();try{
        $blockStmt=$pdo->prepare('SELECT bb.*,r.name AS resource_name FROM booking_blocks bb JOIN resources r ON r.id=bb.resource_id AND r.organization_id=bb.organization_id WHERE bb.id=? AND bb.organization_id=? FOR UPDATE');$blockStmt->execute([$id,$user['organization_id']]);$block=$blockStmt->fetch();if(!$block){$pdo->rollBack();fail('الحجز المؤقت غير موجود.',404,'booking_block_not_found');}
        if(!empty($block['converted_booking_id'])){
            $sameKey=hash_equals((string)($block['conversion_idempotency_key']??''),$idempotencyKey);$sameHash=hash_equals((string)($block['conversion_request_hash']??''),$requestHash);
            if(!$sameKey||!$sameHash){$pdo->rollBack();fail('تم تحويل هذا الحجز المؤقت من قبل.',409,'booking_block_already_converted');}
            $response=json_decode((string)($block['conversion_response_json']??''),true);if(!is_array($response)){$pdo->rollBack();fail('تعذر استعادة نتيجة التحويل.',409,'idempotency_incomplete');}$pdo->commit();$response['idempotent']=true;respond($response);
        }
        if((string)$block['status']!=='active'){$pdo->rollBack();fail('الحجز المؤقت ملغى ولا يمكن تحويله.',409,'booking_block_inactive');}
        $clientStmt=$pdo->prepare("SELECT id,name FROM clients WHERE id=? AND organization_id=? AND status='active' FOR UPDATE");$clientStmt->execute([$clientId,$user['organization_id']]);$client=$clientStmt->fetch();if(!$client){$pdo->rollBack();fail('العميل غير موجود أو غير نشط.',422,'invalid_booking_client');}
        $packageStmt=$pdo->prepare("SELECT cp.*,s.name AS service_name FROM client_packages cp JOIN services s ON s.id=cp.service_id AND s.organization_id=cp.organization_id WHERE cp.id=? AND cp.organization_id=? AND cp.client_id=? AND cp.status='active' AND cp.billing_unit='hour' FOR UPDATE");$packageStmt->execute([$packageId,$user['organization_id'],$clientId]);$package=$packageStmt->fetch();if(!$package){$pdo->rollBack();fail('اختر باقة ساعات نشطة تخص هذا العميل.',422,'invalid_target_package');}
        activatePackageOnFirstBooking($pdo,$package,(string)$block['block_date'],(int)$user['organization_id']);$duration=(int)$block['duration_minutes'];$free=authoritativePackageMinutes($package,'purchased')-authoritativePackageMinutes($package,'held')-authoritativePackageMinutes($package,'consumed');if($free<$duration){$pdo->rollBack();fail('رصيد الباقة لا يكفي مدة الحجز المؤقت.',409,'insufficient_package_balance');}
        $serviceLabel=trim((string)($package['name']??''))?:trim((string)($package['service_name']??''));$notes=trim((string)($block['note']??''));
        $insert=$pdo->prepare("INSERT INTO bookings (organization_id,client_id,client_package_id,service_id,resource_id,client_name,service,date,start_time,end_time,duration_minutes,requested_quantity,status,notes,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?, 'confirmed',?,?)");
        $insert->execute([$user['organization_id'],$clientId,$packageId,$package['service_id'],$block['resource_id'],$client['name'],$serviceLabel,$block['block_date'],$block['start_time'],$block['end_time'],$duration,settlementHours($duration),$notes?:null,$user['id']]);$bookingId=(int)$pdo->lastInsertId();
        $transfer=$pdo->prepare('UPDATE booking_slots SET booking_id=?,booking_block_id=NULL WHERE organization_id=? AND booking_block_id=?');$transfer->execute([$bookingId,$user['organization_id'],$id]);$expectedSlots=(int)ceil($duration/15);if($transfer->rowCount()!==$expectedSlots)fail('تغيرت خانات الموعد أثناء التحويل. لم يتم حفظ أي تغيير.',409,'booking_slot_transfer_conflict');
        mutateLockedPackageQuantities($pdo,$package,0,settlementHours($duration),0,true);insertPackageUsage($pdo,$package,$bookingId,'hold',settlementHours($duration),'تحويل حجز مؤقت إلى موعد عميل','booking:'.$bookingId.':hold',$user['id']);
        $pdo->prepare('INSERT INTO booking_status_history (booking_id,from_status,to_status,note,changed_by) VALUES (?,NULL,?,?,?)')->execute([$bookingId,'confirmed','تحويل الحجز المؤقت إلى حجز عميل',$user['id']]);
        $response=['booking'=>['id'=>$bookingId,'client_id'=>$clientId,'client_name'=>$client['name'],'client_package_id'=>$packageId,'service_id'=>(int)$package['service_id'],'service'=>$serviceLabel,'resource_id'=>(int)$block['resource_id'],'resource_name'=>$block['resource_name'],'date'=>$block['block_date'],'start_time'=>$block['start_time'],'end_time'=>$block['end_time'],'duration_minutes'=>$duration,'requested_quantity'=>settlementHours($duration),'status'=>'confirmed','notes'=>$notes?:null],'block_id'=>$id,'held_minutes'=>$duration,'idempotent'=>false];
        $pdo->prepare("UPDATE booking_blocks SET status='cancelled',cancelled_by=?,cancelled_at=NOW(),converted_booking_id=?,conversion_idempotency_key=?,conversion_request_hash=?,conversion_response_json=? WHERE id=? AND organization_id=? AND status='active'")->execute([$user['id'],$bookingId,$idempotencyKey,$requestHash,json_encode($response,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$id,$user['organization_id']]);
        auditBookingBlockChange($pdo,$user,'convert',$id,$block,$response);audit($pdo,$user,'create','bookings',$bookingId,null,$response['booking']);recordChangeEvent($pdo,(int)$user['organization_id'],$clientId,'bookings','bookings',$bookingId,'created');recordChangeEvent($pdo,(int)$user['organization_id'],$clientId,'client_packages','client_packages',$packageId,'booking_hold');queueClientWhatsAppSummary($pdo,(int)$user['organization_id'],$clientId);$pdo->commit();respond($response,201);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
