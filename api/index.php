...
function audit(PDO $pdo, array $user, string $action, string $entityType, ?int $entityId, mixed $before, mixed $after): int {
    ...
    notifyOwnersOfClientAction($pdo,$user,$action,$entityType,$entityId,$before,$after,$sourceEventId);
...
if (preg_match('#^/client-packages/(\d+)/extend$#',$path,$m)&&$method==='POST'){
    $user=requireUser($user);requireRole($user,['owner']);$id=(int)$m[1];$payload=body();$expires=trim((string)($payload['expires_at']??''));$reason=ownerCorrectionReason($payload);if(!preg_match('/^\d{4}-\d{2}-\d{2}$/',$expires))fail('تاريخ الانتهاء الجديد غير صحيح.',422,'invalid_package_expiry');$pdo->beginTransaction();try{
        ...
        $pdo->prepare("UPDATE client_packages SET expires_at=?,status=IF(status='expired' AND ? >= CURDATE(),'active',status),version=version+1 WHERE id=? AND organization_id=?")->execute([$expires,$expires,$id,$user['organization_id']]);$after=$before;$after['expires_at']=$expires;$after['status']=$before['status']==='expired'&&$expires>=cairoNow()->format('Y-m-d')?'active':$before['status'];$after['version']=(int)($before['version']??1)+1;ownerAdjustment($pdo,$user,'client_packages',$id,'validity',0,0,$reason,$before,$after);audit($pdo,$user,'extend','client_packages',$id,$before,$after+['reason'=>$reason]);$pdo->commit();try{queueClientWhatsAppSummary($pdo,(int)$user['organization_id'],(int)$before['client_id']);}catch(Throwable $notificationError){error_log('[Package validity WhatsApp] '.$notificationError->getMessage());}respond(['id'=>$id,'expires_at'=>$expires,'version'=>$after['version'],'idempotent'=>false]);
    }catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();throw $error;}
}
