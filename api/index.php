...
function clientNotificationTemplate(string $entityType, string $action, mixed $before, mixed $after): ?array {
    ...
    if($entityType==='bookings'){
        ...
        if($action==='admin_reschedule'||array_intersect(['date','start_time','end_time'],array_keys($after)))return ['booking_rescheduled','تم تحديث موعد الحجز','عدّلت الإدارة موعدك إلى: '.$moment.'.','schedule','info'];
        $map=['confirmed'=>['booking_confirmed','تم تأكيد الحجز','تم تأكيد موعدك: '.$moment.'.','success'],'rejected'=>['booking_rejected','تعذر تأكيد الحجز','لم يتم تأكيد الموعد: '.$moment.'.','danger'],'cancelled'=>['booking_cancelled','تم إلغاء الحجز','تم إلغاء الموعد: '.$moment.'.','warning']];if(isset($map[$status])){[$type,$title,$message,$severity]=$map[$status];return [$type,$title,$message,'schedule',$severity];}return null;
    }
