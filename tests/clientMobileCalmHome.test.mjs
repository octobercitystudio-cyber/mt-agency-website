...
test('selected Calm Focus concept is wired to the real four-page client app', () => {
  assert.match(dashboard, /client-app--calm/);
  assert.match(dashboard, /projects=\{projects\}/);
  assert.match(dashboard, /client-topbar-points/);
  assert.doesNotMatch(overview, /client-home-focus-grid|client-home-focus-side/);
  assert.match(overview, /client-package-metrics/);
...
test('real package cards keep authoritative live fields and progressive detail', () => {
  for (const field of [
    ...
    'pkg.starts_at', 'pkg.expires_at', 'pkg.client_notes',
  ]) assert.match(overview, new RegExp(field.replace('.', '\\.')));
  assert.match(overview, /بانتظار أول حجز/);
  assert.match(overview, /الجمعة محسوبة/);
});

test('calm mobile theme covers the four pages and protects small-phone interaction', () => {
  assert.match(styles, /@media\(max-width:680px\)[\s\S]*\.client-app--calm/);
  assert.match(styles, /#f7f4ee/);
  assert.match(styles, /client-package-metrics/);
  assert.match(styles, /min-height:44px/);
  assert.match(styles, /@media\(max-width:350px\)/);
  assert.match(styles, /focus-visible/);
  assert.match(dashboard, /client-app--calm-\$\{activeTab\}/);
  assert.match(dashboard, /activeTab === 'home' \? ' client-app--calm-home' : ''/);
});
