...
test('demo package payment creates one exact payment, allocation and finance row and refreshes package/invoice cents', async () => {
  ...
  resetDemoDatabase(); let db = JSON.parse(storage.get('mt_agency_erp_demo_v12')); const pkg = db.client_packages.find(row => row.id === 203); pkg.source_invoice_id = 703; storage.set('mt_agency_erp_demo_v12', JSON.stringify(db)); activateDemoMode('operations');
...
test('demo package payment validates roles, money, method, outstanding, idempotency mismatch and rollback', async () => {
  ...
  for (const role of ['finance','staff','client']) { resetDemoDatabase(); activateDemoMode(role); const denied = await demoClient.request('/client-packages/201/payments', { method: 'POST', body: JSON.stringify(make()) }); assert.equal(denied.error?.code, 'forbidden'); }
...
test('production endpoint is transactional, package scoped and exact-once', async () => {
  ...
  assert.ok(api.includes("client-packages/(\\d+)/payments")); assert.match(api, /requireRole\(\$user,\['owner','admin','operations'\]\)/); assert.match(api, /SELECT \* FROM client_packages WHERE id=\? AND organization_id=\? FOR UPDATE/); assert.match(api, /SELECT \* FROM invoices WHERE id=\? AND organization_id=\? AND client_id=\? FOR UPDATE/);
