...
test('package payment UI is shared, exact, accessible and responsive', async () => {
  ...
  assert.match(view, /package-payment-button/); assert.match(view, /canPay:\s*canAssign\s*&&\s*packageFinancialSummary/); assert.match(view, /<PackagePaymentModal/); assert.match(view, /bookingPackage\.open \|\| paymentPackage\.open/);
