...
test('package payment UI is shared, exact, accessible and responsive', async () => {
  ...
  assert.match(await load('src/erp/PackageWorkbench.jsx'), /package-payment-button/); assert.match(view, /canPay:\s*canAssign\s*&&\s*packageFinancialSummary/); assert.match(view, /<PackagePaymentModal/); assert.match(view, /bookingPackage\.open \|\| paymentPackage\.open/);
...
test('production endpoint is transactional, package scoped and exact-once', async () => {
  ...
  assert.match(api, /function packagePaymentSchemaReadyFresh/); assert.match(api, /function installPackagePaymentSchema/); assert.match(api, /requirePackagePaymentSchema\(\$pdo\)/); assert.match(api, /package_payments_migration_required/); assert.match(api, /ALTER TABLE payments ADD COLUMN note VARCHAR\(500\)/);
