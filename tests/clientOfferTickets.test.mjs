...
test('private offers remain isolated while client promotions use their own scoped subscription contract', async () => {
  ...
  assert.equal(dashboard.includes('/promotions/public'), false); assert.equal(dashboard.includes('/client/promotions'), true); assert.equal(component.includes('ClientPublicPromotions'), true); assert.equal(component.includes('autoplay'), false); assert.equal(component.includes('onView'), true); assert.match(component, /aria-live="off"/); assert.match(component, /عرض التفاصيل والقبول/); assert.match(component, /بدون تاريخ انتهاء/);
