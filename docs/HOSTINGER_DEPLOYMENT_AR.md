...
3. استورد ملفات قاعدة البيانات بالترتيب: `database/mysql/001_initial_schema.sql` ثم `002_seed_service_catalog.sql` ثم `003_projects_and_content.sql` ثم الترحيلات من `004_attendance.sql` حتى `028_comprehensive_integrity.sql` بالترتيب. يضيف 023 حماية تكرار البيع، و024 سياسة الباقة اليومية، و025 حماية تكرار دفعات الباقات، و026 بدء الصلاحية من أول حجز مؤكد، و027 الربط الآمن بين قيود الموظفين وحساباتهم، و028 أرشيفًا داخليًا آمنًا للحجوزات المحذوفة دون تعديل أي حجز قائم.
...
- إذا كانت قاعدة البيانات موجودة بالفعل، خذ نسخة احتياطية ثم استورد ملفات الترحيل من آخر رقم مطبق حتى `database/mysql/028_comprehensive_integrity.sql` بالترتيب. لا تتخطَّ أي رقم، لأن بيع الباقات والمدفوعات وبداية الصلاحية وحسابات الموظفين وأرشيف الحذف الآمن تعتمد على 023–028.
