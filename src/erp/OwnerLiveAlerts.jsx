...
export default function OwnerLiveAlerts({ alerts, onOpen, settings = false, onDeviceSettings }) {
  return <>
    {!settings && <button type="button" className="owner-notifications__sound" onClick={alerts.enabled ? alerts.mute : alerts.enable} aria-label={alerts.enabled ? 'كتم صوت التنبيهات' : 'تفعيل صوت التنبيهات'} title={alerts.enabled ? 'الصوت تلقائي — اضغط للكتم' : 'اضغط لتفعيل وتجربة صوت التنبيهات'}>{alerts.enabled ? <Volume2/> : <VolumeX/>}<span>{alerts.enabled ? 'الصوت تلقائي' : 'تفعيل الصوت'}</span></button>}
    {settings && <div className="owner-notifications__settings"><strong>الصوت وإشعارات الجهاز</strong><p>تنبيه للحسابات والباقات والمواعيد والمدفوعات الجديدة.</p><div><button type="button" onClick={alerts.enable}><Volume2/> تجربة الصوت</button><button type="button" onClick={() => { onDeviceSettings?.(); window.dispatchEvent(new CustomEvent('mtPushSettings')); }}><BellRing/> إعدادات إشعارات الجهاز</button></div><small role="status">{alerts.message || 'على الموبايل، اسمح بالإشعارات وفعّل صوت التطبيق من إعدادات الجهاز.'}</small></div>}
