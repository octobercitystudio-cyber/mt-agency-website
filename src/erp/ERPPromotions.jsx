  ...
  return <div className="promotion-drawer-overlay" onMouseDown={event => event.target === event.currentTarget && onClose()}><aside ref={drawerRef} className="promotion-drawer" role="dialog" aria-modal="true" aria-labelledby="promotion-drawer-title">
    ...
    <form onSubmit={onSave}>
      ...
      <div className="promotion-form-column">
        ...
        <fieldset><legend><span>03</span> التوقيت</legend><div className="promotion-form-grid"><label>يبدأ في<BusinessDateTimeInput value={form.starts_at} onChange={event => update('starts_at', event.target.value)} required /></label><label>ينتهي في<BusinessDateTimeInput value={form.ends_at} onChange={event => update('ends_at', event.target.value)} required /></label></div><p className="promotion-field-note"><CalendarClock /> تُفسَّر كل المواعيد بتوقيت القاهرة وتظهر للزائر بصيغة 12 ساعة، والاختيار الافتراضي مساءً.</p></fieldset>
