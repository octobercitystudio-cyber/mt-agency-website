import { CalendarClock, PlayCircle, Ban, X } from 'lucide-react';
import { formatBookingDate, formatTime12 } from '../lib/businessFormat';
import useModalDialog from '../hooks/useModalDialog';
import './ERPBookingDetailsDialog.css';

export default function ERPBookingDetailsDialog({ booking, isAdmin, isOwner, busy, error, status, returnFocusRef, onClose, onStart, onCancel, onReschedule, ownerActions }) {
    ...
    <section ref={dialogRef} className="booking-details-dialog" role="dialog" aria-modal="true" aria-labelledby="booking-details-title">
      ...
      <div className="booking-details-body">
        ...
        <div className="booking-details-actions">
          ...
          {isAdmin && !isOwner && !['in_progress', 'completed', 'cancelled', 'منتهي'].includes(booking.status) && <button type="button" className="cancel" disabled={busy === `cancel-${booking.id}`} onClick={onCancel}><Ban />{busy === `cancel-${booking.id}` ? 'جارٍ الإلغاء…' : 'إلغاء الموعد'}</button>}
          {ownerActions}
        </div>
