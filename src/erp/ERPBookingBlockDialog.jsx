...
import BookingBlockConversionForm from './BookingBlockConversionForm';

const newKey = () => globalThis.crypto?.randomUUID?.() || `booking-block-${Date.now()}-${Math.random().toString(16).slice(2)}`;
const defaultDraft = date => ({ date, resource_id: '', start_time: '12:00', end_time: '13:00', title: '', note: '', repeat_daily: false, repeat_end_mode: 'never', repeat_until: '' });
...
export function ERPBookingBlockDialog({ isOpen, date, resources = [], returnFocusRef, onClose, onSuccess }) {
  ...
  const submit = async event => {
    ...
    if (draft.repeat_daily && draft.repeat_end_mode === 'date' && !draft.repeat_until) return setError('حدد تاريخ نهاية التكرار.');
    if (duration < 60 || duration % 15 !== 0) return setError('فترة الحظر يجب أن تكون ساعة على الأقل وبزيادات 15 دقيقة.');
    setBusy(true);
    const { data, error: requestError } = await dataClient.request('/booking-blocks', { method: 'POST', body: JSON.stringify({ ...draft, resource_id: Number(draft.resource_id), repeat_until: draft.repeat_daily && draft.repeat_end_mode === 'date' ? draft.repeat_until : null, idempotency_key: idempotencyKeyRef.current }) });
  ...
  return <div className="booking-block-overlay" onMouseDown={event => event.target === event.currentTarget && !busy && close()}>
    <form ref={dialogRef} className="booking-block-dialog" role="dialog" aria-modal="true" aria-labelledby="booking-block-title" aria-describedby="booking-block-description" onSubmit={submit}>
      <header><span className="booking-block-icon"><LockKeyhole/></span><div><small>إغلاق تشغيلي</small><h2 id="booking-block-title">حظر موعد</h2><p id="booking-block-description">تظهر الفترة للعميل كمشغولة فقط، دون إظهار العنوان أو الملاحظة.</p></div><button type="button" className="booking-block-close" onClick={close} disabled={busy} aria-label="إغلاق"><X/></button></header>
      <div className="booking-block-fields">
        ...
        <label>من الساعة<BusinessTimeSelect required min="00:00" max="23:00" step={15} defaultPeriod="pm" value={draft.start_time} onChange={event => update('start_time', event.target.value)}/></label>
        <label>إلى الساعة<BusinessTimeSelect required min="00:15" max="24:00" step={15} defaultPeriod="pm" value={draft.end_time} onChange={event => update('end_time', event.target.value)}/></label>
        <label className="booking-block-note">عنوان يظهر في التقويم — اختياري<input maxLength="120" value={draft.title} onChange={event => update('title', event.target.value)} placeholder="مثال: معاينة منتجات"/></label>
        <label className="booking-block-note">ملاحظة — اختياري<textarea rows="3" maxLength="1000" value={draft.note} onChange={event => update('note', event.target.value)} placeholder="تظهر معاينتها في بطاقة الموعد"/></label>
      </div>
      <section className="booking-block-repeat" aria-labelledby="booking-block-repeat-title"><div><CalendarRange/><span><strong id="booking-block-repeat-title">تكرار يومي</strong><small>يشمل جميع أيام الأسبوع، بما فيها يوم الجمعة.</small></span><label className="booking-block-switch"><input type="checkbox" checked={draft.repeat_daily} onChange={event => update('repeat_daily', event.target.checked)}/><i aria-hidden="true"/><b>{draft.repeat_daily ? 'نعم' : 'لا'}</b></label></div>{draft.repeat_daily && <div className="booking-block-repeat-until"><label>نهاية التكرار<select value={draft.repeat_end_mode} onChange={event => update('repeat_end_mode', event.target.value)}><option value="never">بدون تاريخ نهاية</option><option value="date">حتى تاريخ محدد</option></select></label>{draft.repeat_end_mode === 'date' && <label>حتى تاريخ<input required type="date" lang="ar" dir="rtl" min={draft.date} value={draft.repeat_until} onChange={event => update('repeat_until', event.target.value)}/></label>}<small>تتكرر نفس الفترة يوميًا. يمكنك إلغاء يوم واحد أو إيقاف التكرار من أي يوم.</small></div>}</section>
...
export function ERPBookingBlockDetailsDialog({ block, busy, error, clients = [], packages = [], services = [], returnFocusRef, onClose, onCancel, onConvert }) {
