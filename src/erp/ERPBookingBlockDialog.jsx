...
import { ArrowLeft, Ban, CalendarRange, Clock3, LockKeyhole, RefreshCw, Trash2, UserRound, X } from 'lucide-react';
...
const defaultDraft = date => ({ date, resource_id: '', start_time: '12:00', end_time: '13:00', title: '', note: '', repeat_daily: false, repeat_until: '' });
...
export function ERPBookingBlockDetailsDialog({ block, busy, error, clients = [], packages = [], returnFocusRef, onClose, onCancel, onConvert }) {
  const close = useCallback(() => onClose(), [onClose]); const dialogRef = useModalDialog(Boolean(block), close, { returnFocusRef }); const [clientId,setClientId]=useState(''); const [packageId,setPackageId]=useState(''); const eligible=packages.filter(pkg=>String(pkg.client_id)===String(clientId)&&pkg.status==='active'&&pkg.billing_unit==='hour'&&(!pkg.starts_at||String(block?.block_date||'')>=String(pkg.starts_at).slice(0,10))&&(!pkg.expires_at||String(block?.block_date||'')<=String(pkg.expires_at).slice(0,10))&&Number(pkg.purchased_minutes??Number(pkg.purchased_quantity||0)*60)-Number(pkg.consumed_minutes??Number(pkg.consumed_quantity||0)*60)-Number(pkg.held_minutes??Number(pkg.held_quantity||0)*60)>=Number(block?.duration_minutes||0)); if (!block) return null;
  return <div className="booking-block-overlay" onMouseDown={event => event.target === event.currentTarget && !busy && close()}><section ref={dialogRef} className="booking-block-dialog booking-block-details" role="dialog" aria-modal="true" aria-labelledby="booking-block-details-title"><header><span className="booking-block-icon"><LockKeyhole/></span><div><small>حجز مؤقت</small><h2 id="booking-block-details-title">{block.title || 'حجز مؤقت'}</h2><p>{block.resource_name || 'مورد الحجز'}</p></div><button type="button" className="booking-block-close" onClick={close} disabled={busy} aria-label="إغلاق"><X/></button></header><dl><div><dt>اليوم</dt><dd>{formatBookingDate(block.block_date)}</dd></div><div><dt>الوقت</dt><dd>{formatTime12(block.start_time)} — {formatTime12(block.end_time)}</dd></div><div><dt>المدة</dt><dd>{formatDurationMinutes(block.duration_minutes)}</dd></div>{block.note && <div className="wide"><dt>الملاحظة</dt><dd>{block.note}</dd></div>}</dl><section className="booking-block-convert"><h3><UserRound/>تحويل إلى حجز عميل</h3><p>سيتم حجز {formatDurationMinutes(block.duration_minutes)} من الباقة في العملية نفسها.</p><div><label>العميل<select value={clientId} onChange={e=>{setClientId(e.target.value);setPackageId('')}}><option value="">اختر العميل</option>{clients.filter(c=>c.status==='active').map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label>باقة الساعات<select value={packageId} onChange={e=>setPackageId(e.target.value)} disabled={!clientId}><option value="">{eligible.length?'اختر الباقة':'لا توجد باقة كافية'}</option>{eligible.map(pkg=><option key={pkg.id} value={pkg.id}>{pkg.name}</option>)}</select></label></div><button type="button" className="primary" disabled={busy||!packageId} onClick={()=>onConvert?.({client_id:Number(clientId),client_package_id:Number(packageId)})}><ArrowLeft/>تحويل وحجز المدة</button></section>{error && <div className="booking-block-error" role="alert"><Ban/><span>{error}</span></div>}<footer><button type="button" onClick={close} disabled={busy}>إغلاق</button><button type="button" className="danger" onClick={() => onCancel('single')} disabled={busy}><Trash2/>إلغاء هذا الحظر</button>{block.series_key && <button type="button" className="danger secondary" onClick={() => onCancel('series')} disabled={busy}><CalendarRange/>إلغاء هذا الحظر وما بعده</button>}</footer></section></div>;
}
