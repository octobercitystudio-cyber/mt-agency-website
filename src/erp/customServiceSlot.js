...
export function customSlotValidation(slot, bookings = []) {
  ...
  if (minutes(slot.start_time) < 0 || minutes(slot.end_time) > 1440 || duration < 60 || duration % 15 !== 0) return { status: 'invalid', available: false, message: 'الحجز متاح طوال اليوم، بحد أدنى ساعة وزيادات 15 دقيقة.' };
