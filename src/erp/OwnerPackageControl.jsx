import PackageLoyaltyControl from './PackageLoyaltyControl';
...
import { calculateDurationMinutes, formatBookingDate, formatEGP, formatPackageQuantity, formatTime12, moneyToCents } from '../lib/businessFormat';
...
export default function OwnerPackageControl({ pkg, person, resources, returnFocusRef, childOpen = false, refreshToken, onClose, onChanged, onDeleted, onNewBooking, onNewPayment }) {
  ...
  const [archive, setArchive] = useState({ mode: '', reason: '', confirmed: false, deleteConfirmation: '' });
  ...
  const purchasedChanged = unit === 'hour'
    ? Math.round(Number(balance.purchased || 0) * 60) !== Math.round(Number(quantities?.purchased || 0) * 60)
    : Number(balance.purchased || 0) !== Number(quantities?.purchased || 0);
  const consumedChanged = unit === 'hour'
    ? Math.round(Number(balance.consumed || 0) * 60) !== Math.round(Number(quantities?.used || 0) * 60)
    : Number(balance.consumed || 0) !== Number(quantities?.used || 0);
  const financeChanged = moneyToCents(finance.total) !== moneyToCents(financial?.total_price)
    || moneyToCents(finance.paid) !== moneyToCents(financial?.paid_amount);
  const detailsChanged = details.name.trim() !== String(info?.name || '').trim()
    ...
    || details.expires !== String(validity?.expires_at || '').slice(0, 10)
    ...
    || (unit === 'hour'
      ? Math.round(Number(details.paymentDue || 0) * 60) !== Math.round(Number(info?.payment_due_quantity || 0) * 60)
      : Number(details.paymentDue || 0) !== Number(info?.payment_due_quantity || 0))
    || Number(details.depositPercent || 0) !== Number(info?.deposit_percent_snapshot || 0)
    || moneyToCents(details.overagePrice) !== moneyToCents(info?.overage_price_snapshot || 0);
  ...
  const deletePackage = async () => {
    ...
    const { data: response, error: requestError } = await dataClient.request(`/client-packages/${pkg.id}`, { method: 'DELETE', body: JSON.stringify({ reason: archive.reason.trim(), confirmation: archive.deleteConfirmation.trim(), expected_version: info.version }) });
    setBusy('');
    if (requestError) return setError(safeUiError(requestError, 'تعذر حذف الباقة وسجلاتها.'));
    await onDeleted?.(response); onClose();
  };
  ...
  return <><div className="packages-modal owner-package-modal" aria-hidden={nestedOpen ? 'true' : undefined} onMouseDown={event => { if (event.target === event.currentTarget && !busy && !nestedOpen) onClose(); }}>
    <section ref={dialogRef} className="owner-package-center" role="dialog" aria-modal={nestedOpen ? undefined : 'true'} aria-hidden={nestedOpen ? 'true' : undefined} inert={nestedOpen ? true : undefined} aria-labelledby="owner-package-title" aria-describedby="owner-package-description">
      ...
      {loading ? <div className="owner-center-state"><RefreshCw className="packages-spin"/><strong>جارٍ تحميل عقد الباقة ومواعيدها…</strong></div> : data && <div className="owner-center-scroll">
        {tab === 'balance' && <section className="owner-balance-panel" role="tabpanel"><div className="owner-metric-strip"><Metric label="إجمالي المشترى" value={quantity(quantities.purchased)}/><Metric label="المستخدم" value={quantity(quantities.used)} tone="used"/><Metric label="محجوز بالمواعيد" value={quantity(quantities.upcoming_held)} tone="held"/><Metric label="متاح لحجز جديد" value={quantity(quantities.available)} tone="available"/><Metric label="المتبقي غير المستهلك" value={quantity(quantities.remaining)}/></div><div className="owner-editor-grid"><Editor title="تصحيح إجمالي الباقة" note={`لا يقل عن المستخدم + المحجوز: ${quantity(purchasedMinimum)}`}>{unit === 'hour' ? <DurationHoursMinutesInput idPrefix="owner-package-purchased" label="الإجمالي الجديد" value={balance.purchased} minMinutes={Math.round(purchasedMinimum * 60)} onChange={value => setBalance({ ...balance, purchased: value })}/> : <label>الإجمالي الجديد<input type="number" min={purchasedMinimum} step={step} value={balance.purchased} onChange={event => setBalance({ ...balance, purchased: event.target.value })}/></label>}<label>سبب تصحيح الإجمالي<textarea rows="3" minLength="5" value={balance.purchasedReason} onChange={event => setBalance({ ...balance, purchasedReason: event.target.value })}/></label><SaveButton busy={busy === 'purchased'} disabled={!purchasedChanged || balance.purchasedReason.trim().length < 5 || Number(balance.purchased) < purchasedMinimum} onClick={savePurchased}>حفظ إجمالي الباقة</SaveButton></Editor><Editor title="تصحيح المستخدم الفعلي" note={`الحد الأقصى بعد خصم المحجوز: ${quantity(consumedMaximum)}`}>{unit === 'hour' ? <DurationHoursMinutesInput idPrefix="owner-package-consumed" label="المستخدم المستهدف" value={balance.consumed} maxMinutes={Math.round(consumedMaximum * 60)} onChange={value => setBalance({ ...balance, consumed: value })}/> : <label>المستخدم المستهدف<input type="number" min="0" max={consumedMaximum} step={step} value={balance.consumed} onChange={event => setBalance({ ...balance, consumed: event.target.value })}/></label>}<label>سبب تصحيح الاستخدام<textarea rows="3" minLength="5" value={balance.consumedReason} onChange={event => setBalance({ ...balance, consumedReason: event.target.value })}/></label><SaveButton busy={busy === 'consumed'} disabled={!consumedChanged || balance.consumedReason.trim().length < 5 || Number(balance.consumed) < 0 || Number(balance.consumed) > consumedMaximum} onClick={saveConsumed}>حفظ المستخدم الموثق</SaveButton></Editor></div><div className="owner-readonly-explain"><Clock3/><div><strong>المحجوز والمتاح لا يُعدّلان يدويًا</strong><p>المحجوز مشتق من المواعيد المؤكدة. استخدم قسم مواعيد الباقة لإضافة أو تعديل أو إلغاء موعد، وسيُعاد حساب المتاح تلقائيًا.</p><button type="button" onClick={() => setTab('bookings')}>إدارة المواعيد</button></div></div></section>}
        {tab === 'finance' && <section role="tabpanel" className="owner-finance-panel"><div className="owner-metric-strip finance"><Metric label="إجمالي السعر" value={formatEGP(finance.total)}/><Metric label="المدفوع" value={formatEGP(finance.paid)} tone="available"/><Metric label="المتبقي" value={formatEGP(financePreview.outstanding)} tone={financePreview.outstanding ? 'held' : 'available'}/><Metric label="رصيد دائن" value={formatEGP(financePreview.credit)}/></div>{Number(financial.outstanding)>0&&<button type="button" className="owner-record-payment" onClick={onNewPayment}><Banknote/> تسجيل دفعة فعلية في مدفوعات العميل <small>{formatEGP(financial.outstanding)} متبقي</small></button>}<PackageLoyaltyControl packageId={pkg.id} onChanged={onChanged}/><Editor title="تصحيح القيمة التجارية" note="استخدم تسجيل دفعة للتحصيل الحقيقي. هذه الأداة مخصصة لتصحيح سعر أو رقم مسجل خطأ مع سبب مراجعة."><div className="owner-fields-grid"><label>إجمالي سعر الباقة<input type="number" min="0" step="0.01" value={finance.total} onChange={event => setFinance({ ...finance, total: event.target.value })}/></label><label>المدفوع المستهدف<input type="number" min="0" step="0.01" value={finance.paid} onChange={event => setFinance({ ...finance, paid: event.target.value })}/></label><label>طريقة التحصيل<select value={finance.method} onChange={event => setFinance({ ...finance, method: event.target.value })}><option value="">اختر عند زيادة المدفوع</option>{Object.entries(PAYMENT_METHODS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div><label>سبب التصحيح المالي<textarea rows="3" minLength="5" value={finance.reason} onChange={event => setFinance({ ...finance, reason: event.target.value })}/></label><SaveButton busy={busy === 'finance'} disabled={!financeChanged || finance.reason.trim().length < 5 || Number(finance.total) < 0 || Number(finance.paid) < 0 || (Number(finance.paid) > Number(financial.paid_amount) && !finance.method)} onClick={saveFinance}>حفظ السعر والمدفوع</SaveButton></Editor></section>}
        {tab === 'details' && <section role="tabpanel" className="owner-details-panel">
          <Editor title="الصلاحية وكل بيانات العقد" note={pendingValidity ? 'اترك التاريخين فارغين ليبدأ العقد تلقائيًا مع أول حجز، أو حددهما يدويًا بصلاحية المالك.' : details.validityMode === 'shooting_day' ? 'هذه باقة يوم تصوير؛ البداية والانتهاء في اليوم نفسه.' : 'لن تُقبل فترة تستبعد موعدًا نشطًا؛ عدّل المواعيد أولًا.'}>
            <div className="owner-fields-grid">
              ...
              {unit === 'hour' ? <DurationHoursMinutesInput idPrefix="owner-package-payment-due" label="حد السداد من الرصيد" value={details.paymentDue} maxMinutes={Math.round(Number(quantities.purchased || 0) * 60)} onChange={value => setDetails({ ...details, paymentDue: value })}/> : <label>حد السداد من الرصيد<input type="number" min="0" max={quantities.purchased} step={step} value={details.paymentDue} onChange={event => setDetails({ ...details, paymentDue: event.target.value })}/></label>}
            ...
            <label>سبب تعديل العقد أو الصلاحية <small>(اختياري — سيُسجل التعديل تلقائيًا)</small><textarea rows="3" value={details.reason} onChange={event => setDetails({ ...details, reason: event.target.value })}/></label>
            <SaveButton busy={busy === 'details'} disabled={!detailsChanged || !details.name.trim() || !details.serviceId || !Number.isInteger(Number(details.validityDays)) || Number(details.validityDays) < 1 || Number(details.validityDays) > 3650 || Number(details.paymentDue) < 0 || Number(details.paymentDue) > Number(quantities.purchased) || Number(details.depositPercent) < 0 || Number(details.depositPercent) > 100 || Number(details.overagePrice) < 0 || (!pendingValidity && (!details.starts || !details.expires || details.expires < details.starts))} onClick={saveDetails}>حفظ كل بيانات العقد</SaveButton>
          </Editor>
          <section className="owner-archive-zone">
            <div className="owner-package-removal-actions">
              <button type="button" onClick={() => setArchive(current => ({ ...current, mode: current.mode === 'archive' ? '' : 'archive', reason: '', confirmed: false, deleteConfirmation: '' }))}><Archive/> أرشفة الباقة</button>
              <button type="button" className="owner-package-delete-entry" onClick={() => setArchive(current => ({ ...current, mode: current.mode === 'delete' ? '' : 'delete', reason: '', confirmed: false, deleteConfirmation: '' }))}><Trash2/> حذف الباقة نهائيًا</button>
            </div>
            {archive.mode === 'archive' && <div><p>الأرشفة توقف العمل بالباقة وتخرجها من المستحقات النشطة، مع حفظ المواعيد والدفعات والسجل.</p><label>سبب الأرشفة<textarea rows="2" minLength="5" value={archive.reason} onChange={event => setArchive({ ...archive, reason: event.target.value })}/></label><label className="owner-confirm-row"><input type="checkbox" checked={archive.confirmed} onChange={event => setArchive({ ...archive, confirmed: event.target.checked })}/> أفهم أن الباقة ستتوقف مع حفظ تاريخها</label><SaveButton danger busy={busy === 'archive'} disabled={!archive.confirmed || archive.reason.trim().length < 5} onClick={archivePackage}>تأكيد أرشفة الباقة</SaveButton></div>}
            {archive.mode === 'delete' && <div className="owner-package-delete-confirm"><p><strong>حذف نهائي لا يمكن التراجع عنه.</strong> سيُحذف عقد الباقة ومواعيدها وحركات الرصيد وتخصيصات الدفع والدفعات والقيود المالية المرتبطة بها وحدها.</p><label>سبب الحذف<textarea rows="2" minLength="5" value={archive.reason} onChange={event => setArchive({ ...archive, reason: event.target.value })}/></label><label>للتأكيد اكتب كلمة «حذف»<input autoComplete="off" value={archive.deleteConfirmation} onChange={event => setArchive({ ...archive, deleteConfirmation: event.target.value })}/></label><SaveButton danger busy={busy === 'delete'} disabled={archive.reason.trim().length < 5 || archive.deleteConfirmation.trim() !== 'حذف'} onClick={deletePackage}>حذف الباقة وكل سجلاتها</SaveButton></div>}
          </section>
        </section>}
...
function BookingGroup({ title, items, unit, drafts, setDrafts, resources, busy, onSave, onDelete }) { return <section className="owner-booking-group"><h4>{title} <small>{items.length}</small></h4><div>{items.map(booking => { const draft = drafts[booking.id] || {}; const changed = draft.date !== String(booking.date || '').slice(0, 10) || draft.start_time !== String(booking.start_time || '').slice(0, 5) || draft.end_time !== String(booking.end_time || '').slice(0, 5) || Number(draft.resource_id || 0) !== Number(booking.resource_id || 0) || String(draft.notes || '') !== String(booking.notes || ''); const valid = Boolean(draft.date && draft.start_time && draft.end_time && draft.resource_id && calculateDurationMinutes(draft.start_time, draft.end_time) > 0); return <article key={booking.id} className={`owner-booking-card status-${booking.status}`}><header><div><strong>{formatBookingDate(booking.date)}</strong><span>{formatTime12(booking.start_time)} — {formatTime12(booking.end_time)}</span></div><b>{STATUS_LABELS[booking.status] || booking.status}</b></header><dl><div><dt>المورد</dt><dd>{booking.resource_name || 'غير محدد'}</dd></div><div><dt>المدة / الكمية</dt><dd>{formatPackageQuantity(booking.requested_quantity || booking.balance_effect, unit)}</dd></div><div><dt>أثره على المحجوز</dt><dd>{formatPackageQuantity(booking.balance_effect, unit)}</dd></div></dl>{booking.notes && <p className="booking-current-note">{booking.notes}</p>}{booking.can_reschedule && <div className="owner-booking-edit"><div className="owner-fields-grid"><label>التاريخ<input type="date" value={draft.date || ''} onChange={event => setDrafts({ ...drafts, [booking.id]: { ...draft, date: event.target.value } })}/></label><label>من<BusinessTimeSelect min="00:00" max="23:45" step={15} required value={draft.start_time || ''} onChange={event => setDrafts({ ...drafts, [booking.id]: { ...draft, start_time: event.target.value } })}/></label><label>إلى<BusinessTimeSelect min="00:15" max="24:00" step={15} required value={draft.end_time || ''} onChange={event => setDrafts({ ...drafts, [booking.id]: { ...draft, end_time: event.target.value } })}/></label><label>المورد<select value={draft.resource_id || ''} onChange={event => setDrafts({ ...drafts, [booking.id]: { ...draft, resource_id: event.target.value } })}>{resources.map(resource => <option key={resource.id} value={resource.id}>{resource.name}</option>)}</select></label></div><label>ملاحظات الموعد<textarea rows="2" value={draft.notes || ''} onChange={event => setDrafts({ ...drafts, [booking.id]: { ...draft, notes: event.target.value } })}/></label><SaveButton busy={busy === `booking-${booking.id}`} disabled={!changed || !valid} onClick={() => onSave(booking)}>حفظ تعديل الموعد</SaveButton></div>}{booking.can_cancel && <div className="owner-cancel-booking"><p>الحذف يزيل الموعد من الحجوزات ويعيد الرصيد المحجوز للعميل دون طلب سبب.</p><SaveButton danger busy={busy === `delete-${booking.id}`} onClick={() => onDelete(booking)}>حذف الموعد</SaveButton></div>}{booking.immutable_reason && <p className="owner-immutable"><ShieldAlert/>{booking.immutable_reason}</p>}</article>; })}</div></section>; }
