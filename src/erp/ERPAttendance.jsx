...
const previewDetails = userId => {
  ...
  return {
    ...
    adjustments: [{ id: Number(userId) * 10 + 1, adjustment_type: 'deduction', amount: 100, minutes: 0, reason: 'تسوية إدارية تجريبية', created_at: `${day(-2)} 18:00:00` }],
  };
};
...
const ERPAttendance = () => {
  ...
  return (
    <main className="attendance-page">
      ...
      {selected && <div className="attendance-overlay" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setSelected(null)}><aside className="attendance-drawer" role="dialog" aria-modal="true" aria-label={`تفاصيل حضور ${selected.full_name}`}><header><div><span>{month}</span><h2>{selected.full_name}</h2><p>{roleLabels[selected.role]} · خصم مقدر {money(selected.total_deduction)}</p></div><button onClick={() => setSelected(null)} aria-label="إغلاق"><X size={20} /></button></header><div className="drawer-breakdown"><div><span>تأخير</span><strong>{selected.late_minutes} د</strong><small>{money(selected.late_deduction)}</small></div><div><span>غياب</span><strong>{selected.absent_days} يوم</strong><small>{money(selected.absence_deduction)}</small></div><div><span>انصراف مبكر</span><strong>{selected.early_leave_minutes} د</strong><small>{money(selected.early_leave_deduction)}</small></div><div><span>تسويات</span><strong>{money(selected.manual_adjustment)}</strong><small>بسبب مسجل</small></div></div><section className="drawer-section"><h3>السجل اليومي</h3>{details.loading ? <p className="drawer-note">جارٍ التحميل…</p> : details.error ? <p className="drawer-note drawer-note--error">{details.error}</p> : details.records.length === 0 ? <p className="drawer-note">لا توجد أيام مسجلة في هذا الشهر.</p> : <div className="daily-records">{details.records.map((record) => <div key={record.id}><time>{record.work_date}</time><span className={`status status--${record.status}`}>{statusLabels[record.status] || record.status}{record.correction_reason ? ' · معدل يدويًا' : ''}</span><bdi>{record.check_in_at ? formatTime12(record.check_in_at) : '—'} → {record.check_out_at ? formatTime12(record.check_out_at) : 'سجل مفتوح'}</bdi>{isOwner && <button onClick={() => setCorrection({ ...record, check_in_at: record.check_in_at?.slice(0, 16) || '', check_out_at: record.check_out_at?.slice(0, 16) || '', correction_reason: '' })}><Edit3 size={14} /> تعديل</button>}</div>)}</div>}</section>{isOwner && !isPreview && <section className="drawer-section"><h3>إضافة تسوية مالية</h3><form className="adjustment-form" onSubmit={saveAdjustment}><label>المبلغ (+ خصم / − تخفيض)<input type="number" step="0.01" required value={adjustment.amount} onChange={(event) => setAdjustment((old) => ({ ...old, amount: event.target.value }))} /></label><label>السبب<input required minLength="5" value={adjustment.reason} onChange={(event) => setAdjustment((old) => ({ ...old, reason: event.target.value }))} placeholder="مثال: إذن معتمد من الإدارة" /></label><button disabled={saving}>حفظ التسوية</button></form></section>}</aside></div>}
