...
import { filterFormationFundEntries } from '../lib/formationFundLedger';
...
const ledgerCopy = {
  contribution: {
    eyebrow: 'دفتر المبالغ الواردة', title: 'المبالغ الواردة', description: 'مساهمات المؤسسين', totalLabel: 'إجمالي المبالغ الواردة', addLabel: 'إضافة مبلغ وارد',
    emptyTitle: 'لا توجد مبالغ واردة تطابق الفلاتر.', emptyHint: 'غيّر حالة القيد أو المؤسس، أو أضف مبلغًا واردًا جديدًا.',
  },
  expense: {
    eyebrow: 'دفتر المصروفات', title: 'مصروفات التأسيس', description: 'المبالغ المصروفة على تجهيز الشركة', totalLabel: 'إجمالي مصروفات التأسيس', addLabel: 'تسجيل مصروف',
    emptyTitle: 'لا توجد مصروفات تأسيس تطابق الفلاتر.', emptyHint: 'غيّر حالة القيد أو المؤسس، أو سجّل مصروف تأسيس جديدًا.',
  },
};

const FormationLedger = ({ entries, founders, currentUser, activeType, statusFilter, founderFilter, total, onTypeChange, setStatusFilter, setFounderFilter, onAdd, onDetails, onEdit, onVoid }) => {
  const copy = ledgerCopy[activeType];
  const handleTabKeyDown = event => {
    let nextType = null;
    if (event.key === 'Home') nextType = 'contribution';
    if (event.key === 'End') nextType = 'expense';
    if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') nextType = activeType === 'contribution' ? 'expense' : 'contribution';
    if (!nextType) return;
    event.preventDefault();
    onTypeChange(nextType);
    window.requestAnimationFrame(() => document.getElementById(`formation-tab-${nextType}`)?.focus());
  };

  return <section className={`formation-ledger formation-ledger--${activeType}`}>
    <div className="formation-ledger-tabs" role="tablist" aria-label="نوع سجل صندوق التأسيس">
      <button id="formation-tab-contribution" type="button" role="tab" aria-selected={activeType === 'contribution'} aria-controls="formation-ledger-panel" tabIndex={activeType === 'contribution' ? 0 : -1} className={activeType === 'contribution' ? 'is-active' : ''} onClick={() => onTypeChange('contribution')} onKeyDown={handleTabKeyDown}>
        <BanknoteArrowUp /><span><strong>المبالغ الواردة</strong><small>مساهمات المؤسسين</small></span>
      </button>
      <button id="formation-tab-expense" type="button" role="tab" aria-selected={activeType === 'expense'} aria-controls="formation-ledger-panel" tabIndex={activeType === 'expense' ? 0 : -1} className={activeType === 'expense' ? 'is-active' : ''} onClick={() => onTypeChange('expense')} onKeyDown={handleTabKeyDown}>
        <BanknoteArrowDown /><span><strong>مصروفات التأسيس</strong><small>تجهيزات ومصروفات الشركة</small></span>
      </button>
    </div>
    <div id="formation-ledger-panel" role="tabpanel" aria-labelledby={`formation-tab-${activeType}`} className="formation-ledger-panel">
      <div className="formation-ledger-header">
        <div className="formation-ledger-intro"><span>{copy.eyebrow}</span><h2>{copy.title}</h2><p>{copy.description}</p></div>
        <div className="formation-ledger-total"><span>{copy.totalLabel}</span><strong>{formatMoney(total)}</strong><small>{entries.length} {entries.length === 1 ? 'قيد معروض' : 'قيود معروضة'} بعد الفلاتر</small></div>
        <button type="button" className={`formation-ledger-add formation-ledger-add--${activeType}`} onClick={onAdd}><Plus /> {copy.addLabel}</button>
      </div>
      <div className="formation-filters" aria-label="فلاتر السجل"><label>حالة القيد<select value={statusFilter} onChange={event => setStatusFilter(event.target.value)}><option value="all">كل الحالات</option><option value="active">نشط</option><option value="voided">مُبطل</option></select></label><label>المؤسس<select value={founderFilter} onChange={event => setFounderFilter(event.target.value)}><option value="all">كل المؤسسين</option>{founders.map(founder => <option key={founder.id} value={founder.id}>{founder.name_ar}</option>)}</select></label></div>
      {entries.length ? <><div className="formation-table-wrap"><table><thead><tr><th>التاريخ</th><th>الحركة</th><th>البيان</th><th>الحساب / التوزيع</th><th>القيمة</th><th>الحالة</th><th>بواسطة</th><th>إجراءات</th></tr></thead><tbody>{entries.map(entry => <tr key={entry.id} className={entry.status === 'voided' ? 'is-voided' : ''}><td>{entry.entry_date}</td><td><span className={`formation-type formation-type--${entry.entry_type}`}>{entry.entry_type === 'contribution' ? 'مساهمة' : 'مصروف'}</span></td><td><strong>{entry.title}</strong><small>{categoryLabels[entry.category] || entry.reference || '—'}</small></td><td>{entry.entry_type === 'contribution' ? entry.founder_name : <button className="formation-link-button" onClick={() => onDetails(entry)}><Eye /> عرض توزيع {entry.allocations.length} حسابات</button>}</td><td className={entry.entry_type === 'expense' ? 'amount-expense' : 'amount-contribution'}>{entry.entry_type === 'expense' ? '−' : '+'}{formatMoney(entry.amount)}</td><td><span className={`formation-status formation-status--${entry.status}`}>{entry.status === 'active' ? 'نشط' : 'مُبطل'}</span>{entry.status === 'voided' && <small>{entry.void_reason}</small>}</td><td>{entry.creator_name}</td><td><div className="formation-ledger-card-actions">{entry.status === 'active' && currentUser?.role === 'owner' && <button className="formation-link-button" onClick={() => onEdit(entry)}>تعديل</button>}{entry.status === 'active' && <button className="formation-void-button" onClick={() => onVoid(entry)}>إبطال القيد</button>}</div></td></tr>)}</tbody></table></div>
        <div className="formation-ledger-cards">{entries.map(entry => <article key={entry.id} className={entry.status === 'voided' ? 'is-voided' : ''}><header><div><span className={`formation-type formation-type--${entry.entry_type}`}>{entry.entry_type === 'contribution' ? 'مساهمة' : 'مصروف'}</span><time>{entry.entry_date}</time></div><span className={`formation-status formation-status--${entry.status}`}>{entry.status === 'active' ? 'نشط' : 'مُبطل'}</span></header><div className="formation-ledger-card-main"><div><h3>{entry.title}</h3><small>{entry.entry_type === 'contribution' ? entry.founder_name : categoryLabels[entry.category] || 'مصروف تأسيس'}</small></div><strong className={entry.entry_type === 'expense' ? 'amount-expense' : 'amount-contribution'}>{entry.entry_type === 'expense' ? '−' : '+'}{formatMoney(entry.amount)}</strong></div><div className="formation-ledger-card-actions">{entry.entry_type === 'expense' && <button className="formation-link-button" onClick={() => onDetails(entry)}><Eye /> التوزيع الدقيق</button>}{entry.status === 'active' && currentUser?.role === 'owner' && <button className="formation-link-button" onClick={() => onEdit(entry)}>تعديل بقيد تصحيح</button>}{entry.status === 'active' && <button className="formation-void-button" onClick={() => onVoid(entry)}>إبطال القيد</button>}</div><details><summary>تفاصيل القيد والتدقيق</summary><dl><div><dt>بواسطة</dt><dd>{entry.creator_name}</dd></div><div><dt>المرجع</dt><dd>{entry.reference || '—'}</dd></div>{entry.status === 'voided' && <div><dt>سبب الإبطال</dt><dd>{entry.void_reason}</dd></div>}</dl></details></article>)}</div></> : <div className="formation-empty"><ReceiptText /><strong>{copy.emptyTitle}</strong><span>{copy.emptyHint}</span></div>}
    </div>
  </section>;
};
...
const ERPFormationFund = () => {
  ...
  const filteredEntries = useMemo(() => filterFormationFundEntries(data?.entries || [], { entryType: activeType, status: statusFilter, founderId: founderFilter }), [data, activeType, statusFilter, founderFilter]);
  const activeTypeTotal = activeType === 'contribution' ? data?.summary.total_contributions : data?.summary.total_expenses;
  ...
  return <main className="formation-page">
    <ERPPageHero icon={Landmark} eyebrow="رأس مال التأسيس · مستقل عن حسابات التشغيل" title="صندوق التأسيس" description="ثلاثة حسابات مؤسسين دقيقة، ورصيد مجمع واحد لتمويل تأسيس الشركة دون خلطه بالخزينة التشغيلية." />
    ...
    <FormationLedger entries={filteredEntries} founders={data?.founders || []} currentUser={currentUser} activeType={activeType} statusFilter={statusFilter} founderFilter={founderFilter} total={activeTypeTotal} onTypeChange={setActiveType} setStatusFilter={setStatusFilter} setFounderFilter={setFounderFilter} onAdd={() => { setSelectedFounder(null); setModal(activeType); }} onDetails={setDetails} onEdit={setEditTarget} onVoid={setVoidTarget} />
