...
const ERPFormationFund = () => {
  ...
  return <main className="formation-page">
    ...
    <section className="formation-ledger"><div className="formation-ledger-header"><div className="formation-section-heading compact"><div><span>السجل الدائم</span><h2>حركات صندوق التأسيس</h2></div></div><div className="formation-filters"><label>نوع الحركة<select value={typeFilter} onChange={e => setTypeFilter(e.target.value)}><option value="all">كل الحركات</option><option value="contribution">المساهمات</option><option value="expense">المصروفات</option><option value="voided">الملغاة</option></select></label><label>المؤسس<select value={founderFilter} onChange={e => setFounderFilter(e.target.value)}><option value="all">كل المؤسسين</option>{data?.founders.map(founder => <option key={founder.id} value={founder.id}>{founder.name_ar}</option>)}</select></label></div></div>
      {filteredEntries.length ? <><div className="formation-table-wrap"><table><thead><tr><th>التاريخ</th><th>الحركة</th><th>البيان</th><th>الحساب / التوزيع</th><th>القيمة</th><th>الحالة</th><th>بواسطة</th><th><span className="sr-only">إجراءات</span></th></tr></thead><tbody>{filteredEntries.map(entry => <tr key={entry.id} className={entry.status === 'voided' ? 'is-voided' : ''}><td>{entry.entry_date}</td><td><span className={`formation-type formation-type--${entry.entry_type}`}>{entry.entry_type === 'contribution' ? 'مساهمة' : 'مصروف'}</span></td><td><strong>{entry.title}</strong><small>{categoryLabels[entry.category] || entry.reference || '—'}</small></td><td>{entry.entry_type === 'contribution' ? entry.founder_name : <button className="formation-link-button" onClick={() => setDetails(entry)}><Eye /> عرض توزيع {entry.allocations.length} حسابات</button>}</td><td className={entry.entry_type === 'expense' ? 'amount-expense' : 'amount-contribution'}>{entry.entry_type === 'expense' ? '−' : '+'}{formatMoney(entry.amount)}</td><td><span className={`formation-status formation-status--${entry.status}`}>{entry.status === 'active' ? 'نشط' : 'مُبطل'}</span>{entry.status === 'voided' && <small>{entry.void_reason}</small>}</td><td>{entry.creator_name}</td><td>{entry.status === 'active' && <button className="formation-void-button" onClick={() => setVoidTarget(entry)}>إبطال القيد</button>}</td></tr>)}</tbody></table></div><div className="formation-ledger-cards">{filteredEntries.map(entry => <article key={entry.id} className={entry.status === 'voided' ? 'is-voided' : ''}><header><div><span className={`formation-type formation-type--${entry.entry_type}`}>{entry.entry_type === 'contribution' ? 'مساهمة' : 'مصروف'}</span><time>{entry.entry_date}</time></div><span className={`formation-status formation-status--${entry.status}`}>{entry.status === 'active' ? 'نشط' : 'مُبطل'}</span></header><div className="formation-ledger-card-main"><div><h3>{entry.title}</h3><small>{entry.entry_type === 'contribution' ? entry.founder_name : categoryLabels[entry.category] || 'مصروف تأسيس'}</small></div><strong className={entry.entry_type === 'expense' ? 'amount-expense' : 'amount-contribution'}>{entry.entry_type === 'expense' ? '−' : '+'}{formatMoney(entry.amount)}</strong></div><div className="formation-ledger-card-actions">{entry.entry_type === 'expense' && <button className="formation-link-button" onClick={() => setDetails(entry)}><Eye /> التوزيع الدقيق</button>}{entry.status === 'active' && <button className="formation-void-button" onClick={() => setVoidTarget(entry)}>إبطال القيد</button>}</div><details><summary>تفاصيل القيد والتدقيق</summary><dl><div><dt>بواسطة</dt><dd>{entry.creator_name}</dd></div><div><dt>المرجع</dt><dd>{entry.reference || '—'}</dd></div>{entry.status === 'voided' && <div><dt>سبب الإبطال</dt><dd>{entry.void_reason}</dd></div>}</dl></details></article>)}</div></> : <div className="formation-empty"><ReceiptText /><strong>لا توجد حركات تطابق الفلاتر.</strong><span>غيّر نوع الحركة أو المؤسس لعرض السجل.</span></div>}
    </section>
