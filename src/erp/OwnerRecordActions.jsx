...
export default function OwnerRecordActions({ user, entity, record, label, onEdit, onChanged, actionLabel = 'حذف / أرشفة', compact = false, className = '' }) {
  ...
  return <>
    <div className={`owner-record-actions ${compact ? 'compact' : ''} ${className}`} aria-label={`تحكم المالك في ${label}`}>
      ...
      <button ref={triggerRef} type="button" className="owner-impact" onClick={() => setOpen(true)} aria-label={`${actionLabel}: ${label}`}>{compact ? <MoreVertical aria-hidden="true" /> : <><ShieldAlert /> {actionLabel}</>}</button>
    </div>
