...
import { Check, Clapperboard, CloudUpload, Film, History, MapPin, Plus, RefreshCw, Save, Search, Trash2, X } from 'lucide-react';
...
const emptyLink = () => ({ title: '', link_kind: 'folder', url: '', is_active: 1 });
...
export default function ERPPostProduction() {
  ...
  const updateStatus = async () => {
    if (!selected || !pendingStatus) return; setBusy('status'); setDialogError(''); const { error: requestError } = await dataClient.request(`/post-production/${selected.id}/status`, { method: 'PATCH', body: JSON.stringify({ status: pendingStatus, expected_version: Number(selected.version) }) }); setBusy('');
  ...
  const correctStatus = async event => {
    event.preventDefault(); if (!selected || currentUser?.role !== 'owner') return; setBusy('status-correction'); setDialogError(''); const target = statusCorrection.status; const { error: requestError } = await dataClient.request(`/owner/post-production/${selected.id}/status-correction`, { method: 'POST', body: JSON.stringify({ status: target, reason: statusCorrection.reason, expected_version: Number(selected.version) }) }); setBusy(''); if (requestError) { setDialogError(requestError.message || 'تعذر تصحيح حالة المونتاج.'); if (requestError.status === 409) await loadJobs(true); return; } setStatusCorrection({ status: target, reason: '' }); announce(`صحح المالك الحالة إلى: ${postProductionMeta(target).label}`); await loadJobs(true);
  };
  ...
  return <main className="post-production-center" dir="rtl">
    ...
    <CompanyPickupScheduleEditor syncEnabled={!currentUser?.is_local_preview} />
