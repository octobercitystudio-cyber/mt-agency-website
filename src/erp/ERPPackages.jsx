...
import { cairoDateKey, centsToMoney, effectivePackageStatus, formatBookingDate, formatDateTime12, formatEGP, formatPackageQuantity, formatTime12, packageFinancialSummary, packageQuantitySummary, remainingCalendarDays } from '../lib/businessFormat';
...
function PackageDetailsDialog({dialogRef,details,onClose,onRetry,onTab}){
  ...
  return <div className="packages-modal packages-details-modal" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><section ref={dialogRef} className="package-statement" role="dialog" aria-modal="true" aria-labelledby="package-details-title" aria-describedby="package-details-description"><header className="package-statement-header"><div><span><PackageCheck/> كشف الباقة التشغيلي</span><h2 id="package-details-title">{packageInfo?.name||details.pkg?.name||'تفاصيل الباقة'}</h2><p id="package-details-description">{packageInfo?`${packageInfo.client.name} · الباقة #${packageInfo.id}${packageInfo.invoice_number?` · ${packageInfo.invoice_number}`:''}`:'نجمع المدفوعات والجلسات والأرصدة في كشف واحد.'}</p></div>{packageInfo&&<StatusBadge status={packageInfo.effective_status}/>}<button data-dialog-initial type="button" className="package-statement-close" onClick={onClose} aria-label="إغلاق تفاصيل الباقة"><X/></button></header>
    {details.loading?<div className="package-statement-state"><RefreshCw className="packages-spin"/><strong>جارٍ إعداد كشف الباقة</strong><p>نراجع التخصيصات والجلسات الفعلية والأرصدة.</p></div>:details.error?<div className="package-statement-state error"><ShieldAlert/><strong>تعذر تحميل التفاصيل</strong><p>{details.error}</p><button onClick={onRetry}>إعادة المحاولة</button></div>:data&&<div className="package-statement-scroll">
      <section className="package-health-strip" aria-label="مؤشرات صحة الباقة"><HealthItem label="تحصيل الباقة" value={`${financial.payment_progress_percent}%`} note={`${money(financial.paid_amount)} مدفوع`} progress={financial.payment_progress_percent} tone="payment"/><HealthItem label="استهلاك الرصيد" value={formatPackageQuantity(quantities.used,packageInfo.billing_unit)} note={`من ${formatPackageQuantity(quantities.purchased,packageInfo.billing_unit)}`} progress={quantities.purchased?Math.min(100,quantities.used/quantities.purchased*100):0} tone="usage"/><HealthItem label="الأيام التقويمية المتبقية" value={(validity.remaining_calendar_days??0).toLocaleString('ar-EG-u-nu-latn')} note="الجمعة محسوبة" progress={validity.state==='expired'?100:validity.state==='near_expiry'?72:35} tone={validity.state}/></section>
      ...
      <section className={`package-statement-validity ${validity.state}`}><header><CalendarClock/><div><span>فترة التعاقد</span><h3>صلاحية الباقة</h3></div></header><dl><div><dt>تاريخ البداية</dt><dd>{validity.starts_at?formatBookingDate(validity.starts_at):'عند أول حجز تصوير'}</dd></div><div><dt>تاريخ الانتهاء</dt><dd>{validity.expires_at?formatBookingDate(validity.expires_at):'يُحسب تلقائيًا'}</dd></div><div><dt>الأيام التقويمية المتبقية</dt><dd>{(validity.remaining_calendar_days??0).toLocaleString('ar-EG-u-nu-latn')} يوم</dd></div></dl><p>يوم أول حجز هو اليوم رقم 1، وتُحسب كل الأيام التالية بما فيها الجمعة.</p></section></div>
