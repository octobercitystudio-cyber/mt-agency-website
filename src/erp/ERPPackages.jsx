...
import { appointmentDurationMinutes, defaultPackageSaleResourceId, normalizePackageSaleAppointments, packageAppointmentUsage, packageCalendarWeek, partitionPackageAppointments, shiftPackageCalendarDate, validatePackageAppointment } from '../lib/packageSaleAppointments';
...
import { organizeClientPackageGroups } from './packageContinuity';
...
export default function ERPPackages() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (failed?.error) setError(safeUiError(failed.error, 'تعذر تحميل الباقات المباعة الآن.'));
      ...
      setAppointment(current => current.resource_id ? current : { ...current, resource_id: defaultPackageSaleResourceId(resourcesResult.data || []) });
  ...
  const openAddDialog = (event, clientId = '') => {
    ...
    setForm({ ...initialForm, client_id: clientId ? String(clientId) : '' }); setFormErrors({}); setSaleBookings([]); setAppointment(initialAppointment(defaultPackageSaleResourceId(resources))); setAppointmentErrors({}); setEditingAppointment(-1); setTemplateResetNotice(''); setClientModalOpen(false); setError('');
    setFormOpen(true);
  };
  ...
  const removeAppointment = index => { setSaleBookings(current => current.filter((_, itemIndex) => itemIndex !== index)); if (editingAppointment === index) { setEditingAppointment(-1); setAppointment(initialAppointment(defaultPackageSaleResourceId(resources))); } else if (editingAppointment > index) { setEditingAppointment(current => current - 1); } };
  ...
  return <div className="sold-packages sold-packages-workbench" dir="rtl">
    <ERPPageHero icon={WalletCards} eyebrow="إدارة المبيعات والرصيد" title="الباقات المباعة" description="باقات كل عميل ومواعيده وحساباته في مكان واحد." actions={canAssign && <button data-variant="primary" onClick={openAddDialog}><PackagePlus/> إضافة باقة لعميل</button>}/>
    <section className="packages-summary" aria-label="ملخص الباقات النشطة"><Metric icon={CheckCircle2} label="الباقات النشطة" value={activePackages.length}/><Metric icon={CalendarClock} label="تنتهي خلال 14 يومًا تقويميًا" value={expiring.length} warning/><Metric icon={Clock3} label="متاح لحجز جديد" value={`${formatDurationMinutes(remainingHours * 60, { compact: true })} / ${remainingReels.toLocaleString('ar-EG')} ريل`}/><Metric icon={CircleDollarSign} label="مستحق على الباقات النشطة" value={money(centsToMoney(outstandingCents))} danger/></section>
    ...
    <form ref={dialogRef} className="packages-dialog large packages-sale-dialog packages-quick-dialog" role="dialog" aria-modal="true" aria-labelledby="add-package-title" aria-describedby="add-package-description" aria-hidden={childOpen ? 'true' : undefined} inert={childOpen ? true : undefined} onSubmit={event => { submitAttemptRef.current = true; onSubmit(event); }} noValidate>
      ...
      <details ref={appointmentsRef} className="packages-progressive-section packages-progressive-appointments">
        ...
        <div className="packages-inline-calendar" aria-label="تقويم توافر المواعيد">
          <header>
            <div><strong>اختر يومًا من التقويم</strong><small>راجع المواعيد المحجوزة ثم اختر الوقت المناسب.</small></div>
