...
export default function ERPPackages() {
  ...
  const [whatsappPackage, setWhatsappPackage] = useState(null);
  const [whatsappMode, setWhatsappMode] = useState('summary');
  ...
  const openAddDialog = (event, clientId = '') => {
    dialogTriggerRef.current = event.currentTarget;
    packageRequestKeyRef.current = globalThis.crypto?.randomUUID?.() || `package-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    setForm({ ...initialForm, client_id: clientId ? String(clientId) : '' }); setFormErrors({}); setSaleBookings([]); setAppointment(initialAppointment()); setAppointmentErrors({}); setEditingAppointment(-1); setTemplateResetNotice(''); setClientModalOpen(false); setError('');
    setFormOpen(true);
  };
  ...
  return <div className="sold-packages" dir="rtl">
    ...
    {loading ? <Empty icon={RefreshCw} title="جارٍ تحميل الباقات" text="نسترجع أرصدة الباقات المباعة من الخادم." spin/> : filtered.length ? <>
      <div className="packages-table-wrap"><table><thead><tr><th>الباقة داخل ملف العميل</th><th>الرصيد</th><th>فترة الصلاحية</th><th>الحالة المالية</th><th>الحالة والإجراءات</th></tr></thead><tbody>{packageGroups.map(group => <Fragment key={group.clientId}>
        <ClientPackageGroupRow group={group} canAdd={canAssign} onAdd={event => openAddDialog(event, group.clientId)} onToggle={() => toggleClientHistory(group.clientId)}/>
        {group.packages.map(pkg => <PackageRow key={pkg.id} {...packageViewProps(pkg, group)}/>) }
      </Fragment>)}</tbody></table></div>
      <div className="packages-mobile-list">{packageGroups.map(group => <ClientPackageGroupCard key={group.clientId} group={group} canAdd={canAssign} onAdd={event => openAddDialog(event, group.clientId)} onToggle={() => toggleClientHistory(group.clientId)}>{group.packages.map(pkg => <PackageCard key={pkg.id} {...packageViewProps(pkg, group)}/>)}</ClientPackageGroupCard>)}</div>
...
function ClientPackageGroupRow({group,canAdd,onAdd,onToggle}){return <tr className="package-client-group-row"><td colSpan="5"><div className="package-client-group-heading"><ClientPackageGroupSummary group={group}/><div className="package-client-group-actions">{canAdd&&<button type="button" className="package-client-add" onClick={onAdd}><PackagePlus/> باقة جديدة لنفس العميل</button>}{group.relatedCount>0&&<button type="button" onClick={onToggle}>{group.expanded?'إخفاء باقي سجل العميل':`عرض ${group.relatedCount.toLocaleString('ar-EG')} باقة سابقة أو أخرى`}<ChevronLeft aria-hidden="true"/></button>}</div></div></td></tr>}
function ClientPackageGroupCard({group,canAdd,onAdd,onToggle,children}){return <section className="package-client-group-card"><header><ClientPackageGroupSummary group={group}/><div className="package-client-group-actions">{canAdd&&<button type="button" className="package-client-add" onClick={onAdd}><PackagePlus/> باقة جديدة</button>}{group.relatedCount>0&&<button type="button" onClick={onToggle}>{group.expanded?'إخفاء باقي السجل':`عرض باقي الباقات (${group.relatedCount.toLocaleString('ar-EG')})`}<ChevronLeft aria-hidden="true"/></button>}</div></header><div className="package-client-group-card__packages">{children}</div></section>}
