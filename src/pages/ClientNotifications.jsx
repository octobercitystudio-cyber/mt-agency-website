...
export default function ClientNotifications({ clientId, onNavigate }) {
  ...
  useEffect(() => {
    ...
    if (loading) return;
    syncAppBadge(unreadCount, { clearSystemNotifications: unreadCount === 0 });
  }, [loading, unreadCount]);
  ...
  const markRead = async item => {
    if (item.read_at) return; const stamp = new Date().toISOString(); updateItems(current => current.map(row => Number(row.id) === Number(item.id) ? { ...row, read_at: stamp } : row)); setUnreadCount(count => Math.max(0, count - 1)); clearSystemNotification(item.id);
  ...
  return <div className="client-notifications">
    ...
    {open && <div className="client-notifications__backdrop" onMouseDown={event => { if (event.target === event.currentTarget) close(); }}>
      <section ref={dialogRef} id="client-notification-center" className="client-notifications__panel" role="dialog" aria-modal="true" aria-labelledby="client-notifications-title">
        ...
        <div className="client-notifications__toolbar"><button type="button" onClick={() => { close(); window.dispatchEvent(new CustomEvent('mtPushSettings')); }}><BellRing/> إعدادات إشعارات الجهاز والصوت</button></div>
