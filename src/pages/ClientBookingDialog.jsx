...
export default function ClientBookingDialog({ open, packages = [], initialPackageId = '', booking = null, triggerRef, onClose, onSuccess, showNotice }) {
  ...
  const dayBalance = selectedPackage?.billing_unit === 'hour' && selectedPackage?.validity_mode_snapshot === 'shooting_day';
  const durationMinutes = dayBalance ? packageMinutes(selectedPackage, 'purchased') : Number.isFinite(durationDraftMinutes) ? Math.max(0, durationDraftMinutes) : 0;
    ...
    <section ref={dialogRef} className="client-modal-card client-booking-request-dialog" role="dialog" aria-modal="true" aria-labelledby="client-booking-request-title">
      ...
      {!eligiblePackages.length ? <div className="client-booking-state" role="status"><PackageCheck/><h3>لا توجد باقة متاحة للحجز</h3><p>تحتاج إلى باقة تصوير فعالة وبها رصيد متاح.</p></div> : <form onSubmit={submit} className="client-booking-guided-form">
        <div className="client-booking-steps">
          ...
          <fieldset className="client-booking-step" disabled={!selectedPackage}><legend><b>2</b> حدّد مدة الجلسة</legend><div className="client-duration-inputs"><label>الساعات<input type="number" min="0" max="10" step="1" inputMode="numeric" disabled={dayBalance} value={dayBalance ? Math.floor(durationMinutes / 60) : hours} onChange={event => updateDurationDraft(event.target.value, minutes)} onBlur={() => setHours(String(Math.min(10, Math.floor(Math.max(0, Number(hours || 0))))))}/></label><label>الدقائق<input type="number" min="0" max="30" step="30" inputMode="numeric" disabled={dayBalance} value={dayBalance ? durationMinutes % 60 : minutes} onChange={event => updateDurationDraft(hours, event.target.value)} onBlur={() => setMinutes(normalizeClientMinuteDraft(minutes))}/></label></div><p className={durationValid ? 'client-duration-total valid' : 'client-duration-total'}><Clock3/> المدة المطلوبة: <strong>{formatDurationMinutes(durationMinutes)}</strong></p><small>{dayBalance ? 'هذا رصيد يوم واحد؛ تُحجز ساعاته كاملة في جلسة متصلة وصلاحيته يوم التصوير فقط.' : 'الحد الأدنى ساعة واحدة، والزيادة كل 30 دقيقة.'}</small>{durationRuleError && <em>اختر مدة من ساعة إلى 10 ساعات بزيادات 30 دقيقة.</em>}{selectedPackage?.billing_unit === 'hour' && availableMinutes + .001 < durationMinutes && <em>رصيد الباقة لا يكفي لهذه المدة.</em>}</fieldset>
        </div>
