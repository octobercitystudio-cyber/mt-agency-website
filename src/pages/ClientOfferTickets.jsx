...
import { formatBookingDate, formatEGP, formatPackageQuantity } from '../lib/businessFormat';
...
export function ClientOfferDetails({ offer, serverOffset = 0, busy, confirm, onConfirm, onCancelConfirm, onAccept }) {
  ...
  return <div className="client-ticket-detail"><header><span><Sparkles/> عرض خاص · {offer.offer_number}</span><OfferSeal status={status}/></header><h2 id="client-offer-title">{offer.title}</h2>
    ...
    <div className="client-ticket-detail__items">{offer.items?.map(item => <article key={item.id}><div><strong>{item.description}</strong><span data-label="الكمية">{item.unit === 'hour' ? formatPackageQuantity(item.quantity, 'hour') : `${item.quantity.toLocaleString('ar-EG')} ${unitLabel(item.unit)}`}</span></div><span data-label="سعر الوحدة">{formatEGP(item.unit_price)}</span><b data-label="الإجمالي">{formatEGP(item.total)}</b></article>)}</div>
