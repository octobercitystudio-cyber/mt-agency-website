...
const demoRequest = async (path, options = {}) => {
  ...
  if ((match = route.match(/^\/booking-blocks\/(\d+)\/convert$/)) && options.method === 'POST') {
    ...
    if (mode === 'existing_package') {
      ...
      overageCents = demoStrictMoneyCents(nextPackage.overage_price_snapshot, 'سعر الساعة الإضافية'); totalCents = demoStrictMoneyCents(nextPackage.total_price, 'إجمالي السعر'); paidCents = demoStrictMoneyCents(nextPackage.paid_amount, 'المدفوع الآن'); if (paidCents > totalCents) throw formationDemoError('المدفوع الآن لا يمكن أن يتجاوز إجمالي السعر.', 'invalid_payment_amount'); if (!['cash', 'bank_transfer', 'vodafone_cash', 'instapay'].includes(nextPackage.payment_method)) throw formationDemoError('طريقة الدفع غير صحيحة.', 'invalid_payment_method'); const name = String(nextPackage.name || '').trim(); if (!name || name.length > 180) throw formationDemoError('اسم الباقة غير صحيح.', 'invalid_package_name');
