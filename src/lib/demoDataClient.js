...
const demoCalendarDays = (database, url, pkg, booking) => {
  ...
  const resources = database.resources.filter(row => belongsToDemoOrganization(row) && Number(row.is_active ?? 1) === 1 && (!booking || Number(row.id) === Number(booking.resource_id || 1)));
  const time = value => `${pad(Math.floor(value / 60))}:${pad(value % 60)}`;
  const free = (date, start, end) => resources.find(resource => getBookingAvailability({ date, start_time: time(start), end_time: time(end), resource_id: resource.id }, database.bookings, { blocks: database.booking_blocks || [], excludeBookingId: booking?.id }).available);
...
const demoRequest = async (path, options = {}) => {
  ...
  if (route === '/booking-blocks' && options.method === 'POST') {
    ...
    const startDate = String(body.date || ''); const repeat = Boolean(body.repeat_daily); const repeatUntil = String(body.repeat_until || ''); const startTime = String(body.start_time || '').slice(0, 5); const endTime = String(body.end_time || '').slice(0, 5); const resourceId = Number(body.resource_id || 0); const idempotencyKey = String(body.idempotency_key || '').trim(); const title = String(body.title || '').trim().slice(0, 120) || 'حجز مؤقت'; const note = String(body.note || '').trim().slice(0, 1000);
    ...
    const inclusiveDays = Math.round((last - first) / 86400000) + 1; if (repeat && inclusiveDays > 90) throw formationDemoError('يمكن تكرار الحظر لمدة 90 يومًا كحد أقصى.', 'booking_block_range_too_long');
    const dates = []; for (const cursor = new Date(first); cursor <= last; cursor.setDate(cursor.getDate() + 1)) dates.push(`${cursor.getFullYear()}-${pad(cursor.getMonth() + 1)}-${pad(cursor.getDate())}`);
  ...
  if (route === '/sync' && (options.method || 'GET') === 'GET') {
    ...
    const visible = tableRows(database, 'change_events').filter(event => belongsToDemoOrganization(event) && (!clientId || Number(event.client_id) === clientId || event.topic === 'services' || (event.client_id == null && event.topic === 'post_production' && event.entity_type === 'pickup_schedule'))).sort((a, b) => Number(a.id) - Number(b.id));
