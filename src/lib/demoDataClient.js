...
const demoRequest = async (path, options = {}) => {
  ...
  if ((match = route.match(/^\/client-packages\/(\d+)\/extend$/)) && options.method === 'POST') {
    ...
    const before = clone(pkg); Object.assign(pkg, { expires_at: expires, status: pkg.status === 'expired' && expires >= dateOnly() ? 'active' : pkg.status, version: Number(pkg.version || 1) + 1, updated_at: nowText() }); addRow(database, 'owner_adjustments', { entity_type: 'client_packages', entity_id: pkg.id, adjustment_type: 'validity', amount_delta_cents: 0, quantity_delta: 0, reason, before_data: before, after_data: clone(pkg) }); demoAudit(database, 'extend', 'client_packages', pkg.id, before, { ...clone(pkg), reason }); writeDatabase(database); return { id: pkg.id, expires_at: expires, version: pkg.version, idempotent: false };
  }
