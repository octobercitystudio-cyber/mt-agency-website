...
import { cairoDateKey, centsToMoney, moneyToCents, packageFinancialSummary, packageQuantitySummary, remainingCalendarDays } from './businessFormat.js';
...
const activateDemoPackageOnFirstBooking = (pkg, bookingDate) => {
  if (!pkg) return;
...
const createDemoDatabase = () => ({
  ...
  finance: [
    ...
    { id: 906, employee_user_id: 1, type: 'مصروف', entry_kind: 'expense', category: 'employee_out_of_pocket', amount: 900, method: 'كاش', detail: 'مشتريات تشغيل - أشرف', date: dateOnly(-6), entity: 'أشرف' },
    { id: 907, employee_user_id: 2, type: 'مصروف', entry_kind: 'expense', category: 'employee_out_of_pocket', amount: 650, method: 'كاش', detail: 'انتقالات تصوير - مروة', date: dateOnly(-5), entity: 'مروة' },
...
const upgradeFinanceDemoCoverage = database => {
  ...
  let changed = false;
  const referenceTables = ['clients', 'services', 'client_packages', 'client_package_payment_requests', 'payment_proofs', 'payments', 'payment_allocations', 'invoices', 'projects', 'offer_items', 'app_notifications'];
...
const employeeFinanceDemoSnapshot = (database, month) => {
  ...
  const employees = database.users.filter(user => user.is_active !== 0 && user.role !== 'client' && ['أشرف', 'اشرف', 'مروة'].includes(String(user.full_name || '').trim().split(/\s+/)[0]));
...
const deleteDemoBooking = (database, bookingId) => {
  ...
  const key = `booking:${booking.id}:deleted`; if (!database.app_notifications.some(item => item.source_event_key === key)) { const notification = addRow(database, 'app_notifications', { client_id: booking.client_id, audience: 'client', type: 'booking_deleted', title: 'تم حذف موعد', message: 'أزيل أحد مواعيدك المجدولة من قائمة الحجوزات.', entity_type: 'bookings', entity_id: booking.id, action_tab: 'schedule', payload: {}, severity: 'warning', source_event_key: key, dedupe_key: key, read_at: null, dismissed_at: null }); addRow(database, 'change_events', { client_id: booking.client_id, topic: 'notifications', entity_type: 'app_notifications', entity_id: notification.id, action: 'created' }); }
...
const demoRequest = async (path, options = {}) => {
  ...
  if (route === '/attendance/employee-accounts/movements' && options.method === 'POST') {
    ...
    if (!employee || employee.is_active === 0 || employee.role === 'client') throw formationDemoError('حساب الموظف غير موجود أو غير نشط.', 'invalid_employee_user');
  ...
  if (route === '/client-packages' && options.method === 'POST') {
    ...
    const startsAt = saleBookings[0]?.date || null; const shootingDate = validityMode === 'shooting_day' ? startsAt : '';
    ...
    const response = { id: row.id, expires_at: expiresAt, payment_id: payment?.id || null, booking_ids: bookingIds, billing_unit: unit, validity_mode_snapshot: validityMode, purchased_quantity: row.purchased_quantity, paid_amount: row.paid_amount, idempotent: false }; Object.assign(request, { status: 'completed', response: clone(response), completed_at: nowText() });
    writeDatabase(working); return response;
  }
  if ((match = route.match(/^\/client-packages\/(\d+)$/)) && options.method === 'PATCH') { requireDemoOwner(); const reason = demoReason(body); const pkg = findById(database, 'client_packages', match[1]); if (!pkg) throw formationDemoError('الباقة غير موجودة.', 'package_not_found'); if (body.expected_version != null && Number(body.expected_version) !== Number(pkg.version || 1)) throw formationDemoError('تم تحديث الباقة من شاشة أخرى. حدّث البيانات ثم أعد المحاولة.', 'stale_package_version'); const starts = String(body.starts_at ?? pkg.starts_at).slice(0, 10); const expires = String(body.expires_at ?? pkg.expires_at).slice(0, 10); if (!/^\d{4}-\d{2}-\d{2}$/.test(starts) || !/^\d{4}-\d{2}-\d{2}$/.test(expires) || expires < starts || (pkg.validity_mode_snapshot === 'shooting_day' && starts !== expires)) throw formationDemoError('بيانات صلاحية الباقة غير صحيحة.', 'invalid_package_details'); const outside = database.bookings.some(booking => Number(booking.client_package_id) === Number(pkg.id) && ['pending','confirmed','alternative_proposed','cancel_requested','late_cancel_requested','in_progress'].includes(booking.status) && (booking.date < starts || booking.date > expires || (pkg.validity_mode_snapshot === 'shooting_day' && booking.date !== starts))); if (outside) throw formationDemoError('توجد مواعيد نشطة خارج فترة الصلاحية الجديدة. عدّل المواعيد أولًا.', 'bookings_outside_package_validity'); const before = clone(pkg); ['name','notes','starts_at','expires_at','status'].forEach(field => { if (Object.prototype.hasOwnProperty.call(body, field)) pkg[field] = body[field]; }); pkg.version = Number(pkg.version || 1) + 1; pkg.updated_at = nowText(); demoAudit(database, 'owner_update_package', 'client_packages', pkg.id, before, { ...clone(pkg), reason }); writeDatabase(database); return clone(pkg); }
  ...
  if ((match = route.match(/^\/client-packages\/(\d+)\/payments$/)) && options.method === 'POST') {
    if (!['owner', 'admin', 'operations'].includes(demoRole)) throw formationDemoError('ليس لديك صلاحية لتسجيل دفعة على الباقة.', 'forbidden');
    ...
    const outstandingCents = summary.outstandingCents - amountCents; demoAudit(working, 'record_package_payment', 'payments', payment.id, null, { client_id: Number(pkg.client_id), client_package_id: packageId, amount: centsToMoney(amountCents), method, remaining: centsToMoney(outstandingCents) }); failAt('audit');
  ...
  if ((match = route.match(/^\/clients\/(\d+)\/payment-history$/)) && (options.method || 'GET') === 'GET') {
    if (!['owner', 'admin', 'operations', 'finance'].includes(demoRole)) throw formationDemoError('ليس لديك صلاحية لعرض سجل دفعات العميل.', 'forbidden');
...
export const activateDemoMode = (role = 'owner', userId = 1) => { demoMode = true; demoRole = role; demoUserId = Number(userId) || 1; readDatabase(); };
export const deactivateDemoMode = () => { demoMode = false; demoRole = 'owner'; demoUserId = 1; demoCredentialSessionVersion = null; };
