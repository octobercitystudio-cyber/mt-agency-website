...
const packageDemoDetails = (database, packageId) => {
  ...
  return {
    package: { id: Number(pkg.id), name: pkg.name, notes: pkg.notes || '', billing_unit: pkg.billing_unit, status: pkg.status, effective_status: effectiveStatus, version: Number(pkg.version || 1), validity_mode_snapshot: pkg.validity_mode_snapshot || 'rolling', source_invoice_id: pkg.source_invoice_id || null, invoice_number: invoice?.invoice_number || null, client: { id: Number(pkg.client_id), name: client?.name || 'عميل', phone: client?.phone1 || null }, service: { id: Number(pkg.service_id), name: service?.name || pkg.name } },
...
const demoRequest = async (path, options = {}) => {
  ...
  if (route === '/client-packages' && options.method === 'POST') {
    ...
    const startsAt = validityMode === 'shooting_day' ? String(body.shooting_date || '') : String(body.starts_at || '');
    if (validityMode === 'shooting_day' && !/^\d{4}-\d{2}-\d{2}$/.test(startsAt)) throw formationDemoError('حدد يوم التصوير للباقة اليومية.', 'shooting_date_required');
    const draft = { ...body, starts_at: startsAt, validity_days: validityMode === 'shooting_day' ? 1 : body.validity_days, client_id: String(body.client_id), service_id: String(body.service_id) }; const errors = validatePackageDraft(draft); if (Object.keys(errors).length) throw formationDemoError(Object.values(errors)[0], 'invalid_package_sale');
    ...
    const expiresAt = validityMode === 'shooting_day' ? startsAt : packageDraftExpiry(draft); if (body.expires_at && body.expires_at !== expiresAt) throw formationDemoError('تاريخ الانتهاء لا يطابق سياسة صلاحية الباقة.', 'expiry_mismatch');
