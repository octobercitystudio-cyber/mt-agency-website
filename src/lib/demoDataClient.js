...
const demoRequest = async (path, options = {}) => {
  ...
  if (route === '/client/studio-availability' && (options.method || 'GET') === 'GET') { if (demoRole !== 'client') throw formationDemoError('غير مصرح.', 'forbidden'); const service = (await registrationDemoCatalog(database)).services.find(row => Number(row.id) === Number(url.searchParams.get('service_id'))); if (!service) throw formationDemoError('اختر باقة متاحة.', 'invalid_service'); if (Number(url.searchParams.get('duration_minutes')) > Number(service.total_hours) * 60) throw formationDemoError('المدة تتجاوز رصيد الباقة.', 'insufficient_package_balance'); return demoCalendarDays(database, url); }
