...
const demoSettleAndComplete = (database, bookingId, body) => {
  ...
  if (mode === 'existing_package') {
    ...
    const minutes = Number(settlement.purchased_minutes); const days = Number(settlement.validity_days); const name = String(settlement.name || '').trim(); const template = findById(working, 'services', settlement.service_id); const totalCents = moneyToCents(settlement.total_price); paidCents = moneyToCents(settlement.initial_paid); const paymentMethod = String(settlement.payment_method || 'cash').trim();
