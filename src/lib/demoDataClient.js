...
import { shiftBookingDate } from './packageBookingCalendar.js';
import { normalizeLoginPhone } from './phoneLogin.js';
import { studioBookingDemoRequest } from './studioBookingDemo.js';
import { registrationDemoCatalog, registrationDemoRequest, registrationDemoUser, authenticateRegistrationDemo, changeRegistrationDemoPassword } from './registrationDemo.js';
import { CLIENT_BOOKING_POLICY, clientWindowError, isClientBookingDateClosed } from './registrationPolicy.js';
...
import { clientAdditionalPhones, clientPhoneFields, normalizeClientPhone } from './clientPhones.js';
...
let demoRegistrationUserId = null;
const currentDemoClientId = database => demoRegistrationUserId ? Number(registrationDemoUser(database || readDatabase(), demoRegistrationUserId)?.client_id || 0) : 1;
let demoMode = false;
let demoRole = 'owner';
let demoUserId = 1;
...
const demoClientBookingTimeError = (message, code) => {
  const error = formationDemoError(message, code); error.status = 422; throw error;
};
const demoClientTimeMinutes = (value, endOfDay = false) => {
  const time = String(value || '').slice(0, 5);
  if (endOfDay && (time === '00:00' || time === '24:00')) return 1440;
  if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(time)) return -1;
  return Number(time.slice(0, 2)) * 60 + Number(time.slice(3, 5));
};
const validateDemoClientBookingGrid = body => {
  const rawDuration = String(body.duration_minutes ?? '').trim();
  if (!/^\d+$/.test(rawDuration)) demoClientBookingTimeError('أرسل مدة الحجز بالدقائق.', 'client_booking_duration_required');
  const duration = Number(rawDuration);
  if (duration < 60 || duration > 600) demoClientBookingTimeError('مدة حجز العميل يجب أن تكون بين ساعة و10 ساعات.', 'client_booking_duration_out_of_range');
  if (duration % 30 !== 0) demoClientBookingTimeError('مدة حجز العميل يجب أن تزيد كل 30 دقيقة.', 'client_booking_duration_increment_invalid');
  const start = demoClientTimeMinutes(body.start_time); const end = demoClientTimeMinutes(body.end_time, true);
  if (start < 0 || start >= 1440 || start % 60 !== 0) demoClientBookingTimeError('وقت بداية حجز العميل يجب أن يكون عند ساعة كاملة ودقائق :00.', 'client_booking_start_grid_invalid');
  if (end < 0 || end % 30 !== 0) demoClientBookingTimeError('وقت نهاية حجز العميل يجب أن ينتهي عند :00 أو :30.', 'client_booking_end_grid_invalid');
  if (end <= start || end > 1440) demoClientBookingTimeError('لا يمكن أن يمتد حجز العميل لما بعد منتصف الليل.', 'client_booking_after_midnight');
  if (end - start !== duration) demoClientBookingTimeError('المدة المرسلة لا تطابق وقت البداية والنهاية المحسوب.', 'client_booking_duration_mismatch');
  const policyError = clientWindowError(body); if (policyError) demoClientBookingTimeError(policyError, 'client_booking_outside_hours');
...
const demoClientHasDay = (database, clientId, date, excludeId = 0) => database.bookings.some(row => Number(row.client_id) === clientId && Number(row.id) !== Number(excludeId) && row.date === date && ['pending', 'confirmed', 'alternative_proposed', 'cancel_requested', 'late_cancel_requested', 'in_progress'].includes(row.status))
  || tableRows(database, 'reschedule_requests').some(row => Number(row.client_id) === clientId && row.status === 'pending' && Number(row.booking_id) !== Number(excludeId) && (row.proposed_date || row.date) === date)
  || tableRows(database, 'studio_booking_requests').some(request => Number(request.client_id) === clientId && request.package_status !== 'rejected' && (request.bookings || []).some(row => row.date === date && row.status === 'pending'));
const demoClientPendingQuantity = (database, pkg, excludeId = 0) => database.bookings.filter(row => Number(row.client_package_id) === Number(pkg?.id) && Number(row.id) !== Number(excludeId) && ['pending', 'alternative_proposed'].includes(row.status)).reduce((sum, row) => sum + Number(row.requested_quantity || row.duration_minutes / 60 || 0), 0);
const demoCalendarDays = (database, url, pkg, booking) => {
  const duration = Number(url.searchParams.get('duration_minutes')), count = Number(url.searchParams.get('days') || 31), startDate = url.searchParams.get('start_date') || earliestClientBookingDate();
  if (!Number.isInteger(duration) || duration < 60 || duration > 600 || duration % 30) throw formationDemoError('مدة الحجز تبدأ من ساعة وبزيادات 30 دقيقة.', 'invalid_booking_duration');
  if (!Number.isInteger(count) || count < 1 || count > 31 || !/^\d{4}-\d{2}-\d{2}$/.test(startDate)) throw formationDemoError('فترة البحث غير صحيحة.', 'availability_window_out_of_range');
  const resources = database.resources.filter(row => belongsToDemoOrganization(row) && Number(row.is_active ?? 1) === 1 && (booking ? Number(row.id) === Number(booking.resource_id || 1) : row.type === 'studio'));
  const time = value => `${pad(Math.floor(value / 60))}:${pad(value % 60)}`;
  const free = (date, start, end) => resources.find(resource => getBookingAvailability({ date, start_time: time(start), end_time: time(end), resource_id: resource.id }, database.bookings, { blocks: calendarBlocks(database, demoOrganizationId, date, date), excludeBookingId: booking?.id }).available);
  const days = Array.from({ length: count }, (_, offset) => {
    const date = shiftBookingDate(startDate, offset), has = demoClientHasDay(database, currentDemoClientId(database), date, booking?.id);
    let reason = clientBookingDateError(date) ? 'past' : isClientBookingDateClosed(date) ? 'friday' : pkg && (pkg.starts_at && date < String(pkg.starts_at).slice(0, 10) || pkg.expires_at && date > String(pkg.expires_at).slice(0, 10)) ? 'outside_validity' : has ? 'already_booked' : null;
    const busy = []; for (let m = 720; m < 1320; m++) { if (!free(date, m, m + 1)) { const last = busy.at(-1); if (last && last.end === m) last.end = m + 1; else busy.push({ start: m, end: m + 1 }); } }
    const slots = [];
    if (!reason) for (let m = 720; m + duration <= 1320; m += 60) { if (booking && clientNoticeIsLate({ date, start_time: time(m) })) continue; const resource = free(date, m, m + duration); if (resource) slots.push({ start_time: time(m), end_time: time(m + duration), resource_id: Number(resource.id) }); }
    if (!reason && !slots.length) reason = booking && clientNoticeIsLate({ date, start_time: '22:00' }) ? 'notice' : 'full';
    return { date, available: slots.length > 0, slots, busy_intervals: busy.map(row => ({ start_time: time(row.start), end_time: time(row.end) })), has_client_booking: has, unavailable_reason: reason };
  });
  return { days, booking_policy: CLIENT_BOOKING_POLICY, duration_minutes: duration, server_time: demoCairoNowIso() };
};
const demoClientBookingAvailability = (database, url) => {
  ...
  const bookingId = Number(url.searchParams.get('booking_id') || 0), booking = bookingId ? findById(database, 'bookings', bookingId) : null;
  if (bookingId && (!booking || Number(booking.client_id) !== currentDemoClientId(database))) throw formationDemoError('الموعد غير موجود.', 'booking_not_found');
  if (booking && clientNoticeIsLate(booking)) throw formationDemoError('التعديل قبل 48 ساعة فعلية باستثناء الجمعة.', 'late_reschedule');
  const pkg = findById(database, 'client_packages', booking?.client_package_id || Number(url.searchParams.get('client_package_id')));
  if (!booking && (!pkg || Number(pkg.client_id) !== currentDemoClientId(database) || pkg.status !== 'active')) throw formationDemoError('الباقة غير فعالة أو لا تخص هذا الحساب.', 'invalid_package');
  const heldCredit = pkg && booking ? demoBookingHeldQuantity(database, booking.id, pkg.id) : 0;
  const pendingExtension = pkg ? tableRows(database, 'reschedule_requests').filter(row => row.status === 'pending' && Number(row.booking_id) !== bookingId).reduce((sum, row) => { const old = findById(database, 'bookings', row.booking_id); if (Number(old?.client_package_id) !== Number(pkg.id)) return sum; const minutes = demoClientTimeMinutes(row.proposed_end_time || row.end_time) - demoClientTimeMinutes(row.proposed_start_time || row.start_time); return sum + Math.max(0, minutes / 60 - demoBookingHeldQuantity(database, old.id, pkg.id)); }, 0) : 0;
  const available = pkg ? Math.max(0, demoPackageAvailable(pkg) - demoClientPendingQuantity(database, pkg, bookingId) - pendingExtension + heldCredit) : 10;
  if (pkg && (pkg.billing_unit === 'hour' ? available * 60 + .001 < Number(url.searchParams.get('duration_minutes')) : available < 1)) throw formationDemoError('رصيد الباقة لا يكفي لهذه المدة.', 'insufficient_package_balance');
  return { ...demoCalendarDays(database, url, pkg, booking), ...(pkg ? { package: { id: pkg.id, name: pkg.name, billing_unit: pkg.billing_unit, available_quantity: available, starts_at: pkg.starts_at, expires_at: pkg.expires_at, minimum_booking_minutes: 60, booking_increment_minutes: 30 } } : {}) };
};
...
const demoSettlementPreview = (database, bookingId, actualMinutes) => {
  ...
  const covered = !pkg ? 0 : pkg.billing_unit === 'hour' ? Math.min(actual, heldMinutes + freeOriginal) : actual;
  const excess = Math.max(0, actual - covered);
  const today = cairoDateKey();
  const eligiblePackages = tableRows(database, 'client_packages').filter(row => Number(row.client_id) === Number(booking.client_id) && Number(row.id) !== Number(pkg?.id) && row.billing_unit === 'hour' && row.status === 'active' && ((!row.starts_at && !row.expires_at) || String(row.starts_at).slice(0, 10) <= today && String(row.expires_at).slice(0, 10) >= today)).map(row => ({ ...row, free_minutes: Math.max(0, demoPackageMinutes(row, 'purchased') - demoPackageMinutes(row, 'consumed') - demoPackageMinutes(row, 'held')) })).filter(row => row.free_minutes >= excess).sort((left, right) => String(left.expires_at || '9999-12-31').localeCompare(String(right.expires_at || '9999-12-31')) || Number(left.id || 0) - Number(right.id || 0)).map(row => ({ id: row.id, name: row.name, service_id: row.service_id, version: Number(row.version || 1), free_minutes: row.free_minutes, remaining_after_minutes: row.free_minutes - excess, expires_at: row.expires_at }));
  ...
  return { actual_minutes: actual, suggested_actual_minutes: suggested, complimentary_seconds: complimentary, held_for_booking_minutes: heldMinutes, free_unheld_original_minutes: freeOriginal, covered_minutes: covered, excess_minutes: excess, unassigned_minutes: pkg ? 0 : actual, requires_package_assignment: !pkg && actual > 0, eligible_packages: eligiblePackages, package_templates: packageTemplates, overage_rate: overageRate, default_mode: !pkg ? 'existing_package' : eligiblePackages.length ? 'existing_package' : Number(overageRate) > 0 ? 'package_overage' : 'custom_invoice', session_version: version, preview_hash: demoSettlementHash(snapshot) };
};
...
const createDemoDatabase = () => ({
  ...
  app_notifications: [
    ...
    { id: 1507, client_id: 1, audience: 'client', type: 'payment_proof_approved', title: 'تم قبول الدفع', message: 'وافقت الإدارة على إثبات الدفع وتم تحديث رصيدك المالي.', entity_type: 'payment_proofs', entity_id: 1203, action_tab: 'finance', payload: {}, severity: 'success', read_at: null, dismissed_at: null, created_at: dateTime(-2, '13:00:00') },
...
class DemoQueryBuilder {
  ...
  async execute() {
    ...
    if (['applicant', 'guest'].includes(demoRole) && this.table !== 'app_config') throw formationDemoError('حسابك قيد المراجعة.', 'forbidden');
    ...
    if (this.method === 'POST') {
      ...
      rows.forEach((row, index) => {
        if (matches(row)) {
          const next = { ...row, ...clone(this.payload), updated_at: nowText() };
          if (this.table === 'clients' && ['phone1', 'additional_phones'].some(field => Object.hasOwn(this.payload, field))) {
            try { Object.assign(next, clientPhoneFields({ ...next, additional_phones: Object.hasOwn(this.payload, 'additional_phones') ? this.payload.additional_phones : clientAdditionalPhones(row) })); }
            catch (error) { throw formationDemoError(error.message, 'invalid_client_phone'); }
          }
          rows[index] = next;
        }
      });
...
const materializeDemoBlocks = (database, from, to) => {
  for (const row of recurringBlocks(database, demoOrganizationId, from, to)) {
    const fields = { ...row }; delete fields.id; addRow(database, 'booking_blocks', fields);
  }
};
const assertDemoBookingAvailable = (database, candidate, excludeBookingId = null, approval = null) => {
  materializeDemoBlocks(database, candidate.date, candidate.date);
  ...
  if (availability.status === 'blocked' && approval && demoRole === 'owner') {
    const blocks = availability.conflicts;
    const token = JSON.stringify([candidate.date, candidate.start_time, candidate.end_time, candidate.resource_id, blocks.map(b => [b.id, b.start_time, b.end_time, b.title, b.note])]);
    if (approval.temporary_booking_confirmation !== token && !approval.temporary_booking_confirmations?.includes(token)) {
      const error = formationDemoError(`الموعد ${candidate.date} من ${candidate.start_time} إلى ${candidate.end_time} محجوز مؤقتًا. هل توافق على استبدال الجزء المتداخل فقط؟ ستظل باقي ساعات الحجز المؤقت محفوظة.`, 'temporary_booking_confirmation_required');
      error.confirmationToken = token; error.status = 409; throw error;
    }
    for (const conflict of blocks) {
      const block = database.booking_blocks.find(b => Number(b.id) === Number(conflict.id));
      const from = bookingTimeToMinutes(block.start_time), to = bookingTimeToMinutes(block.end_time, { end: true });
      const start = bookingTimeToMinutes(candidate.start_time), end = bookingTimeToMinutes(candidate.end_time, { end: true });
      database.booking_slots = (database.booking_slots || []).filter(slot => Number(slot.booking_block_id) !== Number(block.id));
      block.status = 'cancelled'; block.cancelled_by = demoUserId;
      for (const [a, b] of [[from, Math.min(start, to)], [Math.max(end, from), to]]) {
        if (b <= a) continue;
        const fmt = m => `${String(Math.floor(m / 60)).padStart(2, '0')}:${String(m % 60).padStart(2, '0')}`;
        const remainder = clone(block); delete remainder.id;
        const rest = addRow(database, 'booking_blocks', { ...remainder, status: 'active', cancelled_by: null, start_time: fmt(a), end_time: fmt(b), duration_minutes: b - a });
        for (let minute = a; minute < b; minute += 15) addRow(database, 'booking_slots', { organization_id: block.organization_id, resource_id: block.resource_id, booking_id: null, booking_block_id: rest.id, slot_date: block.block_date, slot_start: `${fmt(minute)}:00` });
      }
      demoAudit(database, 'replace_overlap', 'booking_blocks', block.id, conflict, clone(block));
    }
    return { ...availability, available: true, status: 'available' };
  }
...
const demoCompanyPickupSchedule = database => {
  const row = tableRows(database, 'app_config').find(item => belongsToDemoOrganization(item) && item.key === COMPANY_PICKUP_SCHEDULE_KEY);
  if (!row) return emptyCompanyPickupSchedule();
...
const demoPostProductionRows = (database, clientOnly = false) => tableRows(database, 'post_production_jobs')
  .filter(job => !clientOnly || Number(job.client_id) === currentDemoClientId(database) && Number(job.is_client_visible) === 1 && Number(job.needs_review) === 0)
  .map(job => {
...
const demoNotificationTemplate = (entityType, action, before = {}, after = {}) => {
  ...
  if (entityType === 'bookings') {
    ...
    if (action === 'admin_reschedule' || (action !== 'booking_decision' && ['date', 'start_time', 'end_time'].some(key => Object.prototype.hasOwnProperty.call(after || {}, key) && before?.[key] !== after[key]))) return ['booking_rescheduled', 'تم تحديث موعد الحجز', `عدّلت الإدارة موعدك إلى: ${moment}.`, 'schedule', 'info'];
    const states = { confirmed: ['booking_confirmed', 'تم قبول موعد التصوير', `وافقت الإدارة على موعدك: ${moment}.`, 'success'], rejected: ['booking_rejected', 'تم رفض موعد التصوير', `رفضت الإدارة الموعد المطلوب: ${moment}.`, 'danger'], cancelled: ['booking_cancelled', 'تم إلغاء الحجز', `تم إلغاء الموعد: ${moment}.`, 'warning'] }; const state = states[status]; return state ? [state[0], state[1], state[2], 'schedule', state[3]] : null;
  }
  if (entityType === 'reschedule_requests') { if (!['approved', 'rejected'].includes(status)) return null; const moment = demoBookingNotificationMoment(merged); return ['reschedule_update', status === 'approved' ? 'تم قبول تغيير الموعد' : 'تم رفض تغيير الموعد', status === 'approved' ? `تم اعتماد تغيير موعدك إلى: ${moment}.` : `تم رفض الموعد المقترح: ${moment}. موعدك الحالي يظل قائمًا.`, 'schedule', status === 'approved' ? 'success' : 'warning']; }
  ...
  if (entityType === 'payment_proofs' && !['approved', 'rejected'].includes(status)) return null;
...
const demoAudit = (database, action, entityType, entityId, before, after) => {
  demoPackageLoyaltyAudit(database, action, entityType, entityId, after);
...
const demoAuditBookingBlockChange = (database, action, entityId, before, after) => {
  ...
  addRow(database, 'change_events', { client_id: null, topic: 'availability', entity_type: 'calendar_availability', entity_id: null, action: 'changed' });
...
const demoSessionPackageWhatsAppSummary = (database, booking, pkg, actualMinutes, actualReels) => {
  if (!pkg) return null;
  const client = findById(database, 'clients', booking.client_id); if (!client) return null;
  const unit = pkg.billing_unit === 'reel' ? 'reel' : 'hour';
  const purchasedMinutes = unit === 'hour' ? demoPackageMinutes(pkg, 'purchased') : null;
  const consumedMinutes = unit === 'hour' ? demoPackageMinutes(pkg, 'consumed') : null;
  const paymentDueMinutes = unit === 'hour' ? demoPackageMinutes(pkg, 'payment_due') : null;
  const purchasedQuantity = Number(pkg.purchased_quantity || 0); const consumedQuantity = Number(pkg.consumed_quantity || 0); const paymentDueQuantity = Number(pkg.payment_due_quantity || 0);
  const totalCents = moneyToCents(pkg.total_price) + Math.max(0, moneyToCents(pkg.overage_amount)); const paidCents = moneyToCents(pkg.paid_amount); const outstandingCents = Math.max(0, totalCents - paidCents);
  const thresholdReached = outstandingCents > 0 && (unit === 'hour' ? paymentDueMinutes > 0 && consumedMinutes >= paymentDueMinutes : paymentDueQuantity > 0 && consumedQuantity >= paymentDueQuantity);
  return { client_id: Number(client.id), client_name: client.name, client_phone: client.phone1 || '', recipient: client.phone1 || '', client_points: Math.max(0, Number(client.points || 0)), package_id: Number(pkg.id), package_name: pkg.name, billing_unit: unit, today_minutes: actualMinutes, today_quantity: unit === 'reel' ? actualReels : demoSettlementHours(actualMinutes), purchased_minutes: purchasedMinutes, purchased_quantity: purchasedQuantity, consumed_minutes: consumedMinutes, consumed_quantity: consumedQuantity, remaining_minutes: unit === 'hour' ? Math.max(0, purchasedMinutes - consumedMinutes) : null, remaining_quantity: Math.max(0, purchasedQuantity - consumedQuantity), payment_due_minutes: paymentDueMinutes, payment_due_quantity: paymentDueQuantity, payment_due_reached: thresholdReached, total_amount: centsToMoney(totalCents), paid_amount: centsToMoney(paidCents), outstanding_amount: centsToMoney(outstandingCents), expires_at: pkg.expires_at || '' };
};
...
const demoSettleAndComplete = (database, bookingId, body) => {
  ...
  const actual = Number(body.actual_minutes); if (!Number.isSafeInteger(actual) || actual < 0) throw formationDemoError('حدد مدة تصوير صحيحة بالدقائق.', 'invalid_actual_duration');
  ...
  if (actual === 0 && sourceSession.status === 'cancelled' && Number(sourceSession.actual_seconds || 0) === 0) return { booking_id: booking.id, session_id: sourceSession.id, settlement_id: null, post_production_job_id: null, status: 'cancelled', cancelled: true, actual_minutes: 0, covered_minutes: 0, excess_minutes: 0, billable_minutes: 0, released_quantity: 0, billing_unit: findById(database, 'client_packages', booking.client_package_id)?.billing_unit || 'hour', idempotent_replay: true };
  ...
  if (actual === 0) {
    if (sourceSession.status !== 'active') throw formationDemoError('جلسة التصوير لم تعد نشطة.', 'invalid_session_state');
    const working = clone(database); const workBooking = findById(working, 'bookings', booking.id); const session = tableRows(working, 'booking_sessions').find(row => Number(row.booking_id) === Number(booking.id)); const pkg = workBooking.client_package_id ? findById(working, 'client_packages', workBooking.client_package_id) : null; const held = pkg ? demoBookingHeldQuantity(working, workBooking.id, pkg.id) : 0; const heldMinutes = pkg?.billing_unit === 'hour' ? demoBookingHeldMinutes(working, workBooking.id, pkg.id) : null;
    if (pkg && held > 0) { mutateDemoPackageQuantities(pkg, pkg.billing_unit === 'hour' ? { held_minutes: -heldMinutes } : { held: -held }); pkg.version = Number(pkg.version || 1) + 1; addDemoPackageUsage(working, pkg, { booking_id: workBooking.id, movement_type: 'release', quantity: held, quantity_minutes: heldMinutes, reason: 'إلغاء دون خصم', event_key: `booking:${workBooking.id}:cancel:release` }); addRow(working, 'change_events', { client_id: workBooking.client_id, topic: 'packages', entity_type: 'client_packages', entity_id: pkg.id, action: 'booking_release' }); }
    working.booking_slots = tableRows(working, 'booking_slots').filter(slot => Number(slot.booking_id) !== Number(workBooking.id)); const ended = nowText(); const reason = String(body.reason || '').trim() || 'إلغاء الجلسة لإدخال مدة تصوير صفر'; Object.assign(workBooking, { status: 'cancelled', timer_ended_at: ended, actual_seconds: 0, actual_hours: 0, actual_reels: 0, billable_quantity: 0, overage_quantity: 0, overage_amount: 0, session_version: Number(workBooking.session_version || 1) + 1 }); Object.assign(session, { status: 'cancelled', ended_at: ended, actual_seconds: 0, billable_quantity: 0, ended_by: 1, adjustment_reason: reason, settlement_version: Number(session.settlement_version || 1) + 1 }); addRow(working, 'booking_status_history', { booking_id: workBooking.id, from_status: 'in_progress', to_status: 'cancelled', note: reason, changed_by: 1 });
  ...
  if (preview.excess_minutes > 0 && demoRole === 'operations') throw formationDemoError(preview.requires_package_assignment ? 'إسناد وقت الجلسة إلى باقة يحتاج المالك أو الإدارة. ما زالت الجلسة نشطة.' : 'اعتماد الوقت الزائد يحتاج المالك أو الإدارة. ما زالت الجلسة نشطة.', 'settlement_owner_required');
  ...
  if (preview.requires_package_assignment && mode !== 'existing_package') throw formationDemoError('يجب إسناد كامل وقت الجلسة إلى باقة ساعات مؤهلة قبل إيقافها.', 'unassigned_package_required');
  ...
  if (mode === 'existing_package') {
    ...
    if (!['cash','instapay','vodafone_cash'].includes(paymentMethod)) throw formationDemoError('طريقة الدفع غير صحيحة.', 'invalid_payment_method');
...
const requireDemoPaymentMethod = value => { const method = normalizePaymentMethod(value); if (!method) throw formationDemoError('اختر طريقة الدفع: كاش أو انستاباي أو فودافون كاش.', 'invalid_payment_method'); return method; };
...
const demoRequest = async (path, options = {}) => {
  ...
  if (demoRole === 'applicant' || demoRole === 'guest') throw formationDemoError('حسابك قيد المراجعة. تابع صندوق طلباتك.', 'forbidden');
  if (options.method === 'POST' && /^\/(?:finance\/|payments\/|client-packages(?:\/|$)|clients\/\d+\/balance|formation-fund\/|projects\/custom-service|attendance\/employee-accounts\/movements)/.test(route) && !/\/(?:void|legacy-import)$/.test(route)) {
    for (const field of ['method', 'payment_method', 'from_method', 'to_method']) if (Object.hasOwn(body, field) && body[field]) body[field] = requireDemoPaymentMethod(body[field]);
  }
  let match;
  ...
  if (route === '/dashboard/receivables' && (options.method || 'GET') === 'GET') {
    ...
    return calculateDashboardReceivableDetails({ packages: database.client_packages, clients: database.clients, todayKey: cairoDateKey() });
  }
  ...
  if (route === '/auth/password' && options.method === 'PATCH' && demoRegistrationUserId) { const user = await changeRegistrationDemoPassword(database, demoRegistrationUserId, body); writeDatabase(database); return { updated: true, session: { active: true }, user }; }
  ...
  if ((match = route.match(/^\/attendance\/records\/(\d+)\/lateness$/)) && options.method === 'POST') {
    requireDemoOwner(); const record = findById(database, 'attendance_records', match[1]); const action = String(body.action || ''); const reason = String(body.reason || '').trim(); if (!record) throw formationDemoError('سجل الحضور غير موجود.', 'attendance_record_not_found'); if (!['waive', 'restore'].includes(action)) throw formationDemoError('إجراء التأخير غير صحيح.', 'invalid_lateness_action'); if (reason.length < 5) throw formationDemoError('سبب الإجراء مطلوب ويجب أن يكون واضحًا.', 'lateness_reason_required'); const checkInMinutes = record.check_in_at ? Number(String(record.check_in_at).slice(11, 13)) * 60 + Number(String(record.check_in_at).slice(14, 16)) : 0; const rawLate = record.check_in_at ? Math.max(0, checkInMinutes - 12 * 60) : Math.max(0, Number(record.late_minutes || 0)); const charge = calculateAttendanceLateCharge(rawLate); const month = String(record.work_date).slice(0, 7); const adjustments = tableRows(database, 'attendance_adjustments').filter(item => Number(item.attendance_record_id) === Number(record.id) && !item.voided_at && ['late_waiver', 'late_waiver_reversal'].includes(item.adjustment_type)); const waiverNetCents = adjustments.reduce((sum, item) => sum + Math.round(Number(item.amount || 0) * 100), 0); if (action === 'waive' && charge.amount <= 0) throw formationDemoError('هذا اليوم لا يحتوي على تأخير مستحق للخصم.', 'lateness_not_chargeable'); const amountCents = action === 'waive' ? -Math.max(0, Math.round(charge.amount * 100) + waiverNetCents) : Math.max(0, -waiverNetCents); const decorate = () => { const active = tableRows(database, 'attendance_adjustments').filter(item => Number(item.attendance_record_id) === Number(record.id) && !item.voided_at && ['late_waiver', 'late_waiver_reversal'].includes(item.adjustment_type)); const net = active.reduce((sum, item) => sum + Math.round(Number(item.amount || 0) * 100), 0); const grossCents = Math.round(charge.amount * 100); const waivedCents = Math.min(grossCents, Math.max(0, -net)); return { ...clone(record), late_raw_minutes: charge.units > 0 ? rawLate : 0, late_billable_half_hours: charge.units, late_billable_minutes: charge.billableMinutes, late_cost_gross: grossCents / 100, late_waiver_amount: waivedCents / 100, late_cost: Math.max(0, grossCents - waivedCents) / 100, late_waived: grossCents > 0 && waivedCents >= grossCents }; }; if (amountCents === 0) return { record: decorate(), idempotent: true }; const before = decorate(); const adjustment = addRow(database, 'attendance_adjustments', { user_id: record.user_id, attendance_record_id: record.id, adjustment_month: month, adjustment_type: action === 'waive' ? 'late_waiver' : 'late_waiver_reversal', amount: amountCents / 100, minutes: charge.billableMinutes, reason, created_by: 1, voided_at: null, replacement_adjustment_id: null }); const after = decorate(); demoAudit(database, action === 'waive' ? 'waive_lateness' : 'restore_lateness', 'attendance_records', record.id, before, { record: after, adjustment_id: adjustment.id, reason }); writeDatabase(database); return { record: after, adjustment_id: adjustment.id, idempotent: false };
  }
  ...
  if (route.startsWith('/formation-fund')) {
    ...
    if (route === '/formation-fund/contributions' && options.method === 'POST') {
      ...
      const entry = addRow(database, 'formation_fund_entries', { entry_type: 'contribution', founder_id: founder.id, amount: Number(amount.toFixed(2)), title: String(body.title).trim(), category: 'capital', payment_method: body.payment_method || 'cash', reference: body.reference || '', entry_date: body.entry_date, note: body.note || '', allocation_mode: null, status: 'active', created_by: 1 });
  ...
  if (route === '/users/assignees') return clone(database.users.filter(user => !['client', 'applicant'].includes(user.role)));
  ...
  if (route === '/users' && options.method === 'POST') {
    ...
    safeBody.phone = normalizeLoginPhone(body.phone);
    if (!safeBody.phone) throw formationDemoError('رقم الموبايل الأساسي مطلوب للدخول.', 'validation_error');
  ...
  if ((match = route.match(/^\/users\/(\d+)$/)) && options.method === 'PATCH') {
    ...
    if (Object.prototype.hasOwnProperty.call(body, 'phone')) { safeBody.phone = normalizeLoginPhone(body.phone); if (!safeBody.phone) throw formationDemoError('رقم الموبايل الأساسي مطلوب للدخول.', 'validation_error'); }
  ...
  if (route === '/clients' && options.method === 'POST') {
    ...
    const phone1 = normalizeClientPhone(body.phone1);
    if (!name || phone1.length < 10) throw formationDemoError('اسم العميل ورقم الهاتف الصحيح مطلوبان.', 'validation_error');
    const organizationId = 1;
    const duplicate = database.clients.some(client => Number(client.organization_id || 1) === organizationId && normalizeClientPhone(client.phone1) === phone1);
    ...
    let phones;
    try { phones = clientPhoneFields({ ...body, phone1 }); } catch (error) { throw formationDemoError(error.message, 'invalid_client_phone'); }
    const row = addRow(database, 'clients', { ...body, organization_id: organizationId, name, ...phones, preferred_contact: preferredContact, whatsapp_opt_in: body.whatsapp_opt_in == null ? 1 : Number(Boolean(Number(body.whatsapp_opt_in))), status: 'active', color, points: 0, debt: 0, credit: 0 });
  ...
  if ((match = route.match(/^\/client-packages\/(\d+)\/loyalty$/))) {
    requireDemoOwner(); const id = Number(match[1]); const pkg = database.client_packages.find(row => Number(row.id) === id); if (!pkg) throw formationDemoError('الباقة غير موجودة.', 'package_not_found');
    if ((options.method || 'GET') === 'GET') { const state = database.package_loyalty?.find(row => row.client_package_id === id); return { enabled: Boolean(state?.enabled), awarded_points: state?.awarded_points || 0 }; }
    if (options.method !== 'POST' || typeof body.enabled !== 'boolean' || typeof (body.include_paid ?? false) !== 'boolean') throw formationDemoError('إعداد الولاء غير صحيح.', 'invalid_loyalty_enabled');
    const response = syncDemoPackageLoyalty(database, id, body.enabled, body.include_paid ?? false); demoAudit(database, 'loyalty_setting', 'client_packages', id, null, response); writeDatabase(database); return response;
  }
  ...
  if (route === '/client-packages' && options.method === 'POST') {
    ...
    if (body.loyalty_enabled != null && typeof body.loyalty_enabled !== 'boolean') throw formationDemoError('إعداد الولاء غير صحيح.', 'invalid_loyalty_enabled');
    ...
    saleBookings.forEach((booking, index) => {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(booking.date) || startMinutes < 0 || endMinutes > 1440 || durationMinutes < minimum || durationMinutes % increment !== 0) throw formationDemoError('أحد المواعيد لا يطابق حدود الخدمة.', 'invalid_booking_time');
      ...
      if (body.__test_fail_at === 'conflict') throw formationDemoError('الموعد محجوز بالفعل.', 'booking_conflict'); assertDemoBookingAvailable(working, booking, null, body);
    ...
    demoAudit(working, normalizedUpgrade ? 'owner_upgrade_package_create' : 'create', 'client_packages', row.id, null, { ...clone(row), loyalty_enabled: body.loyalty_enabled, upgrade_from_package_id: normalizedUpgrade?.source_package_id || null, upgrade_reason: normalizedUpgrade?.reason || null }); failAt('audit');
  ...
  if ((match = route.match(/^\/client-packages\/(\d+)\/extend$/)) && options.method === 'POST') {
    requireDemoOwner(); const reason = demoReason(body); const pkg = findById(database, 'client_packages', match[1]); if (!pkg) throw formationDemoError('الباقة غير موجودة.', 'package_not_found'); if (body.expected_version != null && Number(body.expected_version) !== Number(pkg.version || 1)) throw formationDemoError('تم تحديث الباقة من شاشة أخرى. حدّث البيانات ثم أعد المحاولة.', 'stale_package_version');
    const expires = String(body.expires_at || '').slice(0, 10); const starts = String(pkg.starts_at || '').slice(0, 10); const mode = String(pkg.validity_mode_snapshot || 'rolling'); if (!/^\d{4}-\d{2}-\d{2}$/.test(expires) || !starts || expires < starts) throw formationDemoError('تاريخ الانتهاء يجب ألا يسبق تاريخ بداية الباقة.', 'invalid_package_expiry'); if (mode === 'shooting_day' && expires !== starts) throw formationDemoError('باقة يوم التصوير يجب أن تبدأ وتنتهي في اليوم نفسه.', 'shooting_day_date_mismatch'); const validityDays = mode === 'shooting_day' ? 1 : Math.round((Date.parse(`${expires}T00:00:00Z`) - Date.parse(`${starts}T00:00:00Z`)) / 86400000) + 1; if (validityDays < 1 || validityDays > 3650) throw formationDemoError('مدة الصلاحية يجب أن تكون بين يوم و3650 يومًا.', 'invalid_package_validity_days'); if (expires === String(pkg.expires_at || '').slice(0, 10) && validityDays === Number(pkg.validity_days_snapshot || 1)) return { id: pkg.id, expires_at: expires, validity_days_snapshot: validityDays, version: Number(pkg.version || 1), idempotent: true };
    const outside = database.bookings.some(booking => Number(booking.client_package_id) === Number(pkg.id) && ['pending','confirmed','alternative_proposed','cancel_requested','late_cancel_requested','in_progress'].includes(booking.status) && (booking.date > expires || (mode === 'shooting_day' && booking.date !== starts))); if (outside) throw formationDemoError('توجد مواعيد نشطة بعد تاريخ الانتهاء الجديد. عدّل المواعيد أولًا.', 'bookings_outside_package_validity');
    const before = clone(pkg); Object.assign(pkg, { expires_at: expires, validity_days_snapshot: validityDays, status: pkg.status === 'expired' && expires >= dateOnly() ? 'active' : pkg.status, version: Number(pkg.version || 1) + 1, updated_at: nowText() }); addRow(database, 'owner_adjustments', { entity_type: 'client_packages', entity_id: pkg.id, adjustment_type: 'validity', amount_delta_cents: 0, quantity_delta: 0, reason, before_data: before, after_data: clone(pkg) }); demoAudit(database, 'extend', 'client_packages', pkg.id, before, { ...clone(pkg), reason }); writeDatabase(database); return { id: pkg.id, expires_at: expires, validity_days_snapshot: validityDays, version: pkg.version, idempotent: false };
  }
  ...
  if ((match = route.match(/^\/client-packages\/(\d+)\/payments$/)) && options.method === 'POST') {
    ...
    const packageId = Number(match[1]); const rawAmount = String(body.amount ?? '').trim(); const method = String(body.method || '').trim(); const reference = String(body.reference || '').trim(); const note = String(body.note || '').trim(); const paymentDate = String(body.payment_date || dateOnly()).trim(); const key = String(body.idempotency_key || '').trim();
    ...
    if (!['cash','instapay','vodafone_cash'].includes(method)) throw formationDemoError('طريقة الدفع غير صحيحة.', 'invalid_payment_method');
    if (reference.length > 120) throw formationDemoError('مرجع الدفعة طويل جدًا.', 'payment_reference_too_long'); if (note.length > 500) throw formationDemoError('ملاحظات الدفعة طويلة جدًا.', 'payment_note_too_long'); if (!/^\d{4}-\d{2}-\d{2}$/.test(paymentDate) || Number.isNaN(Date.parse(`${paymentDate}T00:00:00Z`))) throw formationDemoError('تاريخ الدفعة غير صحيح.', 'invalid_payment_date');
    if (!/^[A-Za-z0-9._:-]{16,128}$/.test(key)) throw formationDemoError('مفتاح حماية الدفعة غير صالح.', 'invalid_idempotency_key');
    const amountCents = moneyToCents(rawAmount); const normalized = { client_package_id: packageId, amount: centsToMoney(amountCents), method, reference, note, payment_date: paymentDate }; const requestHash = JSON.stringify(normalized);
    ...
    addRow(working, 'finance', { type: 'إيراد', entry_kind: 'income', category: 'package_payment', client_id: pkg.client_id, amount: centsToMoney(amountCents), method, detail: note || `دفعة على الباقة: ${pkg.name}`, date: paymentDate, entity: 'الشركة', source_type: 'payment', source_id: payment.id, correlation_id: `payment:${payment.id}`, is_system: 1, created_by: 1, version: 1 }); failAt('finance');
    ...
    const response = { payment_id: payment.id, package_id: packageId, client_id: Number(pkg.client_id), amount: centsToMoney(amountCents), method, payment_date: paymentDate, paid_amount: pkg.paid_amount, outstanding: centsToMoney(outstandingCents), invoice_id: invoice?.id || null, idempotent: false }; Object.assign(request, { status: 'completed', response: clone(response), completed_at: nowText() }); writeDatabase(working); return response;
  }
  ...
  if ((match = route.match(/^\/client-packages\/(\d+)\/commercial-adjustment$/))) {
    requireDemoOwner(); const reason = demoReason(body); const pkg = findById(database, 'client_packages', match[1]); if (!pkg) throw formationDemoError('الباقة غير موجودة.', 'package_not_found'); if (body.expected_version != null && Number(body.expected_version) !== Number(pkg.version || 1)) throw formationDemoError('تم تحديث الباقة من شاشة أخرى. حدّث البيانات ثم أعد المحاولة.', 'stale_package_version'); for (const field of ['target_total_price','target_paid_amount']) if (body[field] != null && !/^\d+(?:\.\d{1,2})?$/.test(String(body[field]).trim())) throw formationDemoError('أدخل السعر والمدفوع بدقة قرشين كحد أقصى.', 'invalid_commercial_target'); const oldTotal = moneyToCents(pkg.total_price); const oldPaid = moneyToCents(pkg.paid_amount); const newTotal = moneyToCents(body.target_total_price ?? pkg.total_price); const newPaid = moneyToCents(body.target_paid_amount ?? pkg.paid_amount); const paidDelta = newPaid - oldPaid; if (paidDelta > 0 && !['cash','instapay','vodafone_cash'].includes(body.method)) throw formationDemoError('حدد طريقة التحصيل عند زيادة المدفوع.', 'payment_method_required'); const ambiguous = pkg.source_invoice_id && database.client_packages.filter(row => Number(row.source_invoice_id) === Number(pkg.source_invoice_id)).length > 1 && database.payment_allocations.some(row => Number(row.invoice_id) === Number(pkg.source_invoice_id) && !row.client_package_id); if (paidDelta && ambiguous) throw formationDemoError('الفاتورة القديمة تضم أكثر من باقة ولا تحتوي توزيعًا دقيقًا.', 'ambiguous_legacy_allocation'); const before = clone(pkg); pkg.total_price = centsToMoney(newTotal); pkg.paid_amount = centsToMoney(newPaid); pkg.version = Number(pkg.version || 1) + 1; const adjustment = addRow(database, 'owner_adjustments', { entity_type: 'client_packages', entity_id: pkg.id, adjustment_type: 'commercial', amount_delta_cents: paidDelta, quantity_delta: newTotal - oldTotal, reason, before_data: before, after_data: clone(pkg) }); if (paidDelta > 0) { const payment = addRow(database, 'payments', { client_id: pkg.client_id, amount: centsToMoney(paidDelta), method: body.method, status: 'approved', reference: `OWNER-ADJ-${adjustment.id}`, version: 1 }); addRow(database, 'payment_allocations', { client_id: pkg.client_id, payment_id: payment.id, client_package_id: pkg.id, invoice_id: pkg.source_invoice_id || null, amount: centsToMoney(paidDelta) }); addRow(database, 'finance', { type: 'إيراد', entry_kind: 'income', category: 'package_paid_correction', client_id: pkg.client_id, amount: centsToMoney(paidDelta), method: body.method, detail: `تصحيح مدفوع الباقة: ${pkg.name}`, date: dateOnly(), entity: 'الشركة', source_type: 'payment', source_id: payment.id, is_system: 1, version: 1 }); } else if (paidDelta < 0) addRow(database, 'finance', { type: 'قيد عكسي', entry_kind: 'reversal', category: 'reversal_income', client_id: pkg.client_id, amount: centsToMoney(Math.abs(paidDelta)), method: body.method || 'cash', detail: `خفض مدفوع الباقة: ${pkg.name}`, date: dateOnly(), entity: 'الشركة', source_type: 'owner_adjustment', source_id: adjustment.id, reversal_reason: reason, is_system: 1, version: 1 }); demoAudit(database, 'commercial_adjustment', 'client_packages', pkg.id, before, { ...clone(pkg), reason, adjustment_id: adjustment.id }); writeDatabase(database); const financial = packageFinancialSummary(pkg); return { id: pkg.id, adjustment_id: adjustment.id, version: pkg.version, financial: { total_price: pkg.total_price, paid_amount: pkg.paid_amount, remaining: centsToMoney(financial.outstandingCents), credit: centsToMoney(Math.max(0, financial.paidCents - financial.totalCents - financial.overageCents)) } };
  }
  if ((match = route.match(/^\/client-packages\/(\d+)$/)) && options.method === 'DELETE') {
    requireDemoOwner(); const reason = demoReason(body); const pkg = findById(database, 'client_packages', match[1]);
    if (!pkg) throw formationDemoError('الباقة غير موجودة.', 'package_not_found');
    if (body.confirmation !== 'حذف') throw formationDemoError('اكتب كلمة حذف لتأكيد حذف الباقة وسجلاتها.', 'hard_delete_confirmation_required');
    if (body.expected_version != null && Number(body.expected_version) !== Number(pkg.version || 1)) throw formationDemoError('تم تحديث الباقة من شاشة أخرى. حدّث البيانات ثم أعد المحاولة.', 'stale_package_version');
    const packageId = Number(pkg.id); const before = clone(pkg);
    const bookings = tableRows(database, 'bookings').filter(row => Number(row.client_package_id) === packageId); const bookingIds = new Set(bookings.map(row => Number(row.id)));
    if (tableRows(database, 'booking_sessions').some(row => bookingIds.has(Number(row.booking_id)) && row.status === 'active')) throw formationDemoError('أنه جلسة التصوير الجارية قبل حذف الباقة.', 'package_session_active');
    const allocations = tableRows(database, 'payment_allocations').filter(row => Number(row.client_package_id) === packageId); const allocationIds = new Set(allocations.map(row => Number(row.id)));
    const directProofs = tableRows(database, 'payment_proofs').filter(row => Number(row.client_package_id) === packageId); const candidatePaymentIds = new Set([...allocations, ...directProofs].map(row => Number(row.payment_id)).filter(Boolean));
    const paymentIds = new Set([...candidatePaymentIds].filter(paymentId => !tableRows(database, 'payment_allocations').some(row => Number(row.payment_id) === paymentId && Number(row.client_package_id) !== packageId) && !tableRows(database, 'payment_proofs').some(row => Number(row.payment_id) === paymentId && Number(row.client_package_id) !== packageId)));
    const proofIds = new Set(tableRows(database, 'payment_proofs').filter(row => Number(row.client_package_id) === packageId || paymentIds.has(Number(row.payment_id))).map(row => Number(row.id)));
    const adjustmentIds = new Set(tableRows(database, 'owner_adjustments').filter(row => row.entity_type === 'client_packages' && Number(row.entity_id) === packageId).map(row => Number(row.id)));
    const financeIds = new Set(tableRows(database, 'finance').filter(row => (['client_package','client_packages','package'].includes(row.source_type) && Number(row.source_id) === packageId) || (row.source_type === 'payment' && paymentIds.has(Number(row.source_id))) || (row.source_type === 'payment_proof' && proofIds.has(Number(row.source_id))) || (row.source_type === 'owner_adjustment' && adjustmentIds.has(Number(row.source_id)))).map(row => Number(row.id)));
    const settlementIds = new Set(tableRows(database, 'session_settlements').filter(row => bookingIds.has(Number(row.booking_id))).map(row => Number(row.id)));
    const deleted = { bookings: bookingIds.size, ledger: tableRows(database, 'package_usage_ledger').filter(row => Number(row.client_package_id) === packageId).length, payment_allocations: allocationIds.size, payment_proofs: proofIds.size, payments: paymentIds.size, finance: financeIds.size };
    const invoiceDeltas = new Map(); allocations.forEach(row => { if (row.invoice_id) invoiceDeltas.set(Number(row.invoice_id), (invoiceDeltas.get(Number(row.invoice_id)) || 0) + moneyToCents(row.amount)); });
    database.post_production_jobs = tableRows(database, 'post_production_jobs').filter(row => !bookingIds.has(Number(row.booking_id)));
    database.session_settlement_allocations = tableRows(database, 'session_settlement_allocations').filter(row => !settlementIds.has(Number(row.settlement_id)) && Number(row.source_client_package_id) !== packageId && Number(row.target_client_package_id) !== packageId);
    database.session_settlements = tableRows(database, 'session_settlements').filter(row => !settlementIds.has(Number(row.id)));
    database.booking_sessions = tableRows(database, 'booking_sessions').filter(row => !bookingIds.has(Number(row.booking_id)));
    database.booking_archives = tableRows(database, 'booking_archives').filter(row => Number(row.client_package_id) !== packageId && !bookingIds.has(Number(row.booking_id)));
    database.reschedule_requests = tableRows(database, 'reschedule_requests').filter(row => !bookingIds.has(Number(row.booking_id)));
    database.booking_status_history = tableRows(database, 'booking_status_history').filter(row => !bookingIds.has(Number(row.booking_id)));
    database.bookings = tableRows(database, 'bookings').filter(row => !bookingIds.has(Number(row.id)));
    database.projects = tableRows(database, 'projects').map(row => Number(row.client_package_id) === packageId ? { ...row, client_package_id: null } : row);
    database.finance = tableRows(database, 'finance').filter(row => !financeIds.has(Number(row.id)));
    database.payment_allocations = tableRows(database, 'payment_allocations').filter(row => !allocationIds.has(Number(row.id)));
    database.payment_proofs = tableRows(database, 'payment_proofs').filter(row => !proofIds.has(Number(row.id)));
    database.payments = tableRows(database, 'payments').filter(row => !paymentIds.has(Number(row.id)));
    database.invoices = tableRows(database, 'invoices').map(row => { const delta = invoiceDeltas.get(Number(row.id)) || 0; if (!delta) return row; const paid = Math.max(0, moneyToCents(row.paid_amount) - delta); return { ...row, paid_amount: centsToMoney(paid), status: ['cancelled','void'].includes(row.status) ? row.status : paid <= 0 ? 'issued' : paid >= moneyToCents(row.total) ? 'paid' : 'partial' }; });
    database.client_package_payment_requests = tableRows(database, 'client_package_payment_requests').filter(row => Number(row.client_package_id) !== packageId);
    database.owner_adjustments = tableRows(database, 'owner_adjustments').filter(row => !(row.entity_type === 'client_packages' && Number(row.entity_id) === packageId));
    database.package_usage_ledger = tableRows(database, 'package_usage_ledger').filter(row => Number(row.client_package_id) !== packageId);
    database.app_notifications = tableRows(database, 'app_notifications').filter(row => !(Number(row.entity_id) === packageId && ['client_package','client_packages','package'].includes(row.entity_type)) && !(bookingIds.has(Number(row.entity_id)) && ['booking','bookings'].includes(row.entity_type)));
    database.change_events = tableRows(database, 'change_events').filter(row => !(Number(row.entity_id) === packageId && ['client_package','client_packages','package'].includes(row.entity_type)) && !(bookingIds.has(Number(row.entity_id)) && ['booking','bookings'].includes(row.entity_type)));
    database.client_packages = tableRows(database, 'client_packages').filter(row => Number(row.id) !== packageId);
    demoAudit(database, 'owner_cascade_delete', 'client_packages', packageId, before, { reason, deleted_records: deleted }); writeDatabase(database);
    return { id: packageId, deleted: true, archived: false, deleted_records: deleted };
  }
  ...
  if (route === '/projects/custom-service' && options.method === 'POST') {
    ...
    if (requiresBooking) {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || start < 0 || end > 1440 || duration < 60 || duration % 15 !== 0) throw formationDemoError('الموعد غير صالح. الحجز متاح كل أيام الأسبوع وعلى مدار اليوم.', 'invalid_project_booking');
      bookingDraft = { resource_id: Number(resource.id), date, start_time: booking.start_time, end_time: booking.end_time };
      assertDemoBookingAvailable(draft, bookingDraft, null, body);
    }
  ...
  if (route === '/client/projects' && (options.method || 'GET') === 'GET') {
    const clientId = currentDemoClientId(database);
  ...
  if (route === '/client/service-history' && (options.method || 'GET') === 'GET') {
    ...
    return buildDemoClientServiceHistory(database, Object.fromEntries(url.searchParams.entries()), currentDemoClientId(database));
  }
  ...
  if (route === '/booking-blocks' && (options.method || 'GET') === 'GET') {
    ...
    const rangeStart = new Date(`${from}T12:00:00Z`), rangeEnd = new Date(`${to}T12:00:00Z`);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(from) || !/^\d{4}-\d{2}-\d{2}$/.test(to) || !Number.isFinite(rangeStart.getTime()) || !Number.isFinite(rangeEnd.getTime()) || rangeEnd < rangeStart || (rangeEnd - rangeStart) / 86400000 > 730) throw formationDemoError('فترة عرض حظر المواعيد غير صحيحة.', 'invalid_booking_block_range');
    materializeDemoBlocks(database, from, to); writeDatabase(database);
  ...
  if (route === '/booking-blocks' && options.method === 'POST') {
    ...
    const startDate = String(body.date || ''); const repeat = Boolean(body.repeat_daily); const repeatUntil = body.repeat_end_mode === 'never' ? '' : String(body.repeat_until || ''); const startTime = String(body.start_time || '').slice(0, 5); const endTime = String(body.end_time || '').slice(0, 5); const resourceId = Number(body.resource_id || 0); const idempotencyKey = String(body.idempotency_key || '').trim(); const title = String(body.title || '').trim().slice(0, 120) || 'حجز مؤقت'; const note = String(body.note || '').trim().slice(0, 1000);
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(startDate) || startMinutes < 0 || endMinutes > 1440 || endMinutes <= startMinutes || duration < 60 || duration % 15 !== 0) throw formationDemoError('فترة الحظر غير صحيحة.', 'invalid_booking_block_time');
    ...
    const normalized = { date: startDate, start_time: startTime, end_time: endTime, resource_id: resourceId, repeat_daily: repeat, repeat_until: repeat ? repeatUntil : null, title, note }; const requestHash = demoSettlementHash(normalized); const replay = database.booking_blocks.find(item => belongsToDemoOrganization(item) && item.idempotency_key === idempotencyKey);
    ...
    const first = new Date(`${startDate}T12:00:00`); const last = new Date(`${repeat && repeatUntil ? repeatUntil : startDate}T12:00:00`); if (Number.isNaN(first.getTime()) || Number.isNaN(last.getTime()) || last < first) throw formationDemoError(repeat ? 'حدد تاريخ نهاية التكرار.' : 'تاريخ الحظر غير صحيح.', repeat ? 'booking_block_repeat_until_required' : 'invalid_booking_block_date');
    if (repeat && body.repeat_end_mode === 'date' && !repeatUntil) throw formationDemoError('حدد تاريخ نهاية التكرار.', 'booking_block_repeat_until_required');
    const inclusiveDays = Math.round((last - first) / 86400000) + 1;
    const dates = []; for (const cursor = new Date(first); cursor <= (inclusiveDays > 90 ? first : last); cursor.setDate(cursor.getDate() + 1)) dates.push(`${cursor.getFullYear()}-${pad(cursor.getMonth() + 1)}-${pad(cursor.getDate())}`);
    const validationDates = new Set(dates);
    if (repeat) {
      const until = repeatUntil || '9999-12-31';
      for (const row of [...database.bookings, ...database.booking_blocks].filter(belongsToDemoOrganization)) { const day = row.block_date || row.date; if (day >= startDate && day <= until) validationDates.add(day); }
      const conflict = firstSeriesOverlap(database, demoOrganizationId, resourceId, startDate, repeatUntil, startTime, endTime);
      if (conflict) throw formationDemoError(`يتعارض التكرار مع حجز مؤقت يوم ${conflict}.`, 'booking_block_conflict');
    }
    for (const blockDate of validationDates) { const availability = getBookingAvailability({ date: blockDate, start_time: startTime, end_time: endTime, resource_id: resourceId }, database.bookings.filter(belongsToDemoOrganization), { blocks: calendarBlocks(database, demoOrganizationId, blockDate, blockDate) }); if (availability.status !== 'available') { const error = formationDemoError(`إحدى الفترات غير متاحة يوم ${blockDate}. لم يتم إنشاء أي حظر.`, availability.status === 'blocked' ? 'booking_block_conflict' : 'booking_conflict'); error.status = 409; throw error; } }
    const working = clone(database); const seriesKey = repeat ? `demo-series-${Date.now()}-${Math.random().toString(16).slice(2)}` : null; const items = dates.map((blockDate, index) => { const block = addRow(working, 'booking_blocks', { organization_id: demoOrganizationId, resource_id: resourceId, resource_name: resource.name, block_date: blockDate, start_time: startTime, end_time: endTime, duration_minutes: duration, title, note: note || null, series_key: seriesKey, idempotency_key: index ? `${idempotencyKey}:${index}` : idempotencyKey, request_hash: requestHash, response_json: null, status: 'active', created_by: demoUserId, cancelled_by: null, cancelled_at: null }); for (let minute = startMinutes; minute < endMinutes; minute += 15) addRow(working, 'booking_slots', { organization_id: demoOrganizationId, booking_id: null, booking_block_id: block.id, resource_id: resourceId, slot_date: blockDate, slot_start: `${pad(Math.floor(minute / 60))}:${pad(minute % 60)}:00` }); return block; });
    if (repeat) addRow(working, 'booking_block_series', { organization_id: demoOrganizationId, resource_id: resourceId, series_key: seriesKey, starts_on: startDate, repeat_until: repeatUntil || null, start_time: startTime, end_time: endTime, duration_minutes: duration, title, note: note || null, status: 'active', created_by: demoUserId });
    const response = { items: clone(items), count: repeat ? (repeatUntil ? inclusiveDays : null) : 1, repeat_daily: repeat, repeat_until: repeatUntil || null, series_key: seriesKey, idempotent: false, skipped_fridays: 0 }; working.booking_blocks.find(item => item.idempotency_key === idempotencyKey).response_json = clone(response); demoAuditBookingBlockChange(working, 'create', items[0]?.id, null, { resource_id: resourceId, dates, start_time: startTime, end_time: endTime, series_key: seriesKey }); writeDatabase(working); return response;
  }
  if ((match = route.match(/^\/booking-blocks\/(\d+)\/convert$/)) && options.method === 'POST') {
    if (!['owner', 'admin', 'operations'].includes(demoRole)) throw formationDemoError('ليس لديك صلاحية لتحويل الحجز المؤقت.', 'forbidden');
    const selected = database.booking_blocks.find(item => Number(item.id) === Number(match[1]) && belongsToDemoOrganization(item)); if (!selected) throw formationDemoError('الحجز المؤقت غير موجود.', 'booking_block_not_found');
    const mode = body.package_mode === 'new_package' ? 'new_package' : 'existing_package'; const nextPackage = mode === 'new_package' && body.new_package && typeof body.new_package === 'object' ? body.new_package : {}; const key = String(body.idempotency_key || '').trim(); if (!/^[A-Za-z0-9:_-]{12,120}$/.test(key)) throw formationDemoError('مفتاح حفظ التحويل غير صحيح.', 'invalid_idempotency_key');
    const normalizedNew = mode === 'new_package' ? { service_id: Number(nextPackage.service_id), name: String(nextPackage.name || '').trim(), billing_unit: String(nextPackage.billing_unit || ''), quantity: String(nextPackage.quantity ?? ''), validity_days: String(nextPackage.validity_days ?? ''), payment_due_quantity: String(nextPackage.payment_due_quantity ?? ''), deposit_percent_snapshot: String(nextPackage.deposit_percent_snapshot ?? ''), overage_price_snapshot: String(nextPackage.overage_price_snapshot ?? ''), total_price: String(nextPackage.total_price ?? ''), paid_amount: String(nextPackage.paid_amount ?? ''), payment_method: String(nextPackage.payment_method || ''), notes: String(nextPackage.notes || '').trim() } : null;
    const hash = demoSettlementHash({ block_id: Number(selected.id), client_id: Number(body.client_id), package_mode: mode, client_package_id: mode === 'existing_package' ? Number(body.client_package_id) : 0, new_package: normalizedNew }); const hasConversion = Boolean(selected.conversion_idempotency_key || selected.conversion_request_hash || selected.conversion_response_json);
    if (hasConversion) { if (selected.conversion_idempotency_key !== key || selected.conversion_request_hash !== hash) throw formationDemoError('تم تحويل هذا الحجز المؤقت من قبل.', 'booking_block_already_converted'); return { ...clone(selected.conversion_response_json), idempotent: true }; }
    const client = findById(database, 'clients', body.client_id); if (selected.status !== 'active') throw formationDemoError('الحجز المؤقت ملغى.', 'booking_block_inactive'); if (!client || client.status !== 'active') throw formationDemoError('العميل غير نشط.', 'invalid_booking_client'); const duration = Number(selected.duration_minutes || 0); if (duration < 15 || duration % 15 !== 0) throw formationDemoError('مدة الحجز المؤقت لا تطابق شبكة الحجز.', 'invalid_booking_time');
    let sourcePackage = null; let service; let preparedPackage = null; let paidCents; let totalCents; let overageCents; let validityMode; let validityDays; let purchasedMinutes; let paymentDueMinutes;
    if (mode === 'existing_package') {
      sourcePackage = findById(database, 'client_packages', body.client_package_id); service = findById(database, 'services', sourcePackage?.service_id); if (!sourcePackage || Number(sourcePackage.client_id) !== Number(client.id) || sourcePackage.status !== 'active' || sourcePackage.billing_unit !== 'hour') throw formationDemoError('اختر باقة ساعات نشطة تخص العميل.', 'invalid_target_package');
      const starts = String(sourcePackage.starts_at || '').slice(0, 10); const expires = String(sourcePackage.expires_at || '').slice(0, 10); if (Boolean(starts) !== Boolean(expires)) throw formationDemoError('تواريخ صلاحية الباقة غير مكتملة.', 'invalid_package_dates'); if (starts && (selected.block_date < starts || selected.block_date > expires)) throw formationDemoError('الحجز المؤقت خارج صلاحية الباقة.', 'booking_outside_package_validity');
      const minimum = Math.max(15, Number(service?.minimum_booking_minutes || 60)); const increment = Math.max(15, Number(service?.booking_increment_minutes || 15)); if (duration < minimum || duration % increment !== 0) throw formationDemoError('مدة الحجز لا تطابق قالب الباقة.', 'invalid_booking_time'); if (demoPackageMinutes(sourcePackage, 'purchased') - demoPackageMinutes(sourcePackage, 'consumed') - demoPackageMinutes(sourcePackage, 'held') < duration) throw formationDemoError('رصيد الباقة لا يكفي.', 'insufficient_package_balance');
    } else {
      service = findById(database, 'services', nextPackage.service_id); if (!belongsToDemoOrganization(service) || !isSellablePackageTemplate(service) || normalizedPackageUnit(service) !== 'hour') throw formationDemoError('اختر قالب باقة ساعات نشطًا وقابلًا للبيع.', 'invalid_package_template'); if (nextPackage.billing_unit !== 'hour') throw formationDemoError('وحدة الباقة الجديدة يجب أن تكون ساعات.', 'invalid_billing_unit');
      const minimum = Math.max(15, Number(service.minimum_booking_minutes || 60)); const increment = Math.max(15, Number(service.booking_increment_minutes || 15)); if (duration < minimum || duration % increment !== 0) throw formationDemoError('مدة الحجز لا تطابق قالب الباقة.', 'invalid_booking_time');
      const quantity = Number(nextPackage.quantity); const paymentDue = Number(nextPackage.payment_due_quantity || 0); purchasedMinutes = demoSettlementMinutes(quantity); paymentDueMinutes = demoSettlementMinutes(paymentDue); if (!(quantity > 0) || Math.abs(quantity * 60 - purchasedMinutes) > .0001 || purchasedMinutes % 15 !== 0 || purchasedMinutes < duration) throw formationDemoError('رصيد الباقة الجديدة يجب أن يغطي الحجز وبزيادات 15 دقيقة.', 'insufficient_package_balance'); if (paymentDue < 0 || paymentDue > quantity || Math.abs(paymentDue * 60 - paymentDueMinutes) > .0001) throw formationDemoError('حد الاستحقاق غير صحيح.', 'invalid_payment_due_quantity');
      const dailyAliases = new Set(['daily', 'daily package', 'day package', 'باقة يومية', 'باقات يومية', 'الباقات اليومية', 'باقة اليوم']); validityMode = service.package_validity_mode === 'shooting_day' || dailyAliases.has(String(service.category || '').trim().toLowerCase()) ? 'shooting_day' : 'rolling'; validityDays = validityMode === 'shooting_day' ? 1 : Number(nextPackage.validity_days); if (!Number.isInteger(validityDays) || validityDays < 1 || validityDays > 3650) throw formationDemoError('مدة صلاحية الباقة غير صحيحة.', 'invalid_package_dates'); const deposit = Number(nextPackage.deposit_percent_snapshot); if (!Number.isFinite(deposit) || deposit < 0 || deposit > 100) throw formationDemoError('نسبة المقدم غير صحيحة.', 'invalid_deposit_percent');
      for (const [field, label] of [['overage_price_snapshot', 'سعر الساعة الإضافية'], ['total_price', 'إجمالي السعر'], ['paid_amount', 'المدفوع الآن']]) { if (!Object.prototype.hasOwnProperty.call(nextPackage, field)) throw formationDemoError(`حقل ${label} مطلوب كنص عشري صريح.`, 'invalid_money_format'); demoStrictMoneyCents(nextPackage[field], label); }
      overageCents = demoStrictMoneyCents(nextPackage.overage_price_snapshot, 'سعر الساعة الإضافية'); totalCents = demoStrictMoneyCents(nextPackage.total_price, 'إجمالي السعر'); paidCents = demoStrictMoneyCents(nextPackage.paid_amount, 'المدفوع الآن'); if (paidCents > totalCents) throw formationDemoError('المدفوع الآن لا يمكن أن يتجاوز إجمالي السعر.', 'invalid_payment_amount'); if (!['cash', 'instapay', 'vodafone_cash'].includes(nextPackage.payment_method)) throw formationDemoError('طريقة الدفع غير صحيحة.', 'invalid_payment_method'); const name = String(nextPackage.name || '').trim(); if (!name || name.length > 180) throw formationDemoError('اسم الباقة غير صحيح.', 'invalid_package_name');
      preparedPackage = { client_id: client.id, service_id: service.id, name, notes: String(nextPackage.notes || '').trim(), billing_unit: 'hour', purchased_quantity: demoSettlementHours(purchasedMinutes), purchased_minutes: purchasedMinutes, held_quantity: 0, held_minutes: 0, consumed_quantity: 0, consumed_minutes: 0, payment_due_quantity: demoSettlementHours(paymentDueMinutes), payment_due_minutes: paymentDueMinutes, deposit_percent_snapshot: deposit, overage_price_snapshot: centsToMoney(overageCents), total_price: centsToMoney(totalCents), paid_amount: centsToMoney(paidCents), starts_at: selected.block_date, expires_at: demoPackageExpiry(selected.block_date, validityDays, validityMode), validity_mode_snapshot: validityMode, validity_days_snapshot: validityDays, status: 'active', version: 1 };
    }
    const working = clone(database); const failAt = stage => { if (body.__test_fail_at === stage) throw formationDemoError('تعطل تجريبي قبل اعتماد العملية.', 'demo_fault_injected'); }; const block = findById(working, 'booking_blocks', selected.id); let target = mode === 'existing_package' ? findById(working, 'client_packages', sourcePackage.id) : addRow(working, 'client_packages', preparedPackage); failAt('package');
    if (mode === 'new_package') { addDemoPackageUsage(working, target, { movement_type: 'opening', quantity: target.purchased_quantity, quantity_minutes: purchasedMinutes, reason: 'إنشاء الباقة أثناء تحويل حجز مؤقت', event_key: `package:${target.id}:opening` }); failAt('opening'); } else activateDemoPackageOnFirstBooking(working, target, block.block_date);
    const booking = addRow(working, 'bookings', { client_id: client.id, client_name: client.name, client_package_id: target.id, service_id: target.service_id, resource_id: block.resource_id, resource_name: block.resource_name, service: target.name, date: block.block_date, start_time: block.start_time, end_time: block.end_time, duration_minutes: duration, requested_quantity: demoSettlementHours(duration), status: 'confirmed', notes: block.note || '' }); failAt('booking'); if (String(block.idempotency_key).startsWith('recurrence:') && !working.booking_slots.some(slot => Number(slot.booking_block_id) === Number(block.id))) { const [hour, minute] = block.start_time.split(':').map(Number); for (let part = hour * 60 + minute; part < hour * 60 + minute + duration; part += 15) addRow(working, 'booking_slots', { organization_id: demoOrganizationId, booking_id: null, booking_block_id: block.id, resource_id: block.resource_id, slot_date: block.block_date, slot_start: `${pad(Math.floor(part / 60))}:${pad(part % 60)}:00` }); } const slots = working.booking_slots.filter(slot => Number(slot.booking_block_id) === Number(block.id)); if (slots.length !== duration / 15) throw formationDemoError('تغيرت خانات الموعد أثناء التحويل.', 'booking_slot_transfer_conflict'); slots.forEach(slot => Object.assign(slot, { booking_id: booking.id, booking_block_id: null })); mutateDemoPackageQuantities(target, { held_minutes: duration }); addDemoPackageUsage(working, target, { booking_id: booking.id, movement_type: 'hold', quantity: demoSettlementHours(duration), quantity_minutes: duration, reason: 'تحويل حجز مؤقت', event_key: `booking:${booking.id}:hold` }); addRow(working, 'booking_status_history', { booking_id: booking.id, from_status: null, to_status: 'confirmed', note: mode === 'new_package' ? 'بيع باقة وتحويل الحجز المؤقت' : 'تحويل الحجز المؤقت إلى حجز عميل', changed_by: demoUserId }); failAt('hold');
    let payment = null; if (mode === 'new_package' && paidCents > 0) { payment = addRow(working, 'payments', { client_id: client.id, client_name: client.name, amount: centsToMoney(paidCents), method: nextPackage.payment_method, status: 'approved', reference: `package-${target.id}-opening`, reviewed_by: demoUserId, reviewed_at: nowText(), version: 1 }); addRow(working, 'payment_allocations', { client_id: client.id, payment_id: payment.id, payment_proof_id: null, client_package_id: target.id, invoice_id: null, amount: centsToMoney(paidCents) }); addRow(working, 'finance', { type: 'إيراد', entry_kind: 'income', category: 'package_payment', client_id: client.id, amount: centsToMoney(paidCents), method: nextPackage.payment_method, detail: `دفعة إنشاء باقة ${target.name}`, date: dateOnly(), entity: 'الشركة', source_type: 'payment', source_id: payment.id, correlation_id: `payment:${payment.id}`, is_system: 1, version: 1 }); } failAt('finance');
    const response = { booking: clone(booking), package_id: target.id, package: mode === 'new_package' ? clone(target) : null, payment_id: payment?.id || null, package_mode: mode, block_id: block.id, held_minutes: duration, remaining_minutes: demoPackageMinutes(target, 'purchased') - demoPackageMinutes(target, 'consumed') - demoPackageMinutes(target, 'held'), idempotent: false }; Object.assign(block, { status: 'cancelled', converted_booking_id: booking.id, conversion_idempotency_key: key, conversion_request_hash: hash, conversion_response_json: clone(response), cancelled_by: demoUserId, cancelled_at: nowText() }); demoAuditBookingBlockChange(working, mode === 'new_package' ? 'convert_with_new_package' : 'convert', block.id, selected, response); if (mode === 'new_package') demoAudit(working, 'create', 'client_packages', target.id, null, clone(target)); demoAudit(working, 'create', 'bookings', booking.id, null, clone(booking)); failAt('audit'); writeDatabase(working); return response;
  }
  if ((match = route.match(/^\/booking-blocks\/(\d+)$/)) && options.method === 'DELETE') {
    if (!['owner', 'admin', 'operations'].includes(demoRole)) throw formationDemoError('ليس لديك صلاحية لإلغاء حظر المواعيد.', 'forbidden'); const scope = url.searchParams.get('scope') || 'single'; if (!['single', 'series'].includes(scope)) throw formationDemoError('نطاق الإلغاء غير صحيح.', 'invalid_booking_block_scope'); const selected = database.booking_blocks.find(item => Number(item.id) === Number(match[1]) && belongsToDemoOrganization(item) && item.status === 'active'); if (!selected) throw formationDemoError('فترة الحظر غير موجودة.', 'booking_block_not_found'); const working = clone(database); if (scope === 'series' && selected.series_key) { const rule = (working.booking_block_series || []).find(row => belongsToDemoOrganization(row) && row.series_key === selected.series_key); if (rule) { rule.repeat_until = shiftBookingDate(selected.block_date, -1); if (rule.repeat_until < rule.starts_on) rule.status = 'cancelled'; } } const ids = working.booking_blocks.filter(item => Number(item.id) === Number(selected.id) || scope === 'series' && selected.series_key && item.series_key === selected.series_key && item.status === 'active' && item.block_date >= selected.block_date).map(item => Number(item.id)); working.booking_blocks.forEach(item => { if (ids.includes(Number(item.id))) Object.assign(item, { status: 'cancelled', cancelled_by: demoUserId, cancelled_at: nowText() }); }); working.booking_slots = working.booking_slots.filter(slot => !ids.includes(Number(slot.booking_block_id))); demoAuditBookingBlockChange(working, scope === 'series' ? 'cancel_series' : 'cancel', selected.id, clone(selected), { cancelled_ids: ids, scope }); writeDatabase(working); return { cancelled: ids.length, ids, scope };
  }
  if (route === '/client/studio-availability' && (options.method || 'GET') === 'GET') { if (demoRole !== 'client') throw formationDemoError('غير مصرح.', 'forbidden'); const service = (await registrationDemoCatalog(database)).services.find(row => Number(row.id) === Number(url.searchParams.get('service_id'))); if (!service) throw formationDemoError('اختر باقة متاحة.', 'invalid_service'); if (service.kind !== 'hourly' && Number(url.searchParams.get('duration_minutes')) > Number(service.total_hours) * 60) throw formationDemoError('المدة تتجاوز رصيد الباقة.', 'insufficient_package_balance'); return demoCalendarDays(database, url); }
  ...
  if (route === '/bookings/request' && options.method === 'POST') {
    const clientId = demoRole === 'client' ? currentDemoClientId(database) : Number(body.client_id || 0);
    ...
    const clientGrid = demoRole === 'client' ? validateDemoClientBookingGrid(body) : null;
    if (demoRole === 'client') { const dateError = clientBookingDateError(body.date); if (dateError) throw formationDemoError(dateError, 'same_day_booking_not_allowed'); if (demoClientHasDay(database, clientId, body.date)) throw formationDemoError('لديك موعد أو طلب في هذا اليوم.', 'client_day_already_booked'); }
    const start = clientGrid?.start ?? (Number(String(body.start_time).slice(0, 2)) * 60 + Number(String(body.start_time).slice(3, 5)));
    let end = clientGrid?.end ?? (Number(String(body.end_time).slice(0, 2)) * 60 + Number(String(body.end_time).slice(3, 5))); if (end === 0) end = 1440;
    if (demoRole === 'client' && pkg?.billing_unit === 'hour' && pkg.validity_mode_snapshot === 'shooting_day' && end - start !== demoPackageMinutes(pkg, 'purchased')) throw formationDemoError('يجب حجز ساعات يوم التصوير كاملة في جلسة واحدة.', 'daily_full_duration_required');
    ...
    assertDemoBookingAvailable(database, { date: body.date, start_time: body.start_time, end_time: body.end_time, resource_id: resourceId }, null, body);
    ...
    if (demoRole === 'client') { const check = new URL('http://demo/client/booking-availability'); check.search = new URLSearchParams({ client_package_id: String(pkg.id), duration_minutes: String(end - start), start_date: body.date, days: '1' }); const day = demoClientBookingAvailability(database, check).days[0]; if (!day.available || !day.slots.some(slot => slot.start_time === String(body.start_time).slice(0,5) && slot.end_time === String(body.end_time).slice(0,5))) throw formationDemoError('هذا الموعد غير متاح.', 'booking_conflict'); }
    ...
    const row = addRow(database, 'bookings', { client_id: clientId, client_name: client?.name || 'عميل تجريبي', client_package_id: pkg?.id || null, service_id: body.service_id, resource_id: resourceId, resource_name: 'الاستديو الرئيسي', service: body.service || service?.name || 'جلسة تصوير', date: body.date, start_time: body.start_time, end_time: body.end_time, duration_minutes: end - start, status, requested_quantity: quantity, requested_reels: body.requested_reels || 0, notes: body.notes || '', payment: 0 });
  ...
  if ((match = route.match(/^\/bookings\/(\d+)\/cancel-request$/))) { const booking = findById(database, 'bookings', match[1]); if (!booking || demoRole === 'client' && Number(booking.client_id) !== currentDemoClientId(database)) throw formationDemoError('الموعد غير موجود.', 'booking_not_found'); if (demoRole === 'client' && booking.status === 'confirmed' && clientNoticeIsLate(booking)) throw formationDemoError('الإلغاء قبل 48 ساعة على الأقل؛ الموعد يظل مؤكدًا وتُخصم مدته من الباقة.', 'late_cancellation'); const before = clone(booking); booking.status = 'cancel_requested'; demoAudit(database, 'cancel_request', 'bookings', booking.id, before, { ...clone(booking), client_id: booking.client_id }); writeDatabase(database); return booking; }
  if ((match = route.match(/^\/bookings\/(\d+)\/admin-reschedule$/))) { const booking = findById(database, 'bookings', match[1]); if (!booking || booking.status !== 'confirmed') throw formationDemoError('الحجز غير موجود أو حالته لا تسمح بتعديل الموعد.', 'invalid_booking_state'); const pkg = booking.client_package_id ? findById(database, 'client_packages', booking.client_package_id) : null; const resource = findById(database, 'resources', body.resource_id ?? booking.resource_id); const date = String(body.date || ''); if (!resource || Number(resource.is_active ?? 1) !== 1) throw formationDemoError('المورد المختار غير متاح.', 'invalid_booking_resource'); if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw formationDemoError('الموعد لا يطابق أيام العمل.', 'invalid_booking_time'); if (pkg && (date < String(pkg.starts_at).slice(0, 10) || date > String(pkg.expires_at).slice(0, 10) || (pkg.validity_mode_snapshot === 'shooting_day' && date !== String(pkg.starts_at).slice(0, 10)))) throw formationDemoError('الموعد الجديد خارج صلاحية الباقة.', 'booking_outside_package_validity'); const nextBase = { ...booking, resource_id: Number(resource.id), date, start_time: body.start_time, end_time: body.end_time }; assertDemoBookingAvailable(database, nextBase, booking.id, body); if (pkg) { const oldHold = demoBookingHeldQuantity(database, booking.id, pkg.id); const oldMinutes = pkg.billing_unit === 'hour' ? demoBookingHeldMinutes(database, booking.id, pkg.id) : null; const nextMinutes = pkg.billing_unit === 'hour' ? demoBookingDurationMinutes(nextBase) : null; const nextQuantity = pkg.billing_unit === 'hour' ? demoSettlementHours(nextMinutes) : Number(booking.requested_quantity || oldHold); const delta = nextQuantity - oldHold; if (delta > demoPackageAvailable(pkg) + 0.000001) throw formationDemoError('رصيد الباقة المتاح لا يكفي للمدة الجديدة.', 'insufficient_package_balance'); mutateDemoPackageQuantities(pkg, pkg.billing_unit === 'hour' ? { held_minutes: nextMinutes - oldMinutes } : { held: delta }); if (oldHold > 0) addDemoPackageUsage(database, pkg, { booking_id: booking.id, movement_type: 'release', quantity: oldHold, quantity_minutes: oldMinutes, reason: 'تحرير حجز الموعد السابق', event_key: `booking:${booking.id}:reschedule-release:${Date.now()}` }); addDemoPackageUsage(database, pkg, { booking_id: booking.id, movement_type: 'hold', quantity: nextQuantity, quantity_minutes: nextMinutes, reason: 'حجز الموعد الجديد', event_key: `booking:${booking.id}:reschedule-hold:${Date.now()}` }); booking.requested_quantity = nextQuantity; } Object.assign(booking, { resource_id: Number(resource.id), resource_name: resource.name, date, start_time: body.start_time, end_time: body.end_time, notes: Object.prototype.hasOwnProperty.call(body, 'notes') ? body.notes : booking.notes }); demoAudit(database, 'admin_reschedule', 'bookings', booking.id, null, clone(booking)); writeDatabase(database); return booking; }
  ...
  if (route === '/studio-sessions/start-direct' && options.method === 'POST') {
    if (!['owner', 'admin', 'operations'].includes(demoRole)) throw formationDemoError('ليس لديك صلاحية لبدء جلسة التصوير.', 'forbidden'); const today = cairoDateKey(); if (String(body.date || today) !== today) throw formationDemoError('يمكن بدء جلسة مباشرة اليوم فقط.', 'session_date_mismatch'); const client = findById(database, 'clients', body.client_id); const resource = findById(database, 'resources', body.resource_id); if (!client || client.status !== 'active') throw formationDemoError('اختر عميلًا نشطًا.', 'invalid_booking_client'); if (!resource || Number(resource.is_active ?? 1) !== 1) throw formationDemoError('الاستديو غير متاح.', 'invalid_booking_resource');
    const key = String(body.idempotency_key || ''); const prior = database.bookings.find(row => row.direct_idempotency_key === key && key); if (prior) return { booking: clone(prior), session: clone(database.booking_sessions.find(row => Number(row.booking_id) === Number(prior.id))), idempotent: true }; const now = new Date(); const startMinutes = Math.floor((now.getHours() * 60 + now.getMinutes()) / 15) * 15; const start = `${pad(Math.floor(startMinutes / 60))}:${pad(startMinutes % 60)}`; const end = String(body.end_time || ''); assertDemoBookingAvailable(database, { date: today, start_time: start, end_time: end, resource_id: resource.id }, null, body); const duration = demoBookingDurationMinutes({ start_time: start, end_time: end }); if (duration < 60 || duration % 15) throw formationDemoError('مدة الجلسة ساعة على الأقل وبزيادات 15 دقيقة.', 'invalid_booking_time'); const working = clone(database); const booking = addRow(working, 'bookings', { client_id: client.id, client_name: client.name, client_package_id: null, service_id: null, resource_id: resource.id, resource_name: resource.name, service: String(body.title || '').trim().slice(0,120) || 'جلسة تصوير مباشرة', date: today, start_time: start, end_time: end, duration_minutes: duration, requested_quantity: demoSettlementHours(duration), status: 'confirmed', notes: String(body.note || '').trim().slice(0,1000), direct_idempotency_key: key }); const startedAt=nowText(); const session=addRow(working,'booking_sessions',{ booking_id:booking.id,client_id:client.id,client_package_id:null,client_name:client.name,service:booking.service,package_name:null,billing_unit:'hour',resource_id:resource.id,date:today,start_time:start,end_time:end,duration_minutes:duration,scheduled_start_at:`${today} ${start}`,started_at:startedAt,started_at_iso:nowIso(),status:'active',start_source:'manual',booking_held_quantity:0,settlement_version:1 }); booking.status='in_progress'; booking.timer_started_at=startedAt; demoAudit(working,'session_start_direct','booking_sessions',session.id,null,clone(session)); writeDatabase(working); return { booking:clone(booking),session:clone(session),idempotent:false };
  }
  if ((match = route.match(/^\/bookings\/(\d+)\/session\/start$/))) {
    ...
    const pkg = booking.client_package_id ? findById(database, 'client_packages', booking.client_package_id) : null;
    if (booking.client_package_id && (!pkg || !['hour', 'reel'].includes(pkg.billing_unit))) throw formationDemoError('زر بدء التصوير متاح فقط لباقات الساعات أو الريلز.', 'unsupported_session_package');
    if (pkg && (Number(pkg.client_id) !== Number(booking.client_id) || pkg.status !== 'active' || String(pkg.starts_at).slice(0, 10) > cairoDateKey() || String(pkg.expires_at).slice(0, 10) < cairoDateKey())) throw formationDemoError('الباقة غير نشطة أو خارج فترة الصلاحية.', 'invalid_session_package');
    const bookingHold = pkg ? demoBookingHeldQuantity(database, booking.id, pkg.id) : 0; if (pkg && bookingHold <= 0) throw formationDemoError('لا يوجد رصيد محجوز لهذا الموعد على الباقة.', 'missing_package_hold');
  ...
  if (route === '/studio-sessions/active') {
    ...
    const visible = demoRole === 'client'
      ? active.filter(item => Number(item.client_id) === currentDemoClientId(database)).map(demoClientActiveSession)
      : active;
    return { items: visible, server_now: nowIso() };
  }

  if (route === '/reschedule-requests' && options.method === 'POST') { const booking = findById(database, 'bookings', body.booking_id); if (demoRole === 'client') { if (!booking || Number(booking.client_id) !== currentDemoClientId(database)) throw formationDemoError('الموعد غير موجود.', 'booking_not_found'); const policyError = clientWindowError({ date: body.date || body.proposed_date, start_time: body.start_time || body.proposed_start_time, end_time: body.end_time || body.proposed_end_time }); if (policyError) throw formationDemoError(policyError, 'client_booking_outside_hours'); if (clientNoticeIsLate(booking)) throw formationDemoError('تغيير الموعد قبل 48 ساعة على الأقل.', 'late_reschedule'); const candidate = { date: body.date || body.proposed_date, start_time: body.start_time || body.proposed_start_time, end_time: body.end_time || body.proposed_end_time }; const duration = demoClientTimeMinutes(candidate.end_time) - demoClientTimeMinutes(candidate.start_time); validateDemoClientBookingGrid({ ...candidate, duration_minutes: duration }); const dateError = clientBookingDateError(candidate.date); if (dateError) throw formationDemoError(dateError, 'same_day_booking_not_allowed'); if (clientNoticeIsLate(candidate)) throw formationDemoError('الموعد البديل يجب أن يسبق بـ 48 ساعة فعلية باستثناء الجمعة.', 'late_reschedule'); const check = new URL('http://demo/client/booking-availability'); check.search = new URLSearchParams({ booking_id: String(booking.id), duration_minutes: String(duration), start_date: candidate.date, days: '1' }); const day = demoClientBookingAvailability(database, check).days[0]; if (!day.available || !day.slots.some(slot => slot.start_time === String(candidate.start_time).slice(0,5) && slot.end_time === String(candidate.end_time).slice(0,5))) throw formationDemoError('الموعد غير متاح أو لديك طلب في هذا اليوم.', 'booking_conflict'); } assertDemoBookingAvailable(database, { ...booking, date: body.proposed_date || body.date, start_time: body.proposed_start_time || body.start_time, end_time: body.proposed_end_time || body.end_time }, booking?.id); const row = addRow(database, 'reschedule_requests', { ...body, proposed_date: body.proposed_date || body.date, proposed_start_time: body.proposed_start_time || body.start_time, proposed_end_time: body.proposed_end_time || body.end_time, client_id: demoRole === 'client' ? currentDemoClientId(database) : body.client_id || booking?.client_id || 1, status: 'pending' }); demoAudit(database, 'create', 'reschedule_requests', row.id, null, clone(row)); writeDatabase(database); return row; }
  if ((match = route.match(/^\/reschedule-requests\/(\d+)\/decision$/))) { const request = findById(database, 'reschedule_requests', match[1]); const booking = body.action === 'approve' ? findById(database, 'bookings', request.booking_id) : null; if (booking) { const pkg = booking.client_package_id ? findById(database, 'client_packages', booking.client_package_id) : null; const nextBooking = { ...booking, date: request.proposed_date, start_time: request.proposed_start_time, end_time: request.proposed_end_time }; assertDemoBookingAvailable(database, nextBooking, booking.id, body); if (pkg && booking.status === 'confirmed') { const oldHold = demoBookingHeldQuantity(database, booking.id, pkg.id); const oldMinutes = pkg.billing_unit === 'hour' ? demoBookingHeldMinutes(database, booking.id, pkg.id) : null; const nextMinutes = pkg.billing_unit === 'hour' ? demoBookingDurationMinutes(nextBooking) : null; const nextQuantity = pkg.billing_unit === 'hour' ? demoSettlementHours(nextMinutes) : Number(booking.requested_quantity || oldHold); const delta = nextQuantity - oldHold; if (delta > demoPackageAvailable(pkg) + 0.000001) throw formationDemoError('رصيد الباقة المتاح لا يكفي للمدة الجديدة.', 'insufficient_package_balance'); mutateDemoPackageQuantities(pkg, pkg.billing_unit === 'hour' ? { held_minutes: nextMinutes - oldMinutes } : { held: delta }); if (oldHold > 0) addDemoPackageUsage(database, pkg, { booking_id: booking.id, movement_type: 'release', quantity: oldHold, quantity_minutes: oldMinutes, reason: 'تحرير الموعد السابق', event_key: `booking:${booking.id}:request-release:${request.id}` }); addDemoPackageUsage(database, pkg, { booking_id: booking.id, movement_type: 'hold', quantity: nextQuantity, quantity_minutes: nextMinutes, reason: 'اعتماد الموعد الجديد', event_key: `booking:${booking.id}:request-hold:${request.id}` }); booking.requested_quantity = nextQuantity; } Object.assign(booking, { date: request.proposed_date, start_time: request.proposed_start_time, end_time: request.proposed_end_time }); } request.status = body.action === 'approve' ? 'approved' : 'rejected'; writeDatabase(database); return request; }
  ...
  if ((match = route.match(/^\/payment-proofs\/(\d+)\/decision$/))) {
    ...
    if (proof.status === 'approved') {
      ...
      const proofMethod = ['cash', 'instapay', 'vodafone_cash'].includes(proof.payment_method) ? proof.payment_method : 'bank_transfer';
  ...
  if (route === '/payment-proofs' && options.method === 'POST') { const proofFile = body.proof; const accounts = { vodafone_cash: '01094084424' }; const paymentMethod = body.payment_method ?? 'vodafone_cash'; if (paymentMethod !== 'vodafone_cash') throw formationDemoError('التحويل متاح عبر فودافون كاش فقط.', 'invalid_payment_method'); const row = addRow(database, 'payment_proofs', { amount: Number(body.amount || 0), payment_method: paymentMethod, transfer_account_snapshot: accounts[paymentMethod], client_package_id: body.client_package_id ? Number(body.client_package_id) : null, invoice_id: body.invoice_id ? Number(body.invoice_id) : null, client_id: demoRole === 'client' ? currentDemoClientId(database) : Number(body.client_id || 1), status: 'pending', original_name: proofFile?.name || 'demo-transfer.jpg', mime_type: proofFile?.type || 'image/jpeg' }); demoAudit(database, 'create', 'payment_proofs', row.id, null, clone(row)); writeDatabase(database); return row; }
  ...
  if (route === '/finance/manual') {
    ...
    const employee = employeeUserId ? findById(database, 'users', employeeUserId) : null; if (employeeUserId && (!employee || ['client', 'applicant'].includes(employee.role) || employee.is_active === 0 || kind !== 'expense')) throw formationDemoError('حساب الموظف المحدد غير صالح لهذا المصروف.', 'invalid_employee_user');
  ...
  if (route === '/post-production/pickup-schedule') {
    ...
    if (!['owner', 'admin', 'operations'].includes(demoRole)) throw formationDemoError('ليس لديك صلاحية لتعديل مواعيد الاستلام.', 'forbidden');
    if (!Number.isSafeInteger(body.expected_revision) || body.expected_revision < 0) throw formationDemoError('نسخة جدول الاستلام مطلوبة.', 'invalid_pickup_revision');
    const desired = validateCompanyPickupSchedule(body); const current = demoCompanyPickupSchedule(database);
    const previous = { enabled: current.enabled, timezone: current.timezone, note: current.note, windows: current.windows };
    if (JSON.stringify(desired) === JSON.stringify(previous) && [current.revision, current.revision - 1].includes(body.expected_revision)) return { ...current, idempotent: true };
    if (body.expected_revision !== current.revision) { const error = formationDemoError('تم تعديل جدول الاستلام من مستخدم آخر. أعد تحميله قبل الحفظ.', 'pickup_revision_conflict'); error.status = 409; throw error; }
    const working = clone(database); const next = { revision: current.revision + 1, ...desired, updated_at: nowIso() };
    const row = tableRows(working, 'app_config').find(item => belongsToDemoOrganization(item) && item.key === COMPANY_PICKUP_SCHEDULE_KEY);
    if (row) { row.value = JSON.stringify(next); row.type = 'json'; row.updated_at = nowText(); }
    else addRow(working, 'app_config', { organization_id: demoOrganizationId, key: COMPANY_PICKUP_SCHEDULE_KEY, value: JSON.stringify(next), type: 'json' });
    addRow(working, 'audit_logs', { organization_id: demoOrganizationId, user_id: demoUserId, action: 'pickup_schedule_updated', entity_type: 'pickup_schedule', entity_id: demoOrganizationId, before_data: JSON.stringify(current), after_data: JSON.stringify(next) });
    addRow(working, 'change_events', { organization_id: demoOrganizationId, client_id: null, topic: 'post_production', entity_type: 'pickup_schedule', entity_id: demoOrganizationId, action: 'pickup_schedule_updated' });
    writeDatabase(working); return { ...next, idempotent: false };
  }
  ...
  if (route === '/client/post-production' && (options.method || 'GET') === 'GET') {
    ...
    return { items: demoPostProductionRows(database, true), pickup_schedule: demoCompanyPickupSchedule(database), server_now: nowIso() };
  }
  ...
  if ((match = route.match(/^\/post-production\/(\d+)\/pickup-availability$/)) && (options.method || 'GET') === 'GET') {
    ...
    if (!job || demoRole === 'client' && (Number(job.client_id) !== currentDemoClientId(database) || Number(job.is_client_visible) !== 1 || Number(job.needs_review) !== 0)) throw formationDemoError('مهمة المونتاج غير موجودة أو غير متاحة.', 'post_production_not_found');
  ...
  if (route === '/sync' && (options.method || 'GET') === 'GET') {
    const cursor = Math.max(0, Number(url.searchParams.get('cursor') || 0)); const clientId = demoRole === 'client' ? currentDemoClientId(database) : null;
    const visible = tableRows(database, 'change_events').filter(event => belongsToDemoOrganization(event) && (!clientId || Number(event.client_id) === clientId || event.topic === 'services' || (event.client_id == null && event.topic === 'post_production' && event.entity_type === 'pickup_schedule') || (event.client_id == null && event.topic === 'availability' && event.entity_type === 'calendar_availability'))).sort((a, b) => Number(a.id) - Number(b.id));
  ...
  if ((match = route.match(/^\/app-notifications\/(\d+)\/read$/)) && options.method === 'POST') { const item = findById(database, 'app_notifications', match[1]); const scoped = item && !item.dismissed_at && (demoRole === 'client' ? item.audience === 'client' && Number(item.client_id) === currentDemoClientId(database) : item.audience === 'staff' || item.audience === 'owner' && Number(item.recipient_user_id) === Number(demoUserId)); if (scoped && !item.read_at) { item.read_at = nowText(); addRow(database, 'change_events', { client_id: item.client_id || null, topic: 'notifications', entity_type: 'app_notifications', entity_id: item.id, action: 'read' }); } writeDatabase(database); return { read: true, changed: Boolean(scoped) }; }
  if (route === '/app-notifications/read-all' && options.method === 'POST') { const upToId = Number(body.up_to_id || 0); const channel = body.channel || ''; let changed = 0; database.app_notifications.forEach(item => { const scoped = !item.dismissed_at && !item.read_at && Number(item.id) <= upToId && (channel === 'client-actions' ? demoRole === 'owner' && item.audience === 'owner' && Number(item.recipient_user_id) === Number(demoUserId) : demoRole === 'client' ? item.audience === 'client' && Number(item.client_id) === currentDemoClientId(database) : item.audience === 'staff'); if (scoped) { item.read_at = nowText(); changed += 1; } }); if (changed) addRow(database, 'change_events', { client_id: demoRole === 'client' ? currentDemoClientId(database) : null, topic: 'notifications', entity_type: 'app_notifications', entity_id: upToId, action: 'read_all' }); writeDatabase(database); return { read: true, changed, up_to_id: upToId }; }
  if ((match = route.match(/^\/app-notifications\/(\d+)\/dismiss$/)) && options.method === 'POST') { const item = findById(database, 'app_notifications', match[1]); const scoped = item && !item.dismissed_at && (demoRole === 'client' ? item.audience === 'client' && Number(item.client_id) === currentDemoClientId(database) : item.audience === 'staff' || item.audience === 'owner' && Number(item.recipient_user_id) === Number(demoUserId)); if (scoped) { item.dismissed_at = nowText(); item.read_at ||= item.dismissed_at; addRow(database, 'change_events', { client_id: item.client_id || null, topic: 'notifications', entity_type: 'app_notifications', entity_id: item.id, action: 'dismissed' }); } writeDatabase(database); return { dismissed: true, changed: Boolean(scoped) }; }

  return { demo: true };
};

export const activateDemoMode = (role = 'owner', userId = 1, organizationId = 1) => { demoRegistrationUserId = null; demoMode = true; demoRole = role; demoUserId = Number(userId) || 1; demoOrganizationId = Number(organizationId) || 1; readDatabase(); };
export const deactivateDemoMode = () => { demoRegistrationUserId = null; demoMode = false; demoRole = 'owner'; demoUserId = 1; demoOrganizationId = 1; demoCsrfReady = false; demoCredentialSessionVersion = null; };
...
export const authenticateDemoClientCredential = async (identifier, password) => {
  ...
  const database = readDatabase(); const identity = normalizeLoginPhone(identifier);
  if (!identity) return null;
  const client = database.clients.find(item => normalizeLoginPhone(item.phone1) === identity);
...
export const authenticateDemoRegistration = async (identifier, password) => {
  if (import.meta.env && !import.meta.env.DEV) return null;
  const user = await authenticateRegistrationDemo(readDatabase(), identifier, password);
  if (!user) return null;
  activateDemoMode(user.role, user.id); demoRegistrationUserId = user.id; return user;
};
export const resumeDemoRegistrationSession = user => {
  const current = registrationDemoUser(readDatabase(), user.id);
  if (!current) return null;
  activateDemoMode(current.role, current.id); demoRegistrationUserId = current.id; return current;
};
