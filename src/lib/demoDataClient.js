...
const demoRequest = async (path, options = {}) => {
  ...
  if ((match = route.match(/^\/booking-blocks\/(\d+)\/convert$/)) && options.method === 'POST') {
    ...
    const client = findById(database, 'clients', body.client_id); const pkg = findById(database, 'client_packages', body.client_package_id); const key = String(body.idempotency_key || '').trim(); const hash = demoSettlementHash({ block_id: Number(selected.id), client_id: Number(body.client_id), client_package_id: Number(body.client_package_id) });
    if (selected.converted_booking_id) { if (selected.conversion_idempotency_key !== key || selected.conversion_request_hash !== hash) throw formationDemoError('تم تحويل هذا الحجز المؤقت من قبل.', 'booking_block_already_converted'); return { ...clone(selected.conversion_response_json), idempotent: true }; }
    if (selected.status !== 'active') throw formationDemoError('الحجز المؤقت ملغى.', 'booking_block_inactive'); if (!client || client.status !== 'active') throw formationDemoError('العميل غير نشط.', 'invalid_booking_client'); if (!pkg || Number(pkg.client_id) !== Number(client.id) || pkg.status !== 'active' || pkg.billing_unit !== 'hour') throw formationDemoError('اختر باقة ساعات نشطة تخص العميل.', 'invalid_target_package');
    const working = clone(database); const block = findById(working, 'booking_blocks', selected.id); const target = findById(working, 'client_packages', pkg.id); activateDemoPackageOnFirstBooking(working, target, block.block_date); if (demoPackageMinutes(target, 'purchased') - demoPackageMinutes(target, 'consumed') - demoPackageMinutes(target, 'held') < Number(block.duration_minutes)) throw formationDemoError('رصيد الباقة لا يكفي.', 'insufficient_package_balance');
    const booking = addRow(working, 'bookings', { client_id: client.id, client_name: client.name, client_package_id: target.id, service_id: target.service_id, resource_id: block.resource_id, resource_name: block.resource_name, service: target.name, date: block.block_date, start_time: block.start_time, end_time: block.end_time, duration_minutes: block.duration_minutes, requested_quantity: demoSettlementHours(block.duration_minutes), status: 'confirmed', notes: block.note || '' }); working.booking_slots.forEach(slot => { if (Number(slot.booking_block_id) === Number(block.id)) Object.assign(slot, { booking_id: booking.id, booking_block_id: null }); }); mutateDemoPackageQuantities(target, { held_minutes: Number(block.duration_minutes) }); addDemoPackageUsage(working, target, { booking_id: booking.id, movement_type: 'hold', quantity: demoSettlementHours(block.duration_minutes), quantity_minutes: Number(block.duration_minutes), reason: 'تحويل حجز مؤقت', event_key: `booking:${booking.id}:hold` });
    const response = { booking: clone(booking), block_id: block.id, held_minutes: Number(block.duration_minutes), idempotent: false }; Object.assign(block, { status: 'cancelled', converted_booking_id: booking.id, conversion_idempotency_key: key, conversion_request_hash: hash, conversion_response_json: clone(response), cancelled_by: demoUserId, cancelled_at: nowText() }); demoAuditBookingBlockChange(working, 'convert', block.id, selected, response); writeDatabase(working); return response;
  }
