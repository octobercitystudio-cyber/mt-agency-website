...
import { STUDIO_TRANSFER_ACCOUNT, STUDIO_SUCCESS_MESSAGE, studioPurchaseSelection, studioDayShares, sortedStudioBookings, studioBookingReady, studioReviewDeadline, validateStudioBookings, validateStudioProof } from './studioBookingPolicy.js';
...
async function handle({ route, method, body, database: db, role, clientId, addRow, writeDatabase, assertAvailable, addUsage, activatePackage, mutatePackage, audit }) {
  ...
  if (route === '/client/studio-booking-requests' && method === 'POST') {
    ...
    let service = (await registrationDemoCatalog(db)).services.find(s => Number(s.id) === Number(payload.service_id)); if (!service) fail('الباقة غير متاحة.', 'invalid_service');
    ...
    service = studioPurchaseSelection(service, payload.selected_hours ?? service.total_hours); if (!service) fail('اختر عدد ساعات صحيحًا من 1 إلى 300 ساعة.', 'invalid_studio_hours');
  ...
  if (action === 'approve' && stage === 'package') {
    ...
    if (!s.hourly_day_allocation) addUsage(db, pkg, { movement_type: 'opening', quantity: s.total_hours, quantity_minutes: pkg.purchased_minutes, reason: 'اعتماد طلب تصوير العميل', event_key: `studio:${request.id}:opening` });
    ...
    if (s.hourly_day_allocation) {
      const shares = studioDayShares(s, request.bookings); s.day_package_ids = {};
      db.payment_allocations = db.payment_allocations.filter(allocation => Number(allocation.payment_id) !== Number(payment.id));
      shares.forEach((share, index) => {
        const fields = { ...pkg, name: `${s.name} · ${share.date}`, purchased_quantity: share.duration_minutes / 60, purchased_minutes: share.duration_minutes, total_price: share.price, paid_amount: share.paid, payment_due_quantity: 0, payment_due_minutes: 0, starts_at: share.date, expires_at: share.date, validity_mode_snapshot: 'shooting_day', validity_days_snapshot: 1 };
        delete fields.id;
        const dayPackage = index ? addRow(db, 'client_packages', fields) : Object.assign(pkg, fields);
        s.day_package_ids[share.id] = dayPackage.id;
        addRow(db, 'payment_allocations', { client_id: request.client_id, payment_id: payment.id, payment_proof_id: proof.id, client_package_id: dayPackage.id, invoice_id: null, amount: share.paid });
        addUsage(db, dayPackage, { movement_type: 'opening', quantity: share.duration_minutes / 60, quantity_minutes: share.duration_minutes, reason: `ساعات يوم التصوير ${share.date}`, event_key: `package:${dayPackage.id}:opening` });
        audit(db, 'create', 'client_packages', dayPackage.id, null, { ...dayPackage, loyalty_enabled: false });
      });
    }
  } else if (action === 'approve') {
    if (!studioBookingReady(request, row)) fail('اعتمد الباقة وراجع الموعد الأسبق أولًا.', 'booking_dependency', 409);
    if (cairoDateTimeToEpoch(`${row.date}T${row.start_time}:00`) <= Date.now()) fail('الموعد المقترح مضى.', 'booking_in_past'); assertAvailable(db, row, null, body);
    const pkg = db.client_packages.find(p => Number(p.id) === Number(request.service_snapshot.day_package_ids?.[row.id] || request.client_package_id)); if (!pkg || pkg.status !== 'active' || Number(pkg.purchased_minutes) - Number(pkg.consumed_minutes) - Number(pkg.held_minutes) < row.duration_minutes) fail('رصيد الباقة لا يكفي.', 'insufficient_package_balance');
