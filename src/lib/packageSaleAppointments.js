...
export const validatePackageAppointment = (candidate, { unit = 'hour', minimumMinutes = 60, incrementMinutes = 15, startsAt = '', expiresAt = '', shootingDate = '', appointments = [], occupied = [], editIndex = -1, nowKey = cairoAppointmentNowKey() } = {}) => {
  ...
  if (timeMinutes(candidate?.start_time) < 0 || timeMinutes(candidate?.end_time) > 1440 || duration < minimumMinutes || duration % incrementMinutes !== 0) errors.time = `الحجز متاح طوال اليوم، بحد أدنى ${formatDurationMinutes(minimumMinutes)} وبزيادات ${formatDurationMinutes(incrementMinutes)}.`;
